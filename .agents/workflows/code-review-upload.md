...
- review wrapper remediation:
  - ChatGPT Pro CDP review should reuse an existing healthy ChatGPT tab before opening a fresh one, because fresh CDP tabs can hit Cloudflare and make the prompt box invisible.
  - Gemini imported-code review may fail-open only when a public `github_repo` and review-packet attachments are both present; in that narrow case the lane stays on `gemini_web.ask` and continues with the attached review packet plus repo URL.
