...
Status: Final v2 after Pro confirmation small revisions
...
Final Pro confirmation verdict: **需要小修后执行**. The plan is accepted after tightening parallelism, keeping only two active orgs, delaying Boss Gallery iteration until after the Commerce Decision Layer, and making asset readiness / claim permissions / rejected direction logging harder requirements.
...
- starts only after `LAB-007 Commerce Decision Layer v0` exists;
...
- A-F gallery skeleton only after Commerce Decision Layer inputs are available;
- 2-3 polished demos only if claim/evidence support is strong enough;
- claim ledger;
- blocked claims.
...
Final Pro tightening: Kimi Code / Claude Code Kimi should handle only bounded evidence and mapping tasks. Main Codex retains high-level decisions, Commerce Decision Layer integration, DTC prototype direction, Boss Gallery narrative and final acceptance.
...
3. **Claude Code: Amazon Method Probe**
   ...
   - task: compare no-key/manual/script/API paths and draft ASIN identity workflow. Main Codex must review before any VOC/design use.
...
5. **Local Codex preferred: Browser / Visual QA Template**
   - files owned: `planning/.../runtime_kernel/qa_templates/`;
   - task: define Browser Harness P0 artifact contract without building a new platform. Delegate only if the worker is explicitly constrained to template drafting.
...
## 11. Post-Pro Revisions Applied

The final Pro confirmation was saved at:
...
1. Keep exactly two active orgs and a read-only archive.
2. Do not treat Skill Foundry or Browser & Vision Lab as active orgs.
3. Do not run broad Amazon crawling; first validate ASIN identity and listing facts.
4. Do not make five full website versions; make concept boards, then primary/fallback.
5. Make `asset_readiness_matrix.csv`, `claim_permission_matrix.csv` and `rejected_direction_log.md` hard Commerce Decision Layer requirements.
6. Keep Evidence Integrity, ASIN Identity, Claim, Design Decision, Browser/Visual QA and Closeout gates mandatory.
7. Keep Full Design Review Scoring, Fresh Agent Review and Skill Curator Review advisory.
8. Restrict Kimi/Claude workers to bounded data/media/reference tasks.
9. Main Codex retains website direction, prototype decisions and Boss Gallery narrative.
10. Boss Gallery iteration happens after Commerce Decision Layer, not before.

## 12. Pro Confirmation Ask
...
## 13. Current Plan Status

This is final v2 for execution. Next step is to mark `PCL-001` and `LAB-001` ready locally, then launch bounded delegated workers only after exact prompts and write scopes are confirmed.
