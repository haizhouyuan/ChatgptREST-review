        ...
        try:
            req_headers.update(
                build_registered_client_hmac_headers(
                    client_lookup=req_headers.get("X-Client-Id") or req_headers.get("X-Client-Name"),
                    client_instance=req_headers.get("X-Client-Instance"),
                    method=method,
                    path=rel_path,
...
def _build_submit_request(args: argparse.Namespace) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    try:
        if args.client_name:
            resolved_client_name = str(args.client_name).strip()
        elif isinstance(payload.get("client"), dict):
            candidate = str((payload.get("client") or {}).get("name") or "").strip()
            if candidate:
    ...
    if resolved_client_name:
        # Keep body/header identity in lockstep so maintenance and alternate
        # registered client flows do not drift into auth mismatch.
        headers["X-Client-Name"] = resolved_client_name
        headers["X-Client-Id"] = resolved_client_name
...
def _cmd_jobs_run(ctx: CliContext, args: argparse.Namespace) -> int:
    ...
    answer_state = get_completion_answer_state(wait_obj)
    ...
    if (status == "completed" and is_research_final(wait_obj)) and not args.skip_answer:
        ...
        if isinstance(answer_obj, dict):
            ...
            if args.only_text:
                print(chunk)
                return 0
    elif status == "completed" and not args.skip_answer:
        result["action_hint"] = "await_research_finality"
        result["answer_state"] = answer_state
        result["authoritative_answer_path"] = get_authoritative_answer_path(wait_obj)
    _print(ctx, result)
    return 0
...
def _git_changed_files(repo_root: Path, diff_spec: str) -> list[str]:
    proc = subprocess.run(
    ...
    if proc.returncode != 0:
        raise CliError(f"git diff failed for {diff_spec}: {proc.stderr[:200]}")
...
def _cmd_repo_bootstrap(ctx: CliContext, args: argparse.Namespace) -> int:
    from chatgptrest.repo_cognition.bootstrap import generate_bootstrap_packet

    packet = generate_bootstrap_packet(
        task_description=str(args.task or ""),
        changed_files=list(args.changed_files or []),
        goal_hint=str(args.goal_hint or ""),
        runtime_mode=str(args.runtime or "quick"),
    )
    _print(ctx, packet)
    return 0


def _cmd_repo_doc_obligations(ctx: CliContext, args: argparse.Namespace) -> int:
    from chatgptrest.repo_cognition.obligations import compute_change_obligations, validate_obligations

    if args.diff:
        changed_files = _git_changed_files(ctx.repo_root, str(args.diff))
    else:
        changed_files = list(args.changed_files or [])

    if not changed_files:
        result = {"ok": True, "obligations": [], "note": "No changed files", "changed_files": []}
        _print(ctx, result)
        return 0
...
def _cmd_repo_closeout(ctx: CliContext, args: argparse.Namespace) -> int:
    script = ctx.repo_root / "scripts" / "chatgptrest_closeout.py"
    if not script.exists():
        raise CliError(f"closeout wrapper not found: {script}")
    command = [
        sys.executable,
        str(script),
        "--agent",
    ...
    if args.pending_reason:
        command.extend(["--pending-reason", str(args.pending_reason)])
    if args.pending_scope:
        command.extend(["--pending-scope", str(args.pending_scope)])
    if args.diff:
        command.extend(["--diff", str(args.diff)])
    if args.changed_files:
        command.extend(["--changed-files", *list(args.changed_files)])
    if bool(args.skip_doc_check):
        command.append("--skip-doc-check")
    if ctx.output == "json":
        command.append("--json")
    proc = subprocess.run(
        command,
        cwd=ctx.repo_root,
        capture_output=ctx.output == "json",
        text=True,
        check=False,
    )
    if ctx.output == "json" and proc.stdout:
...
def _cmd_opencli_doctor(ctx: CliContext, args: argparse.Namespace) -> int:
    """Run opencli doctor."""
    cmd = ["opencli", "doctor"]
    if args.no_live:
        cmd.append("--no-live")
    ...
    try:
        ...
        payload = {"ok": False, "error": "opencli binary not found in PATH"}
        _print(ctx, payload)
        return 1
    except subprocess.TimeoutExpired:
        payload = {"ok": False, "error": "opencli doctor timed out"}
        _print(ctx, payload)
        return 1
    except Exception as exc:
...
def _cmd_opencli_policy(ctx: CliContext, args: argparse.Namespace) -> int:
    """Show opencli execution policy."""
    from chatgptrest.executors.opencli_policy import OpenCLIExecutionPolicy

    policy = OpenCLIExecutionPolicy()
    commands = policy.list_commands()
    ...
    for command_id in commands:
        cmd_policy = policy.get_command_policy(command_id)
        if cmd_policy:
            payload["commands"].append(
                {
                    "command_id": cmd_policy.command_id,
                    "capability_id": cmd_policy.capability_id,
                    "command": cmd_policy.command,
                    "risk_level": cmd_policy.risk_level,
                    "auth_mode": cmd_policy.auth_mode,
                    "browser_mode": cmd_policy.browser_mode,
                    "allowed_args": cmd_policy.allowed_args,
                    "retryable_exit_codes": cmd_policy.retryable_exit_codes,
                }
            )
...
def _cmd_opencli_smoke(ctx: CliContext, args: argparse.Namespace) -> int:
    """Run opencli executor smoke test."""
    smoke_script = ctx.repo_root / "ops" / "run_opencli_executor_smoke.py"
    if not smoke_script.exists():
        payload = {"ok": False, "error": f"Smoke test script not found: {smoke_script}"}
        _print(ctx, payload)
        return 1
    ...
    try:
        ...
        payload = {"ok": False, "error": "Smoke test timed out"}
        _print(ctx, payload)
        return 1
    except Exception as exc:
        payload = {"ok": False, "error": str(exc)}
...
def _default_openmind_memory_db() -> Path:
    return Path(os.environ.get("OPENMIND_MEMORY_DB", "~/.openmind/memory.db")).expanduser()


def _cmd_work_memory_import_manifest(ctx: CliContext, args: argparse.Namespace) -> int:
    ...
    if bool(args.execute) and bool(args.dry_run):
        raise CliError("--dry-run and --execute are mutually exclusive")

    manifest_paths = [str(Path(path).expanduser().resolve()) for path in list(args.manifest or []) if str(path).strip()]
    if not manifest_paths:
        raise CliError("at least one --manifest is required")

    effective_only_gate = str(args.only_gate or ("ready" if args.execute else "all")).strip()
    limit = int(args.limit) if args.limit is not None else None
    memory_db = _default_openmind_memory_db()
    importer = WorkMemoryImporter(MemoryManager(str(memory_db)))

    if args.execute:
        account_id = str(args.account_id or "").strip()
        role_id = str(args.role_id or "").strip()
        if not account_id or not role_id:
            raise CliError("--account-id and --role-id are required with --execute")
        result = importer.execute(
            ...
            only_gate=effective_only_gate,
            limit=limit,
        )
    else:
        result = importer.dry_run(
            manifest_paths,
            only_gate=effective_only_gate,
            limit=limit,
        )
    ...
    payload["memory_db"] = str(memory_db)
    if args.json_out:
        Path(str(args.json_out)).expanduser().write_text(_json_dumps(payload), encoding="utf-8")
    markdown = result.to_markdown()
    if args.report_out:
        Path(str(args.report_out)).expanduser().write_text(markdown, encoding="utf-8")
    if ctx.output == "pretty":
...
def _cmd_work_memory_review_list(ctx: CliContext, args: argparse.Namespace) -> int:
    from chatgptrest.kernel.memory_manager import MemoryManager
    from chatgptrest.kernel.work_memory_importer import WorkMemoryImporter

    memory_db = _default_openmind_memory_db()
    importer = WorkMemoryImporter(MemoryManager(str(memory_db)))
    result = importer.list_review_queue(
        state=str(args.state or "pending").strip(),
...
def _cmd_work_memory_review_resolve(ctx: CliContext, args: argparse.Namespace) -> int:
    from chatgptrest.kernel.memory_manager import MemoryManager
    from chatgptrest.kernel.work_memory_importer import WorkMemoryImporter

    memory_db = _default_openmind_memory_db()
    importer = WorkMemoryImporter(MemoryManager(str(memory_db)))
    result = importer.resolve_review(
        str(args.record_id or "").strip(),
        action=str(args.action or "").strip(),
        reviewer=str(args.reviewer or "").strip(),
        reason=str(args.reason or "").strip(),
        supersedes_decision_id=str(args.supersedes_decision_id or "").strip(),
    )
    payload = result.to_dict()
    payload["memory_db"] = str(memory_db)
    if args.json_out:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    work_memory = sub.add_parser("work-memory", help="local durable work-memory operator commands")
    work_memory_sub = work_memory.add_subparsers(dest="work_memory_cmd", required=True)

    work_memory_import = work_memory_sub.add_parser(
        "import-manifest",
        help="dry-run or execute planning backfill manifest imports into durable work-memory",
    )
    work_memory_import.add_argument("--manifest", action="append", required=True, help="path to import-ready manifest; repeatable")
    work_memory_import.add_argument("--dry-run", action=argparse.BooleanOptionalAction, default=False, help="plan import without writing memory.db")
    work_memory_import.add_argument("--execute", action=argparse.BooleanOptionalAction, default=False, help="execute import into memory.db")
    work_memory_import.add_argument("--account-id", default="", help="required with --execute")
    work_memory_import.add_argument("--role-id", default="", help="required with --execute")
    work_memory_import.add_argument("--session-id", default="", help="optional stable import session override")
    work_memory_import.add_argument("--thread-id", default="", help="optional stable import thread override")
    work_memory_import.add_argument(
        "--only-gate",
        choices=["ready", "manual_review_required", "all"],
        default="",
        help="default is all for dry-run and ready for execute",
    )
    work_memory_import.add_argument("--limit", type=int, default=None, help="maximum number of selected entries to process")
    work_memory_import.add_argument("--json-out", default="", help="write JSON report to file")
    work_memory_import.add_argument("--report-out", default="", help="write Markdown report to file")
    work_memory_import.set_defaults(func=_cmd_work_memory_import_manifest)

    work_memory_review_list = work_memory_sub.add_parser(
        "review-list",
        help="list work-memory import review queue entries",
    )
    work_memory_review_list.add_argument(
        "--state",
        choices=["pending", "resolved", "rolled_back", "all"],
        default="pending",
        help="review queue state filter",
    )
    work_memory_review_list.add_argument("--limit", type=int, default=50, help="maximum number of review items to return")
    work_memory_review_list.add_argument("--json-out", default="", help="write JSON report to file")
    work_memory_review_list.add_argument("--report-out", default="", help="write Markdown report to file")
    work_memory_review_list.set_defaults(func=_cmd_work_memory_review_list)

    work_memory_review_resolve = work_memory_sub.add_parser(
        "review-resolve",
        help="resolve a work-memory import review queue entry",
    )
    work_memory_review_resolve.add_argument("record_id", help="review queue record_id")
    work_memory_review_resolve.add_argument(
        "--action",
        required=True,
        choices=["approve", "reject", "promote", "supersede", "rollback"],
        help="review action",
    )
    work_memory_review_resolve.add_argument("--reviewer", required=True, help="operator/reviewer identity")
    work_memory_review_resolve.add_argument("--reason", required=True, help="review reason")
    work_memory_review_resolve.add_argument(
        "--supersedes-decision-id",
        default="",
        help="required when --action supersede and payload does not already include supersedes_decision_id",
    )
    work_memory_review_resolve.add_argument("--json-out", default="", help="write JSON report to file")
    work_memory_review_resolve.add_argument("--report-out", default="", help="write Markdown report to file")
    work_memory_review_resolve.set_defaults(func=_cmd_work_memory_review_resolve)
    ...
    repo = sub.add_parser("repo", help="repo cognition and governance helpers")
    repo_sub = repo.add_subparsers(dest="repo_cmd", required=True)

    repo_bootstrap = repo_sub.add_parser("bootstrap", help="generate bootstrap-v1 packet")
    repo_bootstrap.add_argument("--task", default="", help="task description")
    repo_bootstrap.add_argument("--changed-files", nargs="*", default=None, help="changed file paths")
    repo_bootstrap.add_argument("--goal-hint", default="", help="goal hint (execution, public_agent, advisor, ...)")
    repo_bootstrap.add_argument("--runtime", choices=["quick", "deep"], default="quick", help="runtime snapshot mode")
    repo_bootstrap.set_defaults(func=_cmd_repo_bootstrap)

    repo_doc = repo_sub.add_parser("doc-obligations", help="check doc/test obligations for a change set")
    repo_doc.add_argument("--diff", default="", help="git diff spec (e.g. HEAD~1..HEAD)")
    repo_doc.add_argument("--changed-files", nargs="*", default=None, help="explicit changed file paths")
    repo_doc.set_defaults(func=_cmd_repo_doc_obligations)

    repo_closeout = repo_sub.add_parser("closeout", help="run repo closeout wrapper with doc-obligation checks")
    repo_closeout.add_argument("--agent", required=True)
    repo_closeout.add_argument("--status", required=True, choices=["completed", "partial"])
    repo_closeout.add_argument("--summary", required=True)
    repo_closeout.add_argument("--pending-reason", default="")
    repo_closeout.add_argument("--pending-scope", default="")
    repo_closeout.add_argument("--diff", default="")
    repo_closeout.add_argument("--changed-files", nargs="*", default=None)
    repo_closeout.add_argument("--skip-doc-check", action="store_true")
    repo_closeout.set_defaults(func=_cmd_repo_closeout)

    opencli = sub.add_parser("opencli", help="opencli executor diagnostics")
    opencli_sub = opencli.add_subparsers(dest="opencli_cmd", required=True)

    opencli_doctor = opencli_sub.add_parser("doctor", help="run opencli doctor")
    opencli_doctor.add_argument("--no-live", action="store_true", help="skip live checks")
    opencli_doctor.set_defaults(func=_cmd_opencli_doctor)

    opencli_policy = opencli_sub.add_parser("policy", help="show opencli execution policy")
    opencli_policy.set_defaults(func=_cmd_opencli_policy)

    opencli_smoke = opencli_sub.add_parser("smoke", help="run opencli executor smoke test")
    opencli_smoke.set_defaults(func=_cmd_opencli_smoke)
