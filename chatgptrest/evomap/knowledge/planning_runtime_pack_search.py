...
def _bundle_generated_at_meta(manifest: Mapping[str, Any]) -> dict[str, Any]:
    generated_at = str(manifest.get("generated_at") or "").strip()
    if not generated_at:
        return {"bundle_generated_at": "", "bundle_age_hours": None, "bundle_freshness": "unknown"}
    try:
        ...
        return {"bundle_generated_at": generated_at, "bundle_age_hours": None, "bundle_freshness": "unknown"}
    return {
        "bundle_generated_at": generated_at,
        "bundle_age_hours": round(age_hours, 3),
        "bundle_freshness": "fresh" if age_hours <= 72.0 else "stale",
    }


def planning_runtime_pack_bundle_status(bundle_dir: str | Path = "") -> dict[str, Any]:
    ...
    if bundle is None:
        return {
            ...
            "bundle_freshness": "missing",
            "checks": {},
            "scope": {},
        }

    manifest_path = bundle / "release_bundle_manifest.json"
    if not manifest_path.exists():
        return {
            "available": False,
            "ready_for_explicit_consumption": False,
    ...
    return {
        "available": True,
        "ready_for_explicit_consumption": bool(manifest.get("ready_for_explicit_consumption", False)),
        "bundle_dir": str(bundle),
        "pack_dir": str(pack_dir),
        "pack_version": pack_dir.name,
        **_bundle_generated_at_meta(manifest),
...
def search_planning_runtime_pack(
    ...
    *,
    ...
    bundle_status = planning_runtime_pack_bundle_status(bundle_dir)
    if not bool(bundle_status.get("available")) or not bool(bundle_status.get("ready_for_explicit_consumption")):
        return []

    bundle = Path(str(bundle_status.get("bundle_dir") or ""))
    pack_dir = Path(str(bundle_status.get("pack_dir") or ""))
    ...
    pack_version = str(bundle_status.get("pack_version") or pack_dir.name)
        ...
        results.append(
            {
                ...
                "planning_pack_meta": {
                    ...
                    "bundle_generated_at": str(bundle_status.get("bundle_generated_at") or ""),
                    "bundle_age_hours": bundle_status.get("bundle_age_hours"),
                    "bundle_freshness": str(bundle_status.get("bundle_freshness") or ""),
