...
from chatgptrest.evomap.knowledge.ingest_quality import GENERIC_HEADING_QUESTIONS
...
_ACTIVITY_EVENT_QUESTION_RE = re.compile(r"^what\s+([a-z0-9_.-]+)\s+event\s+occurred\??$", re.IGNORECASE)
_SYNTHETIC_PLACEHOLDER_QUESTION_RE = re.compile(r"^question\s+for\s+[a-z0-9_.-]+$", re.IGNORECASE)
...
@dataclass
class CanonicalQuestionBackfillStats:
    total: int = 0
    already_set: int = 0
    from_question: int = 0
    from_activity_pattern: int = 0
    skipped_blank: int = 0
    skipped_generic: int = 0


def derive_canonical_question(question: str) -> tuple[str, str]:
    ...
    if not normalized:
        return "", "blank_question"
    if normalized in GENERIC_HEADING_QUESTIONS:
        return "", "generic_heading"
    if _SYNTHETIC_PLACEHOLDER_QUESTION_RE.match(raw):
        return "", "synthetic_placeholder"

    match = _ACTIVITY_EVENT_QUESTION_RE.match(raw)
    if match:
        return f"activity: {match.group(1).lower()}", "activity_pattern"

    collapsed = re.sub(r"\s+", " ", raw).strip()
    return collapsed, "question"


def backfill_canonical_question(db: KnowledgeDB) -> CanonicalQuestionBackfillStats:
    """Populate canonical_question conservatively from question text.

    This backfill is intentionally narrow:
    - generic headings stay empty
    - event questions normalize into `activity: <event_type>`
    - all other specific questions reuse the user-facing question text
    """
    conn = db.connect()
    stats = CanonicalQuestionBackfillStats()
    rows = conn.execute(
        "SELECT atom_id, question, canonical_question FROM atoms"
    ).fetchall()
    stats.total = len(rows)

    updates: list[tuple[str, str]] = []
    for row in rows:
        atom_id, question, canonical_question = row[0], row[1], row[2]
        if canonical_question and str(canonical_question).strip():
            stats.already_set += 1
            continue
        derived, source = derive_canonical_question(str(question or ""))
        if not derived:
            if source == "generic_heading":
                stats.skipped_generic += 1
            else:
                stats.skipped_blank += 1
            continue
        updates.append((derived, atom_id))
        if source == "activity_pattern":
            stats.from_activity_pattern += 1
        else:
            stats.from_question += 1

    if updates:
        conn.executemany(
            "UPDATE atoms SET canonical_question = ? WHERE atom_id = ?",
            updates,
        )
        conn.commit()

    logger.info(
        "canonical_question backfill: total=%d already=%d from_question=%d from_activity=%d blank=%d generic=%d",
        stats.total,
        stats.already_set,
        stats.from_question,
        stats.from_activity_pattern,
        stats.skipped_blank,
...
def build_chains(db: KnowledgeDB, *, apply_promotion_semantics: bool = True) -> ChainStats:
    ...
    When ``apply_promotion_semantics`` is false, this function only backfills
    chain metadata (`chain_id`, `chain_rank`, `is_chain_head`, `superseded_by`)
    and preserves existing promotion status / reason. This is the safe mode for
    live backfills on production data where promotion semantics are governed by
    dedicated promotion runners.
    """
    conn = db.connect()
    stats = ChainStats()
    ...
    if apply_promotion_semantics and stats.atoms_without_canonical > 0:
        conn.execute(
...
class P1Report:
    ...
    canonical_backfill: CanonicalQuestionBackfillStats | None = None
...
def run_p1_migration(db: KnowledgeDB) -> P1Report:
    ...
    # Step 2: backfill canonical_question where safe
    report.canonical_backfill = backfill_canonical_question(db)

    # Step 3: build chains & assign supersession
