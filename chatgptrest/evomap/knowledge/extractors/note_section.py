...
class NoteSectionExtractor(BaseExtractor):
    ...
    def __init__(self, db, source_dirs: list[str], source_label: str = "md",
        ...
        self.path_allowlist = {
            ...
            for path in (path_allowlist or [])
            if path
        }

    def _path_in_source_dirs(self, path: str) -> bool:
        real_path = os.path.realpath(path)
        if not self._normalized_source_dirs:
            return True
        for src_dir in self._normalized_source_dirs:
            if real_path == src_dir or real_path.startswith(f"{src_dir}{os.sep}"):
                return True
        return False

    def _find_md_files(self) -> list[str]:
        """Find all .md files in source directories."""
        if self.path_allowlist:
            files = []
            for path in sorted(self.path_allowlist):
                ...
                if os.path.basename(path).startswith("."):
                    continue
                if not self._path_in_source_dirs(path):
                    continue
                files.append(path)
            return files
