...
class GroundednessWeightProfile:
    name: str
    path: float
    service: float
    staleness: float
...
_GROUNDNESS_WEIGHT_PROFILES: dict[str, GroundednessWeightProfile] = {
    "standard": GroundednessWeightProfile(
        name="standard",
        path=0.40,
        service=0.20,
        staleness=0.15,
    ...
    "planning": GroundednessWeightProfile(
        name="planning",
        path=0.40,
        service=0.15,
        staleness=0.30,
    ...
    "code_procedure": GroundednessWeightProfile(
        name="code_procedure",
        path=0.40,
        service=0.20,
        staleness=0.15,
...
def infer_groundedness_weight_profile(
    *,
    scope_project: str = "",
    doc_source: str = "",
    atom_type: str = "",
    ...
    if profile in _GROUNDNESS_WEIGHT_PROFILES:
        return profile
    if str(scope_project or "").strip().lower() == "planning":
        return "planning"
    if str(doc_source or "").strip().lower() == "planning":
        return "planning"
    if str(atom_type or "").strip().lower() in {"procedure", "qa"}:
        return "code_procedure"
    return "standard"


def resolve_active_groundedness_weights(
    *,
    profile_name: str,
    has_paths: bool,
    has_units: bool,
    has_valid_from: bool,
    has_code_symbols: bool,
) -> dict[str, float]:
    profile = _GROUNDNESS_WEIGHT_PROFILES.get(profile_name, _GROUNDNESS_WEIGHT_PROFILES["standard"])
    return {
        "path": profile.path if has_paths else 0.0,
        "service": profile.service if has_units else 0.0,
        "staleness": profile.staleness if has_paths and has_valid_from else 0.0,
        "code_symbol": profile.code_symbol if has_code_symbols else 0.0,
    }


def weighted_groundedness_score(
    *,
    path_score: float,
    service_score: float,
    ...
    weights = resolve_active_groundedness_weights(
        profile_name=profile_name,
        has_paths=has_paths,
        has_units=has_units,
        has_valid_from=has_valid_from,
        has_code_symbols=has_code_symbols,
    )
    ...
    overall = (
        ...
        + weights["code_symbol"] * code_symbol_score
    ) / total_w
...
def check_atom_groundedness(
    ...
    *,
    ...
    result.weight_profile = infer_groundedness_weight_profile(
        scope_project=scope_project,
        doc_source=doc_source,
        atom_type=atom_type,
        explicit_profile=weight_profile,
    )
    result.overall, _ = weighted_groundedness_score(
        ...
        profile_name=result.weight_profile,
...
def enforce_promotion_gate(
    ...
    *,
    ...
    answer, valid_from = row["answer"], row["valid_from"]
    ...
    result = check_atom_groundedness(
        ...
        doc_source=str(row["doc_source"] or ""),
...
def run_p2_groundedness(db: KnowledgeDB) -> P2Stats:
    ...
    rows = conn.execute(
        ...
        LEFT JOIN episodes e ON e.episode_id = a.episode_id
        LEFT JOIN documents d ON d.doc_id = e.doc_id
        WHERE a.promotion_status IN ('candidate', 'staged')
        """
    ).fetchall()
    ...
    for row in rows:
        atom_id, answer, valid_from = row["atom_id"], row["answer"], row["valid_from"]
        result = check_atom_groundedness(
            atom_id,
            answer,
            valid_from,
