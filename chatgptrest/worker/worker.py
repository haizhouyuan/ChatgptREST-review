...
from chatgptrest.core.web_retry_budget import (
    BROWSER_RETRY_BUDGET_EXCEEDED_EVENT,
    BROWSER_RETRY_SCHEDULED_EVENT,
    evaluate_web_retry_budget,
)
from chatgptrest.core.web_retry_policy import human_visible_web_retry_after_seconds
from chatgptrest.core.prompt_policy import canonical_prompt_head, looks_like_synthetic_or_trivial_agent_prompt
from chatgptrest.core.repair_jobs import create_repair_autofix_job, source_job_uses_synthetic_or_trivial_prompt
from chatgptrest.core.rate_limit import (
    cooldown_wait_seconds,
    set_cooldown_until,
    try_reserve,
    try_reserve_fixed_window,
)
...
from chatgptrest.core.conversation_exports import (
    ...
    _common_prefix_len as _conversation_common_prefix_len,
...
from chatgptrest.core.completion_contract import is_research_contract_params
from chatgptrest.core.repair_gate import RepairGateDecision, classify_repair_gate
from chatgptrest.core.web_same_session_repair import (
    send_phase_requires_same_session_repair_without_thread,
)
...
_DEEP_RESEARCH_PROGRESS_STUB_RE = re.compile(
    r"(?is)"
    r"(?:"
    r"(?:^|\n)\s*update\s*(?:\n|$)[\s\S]{0,3500}(?:^|\n)\s*(?:researching|searching|browsing)\s*(?:\n|$)|"
    r"(?:^|\n)\s*(?:researching|searching|browsing)\s*(?:\n|$)[\s\S]{0,3500}(?:^|\n)\s*update\s*(?:\n|$)|"
    r"(?:^|\n)\s*(?:研究中|调研中|检索中|正在研究|正在调研|正在检索)\s*(?:\n|$)"
    r")"
)
...
_INFRA_ERROR_RE = re.compile(
    ...
    r"ws://127\.0\.0\.1:9222/|"
    r"Page\.goto: Page crashed|"
    r"Target page crashed|"
    r"Page crashed|"
...
def _retry_after_seconds_for_error(*, error_type: str, error: str) -> float:
    if _looks_like_infra_error(error_type, error):
        ...
        return human_visible_web_retry_after_seconds(base, max_seconds=3600.0)
    if _looks_like_ui_transient_error(error_type, error):
        base = float(_env_int("CHATGPTREST_UI_RETRY_AFTER_SECONDS", 90))
        return human_visible_web_retry_after_seconds(base, max_seconds=1800.0)
    return human_visible_web_retry_after_seconds(90.0, max_seconds=1800.0)
...
def _retry_after_seconds_for_wait_phase_error(
    *,
    ...
    if not _looks_like_thread_url(str(kind or ""), conversation_url):
        return default_wait
    if _looks_like_infra_error(error_type, error):
        base = float(_env_int("CHATGPTREST_WAIT_INFRA_RETRY_AFTER_SECONDS", 90))
        return human_visible_web_retry_after_seconds(base, max_seconds=300.0)
    if _looks_like_ui_transient_error(error_type, error):
        base = float(_env_int("CHATGPTREST_WAIT_UI_RETRY_AFTER_SECONDS", 90))
        return human_visible_web_retry_after_seconds(base, max_seconds=180.0)
    return default_wait
...
def _should_release_in_progress_web_job_to_wait(
    *,
    ...
    provider = str(provider_id_for_kind(str(kind or "")) or "").strip().lower()
    if provider == "gemini":
        error_type = str(meta.get("error_type") or "").strip().lower()
        if has_thread_url:
            return True
        if bool(meta.get("wait_handoff_ready")):
            return True
        if error_type in {"geminideepthinkresponsepending", "geminideepthinkthreadpending"}:
            return True
        return bool(meta.get("response_started"))
...
def _looks_like_transient_db_lock(exc: BaseException) -> bool:
...
def _should_worker_auto_autofix(*, kind: str, status: str, error_type: str, error: str) -> bool:
    ...
    if not gate.allow_codex_autofix or not gate.allow_repair_autofix_fallback:
        return False
    ...
    if status == "needs_followup":
        return et in {"waitnoprogresstimeout", "waitnothreadurltimeout", "webretrybudgetexceeded"} or et in pro_regen_error_types
    return _looks_like_infra_error(error_type, error) or _looks_like_ui_transient_error(error_type, error)


def _worker_autofix_gate_decision(
    *,
    kind: str,
    status: str,
    error_type: str,
...
def _worker_autofix_allow_actions(*, kind: str, status: str, error_type: str, error: str) -> list[str]:
    ...
    if status == "needs_followup":
        if et == "proinstantanswerneedsregenerate":
            ...
            allowed.append("restart_driver")
            return allowed
        if et == "webretrybudgetexceeded":
            if is_chatgpt:
                allowed.extend(["clear_blocked", "refresh"])
            allowed.append("restart_driver")
            return allowed
...
def _should_skip_blank_gemini_cooldown_autofix(
    *,
    kind: str,
    status: str,
    error_type: str,
    ...
    if not normalized_kind.startswith("gemini_web."):
        return False
    if str(status or "").strip().lower() != "cooldown":
        return False
    if _looks_like_thread_url(normalized_kind, conversation_url):
        return False
    combined = f"{str(error_type or '').strip().lower()} {str(error or '').strip().lower()}"
    return any(marker in combined for marker in ("timeout", "deadline exceeded", "empty error"))


_GEMINI_PRE_SEND_DEBUG_STEPS = frozenset({
    "open_page",
    "new_chat",
...
def _should_fail_closed_blank_gemini_send_cooldown(
    *,
    kind: str,
    status: str,
    phase: str,
    ...
    if str(phase or "").strip().lower() != "send":
        return False
    debug_step = str((meta or {}).get("debug_step") or "").strip().lower()
    if debug_step in _GEMINI_PRE_SEND_DEBUG_STEPS:
        return False
    return _should_skip_blank_gemini_cooldown_autofix(
        kind=kind,
        status=status,
        error_type=error_type,
        error=error,
...
def _should_fail_closed_chatgpt_send_same_session_repair(
    *,
    kind: str,
    phase: str,
    error_type: str,
    ...
    normalized_kind = str(kind or "").strip().lower()
    if not normalized_kind.startswith("chatgpt_web."):
        return False
    send_evidence = dict((meta or {}).get("send_phase_evidence") or {})
    recovered_conversation_url = str(send_evidence.get("recovered_conversation_url") or "").strip()
    if send_evidence.get("idempotency_sent") is False and not recovered_conversation_url:
        return False
...
async def _maybe_submit_worker_autofix(
    *,
    ...
    if _should_skip_blank_gemini_cooldown_autofix(
        kind=kind,
        status=status,
        error_type=error_type,
        error=error,
        ...
        payload = {
            ...
            "reason": "blank_gemini_cooldown_without_thread",
        }
        with connect(cfg.db_path) as conn:
            conn.execute("BEGIN IMMEDIATE")
            insert_event(conn, job_id=str(job_id), type="auto_autofix_skipped_blank_gemini_cooldown", payload=payload)
            conn.commit()
        artifacts.append_event(
            cfg.artifacts_dir,
            str(job_id),
            type="auto_autofix_skipped_blank_gemini_cooldown",
            payload=payload,
        )
        return

    gate = _worker_autofix_gate_decision(
        kind=kind,
        status=status,
        error_type=error_type,
        error=error,
    ...
    if not gate.allow_codex_autofix or not gate.allow_repair_autofix_fallback:
        ...
        with connect(cfg.db_path) as conn:
            conn.execute("BEGIN IMMEDIATE")
            insert_event(conn, job_id=str(job_id), type="auto_autofix_skipped_by_gate", payload=payload)
            conn.commit()
        artifacts.append_event(
            cfg.artifacts_dir,
            str(job_id),
            type="auto_autofix_skipped_by_gate",
            payload=payload,
        )
        return
...
def _deep_research_is_progress_stub(text: str) -> bool:
    ...
    if len(trimmed) > 5000:
        return False
    return bool(_DEEP_RESEARCH_PROGRESS_STUB_RE.search(trimmed))
...
def _deep_research_export_should_finalize(text: str) -> bool:
    ...
    if _deep_research_is_ack(trimmed):
        return False
    if _deep_research_is_progress_stub(trimmed):
        return False
...
def _deep_research_pending_wait_reason(text: str) -> str | None:
    trimmed = str(text or "").strip()
    if not trimmed:
        return None
    trimmed = _unwrap_response_envelope_text(trimmed)
    if _deep_research_is_ack(trimmed):
        return "deep_research_ack"
    if _deep_research_is_progress_stub(trimmed):
        return "deep_research_progress_stub"
    if len(trimmed) <= 2500 and ("implicit_link" in trimmed or "connector_openai_deep_research" in trimmed):
        return "deep_research_tool_stub"
    return None
...
def _should_reconcile_export_answer(
    *,
    ...
    quality = _classify_answer_quality(
        trimmed,
        answer_chars=len(trimmed),
        question_text=question,
    )
    if quality in {
        "suspect_meta_commentary",
        "suspect_context_acquisition_failure",
        "suspect_review_shallow_verdict",
    }:
...
def _should_downgrade_when_export_missing_reply(
    *,
    ...
    thinking_preset_requested: bool = False,
    ...
    export_guard = dict(export_guard_info or {})
    info: dict[str, Any] = {"threshold": threshold, "answer_chars": len(trimmed)}
    if export_guard:
        info["thread_contaminated_after_match"] = bool(export_guard.get("thread_contaminated_after_match"))
        info["next_role_after_match"] = export_guard.get("next_role_after_match")
        try:
            info["subsequent_user_turn_count"] = int(export_guard.get("subsequent_user_turn_count") or 0)
        except Exception:
            info["subsequent_user_turn_count"] = 0
    if bool(export_guard.get("thread_contaminated_after_match")):
        info["reason"] = "thread_contaminated_by_subsequent_user_turn"
        info["terminal_action"] = "needs_followup"
        return True, info
    if not trimmed:
    ...
    if len(trimmed) < threshold:
        ...
        return True, info
    answer_source = str(export_guard.get("answer_source") or "").strip().lower()
    if answer_source == "matched_but_missing_assistant":
        prompt_echo, prompt_echo_info = _looks_like_prompt_or_question_echo(
            current_answer=trimmed,
            question_text=question_text,
        )
        if prompt_echo:
            info.update(prompt_echo_info)
            info["reason"] = "matched_missing_assistant_prompt_echo"
            return True, info
        if (
            bool(thinking_preset_requested or deep_research_requested)
            and bool(export_guard.get("export_has_in_progress"))
            and str(export_guard.get("next_role_after_match") or "").strip().lower() == "assistant"
        ):
            current_quality = _classify_answer_quality(
                trimmed,
                answer_chars=len(trimmed),
                question_text=question_text or None,
            )
            info["current_answer_quality"] = current_quality
            info["reason"] = "matched_assistant_still_in_progress"
            return True, info
    if answer_source == "matched_in_progress_partial":
        if bool(export_guard.get("current_answer_matches_pre_match_assistant")):
            info["reason"] = "matched_in_progress_partial_current_answer_is_previous_assistant"
            for key in (
                "pre_match_assistant_match_kind",
                "pre_match_assistant_index",
                "pre_match_assistant_chars",
                "pre_match_assistant_common_prefix_len",
            ):
                if key in export_guard:
                    info[key] = export_guard.get(key)
            return True, info
        current_quality = _classify_answer_quality(
            trimmed,
            answer_chars=len(trimmed),
        ...
        info["current_answer_quality"] = current_quality
        if current_quality != "final":
            info["reason"] = "matched_in_progress_partial_current_answer_not_final"
            return True, info
        prompt_echo, prompt_echo_info = _looks_like_prompt_or_question_echo(
            current_answer=trimmed,
            question_text=question_text,
        ...
        if prompt_echo:
            info.update(prompt_echo_info)
            info["reason"] = "matched_in_progress_partial_prompt_echo"
            return True, info
    info["reason"] = "answer_sufficient"
    return False, info


def _current_answer_matches_pre_match_assistant(
    *,
    current_answer: str,
    export_obj: dict[str, Any] | None,
    matched_user_index: Any,
    question_text: str = "",
) -> tuple[bool, dict[str, Any]]:
    """
    Detect a driver/DOM extraction that returned an assistant turn before the matched user prompt.

    This closes the follow-up stale-answer hole where the conversation export correctly anchors the
    current user turn but has only an in-progress/partial reply, while the DOM extractor returns a
    long, final-looking answer from an older turn.
    """
    info: dict[str, Any] = {"current_answer_matches_pre_match_assistant": False}
    raw_norm = _normalize_text(current_answer)
    if not raw_norm:
        return False, info
    if not isinstance(export_obj, dict):
        info["pre_match_assistant_check_skipped"] = "missing_export_obj"
        return False, info
    if not isinstance(matched_user_index, int) or matched_user_index <= 0:
        info["pre_match_assistant_check_skipped"] = "missing_matched_user_index"
        return False, info

    try:
        messages = _conversation_export_messages(
            export_obj,
            include_roles={"user", "assistant"},
            include_hidden=False,
        )
    except Exception:
        info["pre_match_assistant_check_skipped"] = "messages_unavailable"
        return False, info

    for idx, message in enumerate(messages[:matched_user_index]):
        if not isinstance(message, dict):
            continue
        if str(message.get("role") or "").strip().lower() != "assistant":
            continue
        prev_norm = _normalize_text(str(message.get("text") or ""))
        if not prev_norm:
            continue

        match_kind: str | None = None
        common_prefix_len = _conversation_common_prefix_len(prev_norm, raw_norm, 400)
        if raw_norm == prev_norm:
            match_kind = "exact"
        elif len(raw_norm) >= 200 and raw_norm in prev_norm:
            match_kind = "current_answer_inside_previous_assistant"
        elif len(prev_norm) >= 200 and prev_norm in raw_norm:
            match_kind = "previous_assistant_inside_current_answer"
        elif common_prefix_len >= 200:
            match_kind = "long_common_prefix"

        if match_kind:
            info.update(
                {
                    "current_answer_matches_pre_match_assistant": True,
                    "pre_match_assistant_match_kind": match_kind,
                    "pre_match_assistant_index": idx,
                    "pre_match_assistant_chars": len(prev_norm),
                    "pre_match_assistant_common_prefix_len": common_prefix_len,
                }
            )
            return True, info
    ...
    if prompt_echo:
        info.update(prompt_echo_info)
        info["current_answer_matches_question_prompt"] = True
        return True, info

    return False, info


def _looks_like_prompt_or_question_echo(
    *,
    current_answer: str,
    question_text: str,
) -> tuple[bool, dict[str, Any]]:
    answer = _normalize_text(current_answer)
    ...
    if answer == question:
        info["prompt_echo_match"] = "exact"
        return True, info

    if len(answer) >= 200 and answer in question:
        info["prompt_echo_match"] = "answer_in_question"
        return True, info

    question_prefix = question[: min(len(question), max(400, len(answer)))]
    if len(answer) >= 200 and question_prefix.startswith(answer):
        info["prompt_echo_match"] = "question_prefix"
        return True, info

    common_prefix_len = 0
    limit = min(len(answer), len(question), 4000)
    while common_prefix_len < limit and answer[common_prefix_len] == question[common_prefix_len]:
        common_prefix_len += 1
    info["prompt_echo_common_prefix_len"] = common_prefix_len
    ...
    marker_count = sum(1 for marker in section_markers if marker in answer)
    info["prompt_echo_marker_count"] = marker_count

    if common_prefix_len >= max(200, int(min(len(answer), len(question)) * 0.6)):
        info["prompt_echo_match"] = "long_common_prefix"
        return True, info

    if marker_count >= 4 and common_prefix_len >= 120:
        info["prompt_echo_match"] = "compiled_prompt_scaffold"
        return True, info
...
_WAIT_ACTIVE_FINALIZATION_NO_PROGRESS_TIMEOUT_SECONDS = 45 * 60
_WAIT_NO_PROGRESS_NO_THREAD_URL_TIMEOUT_SECONDS = 30 * 60
_WAIT_NO_PROGRESS_RETRY_AFTER_SECONDS = 10 * 60
_WAIT_PARTIAL_ANSWER_PROGRESS_MIN_CHARS = 80
_WAIT_ACTIVE_FINALIZATION_PROGRESS_EPSILON = 0.1
_CHATGPT_MAX_FILES_PER_PROMPT = 20
_CHATGPT_WAIT_REGENERATE_MAX_ATTEMPTS = 1
_CHATGPT_WAIT_REGENERATE_REQUEUE_SECONDS = 30
_CHATGPT_WAIT_REGENERATE_TOOL_TIMEOUT_SECONDS = 120
_WAIT_NO_PROGRESS_NON_PROGRESS_EVENT_TYPES = frozenset(
    {
        ...
        "lease_renewed",
        "wait_requeued",
        # Retry scheduling is orchestration noise, not evidence that the wait-phase answer progressed.
        BROWSER_RETRY_SCHEDULED_EVENT,
        ...
        # Active-finalization observation is useful for retry-budget accounting, but repeated sightings
        # of the same in-progress tool node must not reset the long-tail no-progress timeout.
        "wait_active_finalization_observed",
        # Prompt echoes are evidence of a bad/stalled extraction state, not assistant progress.
        "wait_partial_answer_echo_observed",
        # Deep Research pending sightings are orchestration state. The first visible status card
        # is still tracked by wait_partial_answer_progressed; repeated pending sightings must not
        # hide a truly stalled research run.
        "wait_deep_research_pending_observed",
        # Per-poll observability only; emitting timing telemetry should not hide a stalled wait job.
        "worker_timing",
    }
)
_SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_THRESHOLD = 8
_SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_MAX_AGE_SECONDS = 20 * 60


def _suspicious_short_answer_loop_breaker_threshold() -> int:
    return max(0, int(_env_int("CHATGPTREST_SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_THRESHOLD", _SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_THRESHOLD)))


def _suspicious_short_answer_loop_breaker_max_age_seconds() -> float:
    return max(0.0, float(_env_int("CHATGPTREST_SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_MAX_AGE_SECONDS", _SUSPICIOUS_SHORT_ANSWER_LOOP_BREAKER_MAX_AGE_SECONDS)))
...
def _min_chars_guard_should_complete_under_min_chars(
    *,
    ...
    complete_final: bool = False,
    trivial_prompt: bool = False,
    research_contract: bool = False,
    ...
    details = {
        ...
        "complete_final": bool(complete_final),
    }

    if trivial_prompt and answer_chars > 0:
        details["decision_reason"] = "trivial_prompt_short_answer"
        return True, details
    if slack > 0 and missing <= slack:
        details["decision_reason"] = "near_miss"
        return True, details
    if semantically_final and (not deep_research) and (not thinking_preset) and (not research_contract):
        ...
        if answer_chars >= semantic_threshold:
            ...
            return True, details
    if research_contract and semantically_final and complete_final and answer_chars > 0:
        details["decision_reason"] = "semantic_final_under_min_chars"
        details["research_contract_blocked"] = True
        details["terminal_action"] = "needs_followup"
...
def _completion_guard_retry_after_seconds() -> float:
    return human_visible_web_retry_after_seconds(float(_MIN_CHARS_GUARD_RETRY_BASE_SECONDS), max_seconds=1800.0)


def _deep_research_pending_retry_after_seconds() -> float:
    raw = _env_int("CHATGPTREST_DEEP_RESEARCH_PENDING_RETRY_AFTER_SECONDS", 10 * 60)
    return max(60.0, min(float(raw), 60.0 * 60.0))


def _legacy_trivial_wait_loop_breaker_threshold() -> int:
    return max(0, int(_env_int("CHATGPTREST_LEGACY_TRIVIAL_WAIT_LOOP_BREAKER_THRESHOLD", 3)))


def _looks_like_legacy_trivial_wait_prompt(*, question_text: str, export_guard_info: dict[str, Any] | None) -> bool:
    head = canonical_prompt_head(question_text)
    if looks_like_synthetic_or_trivial_agent_prompt(head):
        return True
    export_guard = dict(export_guard_info or {})
    matched_preview = canonical_prompt_head(str(export_guard.get("matched_user_text_preview") or ""))
    if matched_preview and looks_like_synthetic_or_trivial_agent_prompt(matched_preview):
        return True
...
def _legacy_trivial_wait_loop_should_finalize(
    *,
    conn: Any,
    job_id: str,
    question_text: str,
    export_guard_info: dict[str, Any] | None,
    downgrade_reason: str,
) -> tuple[bool, dict[str, Any]]:
    if str(downgrade_reason or "") != "answer_quality_suspect_short_answer":
        return False, {}
    threshold = _legacy_trivial_wait_loop_breaker_threshold()
    if threshold <= 0:
        return False, {}
    if not _looks_like_legacy_trivial_wait_prompt(question_text=question_text, export_guard_info=export_guard_info):
        return False, {}
    try:
        row = conn.execute(
            """
            ...
            FROM job_events
            WHERE job_id = ?
              AND type = 'completion_guard_downgraded'
              AND coalesce(json_extract(payload_json, '$.reason'), '') = 'answer_quality_suspect_short_answer'
            """,
            (str(job_id),),
        ).fetchone()
        prior_count = int((row["c"] if row is not None else 0) or 0)
    except Exception:
        prior_count = 0
    should_finalize = (prior_count + 1) >= threshold
    return should_finalize, {
        "decision_reason": "legacy_trivial_wait_loop_breaker",
        "prior_downgrade_count": prior_count,
        "threshold": threshold,
        "prompt_head": canonical_prompt_head(question_text)[:120],
    }


def _suspicious_short_answer_loop_should_fail_closed(
    *,
    conn: Any,
    job_id: str,
    question_text: str,
    ...
    answer_chars: int,
    deep_research: bool,
    thinking_preset: bool,
) -> tuple[bool, dict[str, Any]]:
    if str(downgrade_reason or "") != "answer_quality_suspect_short_answer":
        return False, {}
    if deep_research or thinking_preset:
        return False, {}
    if looks_like_synthetic_or_trivial_agent_prompt(canonical_prompt_head(question_text)):
        return False, {}
    threshold = _suspicious_short_answer_loop_breaker_threshold()
    if threshold <= 0:
        return False, {}
    max_age_seconds = _suspicious_short_answer_loop_breaker_max_age_seconds()
    try:
        rows = conn.execute(
            """
            SELECT ts, payload_json
    ...
    for row in rows:
        ...
        if isinstance(payload, dict):
            try:
                previous_max_answer_chars = max(previous_max_answer_chars, int(payload.get("answer_chars") or 0))
            except Exception:
                pass
    now = float(time.time())
    age_seconds = float(now - first_ts) if first_ts is not None else 0.0
    triggered_by_threshold = (prior_count + 1) >= threshold
    triggered_by_age = bool(first_ts is not None and max_age_seconds > 0 and age_seconds >= max_age_seconds)
    if not (triggered_by_threshold or triggered_by_age):
        return False, {}
    if int(answer_chars) > int(previous_max_answer_chars):
        return False, {
            "decision_reason": "answer_chars_improving",
            "prior_downgrade_count": prior_count,
            "threshold": threshold,
            "previous_max_answer_chars": previous_max_answer_chars,
    ...
    return True, {
        "decision_reason": "suspicious_short_answer_loop_breaker",
        "prior_downgrade_count": prior_count,
        "threshold": threshold,
        "previous_max_answer_chars": previous_max_answer_chars,
        "answer_chars": int(answer_chars),
        "age_seconds": round(age_seconds, 3),
        "max_age_seconds": max_age_seconds,
        "triggered_by_threshold": triggered_by_threshold,
        "triggered_by_age": triggered_by_age,
        "prompt_head": canonical_prompt_head(question_text)[:160],
    }
...
def _wait_active_finalization_no_progress_timeout_seconds() -> float:
    raw = _env_int(
        "CHATGPTREST_WAIT_ACTIVE_FINALIZATION_NO_PROGRESS_TIMEOUT_SECONDS",
        _WAIT_ACTIVE_FINALIZATION_NO_PROGRESS_TIMEOUT_SECONDS,
    )
    return max(0.0, float(raw))
...
def _browser_retry_budget_payload(
    *,
    snapshot: Any,
    kind: str | None,
    phase: str,
    ...
    payload = dict(snapshot.as_payload() if snapshot is not None else {})
    payload.update(
        {
            ...
            "error_preview": (str(error or "").strip()[:240] or None),
...
def _store_browser_retry_or_fail_closed(
    conn: Any,
    *,
    artifacts_dir: Path,
    job: Any,
    ...
    error_type: str | None,
    error: str | None,
    release_wait: bool = False,
) -> tuple[str, str | None, str | None]:
    if not is_provider_web_kind(job.kind):
        if release_wait:
            release_for_wait(
                conn,
            ...
            return JobStatus.IN_PROGRESS.value, error_type, error
        ...
        return status.value, error_type, error

    snapshot = evaluate_web_retry_budget(conn, job_id=job.job_id, kind=job.kind)
    retry_status = "wait" if release_wait else (status.value if status is not None else "retry")
    event_payload = _browser_retry_budget_payload(
        snapshot=snapshot,
        kind=job.kind,
        phase=phase,
        status=retry_status,
    ...
    if snapshot.exceeded:
        insert_event(
            conn,
            job_id=job.job_id,
            type=BROWSER_RETRY_BUDGET_EXCEEDED_EVENT,
        ...
        fail_closed_error_type = "WebRetryBudgetExceeded"
        provider_label = str(snapshot.provider_id or provider_id_for_kind(job.kind) or "web")
        fail_closed_error = (
            f"{provider_label} browser-visible retry budget exceeded within {snapshot.window_seconds}s "
            f"(job={snapshot.recent_job_retry_count}/{snapshot.max_per_job}, "
            f"provider={snapshot.recent_provider_retry_count}/{snapshot.max_per_provider}); "
            "fail closed to avoid non-human retry cadence and provider rate-limit escalation"
        )
        store_error_result(
            conn,
            artifacts_dir=artifacts_dir,
            ...
            worker_id=worker_id,
            lease_token=lease_token,
            error_type=fail_closed_error_type,
            error=fail_closed_error,
            status=JobStatus.NEEDS_FOLLOWUP,
        )
        return JobStatus.NEEDS_FOLLOWUP.value, fail_closed_error_type, fail_closed_error
    ...
    insert_event(
        conn,
        job_id=job.job_id,
        type=BROWSER_RETRY_SCHEDULED_EVENT,
        payload=event_payload,
    )
    artifacts.append_event(
        artifacts_dir,
    ...
    return (JobStatus.IN_PROGRESS.value if release_wait else status.value), error_type, error
...
def _latest_job_event_payload(
    ...
    *,
    ...
    row = conn.execute(
        ...
        LIMIT 1
        """,
        (job_id, str(event_type or "").strip()),
    ).fetchone()
    if row is None:
        return {}
    return _safe_payload_obj(row[0])


def _latest_wait_active_finalization_event(
    conn: Any,
    *,
    job_id: str,
) -> dict[str, Any]:
    row = conn.execute(
        ...
        FROM job_events
        WHERE job_id = ?
          AND type IN ('wait_active_finalization_progressed', 'wait_active_finalization_observed')
        ORDER BY id DESC
        LIMIT 1
        """,
        (job_id,),
    ...
    payload["_event_ts"] = row[0]
    payload["_event_type"] = row[1]
    return payload


def _wait_active_finalization_is_live(payload: dict[str, Any]) -> bool:
    ...
    try:
        ...
        active_count = 0
    try:
        in_progress_count = int(payload.get("in_progress_count") or 0)
    except Exception:
        in_progress_count = 0
    return active_count > 0 or in_progress_count > 0


def _wait_partial_answer_progress_payload(
    *,
    conn: Any,
    job_id: str,
    answer_text: str,
    ...
    preview = _normalize_text(_unwrap_response_envelope_text(str(answer_text or "")))
    if len(preview) < _WAIT_PARTIAL_ANSWER_PROGRESS_MIN_CHARS:
        return None

    prompt_echo, prompt_echo_info = _looks_like_prompt_or_question_echo(
        current_answer=preview,
        question_text=question_text,
    )
    event_type = "wait_partial_answer_echo_observed" if prompt_echo else "wait_partial_answer_progressed"
    previous = _latest_job_event_payload(conn, job_id=job_id, event_type=event_type)
    previous_preview = _normalize_text(str(previous.get("preview") or ""))
    previous_chars = 0
    try:
        previous_chars = int(previous.get("answer_chars") or 0)
    except Exception:
        previous_chars = 0

    answer_chars = len(preview)
    current_preview_head = preview[:240]
    if previous_preview == current_preview_head and answer_chars <= previous_chars:
        return None

    payload: dict[str, Any] = {
        "answer_chars": int(answer_chars),
        "answer_quality": _classify_answer_quality(preview, answer_chars=answer_chars),
        "preview": current_preview_head,
        "_event_type": event_type,
    }
    if prompt_echo:
        payload["prompt_echo"] = True
        payload.update(prompt_echo_info)
    if previous_preview:
        payload["previous_answer_chars"] = int(previous_chars)
        payload["changed"] = bool(previous_preview != current_preview_head)
        payload["delta_chars"] = int(answer_chars - previous_chars)
    else:
        payload["changed"] = True
    return payload


def _wait_active_finalization_progress_payload(
    *,
    conn: Any,
    job_id: str,
    ...
    if not isinstance(details, dict) or not details:
        return None
    raw_progress = details.get("best_progress")
    if not isinstance(raw_progress, (int, float)):
        return None
    progress = float(raw_progress)

    previous = _latest_job_event_payload(conn, job_id=job_id, event_type="wait_active_finalization_progressed")
    previous_progress = previous.get("best_progress")
    try:
        previous_progress_f = float(previous_progress) if previous_progress is not None else None
    except Exception:
        previous_progress_f = None

    if previous_progress_f is not None and progress <= (previous_progress_f + _WAIT_ACTIVE_FINALIZATION_PROGRESS_EPSILON):
        return None

    payload = {
        "in_progress_count": int(details.get("in_progress_count") or 0),
        "active_count": int(details.get("active_count") or 0),
        "best_progress": progress,
        "best_role": details.get("best_role"),
        "best_channel": details.get("best_channel"),
        "best_content_type": details.get("best_content_type"),
    }
    if previous_progress_f is not None:
        payload["previous_best_progress"] = previous_progress_f
        payload["progress_delta"] = round(progress - previous_progress_f, 6)
    return payload


def _record_wait_progress_signals(
    conn: Any,
    *,
    artifacts_dir: Path,
    job_id: str,
    current_answer: str,
    active_finalization_details: dict[str, Any] | None,
    question_text: str = "",
) -> list[str]:
    recorded: list[str] = []

    partial_payload = _wait_partial_answer_progress_payload(
        conn=conn,
        job_id=job_id,
    ...
    if partial_payload is not None:
        partial_event_type = str(partial_payload.pop("_event_type", "") or "wait_partial_answer_progressed")
        insert_event(conn, job_id=job_id, type=partial_event_type, payload=partial_payload)
        artifacts.append_event(
            artifacts_dir,
            job_id,
            type=partial_event_type,
            payload=partial_payload,
        )
        recorded.append(partial_event_type)

    finalization_payload = _wait_active_finalization_progress_payload(
        conn=conn,
        job_id=job_id,
        details=active_finalization_details,
    )
    if finalization_payload is not None:
        insert_event(conn, job_id=job_id, type="wait_active_finalization_progressed", payload=finalization_payload)
        artifacts.append_event(
            artifacts_dir,
            job_id,
            type="wait_active_finalization_progressed",
            payload=finalization_payload,
        )
        recorded.append("wait_active_finalization_progressed")
    elif active_finalization_details is None:
        previous_finalization = _latest_wait_active_finalization_event(conn, job_id=job_id)
        if _wait_active_finalization_is_live(previous_finalization):
            clear_payload = {
                "in_progress_count": 0,
                "active_count": 0,
                "best_progress": None,
                ...
                "cleared": True,
                "previous_event_type": previous_finalization.get("_event_type"),
                "previous_event_ts": previous_finalization.get("_event_ts"),
                "previous_best_progress": previous_finalization.get("best_progress"),
            }
            insert_event(conn, job_id=job_id, type="wait_active_finalization_observed", payload=clear_payload)
            artifacts.append_event(
                artifacts_dir,
                job_id,
                type="wait_active_finalization_observed",
                payload=clear_payload,
            )
            recorded.append("wait_active_finalization_cleared")

    return recorded


def _wait_regenerate_attempt_count(
    conn: Any,
    *,
    job_id: str,
) -> int:
    row = conn.execute(
        "SELECT COUNT(1) AS n FROM job_events WHERE job_id = ? AND type = ?",
        (job_id, "wait_regenerate_requested"),
    ).fetchone()
    if row is None:
        return 0
    try:
...
async def _maybe_regenerate_chatgpt_wait_stall(
    *,
    conn: Any,
    artifacts_dir: Path,
    job: Any,
    ...
    current_answer: str,
    timeout_decision: dict[str, Any],
) -> bool:
    if tool_caller is None:
        return False
    if str(job.kind or "").strip().lower() != "chatgpt_web.ask":
        return False
    if str((timeout_decision.get("payload") or {}).get("reason") or "") != "no_progress":
        return False
    if not _looks_like_thread_url(str(job.kind or ""), conversation_url):
        return False
    preset = str((params_obj or {}).get("preset") or "").strip().lower()
    if preset not in _THINKING_PRESETS:
        return False
    attempts = _wait_regenerate_attempt_count(conn, job_id=str(job.job_id))
    if attempts >= _CHATGPT_WAIT_REGENERATE_MAX_ATTEMPTS:
        return False

    answer_text = str(current_answer or "").strip()
    answer_quality = (
        _classify_answer_quality(answer_text, answer_chars=len(answer_text))
        if answer_text
        else "missing"
    )
    min_chars_required = max(0, int((params_obj or {}).get("min_chars") or 0))
    if answer_quality == "final" and len(answer_text) >= max(200, min_chars_required):
        return False

    tool_timeout = max(
        60,
        min(
            _CHATGPT_WAIT_REGENERATE_TOOL_TIMEOUT_SECONDS,
            int((params_obj or {}).get("wait_timeout_seconds") or _CHATGPT_WAIT_REGENERATE_TOOL_TIMEOUT_SECONDS),
        ),
    )
    payload: dict[str, Any] = {
        "attempt_index": int(attempts + 1),
        "phase": "wait",
        "reason": "wait_no_progress_suspect_partial_answer",
    ...
    try:
        regen = await asyncio.to_thread(
            ...
            tool_args={
                ...
                "min_chars": int(min_chars_required),
            },
            timeout_sec=float(tool_timeout) + 60.0,
        )
    except Exception as exc:
        payload["result_status"] = "error"
        payload["error_type"] = type(exc).__name__
        payload["error"] = str(exc)[:800]
        insert_event(conn, job_id=job.job_id, type="wait_regenerate_failed", payload=payload)
        artifacts.append_event(artifacts_dir, job.job_id, type="wait_regenerate_failed", payload=payload)
        return False

    regen_status = str((regen or {}).get("status") or "").strip().lower() if isinstance(regen, dict) else ""
    payload["result_status"] = regen_status or None
    if isinstance(regen, dict):
        if regen.get("conversation_url"):
            payload["result_conversation_url"] = str(regen.get("conversation_url") or "").strip()
        if regen.get("error_type"):
            payload["error_type"] = str(regen.get("error_type") or "")
        if regen.get("error"):
            payload["error"] = str(regen.get("error") or "")[:800]
    insert_event(conn, job_id=job.job_id, type="wait_regenerate_requested", payload=payload)
    artifacts.append_event(artifacts_dir, job.job_id, type="wait_regenerate_requested", payload=payload)
    if regen_status not in {"completed", "in_progress"}:
        return False
    ...
    release_for_wait(
        ...
        worker_id=worker_id,
        lease_token=lease_token,
        not_before=float(time.time() + _CHATGPT_WAIT_REGENERATE_REQUEUE_SECONDS),
    )
    return True
...
def _wait_no_progress_timeout_decision(
    *,
    ...
    preset = str((params or {}).get("preset") or "").strip().lower()
    thinking_preset = bool(preset in _THINKING_PRESETS)
    active_finalization_payload = _latest_wait_active_finalization_event(conn, job_id=str(job.job_id))
    active_finalization_live = bool(thinking_preset and _wait_active_finalization_is_live(active_finalization_payload))
    active_finalization_timeout_seconds = _wait_active_finalization_no_progress_timeout_seconds()
    ...
    if (
        reason == "no_progress"
        and active_finalization_live
        and active_finalization_timeout_seconds > effective_timeout_seconds
    ):
        effective_timeout_seconds = active_finalization_timeout_seconds
        if age_seconds < effective_timeout_seconds:
            return None
    ...
    if active_finalization_live:
        payload["active_finalization_grace_applied"] = bool(
            active_finalization_timeout_seconds >= effective_timeout_seconds
        )
        payload["active_finalization_timeout_seconds"] = round(float(active_finalization_timeout_seconds), 3)
        payload["active_finalization_event_type"] = active_finalization_payload.get("_event_type")
        payload["active_finalization_event_ts"] = active_finalization_payload.get("_event_ts")
        payload["active_finalization_best_progress"] = active_finalization_payload.get("best_progress")
...
def _render_attachment_bundle_section(
    *,
    file_name: str,
    member_bytes: int,
    included_bytes: int,
...
def _read_attachment_bundle_text(
    *,
    path: Path,
    max_bytes: int,
) -> tuple[str | None, str, int, int, bool]:
    suffix = path.suffix.lower()
    if suffix not in _ZIP_BUNDLE_ALLOWED_SUFFIXES:
        return None, "unsupported_suffix", 0, 0, False
    try:
    ...
    try:
        ...
        return None, "read_failed", stat_size, 0, False
    if b"\x00" in blob:
        return None, "binary_detected", stat_size or len(blob), 0, False
    original_bytes = len(blob)
    clipped = blob[: int(max_bytes)]
    truncated = len(clipped) < original_bytes
    try:
        text = clipped.decode("utf-8", errors="replace").strip()
    except Exception:
        return None, "decode_failed", stat_size or original_bytes, len(clipped), truncated
    if not text:
        return None, "empty_text", stat_size or original_bytes, len(clipped), truncated
    return text, "ok", stat_size or original_bytes, len(clipped), truncated


def _maybe_bundle_many_attachments_for_chatgpt(
    *,
    artifacts_dir: Path,
    job_id: str,
    file_paths: list[str],
    ...
    if len(uniq) <= _CHATGPT_MAX_FILES_PER_PROMPT:
        return file_paths, None

    max_files = _CHATGPT_MAX_FILES_PER_PROMPT
    ...
    bundle_path = inputs_dir / "CHATGPT_ATTACH_BUNDLE.md"
    index_path = inputs_dir / "CHATGPT_ATTACH_INDEX.md"
    ...
    for raw in uniq:
        path = Path(raw)
        text, reason, member_bytes, included_bytes, truncated = _read_attachment_bundle_text(
            path=path,
        ...
        if text is None:
            ...
            continue
        section = _render_attachment_bundle_section(
            file_name=path.name,
        ...
        if bundle_bytes + section_bytes > bundle_max_bytes:
            passthrough.append(raw)
            skipped.append({"file": path.name, "reason": "bundle_size_cap"})
            continue
        bundle_parts.append(section)
        bundle_bytes += section_bytes
        bundled_names.append(path.name)

    if not bundle_parts:
        return file_paths, {
            "ok": False,
            "reason": "attachment_bundle_empty",
            "requested_count": int(len(uniq)),
            "max_files_per_prompt": int(max_files),
        }

    header = (
        "# CHATGPT_ATTACH_BUNDLE（ChatgptREST 自动生成）\n\n"
        "由于当前请求附件数量超过 ChatGPT composer 的稳定上限，"
        "ChatgptREST 已将可读文本类附件合并为本 bundle，供模型统一阅读。\n\n"
    )
    bundle_path.write_text(header + "".join(bundle_parts), encoding="utf-8", errors="replace")

    final_paths = list(passthrough)
    if len(final_paths) >= max_files:
        final_paths = final_paths[: max_files - 2]
    final_paths.append(bundle_path.as_posix())

    lines: list[str] = []
    lines.append("# CHATGPT_ATTACH_INDEX（ChatgptREST 自动生成）")
    lines.append("")
    lines.append(f"- requested_files: `{len(uniq)}`")
    lines.append(f"- upload_limit: `{max_files}`")
    lines.append(f"- bundled_files: `{len(bundled_names)}`")
    lines.append(f"- passthrough_files: `{len(final_paths) - 1}`")
    lines.append("")
    lines.append("## Bundled into CHATGPT_ATTACH_BUNDLE.md")
    for name in bundled_names:
        lines.append(f"- {name}")
    if final_paths[:-1]:
        lines.append("")
        lines.append("## Uploaded separately")
        for raw in final_paths[:-1]:
            lines.append(f"- {Path(raw).name}")
    if skipped:
        lines.append("")
        lines.append("## Bundle skip reasons")
        for item in skipped[:200]:
            lines.append(f"- {item.get('file')}: {item.get('reason')}")
    index_path.write_text("\n".join(lines) + "\n", encoding="utf-8", errors="replace")

    if len(final_paths) >= max_files:
        final_paths = final_paths[: max_files - 1]
    final_paths.append(index_path.as_posix())

    info = {
        "ok": True,
        "reason": "attachment_count_cap",
        "requested_count": int(len(uniq)),
        "max_files_per_prompt": int(max_files),
        "bundled_count": int(len(bundled_names)),
        "passthrough_count": int(max(0, len(final_paths) - 2)),
        "generated_files": [bundle_path.name, index_path.name],
        "bundled_files": bundled_names[:100],
        "skipped": skipped[:100],
...
def _conversation_export_active_finalization_details(*, obj: dict[str, Any]) -> dict[str, Any] | None:
    mapping = obj.get("mapping")
    ...
    for node in mapping.values():
        ...
        if not isinstance(raw_create_time, (int, float)):
            continue
        create_time = float(raw_create_time)
        if latest_user_create_time is None or create_time > latest_user_create_time:
            latest_user_create_time = create_time
    ...
    for node in mapping.values():
        ...
        if not isinstance(msg, dict):
            continue
        if str(msg.get("status") or "").strip().lower() != "in_progress":
            continue
        raw_create_time = msg.get("create_time")
        if (
            latest_user_create_time is not None
            and isinstance(raw_create_time, (int, float))
            and float(raw_create_time) < latest_user_create_time
        ):
            continue
        in_progress_count += 1
        meta = msg.get("metadata")
        if not isinstance(meta, dict):
            continue
        progress = meta.get("pro_progress")
        is_finalizing = bool(meta.get("is_finalizing"))
        has_progress = isinstance(progress, (int, float))
        if not is_finalizing and not has_progress:
            continue
        active_count += 1
        progress_val = float(progress) if has_progress else None
        if progress_val is not None and (best_progress is None or progress_val > best_progress):
            best_progress = progress_val
            author = msg.get("author")
            best_role = str((author or {}).get("role") or "").strip().lower() or None if isinstance(author, dict) else None
            best_channel = str(msg.get("channel") or "").strip() or None
            content = msg.get("content")
            best_content_type = (
                str((content or {}).get("content_type") or "").strip().lower() or None if isinstance(content, dict) else None
            )
        elif best_progress is None and is_finalizing:
            author = msg.get("author")
            best_role = str((author or {}).get("role") or "").strip().lower() or None if isinstance(author, dict) else None
            best_channel = str(msg.get("channel") or "").strip() or None
            content = msg.get("content")
...
async def _choose_chatgpt_current_answer_candidate(
    *,
    tool_caller: ToolCaller | None,
    result: ExecutorResult,
    current_answer: str,
) -> tuple[str, str, dict[str, Any]]:
    chosen = str(current_answer or "")
    source = "current_answer"
    details: dict[str, Any] = {}
    meta = dict(getattr(result, "meta", None) or {})
    if tool_caller is None or not bool(meta.get("answer_saved")):
        return chosen, source, details
    ...
    if not chosen.strip() or _should_prefer_conversation_answer(candidate=rehydrated, current=chosen):
        chosen = rehydrated
        source = "answer_id"
        details["answer_rehydrated"] = True
        details["answer_rehydrated_chars"] = len(rehydrated)
    return chosen, source, details
...
_CONVERSATION_EXPORT_429_COOLDOWN_KEY = "chatgpt_web_conversation_export:backend_429"


def _conversation_export_backend_status(export_res: Any) -> int | None:
    if not isinstance(export_res, dict):
        return None
    for key in ("backend_status", "status_code", "http_status"):
        value = export_res.get(key)
        try:
...
def _conversation_export_result_is_backend_rate_limited(export_res: Any) -> bool:
    status = _conversation_export_backend_status(export_res)
    if status == 429:
        return True
    if not isinstance(export_res, dict):
        return False
    text = " ".join(
        str(export_res.get(key) or "")
        for key in ("error", "backend_error", "message", "detail")
    ).lower()
    return "http 429" in text or "too many requests" in text or "rate limit" in text


def _conversation_export_429_cooldown_seconds(export_res: Any) -> float:
    base = max(60, _env_int("CHATGPTREST_CONVERSATION_EXPORT_429_COOLDOWN_SECONDS", 600))
    max_seconds = max(base, _env_int("CHATGPTREST_CONVERSATION_EXPORT_429_COOLDOWN_MAX_SECONDS", 3600))
    retry_after = None
    ...
    try:
        ...
        retry_after_s = 0.0
    if retry_after_s > 0:
        return min(float(max_seconds), max(float(base), retry_after_s))
    return human_visible_web_retry_after_seconds(float(base), max_seconds=float(max_seconds))
...
async def _maybe_export_conversation(
    *,
    ...
    dom_during_429_cooldown = _truthy_env("CHATGPTREST_CONVERSATION_EXPORT_DOM_DURING_429_COOLDOWN", True)
    dom_only_due_to_429_cooldown = False
    ...
    if cooldown_until > 0 and now < cooldown_until:
        local_backend_429 = str(state.get("last_error_type") or "").strip() == "ConversationExportBackendRateLimited"
        if force and dom_during_429_cooldown and local_backend_429:
            dom_only_due_to_429_cooldown = True
        else:
            return
    ...
    def _record_backend_skipped_for_dom_only(*, wait_seconds: float, reason: str) -> None:
        payload = {
            "reason": str(reason or "backend_429_cooldown"),
            "wait_seconds": round(float(wait_seconds), 3),
            "force": bool(force),
            "backend_mode": "dom_only",
        }
        try:
            artifacts.append_event(cfg.artifacts_dir, job_id, type="conversation_export_backend_skipped", payload=payload)
        except Exception:
            pass
        try:
            with connect(cfg.db_path) as conn:
                conn.execute("BEGIN IMMEDIATE")
                insert_event(conn, job_id=job_id, type="conversation_export_backend_skipped", payload=payload)
                conn.commit()
        except Exception:
            pass
    ...
    try:
        with connect(cfg.db_path) as conn:
            conn.execute("BEGIN IMMEDIATE")
            global_429_wait = cooldown_wait_seconds(conn, key=_CONVERSATION_EXPORT_429_COOLDOWN_KEY)
            conn.commit()
        if global_429_wait > 0:
            if force and dom_during_429_cooldown:
                dom_only_due_to_429_cooldown = True
                _record_backend_skipped_for_dom_only(
                    wait_seconds=float(global_429_wait),
                    reason="global_429_cooldown",
                )
            else:
                _record_skipped(wait_seconds=float(global_429_wait), reason="global_429_cooldown")
                return
    except Exception:
        pass
    ...
    def _record_backend_rate_limit_cooldown(export_result: Any) -> tuple[float, float]:
        cooldown_seconds = _conversation_export_429_cooldown_seconds(export_result)
        cooldown_until_ts = float(time.time() + float(cooldown_seconds))
        payload = {
            "reason": "backend_429",
            "cooldown_seconds": round(float(cooldown_seconds), 3),
            "cooldown_until": float(cooldown_until_ts),
            "backend_status": _conversation_export_backend_status(export_result),
            "export_kind": (
                str(export_result.get("export_kind") or "")
        ...
        try:
            with connect(cfg.db_path) as conn:
                conn.execute("BEGIN IMMEDIATE")
                effective_until = set_cooldown_until(
                    conn,
                    key=_CONVERSATION_EXPORT_429_COOLDOWN_KEY,
                    until_ts=cooldown_until_ts,
                    reason="backend_429",
                    payload=payload,
                )
                payload["cooldown_until"] = float(effective_until)
                payload["cooldown_seconds"] = round(max(0.0, float(effective_until) - time.time()), 3)
                insert_event(conn, job_id=job_id, type="conversation_export_backend_rate_limited", payload=payload)
                conn.commit()
        except Exception:
            pass
        try:
            artifacts.append_event(
                cfg.artifacts_dir,
                job_id,
                type="conversation_export_backend_rate_limited",
                payload=payload,
            )
        except Exception:
            pass
        return float(payload.get("cooldown_until") or cooldown_until_ts), float(payload.get("cooldown_seconds") or cooldown_seconds)

    export_res = None
    tool_args: dict[str, Any] = {"conversation_url": str(conversation_url).strip()}
    if dom_only_due_to_429_cooldown:
        tool_args["backend_mode"] = "dom_only"
        tool_args["allow_dom_fallback"] = True
    ...
    for attempt in range(2):
        try:
            ...
            try:
                ...
                exc_result = {"error_type": type(exc).__name__, "error": str(exc)}
                backend_rate_limited = _conversation_export_result_is_backend_rate_limited(exc_result)
                if backend_rate_limited:
                    cooldown_until_ts, backoff = _record_backend_rate_limit_cooldown(exc_result)
                else:
                    cooldown_until_ts = float(time.time() + backoff)
                payload = dict(prev)
                payload.setdefault("version", 1)
                payload["last_fail_at"] = float(time.time())
                payload["consecutive_failures"] = int(fail_count)
                payload["cooldown_until"] = float(cooldown_until_ts)
                payload["last_error_type"] = (
                    "ConversationExportBackendRateLimited" if backend_rate_limited else type(exc).__name__
                )
    ...
    if not isinstance(export_res, dict) or not bool(export_res.get("ok")):
        try:
            ...
            fail_count = int(prev.get("consecutive_failures") or 0) + 1
            backoff = min(float(fail_backoff_max_seconds), float(fail_backoff_base_seconds) * (2.0 ** float(fail_count - 1)))
            backend_rate_limited = _conversation_export_result_is_backend_rate_limited(export_res)
            if backend_rate_limited:
                cooldown_until_ts, backoff = _record_backend_rate_limit_cooldown(export_res)
            else:
                cooldown_until_ts = float(time.time() + backoff)
            payload = dict(prev)
            payload.setdefault("version", 1)
            ...
            payload["cooldown_until"] = float(cooldown_until_ts)
            payload["last_error_type"] = (
                "ConversationExportBackendRateLimited" if backend_rate_limited else "ConversationExportFailed"
            )
    ...
    try:
        ...
        if backend_rate_limited:
            cooldown_until_ts, _cooldown_seconds = _record_backend_rate_limit_cooldown(export_res)
            try:
                fail_count = int(payload.get("consecutive_failures") or 0) + 1
            except Exception:
                fail_count = 1
        payload.setdefault("version", 1)
        ...
        if backend_rate_limited:
            ...
            payload["consecutive_failures"] = int(fail_count)
            payload["cooldown_until"] = float(cooldown_until_ts)
            payload["last_error_type"] = "ConversationExportBackendRateLimited"
            payload["last_error"] = "backend conversation fetch returned HTTP 429; DOM fallback export saved"
        else:
...
async def _claim_next_job_with_db_lock_retry(
    *,
    db_path: Path,
    artifacts_dir: Path,
    worker_id: str,
    ...
    attempts = max(1, min(10, _env_int("CHATGPTREST_WORKER_CLAIM_DB_LOCK_RETRY_ATTEMPTS", 4)))
    base_sleep = max(0.05, min(5.0, float(_env_int("CHATGPTREST_WORKER_CLAIM_DB_LOCK_RETRY_BASE_MS", 200)) / 1000.0))
    for attempt in range(attempts):
        try:
            ...
            if not _looks_like_transient_db_lock(exc) or attempt >= attempts - 1:
                raise
            await asyncio.sleep(min(2.0, base_sleep * (2**attempt)))
    return None
...
async def _run_once(
    *,
    ...
    kind_prefix = str(kind_prefix or "").strip() or None
    job = await _claim_next_job_with_db_lock_retry(
        db_path=db_path,
        artifacts_dir=artifacts_dir,
        worker_id=worker_id,
        lease_ttl_seconds=lease_ttl_seconds,
    ...
    def _record_job_event(event_type: str, payload: dict[str, Any]) -> None:
        try:
            with connect(db_path) as conn:
                conn.execute("BEGIN IMMEDIATE")
                insert_event(conn, job_id=job.job_id, type=event_type, payload=dict(payload))
                conn.commit()
            artifacts.append_event(artifacts_dir, job.job_id, type=event_type, payload=dict(payload))
        except Exception:
            pass
    ...
    async def _run_executor_step(
        *,
        ...
        if lease_lost_event.is_set():
            ...
            return None, "lease_lost"
        if cancel_event.is_set():
            if is_web_ask_kind(job.kind) and exec_phase in {"send", "full"}:
                _record_job_event(
                    "inflight_send_cancel_observed",
                    {
                        "kind": job.kind,
                        "phase": exec_phase,
                        "detail": "cancel requested after provider send executor started; waiting for executor result to preserve provider-side evidence",
                    },
                )
                try:
                ...
                meta = dict(getattr(cancel_result, "meta", None) or {})
                _record_job_event(
                    "inflight_send_cancel_completed",
                    {
                        "kind": job.kind,
                        "phase": exec_phase,
                        "executor_status": str(getattr(cancel_result, "status", "") or ""),
                        "conversation_url": str(meta.get("conversation_url") or "").strip() or None,
                    },
                )
                return cancel_result, "canceled_after_inflight_send"
            task.cancel()
            await asyncio.gather(task, return_exceptions=True)
            return None, "canceled"
    ...
    try:
        ...
        if (
            ...
            if len(raw_paths) > _CHATGPT_MAX_FILES_PER_PROMPT:
                try:
                    new_paths, bundle_info = _maybe_bundle_many_attachments_for_chatgpt(
                        artifacts_dir=artifacts_dir,
                        job_id=job.job_id,
                        file_paths=raw_paths,
                    )
                except Exception as exc:
                    new_paths, bundle_info = raw_paths, {
                        "ok": False,
                        "reason": "attachment_bundle_exception",
                        "error": f"{type(exc).__name__}: {exc}",
                    }
                ...
                if isinstance(bundle_info, dict):
                    ev_type = (
                        ...
                        if bool(bundle_info.get("ok"))
                        else "chatgpt_attachments_bundle_failed"
                    )
                    try:
                        payload = dict(bundle_info)
                        with connect(db_path) as conn:
                            conn.execute("BEGIN IMMEDIATE")
                ...
                if isinstance(bundle_info, dict) and bool(bundle_info.get("ok")) and new_paths != raw_paths:
                    exec_input = dict(input_obj)
                    exec_input["file_paths"] = list(new_paths)
                    raw_paths = list(new_paths)
        ...
        if abort == "canceled_after_inflight_send":
            _record_job_event(
                "inflight_send_cancel_recorded",
                {
                    "kind": job.kind,
                    "phase": exec_phase,
                    "conversation_url": conversation_url or None,
                    "detail": "job remains canceled, but provider-side send result was observed and recorded",
                },
            )
            with connect(db_path) as conn:
                conn.execute("BEGIN IMMEDIATE")
                store_canceled_result(
                    ...
                    worker_id=worker_id,
                    lease_token=lease_token,
                    reason="cancel requested after provider send started",
                )
                conn.commit()
            return True
        ...
        if (
            ...
            active_finalization_info: dict[str, Any] | None = None
            question_text = str(input_obj.get("question") or "")
            try:
                min_chars_required = max(0, int((params_obj or {}).get("min_chars") or 0))
            except Exception:
                min_chars_required = 0
            current_answer_text = str(getattr(result, "answer", "") or "")
            current_answer_candidate, current_answer_source, current_answer_details = (
                await _choose_chatgpt_current_answer_candidate(
                    tool_caller=tool_caller,
                    result=result,
            ...
            if export_obj is not None:
                ...
                active_finalization_info = _conversation_export_active_finalization_details(obj=export_obj)
                ...
                try:
                    meta_ref = getattr(result, "meta", None)
                    if isinstance(meta_ref, dict) and active_finalization_info:
                        meta_ref["export_active_finalization"] = True
                        meta_ref["export_active_finalization_details"] = dict(active_finalization_info)
                except Exception:
                    pass

            if (
                status_raw == JobStatus.IN_PROGRESS.value
                and bool(match_info.get("matched"))
                and not str(cand or "").strip()
            ):
                should_downgrade, warn_info = _should_downgrade_when_export_missing_reply(
                    current_answer=current_answer_candidate,
                    min_chars_required=min_chars_required,
                    question_text=question_text,
                    export_guard_info=match_info,
                    thinking_preset_requested=thinking_preset_requested,
                    deep_research_requested=deep_research_requested,
                )
                if should_downgrade and warn_info.get("terminal_action") == "needs_followup":
                    try:
                        payload = {
                            "reason": str(warn_info.get("reason") or "thread_contaminated_by_subsequent_user_turn"),
                            "export_path": export_path.as_posix(),
                            **warn_info,
                        }
                        if match_info:
                            payload["export_guard"] = {
                                "match_kind": match_info.get("match_kind"),
                                "match_strength": match_info.get("match_strength"),
                                "matched_user_index": match_info.get("matched_user_index"),
                                "export_messages_len": match_info.get("export_messages_len"),
                                "next_role_after_match": match_info.get("next_role_after_match"),
                                "subsequent_user_turn_count": match_info.get("subsequent_user_turn_count"),
                            }
                        with connect(db_path) as conn:
                            conn.execute("BEGIN IMMEDIATE")
                            insert_event(
                                conn,
                                job_id=job.job_id,
                                type="completion_guard_thread_contaminated",
                                payload=payload,
                            )
                            conn.commit()
                        artifacts.append_event(
                    ...
                    meta["completion_guard"] = {
                        "type": "export_missing_reply",
                        "action": "needs_followup",
                        **warn_info,
                    }
                    meta.setdefault("error_type", "ConversationThreadContaminated")
                    meta.setdefault(
                        "error",
                        (
                            "conversation export shows a subsequent user turn before any assistant reply "
                            "for this job; stop waiting on the current thread and escalate"
                        ),
                    )
                    result = ExecutorResult(
                        status=JobStatus.NEEDS_FOLLOWUP.value,
                        answer=current_answer_candidate or current_answer_text,
                        answer_format=str(getattr(result, "answer_format", "") or "text"),
                        meta=meta,
                    )
                    status_raw = JobStatus.NEEDS_FOLLOWUP.value
                elif not should_downgrade:
                    reconcile_ok, reconcile_info = _should_reconcile_export_answer(
                        candidate=current_answer_candidate,
                        deep_research=deep_research_requested,
                    ...
                    if reconcile_ok:
                        ...
                        meta["export_missing_reply"] = {
                            "action": "ignored",
                            "finalized_from": current_answer_source,
                            **warn_info,
                        ...
                        try:
                            payload = {
                                ...
                                "export_path": export_path.as_posix(),
                                "chosen_source": current_answer_source,
                                "chosen_answer_chars": len(str(current_answer_candidate or "")),
                            ...
                            if match_info:
                                payload["export_guard"] = {
                                    ...
                                    "matched_user_index": match_info.get("matched_user_index"),
                                    "export_messages_len": match_info.get("export_messages_len"),
                                    "export_last_role": match_info.get("export_last_role"),
                                    "subsequent_user_turn_count": match_info.get("subsequent_user_turn_count"),
                                }
                            with connect(db_path) as conn:
                                conn.execute("BEGIN IMMEDIATE")
                                ...
                                ev_type = (
                                    "answer_completed_from_wait_answer"
                                    if current_answer_source == "current_answer"
                                    else "answer_completed_from_answer_id"
                                )
                                insert_event(
                                    conn,
                                    job_id=job.job_id,
                        ...
                        result = ExecutorResult(
                            ...
                            answer=current_answer_candidate,
                            answer_format=str(getattr(result, "answer_format", "") or "text"),
                            meta=meta,
                        )
                        ...
                        try:
                            ...
                            for key in ("connector", "path", "keys", "answer_quality"):
            ...
            deep_research_pending_reason = (
                _deep_research_pending_wait_reason(cand or "")
                if deep_research_requested and isinstance(cand, str) and cand.strip()
                else None
            )
            ...
            if (
                ...
                and cand
                and cand.strip()
                and deep_research_pending_reason in {"deep_research_ack", "deep_research_progress_stub", "deep_research_tool_stub"}
            ):
                meta = dict(getattr(result, "meta", None) or {})
                meta["conversation_url"] = stable_conversation_url
                meta["export_path"] = export_path.as_posix()
                meta["export_dom_fallback"] = export_dom_fallback
                meta["deep_research_pending_wait"] = True
                meta["deep_research_pending_reason"] = deep_research_pending_reason
                meta["deep_research_pending_answer_chars"] = len(str(cand or ""))
                meta["retry_after_seconds"] = max(
                    float(meta.get("retry_after_seconds") or 0),
                    _deep_research_pending_retry_after_seconds(),
                )
                if match_info:
                    meta["export_match"] = match_info
                try:
                    payload = {
                        "reason": deep_research_pending_reason,
                        "export_path": export_path.as_posix(),
                        "export_answer_chars": len(str(cand or "")),
                        "export_dom_fallback": export_dom_fallback,
                        "status_before": status_raw,
                    }
                    if match_info:
                        payload["export_guard"] = match_info
                    with connect(db_path) as conn:
                        conn.execute("BEGIN IMMEDIATE")
                        insert_event(conn, job_id=job.job_id, type="deep_research_pending_from_export", payload=payload)
                        conn.commit()
                    artifacts.append_event(
                        artifacts_dir,
                        job.job_id,
                        type="deep_research_pending_from_export",
                        payload=payload,
                    )
                except Exception:
                    pass
                result = ExecutorResult(
                    ...
                    answer=str(cand or "") if deep_research_pending_reason != "deep_research_tool_stub" else str(getattr(result, "answer", "") or ""),
                    answer_format=str(getattr(result, "answer_format", "") or "text"),
                    meta=meta,
                )
                status_raw = JobStatus.IN_PROGRESS.value
        ...
        if job.kind == "chatgpt_web.ask" and status_raw == JobStatus.COMPLETED.value:
            ...
            if export_obj is not None:
                ...
                if matched and not export_answer_text.strip():
                    if isinstance(export_guard_info.get("matched_user_index"), int):
                        _, stale_match_info = _current_answer_matches_pre_match_assistant(
                            current_answer=trimmed_answer,
                            export_obj=export_obj,
                            matched_user_index=export_guard_info.get("matched_user_index"),
                            question_text=question_text,
                        )
                        export_guard_info.update(stale_match_info)

                    should_downgrade, warn_info = _should_downgrade_when_export_missing_reply(
                        current_answer=trimmed_answer,
                        min_chars_required=min_chars_required,
                        question_text=question_text,
                        export_guard_info=export_guard_info,
                        thinking_preset_requested=thinking_preset_requested,
                        deep_research_requested=deep_research_requested,
                    )
                    if should_downgrade:
                        if warn_info.get("terminal_action") == "needs_followup" and (not stub):
                            try:
                                payload = {
                                    "reason": str(warn_info.get("reason") or "thread_contaminated_by_subsequent_user_turn"),
                                    "export_path": export_path.as_posix(),
                                ...
                                if export_guard_info:
                                    payload["export_guard"] = {
                                        ...
                                        "next_role_after_match": export_guard_info.get("next_role_after_match"),
                                        "subsequent_user_turn_count": export_guard_info.get("subsequent_user_turn_count"),
                                    }
                                with connect(db_path) as conn:
                                    conn.execute("BEGIN IMMEDIATE")
                                    insert_event(
                    ...
                    if did_override:
                        ...
                        try:
                            ...
                            matched_old, matched_old_info = _current_answer_matches_pre_match_assistant(
                                current_answer=trimmed_answer,
                                export_obj=export_obj,
            ...
            if downgrade_reason is None and thinking_preset_requested:
                meta_for_guard = dict(getattr(result, "meta", None) or {})
                wait_observations = meta_for_guard.get("wait_observations")
                send_timed_out = bool(
                    isinstance(wait_observations, dict)
                    and wait_observations.get("timed_out") is True
                )
                export_confirms_final = bool(
                    isinstance(export_guard_info, dict)
                    and str(export_guard_info.get("answer_source") or "").strip().lower() == "matched_window_longest"
                    and not bool(export_guard_info.get("export_has_in_progress"))
                    and str(export_guard_info.get("chosen_assistant_status") or "").strip().lower() != "in_progress"
                )
                if send_timed_out and not export_confirms_final:
                    downgrade_reason = "thinking_send_timeout_without_complete_export"
            ...
            trivial_prompt_short_answer_ok = bool(
                trimmed_answer and looks_like_synthetic_or_trivial_agent_prompt(question_text)
            )
            suspicious_short_answer_fail_closed = False
            suspicious_short_answer_details: dict[str, Any] = {}
            if downgrade_reason is None:
                ...
                if trivial_prompt_short_answer_ok and _aq_quality == "suspect_short_answer":
                    _aq_quality = "final"
                elif _aq_quality == "suspect_short_answer":
                    should_finalize_legacy_trivial_loop, legacy_loop_details = False, {}
                    try:
                        with connect(db_path) as conn:
                            should_finalize_legacy_trivial_loop, legacy_loop_details = _legacy_trivial_wait_loop_should_finalize(
                                conn=conn,
                                job_id=job.job_id,
                                question_text=question_text,
                                export_guard_info=export_guard_info,
                                downgrade_reason="answer_quality_suspect_short_answer",
                            )
                    except Exception:
                        should_finalize_legacy_trivial_loop, legacy_loop_details = False, {}
                    if should_finalize_legacy_trivial_loop:
                        _aq_quality = "final"
                        try:
                            payload = {
                            ...
                            with connect(db_path) as conn:
                                ...
                                insert_event(
                                    conn,
                                    job_id=job.job_id,
                                    type="completion_guard_legacy_trivial_loop_broken",
                                    payload=payload,
                                )
                                conn.commit()
                            artifacts.append_event(
                        ...
                        try:
                            with connect(db_path) as conn:
                                suspicious_short_answer_fail_closed, suspicious_short_answer_details = (
                                    _suspicious_short_answer_loop_should_fail_closed(
                                        conn=conn,
                                        job_id=job.job_id,
                                        question_text=question_text,
                                        downgrade_reason="answer_quality_suspect_short_answer",
                            ...
                            suspicious_short_answer_fail_closed, suspicious_short_answer_details = False, {}
                        if suspicious_short_answer_fail_closed:
                            try:
                                payload = {
                                    "reason": "answer_quality_suspect_short_answer",
                                    **suspicious_short_answer_details,
                                }
                                with connect(db_path) as conn:
                                    conn.execute("BEGIN IMMEDIATE")
                                    insert_event(
                                        conn,
                                        job_id=job.job_id,
                                        type="completion_guard_suspicious_short_answer_loop_broken",
                                        payload=payload,
                                    )
                                    conn.commit()
                                artifacts.append_event(
            ...
            if downgrade_reason is None and min_chars_required > 0 and len(trimmed_answer) < min_chars_required:
                ...
                research_completion_contract = is_research_contract_params(params_obj)
                try:
                    complete_final_answer = bool(export_guard_info and export_guard_info.get("chosen_assistant_is_complete"))
                    if complete_final_answer and str((export_guard_info or {}).get("chosen_assistant_finish_type") or "").strip().lower() not in {
                        "stop",
                        "complete",
                        "completed",
                    ...
                    if complete_final_answer and bool((export_guard_info or {}).get("export_has_in_progress")):
                        complete_final_answer = False
                    with connect(db_path) as conn:
                        should_complete, details = _min_chars_guard_should_complete_under_min_chars(
                            ...
                            complete_final=bool(complete_final_answer),
                            trivial_prompt=trivial_prompt_short_answer_ok,
                            research_contract=bool(research_completion_contract),
                        )
                except Exception:
                    should_complete, details = False, {}
                ...
                if should_complete and (not stub):
                    ...
                    status_raw = JobStatus.COMPLETED.value
                elif details.get("terminal_action") == "needs_followup" and (not stub):
                    try:
                        payload = {
                            "reason": "answer_too_short_for_min_chars",
                            "action": "research_completion_not_final",
                            **details,
                        }
                        with connect(db_path) as conn:
                            conn.execute("BEGIN IMMEDIATE")
                            insert_event(
                                conn,
                                job_id=job.job_id,
                                type="completion_guard_research_contract_blocked",
                                payload=payload,
                            )
                            conn.commit()
                        artifacts.append_event(
                    ...
                    meta["min_chars_required"] = min_chars_required
                    meta["min_chars_met"] = False
                    meta.setdefault("error_type", "ResearchCompletionNotFinal")
                    meta.setdefault(
                        "error",
                        (
                            f"research answer remained under min_chars={min_chars_required} "
                            f"until {details.get('decision_reason')}; escalate instead of marking completed"
                        ),
                    )
                    result = ExecutorResult(
                        status=JobStatus.NEEDS_FOLLOWUP.value,
                    ...
                    downgrade_reason = (
                        "research_completion_not_final"
                        if details.get("research_contract_blocked")
                        else "answer_too_short_for_min_chars"
                    )
            ...
            if downgrade_reason and (not stub):
                ...
                try:
                    ...
                    elif suspicious_short_answer_fail_closed:
                        payload["action"] = "needs_followup"
                        payload["loop_breaker"] = dict(suspicious_short_answer_details or {})
                    if export_guard_info:
                ...
                if deep_research_requested and downgrade_reason in {"deep_research_ack", "deep_research_not_final"}:
                    retry_after = max(float(retry_after), _deep_research_pending_retry_after_seconds())
                if should_regenerate_same_turn:
                    ...
                    meta["error_type"] = "SuspiciousShortAnswerLoop"
                    meta["error"] = (
                        "answer quality remained suspect_short_answer across repeated completion guards "
                        "without material improvement; fail closed instead of waiting indefinitely"
                    )
                    meta["retry_after_seconds"] = retry_after
                    meta["not_before"] = float(time.time() + float(retry_after))
                    meta.setdefault("completion_guard", {})
                    ...
                    meta["completion_guard"].update(
                        {
                            "type": "answer_quality",
                            "action": "needs_followup_loop_breaker",
                            "reason": downgrade_reason,
                            "loop_breaker": dict(suspicious_short_answer_details or {}),
                        }
                    )
                    result = ExecutorResult(
                        status=JobStatus.NEEDS_FOLLOWUP.value,
                    ...
                    if downgrade_reason == "deep_research_ack":
                        ...
                        meta.setdefault("error", "deep research pending/stub output observed; waiting for full report")
                    ...
                    elif downgrade_reason == "thinking_send_timeout_without_complete_export":
                        meta.setdefault(
                            "error",
                            "thinking preset reached the send-stage timeout without a complete conversation export; waiting",
                        )
                    elif downgrade_reason == "research_completion_not_final":
                        meta.setdefault(
                            "error",
                            (
                                f"research answer remained below min_chars={min_chars_required} "
                                "without a trustworthy final report; waiting instead of marking completed"
                            ),
                        )
                    ...
                    if deep_research_requested and downgrade_reason in {"deep_research_ack", "deep_research_not_final"}:
                        meta["deep_research_pending_wait"] = True
                        meta["deep_research_pending_reason"] = (
                            _deep_research_pending_wait_reason(trimmed_answer) or downgrade_reason
                        )
                        meta["deep_research_pending_answer_chars"] = len(trimmed_answer)
                        meta["retry_after_seconds"] = retry_after
                        meta["not_before"] = float(time.time() + float(retry_after))
                    else:
        ...
        if getattr(result, "status", None) == "completed":
            if job.kind == "chatgpt_web.conversation_export":
                ...
                extract_answer_job = str(job.kind or "").strip().endswith(".extract_answer")
                export_path = artifacts_dir / "jobs" / job.job_id / "conversation.json"
                ...
                if export_path.exists():
                    existing_export_answer = _extract_answer_from_conversation_export(
                        ...
                        question=question_text,
                        deep_research=deep_research_requested,
                        allow_fallback_last_assistant=extract_answer_job,
                    )
                ...
                if extract_answer_job and not str(answer or "").strip():
                    with connect(db_path) as conn:
                        conn.execute("BEGIN IMMEDIATE")
                        try:
                            store_error_result(
                                ...
                                worker_id=worker_id,
                                lease_token=lease_token,
                                error_type="ExtractAnswerEmpty",
                                error="conversation export did not contain an extractable assistant answer",
                                status=JobStatus.NEEDS_FOLLOWUP,
                            )
                            conn.commit()
                        except (LeaseLost, AlreadyFinished):
            ...
            if status_raw == JobStatus.IN_PROGRESS.value and is_web_ask_kind(job.kind):
                ...
                if exec_phase == "wait":
                    try:
                        with connect(db_path) as conn:
                            conn.execute("BEGIN IMMEDIATE")
                            _record_wait_progress_signals(
                                conn,
                                artifacts_dir=artifacts_dir,
                                job_id=job.job_id,
                                current_answer=str(getattr(result, "answer", "") or ""),
                                active_finalization_details=(
                                    dict(meta.get("export_active_finalization_details") or {})
                                    if isinstance(meta.get("export_active_finalization_details"), dict)
                                    else None
                                ),
                                question_text=str(input_obj.get("question") or ""),
                            )
                ...
                if timeout_decision is not None:
                    try:
                        with connect(db_path) as conn:
                            conn.execute("BEGIN IMMEDIATE")
                            regenerated = await _maybe_regenerate_chatgpt_wait_stall(
                                conn=conn,
                                artifacts_dir=artifacts_dir,
                                job=job,
                                worker_id=worker_id,
                                ...
                                params_obj=params_obj,
                                conversation_url=(
                                    str(conversation_url or "").strip()
                                    or str(job.conversation_url or "").strip()
                                ),
                                current_answer=str(getattr(result, "answer", "") or ""),
                    ...
                    try:
                        with connect(db_path) as conn:
                            ...
                            if should_release_for_wait:
                                active_finalization = bool(meta.get("export_active_finalization"))
                                deep_research_pending_wait = bool(meta.get("deep_research_pending_wait"))
                                if active_finalization or deep_research_pending_wait:
                                    if active_finalization:
                                        details = dict(meta.get("export_active_finalization_details") or {})
                                        ...
                                        event_type = "wait_deep_research_pending_observed"
                                        payload = {
                                            "phase": "wait",
                                            "error_type": "InProgress",
                                            "reason": str(meta.get("deep_research_pending_reason") or "deep_research_pending"),
                                            "answer_chars": int(meta.get("deep_research_pending_answer_chars") or 0),
                                            "retry_after_seconds": float(retry_after if retry_after is not None else 60),
                                            "error_preview": "deep research is still running; pending/progress output observed",
                                        }
                                        export_path_meta = str(meta.get("export_path") or "").strip()
                                        if export_path_meta:
                                            payload["export_path"] = export_path_meta
                                    insert_event(
                                        conn,
                                        job_id=job.job_id,
                                        type=event_type,
                                    ...
                                    status_raw, error_type, error = _store_browser_retry_or_fail_closed(
                                        conn,
                                        artifacts_dir=artifacts_dir,
                                        job=job,
                                        worker_id=worker_id,
                ...
                with connect(db_path) as conn:
                    ...
                    try:
                        if status_raw == JobStatus.CANCELED.value:
                            ...
                            chatgpt_send_manual_repair = _should_fail_closed_chatgpt_send_same_session_repair(
                                kind=str(job.kind or ""),
                                phase=(retry_phase or exec_phase or "send"),
                                error_type=error_type,
                                error=error,
                                conversation_url=(conversation_url or str(job.conversation_url or "").strip()),
                            ...
                            blank_gemini_send_cooldown = _should_fail_closed_blank_gemini_send_cooldown(
                                kind=str(job.kind or ""),
                                status=status_raw,
                                phase=(retry_phase or exec_phase or "send"),
                                error_type=error_type,
                            ...
                            if chatgpt_send_manual_repair:
                                fail_closed_error = (
                                    "ChatGPT send hit a same-session repair condition without stable "
                                    "conversation/thread evidence; fail closed instead of repeated cooldown retries. "
                                    f"Original: {error_type}: {error}"
                                )
                                payload = {
                                    "status": status_raw,
                                    "phase": (retry_phase or exec_phase or "send"),
                                    "error_type": error_type,
                                    "reason": "chatgpt_send_same_session_repair_without_thread",
                                }
                                send_evidence = dict(meta.get("send_phase_evidence") or {})
                                if send_evidence:
                                    payload["send_phase_evidence"] = send_evidence
                                insert_event(
                                    conn,
                                    job_id=job.job_id,
                                    type="chatgpt_send_manual_repair_fail_closed",
                                    payload=payload,
                                )
                                artifacts.append_event(
                                    artifacts_dir,
                                ...
                                status_raw = JobStatus.ERROR.value
                                error = fail_closed_error
                            elif blank_gemini_send_cooldown:
                                fail_closed_error_type = "GeminiBlankSendTimeout"
                                fail_closed_error = (
                                    "Gemini send failed without stable conversation_url/thread evidence; "
                                    f"fail closed instead of repeated cooldown retries. Original: {error_type}: {error}"
                                )
                                payload = {
                                    "status": status_raw,
                                    "phase": (retry_phase or exec_phase or "send"),
                                    "error_type": error_type,
                                    "reason": "blank_gemini_cooldown_without_thread_fail_closed",
                                }
                                insert_event(
                                    conn,
                                    job_id=job.job_id,
                                    type="blank_gemini_cooldown_fail_closed",
                                    payload=payload,
                                )
                                artifacts.append_event(
                                    artifacts_dir,
                                ...
                                if status_raw == JobStatus.NEEDS_FOLLOWUP.value:
                                    store_retryable_result(
                                        conn,
                                        artifacts_dir=artifacts_dir,
                                        job_id=job.job_id,
                            ...
                            active_finalization = bool(meta.get("export_active_finalization"))
                            deep_research_pending_wait = bool(meta.get("deep_research_pending_wait"))
                            if is_web_ask_kind(job.kind) and exec_phase == "wait" and (active_finalization or deep_research_pending_wait):
                                if active_finalization:
                                    details = dict(meta.get("export_active_finalization_details") or {})
                                    event_type = "wait_active_finalization_observed"
                                    payload = {
                                        "phase": "wait",
                                        "error_type": "InProgress",
                                        "error_preview": "job still in progress; active finalization observed in export",
                                        **details,
                                    }
                                else:
                                    event_type = "wait_deep_research_pending_observed"
                            ...
                            chatgpt_send_manual_repair = _should_fail_closed_chatgpt_send_same_session_repair(
                                kind=str(job.kind or ""),
                                phase=(retry_phase or exec_phase or "send"),
                                error_type=(classified_error_type if retryable_error else error_type),
                                error=error,
                                conversation_url=(conversation_url or str(job.conversation_url or "").strip()),
                                conversation_id=str(getattr(job, "conversation_id", "") or ""),
                                meta=meta,
                            ...
                            blank_gemini_send_cooldown = _should_fail_closed_blank_gemini_send_cooldown(
                                kind=str(job.kind or ""),
                                status=JobStatus.COOLDOWN.value,
                                phase=(retry_phase or exec_phase or "send"),
                                error_type=(classified_error_type if retryable_error else error_type),
                                error=error,
                                conversation_url=(conversation_url or str(job.conversation_url or "").strip()),
                            ...
                            if chatgpt_send_manual_repair:
                                fail_closed_error_type = classified_error_type if retryable_error else error_type
                                fail_closed_error = (
                                    "ChatGPT send hit a same-session repair condition without stable "
                                    "conversation/thread evidence; fail closed instead of repeated cooldown retries. "
                                    f"Original: {fail_closed_error_type}: {error}"
                                )
                                payload = {
                                    "status": JobStatus.COOLDOWN.value,
                                    "phase": (retry_phase or exec_phase or "send"),
                                ...
                                fail_closed_error = (
                                    "Gemini send failed without stable conversation_url/thread evidence; "
                                    f"fail closed instead of repeated cooldown retries. Original: {classified_error_type}: {error}"
                                )
                                payload = {
                                    "status": JobStatus.COOLDOWN.value,
                                    "phase": (retry_phase or exec_phase or "send"),
