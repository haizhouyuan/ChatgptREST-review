...
def _job_answer(
    ...
    *,
    ...
    qs = urllib.parse.urlencode(
        {"offset": max(0, int(offset)), "max_chars": max(1, int(max_chars))},
        doseq=False,
    )
    req = _build_request(
        url=f"{base}/v1/jobs/{urllib.parse.quote(str(job_id).strip())}/answer?{qs}",
        method="GET",
        api_key=api_key,
    )
    with _open_with_recovery(req, timeout=timeout) as resp:
...
async def advisor_agent_answer(
    ...
    """Fetch the canonical answer for an agent session.

    For job-backed sessions this resolves the authoritative job answer through
    the jobs API. For no-job sessions it falls back to last_answer.
    """
    base = _base_url()
    api_key = _api_key()

    try:
        session = await asyncio.to_thread(_session_status, base, api_key, session_id, timeout=30.0)
    except urllib.error.HTTPError as e:
        return _structured_http_error_result(e, session_id=session_id)
    except Exception as e:
        return {
    ...
    if not isinstance(session, dict):
        return {"ok": False, "session_id": session_id, "error": "invalid_session_payload"}

    authoritative_job_id = str(
        session.get("authoritative_job_id") or session.get("job_id") or ""
    ).strip()
    answer_state = str(session.get("answer_state") or "").strip().lower()
    if authoritative_job_id:
        try:
            ...
            if isinstance(answer_payload, dict) and answer_payload.get("ok"):
                answer_text = str(answer_payload.get("answer") or "")
                answer_chars = answer_payload.get("answer_chars")
                if answer_chars is None:
                    answer_chars = len(answer_text)
                return {
                    "ok": True,
                    "session_id": str(session.get("session_id") or session_id),
                    "answer_state": answer_state or "final",
                    "answer": answer_text,
                    "answer_chars": int(answer_chars or 0),
                    "offset": int(answer_payload.get("offset") or max(0, int(offset))),
                    "has_more": bool(answer_payload.get("has_more")),
                    "source": "job_answer_api",
                    "authoritative_job_id": authoritative_job_id,
                    "authoritative_answer_path": str(session.get("authoritative_answer_path") or "").strip() or None,
                }
        except urllib.error.HTTPError as e:
            return _structured_http_error_result(e, session_id=session_id)
        except Exception as e:
    ...
    last_answer = str(session.get("last_answer") or "")
    if last_answer.strip():
        sliced = last_answer[max(0, int(offset)): max(0, int(offset)) + max(1, int(max_chars))]
        end_pos = max(0, int(offset)) + len(sliced)
        return {
            "ok": True,
            "session_id": str(session.get("session_id") or session_id),
            "answer_state": answer_state or ("final" if str(session.get("status") or "").strip().lower() == "completed" else "partial"),
            "answer": sliced,
            "answer_chars": len(last_answer),
            "offset": max(0, int(offset)),
            "has_more": end_pos < len(last_answer),
            "source": "last_answer",
        }
