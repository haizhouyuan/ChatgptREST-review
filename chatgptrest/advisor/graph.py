...
def execute_quick_ask(state: AdvisorState) -> dict:
    ...
    if svc.memory:
        try:
            ...
            assembler = ContextAssembler(svc.memory, svc.kb_hub, evomap_db=svc.evomap_knowledge_db)
            pack = assembler.build(query=msg, session_id=session_id)
            context_system_prompt = assembler.to_system_prompt(pack)
        except Exception as e:
