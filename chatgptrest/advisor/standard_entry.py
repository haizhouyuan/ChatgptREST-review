...
def standard_entry_pipeline(
    ...
    try:
        ...
        checked_agent = request.target_agent
        ...
        result["recommended_skills"] = list(skill_check.recommended_skills)
        result["recommended_bundles"] = list(skill_check.recommended_bundles)
        result["unmet_capabilities"] = list(skill_check.unmet_capabilities)
        result["decision_reasons"] = list(skill_check.decision_reasons)
        result["fallback_plan"] = list(skill_check.fallback_plan)
        selected_skills: list[str] = []
        selected_bundles: list[str] = []
        if skill_check.passed:
            registry = get_canonical_registry()
            agent_profile = registry.get_agent_profile(checked_agent)
            if agent_profile is not None:
                ...
                selected_skills = registry.available_skill_ids_for_bundles(
                    ...
                    platform=skill_check.platform,
                )
        emit_skill_resolution_signals(
            trace_id=request.trace_id,
            source="advisor.standard_entry",
            agent_id=checked_agent,
            task_type=task_type,
            platform=skill_check.platform,
            recommended_skills=skill_check.recommended_skills,
            recommended_bundles=skill_check.recommended_bundles,
            selected_skills=selected_skills,
            selected_bundles=selected_bundles,
            unmet_capabilities=skill_check.unmet_capabilities,
            extra={"entry_source": request.source},
        )

        if not skill_check.passed:
            market_candidates = find_market_candidates_for_unmet(skill_check.unmet_capabilities)
            gaps = get_capability_gap_recorder().promote_unmet(
                ...
                platform=skill_check.platform,
                unmet_capabilities=skill_check.unmet_capabilities,
                suggested_agent=skill_check.suggested_agent or "",
                context={
                    "entry_source": request.source,
                    "question_preview": request.question[:200],
                    "recommended_bundles": list(skill_check.recommended_bundles),
                    "recommended_skills": list(skill_check.recommended_skills),
                },
            )
            result["skill_gap"] = True
            result["skill_status"] = skill_check.status
            result["capability_gap_ids"] = [gap.gap_id for gap in gaps]
            result["market_candidates"] = market_candidates
            if market_candidates:
                result["fallback_plan"].append(
                    {
