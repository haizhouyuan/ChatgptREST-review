...
DEFAULT_ARTIFACT_ROOT = REPO_ROOT / "artifacts" / "monitor"
...
def _summarize_production_regression(*, artifact_root: Path) -> dict[str, Any]:
    candidates = sorted(artifact_root.glob("**/openmind_production_regression_*.json"))
    if not candidates:
        return {
            "status": "missing",
            "evidence_path": "",
    ...
    failed = sum(1 for item in bundles if not bool((item or {}).get("ok")))
    return {
...
def build_shared_cognition_status_board() -> dict[str, Any]:
    ...
    production_regression = _summarize_production_regression(artifact_root=artifact_root)
    ...
    blockers: list[str] = []
    if production_regression["status"] != "ok":
        blockers.append("production_regression_not_green")
    ...
    return {
        ...
        "shared_cognition": {
            ...
            "owner_scope_ready": not any(
                blocker in blockers
                for blocker in (
                    "production_regression_not_green",
...
def render_shared_cognition_status_board_markdown(payload: dict[str, Any]) -> str:
    ...
    semantic = board.get("production_regression") if isinstance(board.get("production_regression"), dict) else {}
    ...
    lines = [
        ...
        f"- bundles: `{semantic.get('bundle_count', 0)}`",
        f"- failed_bundles: `{semantic.get('failed_bundles', 0)}`",
