...
_PROVIDER_SPECS: tuple[ProviderSpec, ...] = (
    ...
    ProviderSpec(
        provider_id="qwen",
        kind_namespace="qwen_web.",
        ask_kind="qwen_web.ask",
        rate_limit_key="qwen_web_send",
        min_interval_attr="qwen_min_prompt_interval_seconds",
        supported_presets=frozenset({"auto", "deep_thinking", "deep_research"}),
        preset_aliases=_QWEN_PRESET_ALIASES,
    ),
)
...
_QWEN_THREAD_URL_RE = re.compile(r"https?://(?:[^/]*\.)?qianwen\.com/chat/[0-9a-f]{32}\b", re.I)
...
def looks_like_thread_url(kind: str, url: str | None) -> bool:
    ...
    if spec.provider_id == "qwen":
        return bool(_QWEN_THREAD_URL_RE.search(raw))
    return False
