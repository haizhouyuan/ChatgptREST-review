...
_PROVIDER_SPECS: tuple[ProviderSpec, ...] = (
    ProviderSpec(
        ...
        default_preset="auto",
        has_true_non_pro=True,
        auto_semantics="true_non_pro",
        safe_non_pro_fallback="auto",
        deep_research_support="flag_only",
        premium_presets=frozenset({"pro_extended", "thinking_heavy", "thinking_extended", "deep_research"}),
    ...
    ProviderSpec(
        ...
        default_preset="pro",
        has_true_non_pro=False,
        auto_semantics="alias_to_pro",
        safe_non_pro_fallback=None,
        deep_research_support="tool_path",
        premium_presets=frozenset({"pro", "deep_think"}),
...
_SPEC_BY_PROVIDER_ID: dict[str, ProviderSpec] = {spec.provider_id: spec for spec in _PROVIDER_SPECS}
...
def provider_spec_for_provider_id(provider_id: str | None) -> ProviderSpec | None:
    return _SPEC_BY_PROVIDER_ID.get(str(provider_id or "").strip())
...
def provider_capability_profile_for_kind(kind: str | None) -> dict[str, object] | None:
    spec = provider_spec_for_ask_kind(kind) or provider_spec_for_kind(kind)
    if spec is None:
        return None
    return spec.capability_profile()


def provider_capability_profile_for_provider(provider_id: str | None) -> dict[str, object] | None:
...
def safe_non_pro_preset_for_kind(kind: str | None) -> str | None:
    spec = provider_spec_for_ask_kind(kind)
    if spec is None:
        return None
    return spec.safe_non_pro_fallback
        ...
        raise PresetValidationError(
            {
                ...
                "provider_capability_profile": spec.capability_profile(),
