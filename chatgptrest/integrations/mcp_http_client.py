...
def _should_retry_with_fresh_session(exc: McpHttpError) -> bool:
    text = str(exc or "").strip().lower()
    if not text:
        return False
    if "deadline exceeded" in text or "stream timeout" in text:
        return False
...
def _should_retry_initialize_handshake(exc: McpHttpError) -> bool:
    text = str(exc or "").strip().lower()
    if not text:
        return False
    return any(marker in text for marker in (
...
@dataclass(frozen=True)
class McpHttpInitializeHandshake:
...
def _jsonrpc_post_stream(
    ...
    *,
    ...
    # Allow slower provider sends to use the caller deadline up to a bounded ceiling,
    # instead of force-cutting every long-lived stream at 60s.
...
def mcp_http_initialize_handshake(
    url: str,
    *,
    client_name: str,
    client_version: str,
    ...
    last_exc: McpHttpError | None = None
    for attempt in range(2):
        try:
            ...
            return McpHttpInitializeHandshake(session=session, result=result)
        except McpHttpError as exc:
            last_exc = exc
            if attempt >= 1 or not _should_retry_initialize_handshake(exc):
                raise
            time.sleep(0.2)
    if last_exc is not None:
        raise last_exc
    raise McpHttpError("initialize failed without explicit error")
...
def mcp_http_jsonrpc_call(
    ...
    *,
    ...
    try:
        ...
        try:
            if status == 202:
                raise McpHttpError(f"{method} returned 202 Accepted (unexpected)")
            if status != 200:
                raw = fp.read() if hasattr(fp, "read") else b""
                snippet = raw.decode("utf-8", errors="replace")[:500].strip()
                raise McpHttpError(f"{method} failed: HTTP {status} {snippet}")
            ctype = (_read_header(resp_headers, "content-type") or "").lower()
            if ctype.startswith("application/json"):
                raw = fp.read()
                msg = json.loads(raw.decode("utf-8", errors="replace"))
        ...
        if not isinstance(msg, dict) or msg.get("id") != req_id:
            raise McpHttpError(f"{method} bad response: {str(msg)[:300]}")
        if "error" in msg:
            raise McpHttpError(f"{method} error: {json.dumps(msg['error'], ensure_ascii=False)[:500]}")
        return msg
    except McpHttpError:
        raise
    except Exception as exc:
        raise McpHttpError(f"{method} transport failure: {_describe_exc(exc)}") from exc


def mcp_http_list_tools(session: McpHttpSession, *, timeout_sec: float = 30.0) -> list[Dict[str, Any]]:
    message = mcp_http_jsonrpc_call(session, method="tools/list", params={}, timeout_sec=timeout_sec)
    result = message.get("result") or {}
    if not isinstance(result, dict):
        raise McpHttpError(f"tools/list bad result: {json.dumps(message, ensure_ascii=False)[:500]}")
    tools = result.get("tools") or []
    if not isinstance(tools, list):
        raise McpHttpError(f"tools/list missing tools array: {json.dumps(message, ensure_ascii=False)[:500]}")
...
def mcp_http_call_tool(
    ...
    *,
    ...
    try:
        ...
        raise McpHttpError(f"tools/call transport failure: {_describe_exc(exc)}") from exc
...
class McpHttpClient:
    ...
    def list_tools(self, *, timeout_sec: float = 30.0) -> list[Dict[str, Any]]:
        session = self._ensure_session()
        try:
            return mcp_http_list_tools(session, timeout_sec=timeout_sec)
        except McpHttpError as exc:
    ...
    def call_tool(self, *, tool_name: str, tool_args: Dict[str, Any], timeout_sec: float = 600.0) -> Dict[str, Any]:
        ...
        try:
            ...
            if not _should_retry_with_fresh_session(exc):
                raise
            # Retry once with a fresh session when the current MCP session likely rotated
            # or the transport could not even establish a stable request.
            self._session = None
            session = self._ensure_session()
            return mcp_http_call_tool(session, tool_name=tool_name, tool_args=tool_args, timeout_sec=timeout_sec)
