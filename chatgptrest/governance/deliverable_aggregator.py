...
from chatgptrest.core.completion_contract import get_authoritative_answer_path
...
def aggregate_answers(
    ...
    *,
    ...
    for jid in job_ids:
        answer_path = _resolve_job_answer_path(artifacts, jid)
...
def _resolve_job_answer_path(artifacts: Path, job_id: str) -> Path | None:
    job_dir = artifacts / "jobs" / job_id
    ...
    if result_path.exists():
        ...
        if isinstance(parsed, dict):
            authoritative = get_authoritative_answer_path(parsed)
            if authoritative:
                candidate = artifacts / authoritative
                if candidate.exists():
                    return candidate
    for ext in ("md", "txt"):
        candidate = job_dir / f"answer.{ext}"
        if candidate.exists():
            return candidate
    return None
