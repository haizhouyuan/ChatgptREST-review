...
class TaskStateMachine:
    ...
    def inject_signal(
        ...
        *,
        ...
        """Inject an external signal for the task with deduplication.

        Signals are deduplicated by signal_type - if a signal of the same type
        has already been received and is still pending, the existing signal is
        ...
        with task_db_conn(self.db_path) as conn:
            ...
            # Check for existing pending signal of same type (deduplication)
            existing = conn.execute("""
                SELECT signal_id FROM task_external_signals
                WHERE task_id = ? AND signal_type = ? AND processed_at IS NULL
                LIMIT 1
            """, (self.task_id, signal_type)).fetchone()

            if existing:
                # Return existing signal instead of creating duplicate
