...
from chatgptrest.task_runtime.task_finalization import TaskFinalizationService, finalize_task
...
class FinalizeTaskResponse(BaseModel):
    """Response from task finalization."""
    task_id: str
    success: bool
    previous_status: str
    final_status: str
...
async def finalize_task_endpoint(
    task_id: str,
    request: FinalizeTaskRequest | None = None,
) -> FinalizeTaskResponse:
    """Finalize a task - orchestrates the complete finalization path.

    This endpoint implements the real runtime orchestration:
    1. Validates task is in PROMOTED status with valid outcome
    ...
    This is the real caller that connects publish_to_delivery, distill_outcome,
    and complete_task into a single runtime path.
    """
    try:
        # Parse memory_db_path if provided
        memory_db_path = None
        if request and request.memory_db_path:
            memory_db_path = Path(request.memory_db_path)
        ...
        return FinalizeTaskResponse(
            ...
            error=result.error,
            delivery_projection=result.delivery_projection,
            memory_distillation=result.memory_distillation,
        )
