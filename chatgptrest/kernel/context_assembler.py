...
from chatgptrest.governance.authority_anchor import load_authority_anchor
...
class ContextPack:
    ...
    authority_anchor: Any | None = None
...
class ContextAssembler:
    ...
    SOURCE_PRIORITY = {
        "authority": 0,    # project authority anchor
    ...
    def build(
        ...
        *,
        ...
        semantic = self._memory.get_semantic(domain="user_profile", project_id=project_id)
        if semantic:
    ...
    def _apply_budget(self, pack: ContextPack) -> ContextPack:
        ...
        used = 0
        kept_sources = []
        authority_budget = max(0, min(pack.budget.authority_anchor, available))
        for source in [item for item in pack.sources if item.source_type == "authority"]:
            if used >= available:
                break
            allowance = authority_budget if authority_budget > 0 else max(0, available - used)
            if allowance <= 0:
                break
            if source.token_count <= allowance:
                kept_sources.append(source)
                used += source.token_count
                authority_budget = max(0, authority_budget - source.token_count)
                continue
            if allowance > 50:
                kept_sources.append(
                    ContextSource(
                        ...
                        content=source.content[: allowance * 4],
                        token_count=allowance,
                        metadata={**source.metadata, "truncated": True, "protected": True},
                    )
                )
                used += allowance
                authority_budget = 0
    ...
    def to_system_prompt(self, pack: ContextPack) -> str:
        ...
        if "authority" in sources_by_type:
            parts.append(f"## Authority Anchor\n{sources_by_type['authority'].content}")
    ...
    def _format_evomap_hits(self, hits: list) -> str:
        ...
        for i, sa in enumerate(hits, 1):
            atom = sa.atom
            layer = str(getattr(sa, "retrieval_layer", "") or "").strip()
            bucket = str(getattr(sa, "source_bucket", "") or "").strip()
            label_parts = [f"score={sa.final_score:.2f}"]
            if layer:
                label_parts.append(layer)
            if bucket:
                label_parts.append(bucket)
            score_label = f"[{' | '.join(label_parts)}]"
            q = atom.question[:120]
