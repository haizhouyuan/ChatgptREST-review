...
CREATE INDEX IF NOT EXISTS idx_memory_dedup_scope
    ON memory_records(fingerprint, category, agent_id, role_id, session_id, account_id, thread_id, project_id);
...
class MemoryManager:
    ...
    def _upgrade_schema(self, conn: sqlite3.Connection) -> None:
        ...
        new_cols = []
        for col in ["session_id", "agent_id", "role_id", "account_id", "thread_id", "project_id"]:
        ...
        try:
            conn.execute(
                ...
                SET
                    ...
                    project_id = COALESCE(
                        NULLIF(project_id, ''),
                        NULLIF(json_extract(source, '$.project_id'), ''),
                        NULLIF(json_extract(value, '$.project_id'), ''),
                        ''
                    )
                WHERE
                    session_id = ''
    ...
    def stage(self, record: MemoryRecord) -> str:
        ...
        inferred_project_id = self._infer_project_id(record.value)
        if inferred_project_id and not str(record.source.get("project_id") or "").strip():
            record.source["project_id"] = inferred_project_id
            ...
            conn.execute(
                """UPDATE memory_records
                   SET value = ?, confidence = ?, source = ?, updated_at = ?,
                       agent_id = ?, role_id = ?, session_id = ?, account_id = ?, thread_id = ?, project_id = ?
                   WHERE record_id = ?""",
                (json.dumps(record.value, ensure_ascii=False),
                 ...
                 now,
                 dedup_params[0], dedup_params[1], dedup_params[2], dedup_params[3], dedup_params[4], dedup_params[5],
                 existing["record_id"]),
            )
            conn.commit()
            src = record.source if isinstance(record.source, dict) else {}
        ...
        conn.execute(
            """INSERT INTO memory_records
               (record_id, tier, category, key, value, confidence,
                ...
                created_at, updated_at, agent_id, role_id, session_id, account_id, thread_id, project_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
    ...
    def get_working_context(self, session_id: str = "",
                            limit: int = 20,
                            project_id: str = "") -> list[MemoryRecord]:
        """Get current working memory (hot context)."""
        conn = self._conn()
        clauses = ["tier = 'working'"]
        params: list[Any] = []
        if session_id:
    ...
    @staticmethod
    def _infer_project_id(value: Any) -> str:
        if not isinstance(value, dict):
            return ""
        direct = str(value.get("project_id") or "").strip()
        if direct:
            return direct
        refs = value.get("project_refs")
