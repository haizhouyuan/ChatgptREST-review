...
class MemoryManager:
    ...
    _IDENTITY_FIELDS = ("agent", "role", "session_id", "account_id", "thread_id")
    ...
    def get_episodic(self, query: str = "", category: str = "",
        ...
        Args:
            agent_id: If set, only return records created by this agent/component
                      (matches json_extract(source, '$.agent')).
            role_id: If set, only return records from this business role
                     (matches json_extract(source, '$.role')).
            session_id: If set, only return records from this session
                        (tenant isolation via json_extract(source, '$.session_id')).
        ...
        if session_id:
            clauses.append("json_extract(source, '$.session_id') = ?")
        ...
        if account_id:
            clauses.append("json_extract(source, '$.account_id') = ?")
            params.append(account_id)
        if thread_id:
            clauses.append("json_extract(source, '$.thread_id') = ?")
