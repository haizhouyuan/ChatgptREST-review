...
from chatgptrest.evomap.knowledge.retrieval import (
    ...
    summarize_retrieval_layers,
)
from chatgptrest.governance.authority_anchor import (
...
class ContextResolver:
    ...
    def __init__(self, runtime: ContextRuntimeDeps):
    ...
    def resolve(self, options: ContextResolveOptions) -> ContextResolveResult:
        ...
        for source in pack.sources:
            ...
            if kind not in requested_sources and source.source_type != "authority":
                continue
        ...
        authority_block = next((block for block in blocks if block.source_type == "authority"), None)
        ...
        if "graph" in requested_sources and include_personal_graph and not pack.evomap_hits:
            if self._should_require_personal_graph(options=options, pack=pack):
                degraded_sources.append("personal_graph_empty")
            else:
                optional_gap_sources.append("personal_graph_empty")
        ...
        authority_governance = self._build_authority_governance(
            authority_anchor=getattr(pack, "authority_anchor", None),
            authority_block=authority_block,
            blocks=blocks,
        )
        if authority_governance.get("stale_status") == "stale":
            degraded_sources.append("authority_anchor_stale")
        if authority_governance.get("missing_docs"):
            degraded_sources.append("authority_docs_missing")
        if authority_governance.get("schema_gaps"):
            degraded_sources.append("authority_schema_partial")
        ...
        return ContextResolveResult(
            ...
            metadata={
                ...
                "optional_gap_sources": optional_gap_sources,
    ...
    @staticmethod
    def _should_require_personal_graph(*, options: ContextResolveOptions, pack: Any) -> bool:
        if not str(options.project_id or "").strip():
            return True
        if getattr(pack, "evomap_hits", None):
            return True
        has_project_grounding = bool(getattr(pack, "authority_anchor", None)) or bool(getattr(pack, "planning_pack_hits", None)) or bool(getattr(pack, "kb_hits", None))
        return not has_project_grounding
    ...
    def _build_policy_hints(
        ...
        *,
        ...
        if "authority_anchor_stale" in degraded_sources:
            hints.append("Authority anchor is stale; refresh the project anchor before trusting lower-priority recall.")
        if "authority_docs_missing" in degraded_sources:
            hints.append("Some authority docs are missing or empty; treat authority-derived context as partially degraded.")
            ...
            return [
                {
                    ...
                    "retrieval_layer": str(getattr(scored, "retrieval_layer", "") or ""),
                    "retrieval_source": str(getattr(scored, "retrieval_source", "") or ""),
                    "source_bucket": str(getattr(scored, "source_bucket", "") or ""),
                    "promotion_status": str(getattr(scored.atom, "promotion_status", "") or ""),
                    "source_ref": str(getattr(scored, "source_ref", "") or ""),
                    "vector_score": round(float(getattr(scored, "vector_score", 0.0) or 0.0), 4),
                }
    ...
    def _compose_prompt_prefix(
        ...
        *,
        assembler: Any,
        prompt_pack: Any,
        authority_block: ContextBlock | None,
        ...
        sections: list[str] = []
        authority_section = (
            f"## Authority Anchor\n{authority_block.text}" if authority_block is not None else ""
        )
        planning_section = (
        ...
        prompt_pack_without_authority = copy.copy(prompt_pack)
        prompt_pack_without_authority.sources = [
            source for source in prompt_pack.sources if source.source_type != "authority"
        ]
        base_prompt = assembler.to_system_prompt(prompt_pack_without_authority)
        if base_prompt:
    ...
    def _build_authority_governance(
        self,
        *,
        ...
        if authority_block is None or authority_anchor is None:
            return {
                "present": False,
                "stale_status": "absent",
                "missing_docs": [],
        ...
        authority_metadata = dict(authority_block.metadata if isinstance(authority_block.metadata, dict) else {})
        conflicts = [
            item.__dict__.copy()
            for item in detect_authority_conflicts(
        ...
        return {
            "present": True,
            "project_id": authority_metadata.get("project_id"),
            "anchor_path": authority_metadata.get("anchor_path"),
            "owner": authority_metadata.get("owner"),
            "last_reviewed_at": authority_metadata.get("last_reviewed_at"),
            "stale_threshold_days": authority_metadata.get("stale_threshold_days"),
            "stale_status": authority_metadata.get("stale_status"),
            "missing_docs": list(authority_metadata.get("missing_docs") or []),
            "schema_gaps": list(authority_metadata.get("schema_gaps") or []),
            "current_phase_framing": authority_metadata.get("current_phase_framing") or "",
            "pinned_authority_inputs": list(authority_metadata.get("pinned_authority_inputs") or []),
            "conflicts": conflicts,
    ...
    def _build_retrieval_plan(
        *,
        ...
        if any(block.source_type == "authority" for block in blocks):
            plan.append(
                {
                    "kind": "authority",
                    "plane": _SOURCE_PLANE_MAP["authority"],
                    "resolved": True,
                    "block_count": block_counts.get("authority", 0),
                    "reason": "project-scoped authority anchor was loaded and elevated ahead of lower-priority context",
                }
            )
        for kind in requested_sources:
...
class _LocalOnlyContextAssembler(ContextAssembler):
    ...
    def build(
        ...
        *,
        ...
        if project_id:
            authority_anchor = load_authority_anchor(project_id)
            if authority_anchor is not None:
                pack.authority_anchor = authority_anchor
                content = authority_anchor.to_prompt_text()
                pack.sources.append(
                    ContextSource(
                        ...
                        metadata=authority_anchor.to_runtime_metadata(),
                    )
                )
        ...
        if self._evomap_db is not None:
            ...
            if evomap_retrieve is not None:
                try:
                    ...
                    planning_role = str(role_id or "").strip().lower() == "planning"
                    retrieval_surface = (
                        RetrievalSurface.PLANNING_EXPLICIT_PATH
                        if planning_role
                        else RetrievalSurface.USER_HOT_PATH
                    )
                    ...
                    setattr(pack, "evomap_surface", retrieval_surface.value)
                    if evomap_hits:
                        ...
                        pack.sources.append(
                            ContextSource(
                                ...
                                metadata={
                                    ...
                                    "retrieval_surface": retrieval_surface.value,
                                    "retrieval_layer_counts": summarize_retrieval_layers(evomap_hits),
