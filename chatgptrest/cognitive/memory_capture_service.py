...
from chatgptrest.kernel.work_memory_objects import is_work_memory_category
...
class MemoryCaptureService:
    def __init__(self, runtime: MemoryRuntimeDeps):
    ...
    def _capture_one(self, item: MemoryCaptureItem) -> MemoryCaptureItemResult:
        ...
        if item.require_complete_identity and identity_gaps:
            blocked_by = ["identity"]
        ...
        if quality_gate and not quality_gate.get("allowed", False):
            blocked_by = list(quality_gate.get("blocked_by") or ["policy"])
            self._emit_capture_event(
                item=item,
                trace_id=trace_id,
                event_type="memory.capture.blocked",
        ...
        if is_work_memory_category(item.category):
            payload = dict(item.object_payload or {})
            if item.project_id.strip() and not str(payload.get("project_id") or "").strip():
                payload["project_id"] = item.project_id.strip()
            work_memory_result = WorkMemoryManager(memory).write_from_capture(
                category=item.category.strip() or "captured_memory",
                title=title,
            ...
            event_type = "memory.capture" if work_memory_result.ok else "memory.capture.blocked"
            self._emit_capture_event(
                item=item,
                trace_id=trace_id,
                event_type=event_type,
                data={
                    "record_id": work_memory_result.record_id,
                    "title": title,
                    "category": item.category.strip() or "captured_memory",
                    "source_ref": item.source_ref.strip(),
                    "duplicate": work_memory_result.duplicate,
                    "cross_session_visible": work_memory_result.ok,
                    "role_id": item.role_id.strip(),
                    "identity_gaps": list(identity_gaps),
                    "blocked_by": list(work_memory_result.blocked_by),
                    "provenance_quality": provenance_quality,
                    "quality_gate": dict(quality_gate),
                    "require_complete_identity": bool(item.require_complete_identity),
                    "project_id": item.project_id.strip(),
                    "work_memory": dict(work_memory_result.work_memory),
                    "review_status": work_memory_result.review_status,
                    "active": work_memory_result.active,
                    "promotion_state": work_memory_result.promotion_state,
                    "superseded_record_id": work_memory_result.superseded_record_id,
                    "governance": dict(work_memory_result.governance),
                },
            )
            return MemoryCaptureItemResult(
                ok=work_memory_result.ok,
                trace_id=trace_id,
                title=title,
                record_id=work_memory_result.record_id,
                category=item.category.strip() or "captured_memory",
                tier=work_memory_result.tier,
                duplicate=work_memory_result.duplicate,
                message=work_memory_result.message,
                provenance_quality=provenance_quality,
                identity_gaps=identity_gaps,
                blocked_by=list(work_memory_result.blocked_by),
                quality_gate=quality_gate,
                audit_trail=work_memory_result.audit_trail,
                work_memory=work_memory_result.work_memory,
                review_status=work_memory_result.review_status,
                active=work_memory_result.active,
                promotion_state=work_memory_result.promotion_state,
                superseded_record_id=work_memory_result.superseded_record_id,
                governance=work_memory_result.governance,
            )
        ...
        self._emit_capture_event(
            ...
            data={
                ...
                "role_id": item.role_id.strip(),
                "identity_gaps": list(identity_gaps),
                "blocked_by": list(quality_gate.get("blocked_by") or []),
                "provenance_quality": provenance_quality,
                "quality_gate": dict(quality_gate),
                "require_complete_identity": bool(item.require_complete_identity),
                "project_id": item.project_id.strip(),
