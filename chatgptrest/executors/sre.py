...
_DEFAULT_SRE_MODEL = "gpt-5.3-codex-spark"
...
def _default_sre_model() -> str:
    raw = _string_param(os.environ.get("CHATGPTREST_CODEX_SRE_MODEL_DEFAULT"))
    if raw:
        return raw
    raw = _string_param(os.environ.get("CODEX_SRE_MODEL"))
    return raw or _DEFAULT_SRE_MODEL


def _sre_reasoning_effort() -> str:
    return _string_param(os.environ.get("CHATGPTREST_CODEX_SRE_REASONING_EFFORT"), "low").lower()


def _sre_fresh_reasoning_effort() -> str:
    return _string_param(os.environ.get("CHATGPTREST_CODEX_SRE_FRESH_REASONING_EFFORT"), "medium").lower()


def _sre_fail_open_to_manual() -> bool:
    return _bool_param(os.environ.get("CHATGPTREST_CODEX_SRE_FAIL_OPEN_TO_MANUAL"), True)


def _codex_result_meta(result: Any) -> dict[str, Any]:
    return {
        "ok": bool(result.ok),
...
def _manual_decision_for_codex_failure(
    *,
    lane_id: str,
    model: str,
    resume_error: dict[str, Any] | None,
    fresh_error: dict[str, Any] | None,
) -> dict[str, Any]:
    error_summary = _string_param((fresh_error or {}).get("error")) or _string_param((resume_error or {}).get("error"))
    details: list[str] = []
    if resume_error:
        details.append("resume failed")
    if fresh_error:
        details.append("fresh diagnosis failed")
    if not details:
        details.append("codex unavailable")
    return {
        "summary": "Codex diagnosis unavailable; operator review is required.",
        "root_cause": truncate_text(
            f"SRE controller could not obtain a stable Codex decision for lane={lane_id} model={model}. "
            f"Failure chain: {', '.join(details)}. {error_summary}".strip(),
            limit=8000,
        ),
        "route": "manual",
        "confidence": "medium",
        "rationale": "Fail open to a structured manual route instead of retrying Codex blindly.",
        "recommended_actions": [
            "Inspect the incident pack and lane report for the recorded Codex error.",
            "Retry the controller only after the Codex lane is healthy if automation is still needed.",
        ],
        "gitnexus_queries": [],
        "runtime_fix": {},
        "open_pr": {},
        "notes": ["codex_error_manual_fallback"],
    }
...
def execute_sre_fix_request_controller(
    *,
    ...
    requested_model = _string_param(params_obj.get("model")) or None
    model = requested_model or _default_sre_model()
    model_explicit = requested_model is not None
    ...
    resume_reasoning_effort = _sre_reasoning_effort()
    fresh_reasoning_effort = _sre_fresh_reasoning_effort()
    codex_fail_open_to_manual = _sre_fail_open_to_manual()
    ...
    with _lane_lock(lane_dir):
        ...
        resume_result_meta: dict[str, Any] | None = None
        fresh_result_meta: dict[str, Any] | None = None
        if decision_override is not None:
            ...
            result = codex_resume_last_message_json(
                ...
                cwd=lane_dir,
                config_overrides=[f'model_reasoning_effort="{resume_reasoning_effort}"'],
            )
            resume_result_meta = _codex_result_meta(result)
            if not result.ok:
                resume_error = {
                    "returncode": result.returncode,
                    "error_type": result.error_type,
        ...
        if decision_override is None and heuristic_decision is None and (not use_resume or run_mode == "resume_fallback_fresh"):
            result = codex_exec_with_schema(
                ...
                cd=lane_dir,
                sandbox="read-only",
                config_overrides=[f'model_reasoning_effort="{fresh_reasoning_effort}"'],
            )
            fresh_result_meta = _codex_result_meta(result)
            if not result.ok:
                if codex_fail_open_to_manual:
                    run_mode = "codex_error_manual_fallback"
                    decision = _normalize_decision(
                        _manual_decision_for_codex_failure(
                            lane_id=lane_id,
                            model=model,
                            resume_error=resume_error,
                            fresh_error=fresh_result_meta,
                        )
                    )
                else:
                ...
                if reused_downstream is not None and dict(reused_downstream.get("downstream") or {}).get("kind") == "repair.autofix":
                    ...
                    try:
                        repair_job = create_repair_autofix_job(
                            ...
                            timeout_seconds=timeout_seconds,
                            model=(model if model_explicit else None),
        ...
        report = {
            ...
            "codex": {
                ...
                "fresh_reasoning_effort": fresh_reasoning_effort,
                "fail_open_to_manual": codex_fail_open_to_manual,
                "resume_result": resume_result_meta,
                "fresh_result": fresh_result_meta,
            },
            "repo": {
