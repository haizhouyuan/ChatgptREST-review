...
def execute_sre_fix_request_controller(
    *,
    ...
    with _lane_lock(lane_dir):
        ...
        playbook_excerpt = read_text((_REPO_ROOT / "docs" / "repair_agent_playbook.md").resolve(), limit=6_000)
        bootstrap_memory = load_maintagent_bootstrap_memory(max_chars=12_000)
        repo_memory = load_maintagent_repo_memory(max_chars=6000)
        ...
        _append_section(prompt_lines, title="Lane history", body=_json_preview(history))
        if manifest:
            _append_section(prompt_lines, title="Lane manifest", body=_json_preview(manifest))
        _append_section(prompt_lines, title="Maintagent bootstrap memory", body=str(bootstrap_memory.get("text") or ""))
        _append_section(prompt_lines, title="Maintagent repo memory", body=str(repo_memory.get("text") or ""))
        if issue_payload is not None:
            _append_section(prompt_lines, title="Issue summary", body=_json_preview(issue_payload))
        if issue_events:
            _append_section(prompt_lines, title="Recent issue events", body=_json_preview(issue_events, limit=10_000))
        if context_pack is not None:
            _append_section(prompt_lines, title="Provided context pack", body=_json_preview(context_pack, limit=12_000))
        if target_job_context is not None:
            _append_section(prompt_lines, title="Target job summary", body=_json_preview(target_job_context.get("summary")))
            _append_section(prompt_lines, title="Target result.json", body=str(target_job_context.get("result_json") or ""))
            _append_section(prompt_lines, title="Target run_meta.json", body=str(target_job_context.get("run_meta_json") or ""))
            _append_section(prompt_lines, title="Target events tail", body=str(target_job_context.get("events_tail") or ""))
            _append_section(prompt_lines, title="Target answer excerpt", body=str(target_job_context.get("answer_text") or ""))
        if gitnexus_payload:
            _append_section(prompt_lines, title="GitNexus query result", body=_json_preview(gitnexus_payload, limit=16_000))
        if playbook_excerpt:
            _append_section(prompt_lines, title="repair_agent_playbook.md excerpt", body=playbook_excerpt)
