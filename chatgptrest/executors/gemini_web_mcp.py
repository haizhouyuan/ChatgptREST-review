...
from chatgptrest.core.web_retry_policy import (
    human_visible_web_retry_after_int,
    human_visible_web_retry_after_seconds,
)
...
def _gemini_send_retry_delay() -> float:
    return human_visible_web_retry_after_seconds(_cfg.send_retry_delay, max_seconds=600.0)
...
def _is_retryable_gemini_send_exception(exc: BaseException) -> bool:
    combined = f"{type(exc).__name__}: {exc}".lower()
    if "deadline exceeded" in combined or "sse stream timeout" in combined:
        return False
    return (
        "connection refused" in combined
        or "econnrefused" in combined
        or "transport error" in combined
    )


def _looks_like_gemini_send_tool_timeout(exc: BaseException) -> bool:
    combined = f"{type(exc).__name__}: {exc}".lower()
    return any(
        marker in combined
        for marker in (
...
def _gemini_single_inline_max_bytes() -> int:
    return min(_gemini_bundle_per_file_max_bytes(), 12_000)


def _maybe_inline_single_text_attachment(
    *,
    question: str,
    ...
    if (
        ...
        or len(file_paths) != 1
        or not isinstance(original_file_paths, list)
        or len(original_file_paths) != 1
    ):
        return question, file_paths, None
    raw = str(file_paths[0] or "").strip()
    if not raw:
        return question, file_paths, None
    path = Path(raw)
    inline_max_bytes = _gemini_single_inline_max_bytes()
    try:
        file_size_bytes = int(path.stat().st_size)
    except Exception:
        return question, file_paths, {"single_text_inline_skipped": True, "inline_skip_reason": "stat_failed"}
    if file_size_bytes > inline_max_bytes:
        return question, file_paths, {
            "single_text_inline_skipped": True,
            "inline_skip_reason": "file_too_large",
            "inline_max_bytes": int(inline_max_bytes),
            "file_size_bytes": int(file_size_bytes),
        }
    text, reason = _read_text_snippet_from_file(path, max_bytes=inline_max_bytes)
    if not text:
        return question, file_paths, {"single_text_inline_skipped": True, "inline_skip_reason": reason}
    ...
    q = str(question or "").rstrip()
    if q:
        q += "\n\n"
    q += (
        "## Inlined Attachment Context\n"
        "The following content was inlined by ChatgptREST from the attached file. "
        "Treat it as the attachment contents for this request.\n\n"
        f"{section}"
    )
    return q, [], {"single_text_inlined": True, "inline_text_count": 1, "inlined_files": [path.name]}
...
def _gemini_wait_transient_retry_after_seconds() -> int:
    return human_visible_web_retry_after_int(
        GeminiExecutorConfig().wait_transient_retry_after_seconds,
        max_seconds=300,
        jitter=False,
    )
...
class GeminiWebMcpExecutor(BaseExecutor):
    ...
    async def _run_ask(self, *, job_id: str, input: dict[str, Any], params: dict[str, Any]) -> ExecutorResult:
        ...
        inline_attachment_meta: dict[str, Any] | None = None
        ...
        if phase != "wait" and _gemini_attachment_preprocess_enabled() and isinstance(file_paths, list) and file_paths:
            try:
                ...
                question, file_paths, inline_attachment_meta = _maybe_inline_single_text_attachment(
                    question=question,
                    file_paths=file_paths,
                    original_file_paths=raw_file_paths_for_hint,
                    deep_research=bool(deep_research_effective),
                )
                if inline_attachment_meta:
                    attachment_preprocess_meta = dict(attachment_preprocess_meta or {"enabled": True, "deep_research": bool(deep_research_effective)})
                    attachment_preprocess_meta.update(dict(inline_attachment_meta))
                    attachment_preprocess_meta["uploaded_count"] = int(len(file_paths or []))
            except Exception as exc:
        ...
        def _idempotency_record_to_send_recovery_result(
            *,
            record: dict[str, Any],
            ...
            if not isinstance(record, dict):
                return None, ""
            recovered_url = _conversation_url_from_idempotency_record(record)
            record_result = record.get("result") if isinstance(record.get("result"), dict) else None
            if isinstance(record_result, dict):
                recovered = dict(record_result)
                if recovered_url and not str(recovered.get("conversation_url") or "").strip():
                    recovered["conversation_url"] = recovered_url
                if (
                    str(recovered.get("status") or "").strip().lower() == "in_progress"
                    and str(recovered.get("conversation_url") or "").strip()
                ):
                    recovered.setdefault("wait_handoff_ready", True)
                recovered.setdefault(
                    "send_exception_recovery",
                    {
                        "applied": True,
                        "source": "idempotency_result",
                        "exception": f"{type(send_exc).__name__}: {send_exc}" if send_exc is not None else None,
                    },
                )
                recovered = _merge_idempotency_recovery_meta(recovered, recovered_url)
                return recovered, str(recovered.get("conversation_url") or recovered_url or "").strip()

            record_status = str(record.get("status") or "").strip().lower()
            if record_status != "in_progress":
                return None, recovered_url

            sent = bool(record.get("sent"))
            if sent or recovered_url:
                pending_url = recovered_url or "https://gemini.google.com/app"
                recovered = {
                    "ok": False,
                    "status": "in_progress",
                    "answer": "",
                    ...
                    "error": (
                        "Gemini send timed out after the driver created an in-progress idempotency record; "
                        "recover via wait/sidebar instead of resending immediately."
                    ),
                    "wait_handoff_ready": True,
                    "wait_handoff_reason": "idempotency_send_in_progress",
                    "retry_after_seconds": 15,
                    "not_before": _now() + 15.0,
                    "send_exception_recovery": {
                        "applied": True,
                ...
                recovered = _merge_idempotency_recovery_meta(recovered, recovered_url or pending_url)
                return recovered, pending_url

            if send_exc is not None and _looks_like_gemini_send_tool_timeout(send_exc):
                recovered = {
                    "ok": False,
                    "status": "in_progress",
                    "answer": "",
                    "conversation_url": "https://gemini.google.com/app",
                    "error_type": "GeminiSendPendingUnknown",
                    "error": (
                        "Gemini send tool timed out after idempotency began but before stable thread evidence was "
                        "captured; recover via wait/sidebar instead of resending immediately."
                    ),
                    "wait_handoff_ready": True,
                    "wait_handoff_reason": "timeout_blank_idempotency_in_progress",
                    "retry_after_seconds": 15,
                    "not_before": _now() + 15.0,
                    "send_exception_recovery": {
                        "applied": True,
                        "source": "blank_in_progress_timeout",
                        "exception": f"{type(send_exc).__name__}: {send_exc}",
                    },
                }
                return recovered, ""
        ...
        async def _recover_send_exception_from_idempotency(
            *,
            tool_name: str,
            key: str,
            ...
            if not str(tool_name or "").strip():
                return None, ""
            if not _looks_like_gemini_send_tool_timeout(send_exc):
                return None, ""
            ...
            for attempt in range(attempts):
                ...
                if isinstance(res, dict):
                    record = res.get("record") if isinstance(res.get("record"), dict) else {}
                    recovered, recovered_url = _idempotency_record_to_send_recovery_result(
                        record=record,
                        send_exc=send_exc,
                    )
                    if recovered is not None:
                        return recovered, recovered_url
                    record_status = str(record.get("status") or "").strip().lower()
                    if record_status not in {"", "in_progress"}:
                        break
        ...
        if phase == "wait":
            ...
            if drive_files:
                tool_args["drive_files"] = drive_files
            if github_repo and enable_import_code:
                tool_args["github_repo"] = github_repo
                tool_args["enable_import_code"] = True
            elif github_repo:
                # Keep review-safe repo context for prompt grounding and Drive attach
                # fail-open, but do not trigger Gemini Import code tooling.
                tool_args["repo_context_hint"] = github_repo
            ...
            for send_attempt in range(send_max_retries + 1):
                try:
                    ...
                    recovered_result, recovered_url = await _recover_send_exception_from_idempotency(
                        tool_name=driver_tool_name,
                        key=str(tool_args.get("idempotency_key") or primary_key),
                        send_exc=send_exc,
                    )
                    if recovered_result is not None:
                        result = recovered_result
                        if recovered_url:
                            recovered_wait_url = recovered_url
                        logger.warning(
                            "gemini send exception recovered from idempotency: attempt=%d/%d tool=%s error=%s",
                            send_attempt + 1,
                            send_max_retries + 1,
                            driver_tool_name,
                            send_exc,
                        ...
                        break
                    if _is_retryable_gemini_send_exception(send_exc) and send_attempt < send_max_retries:
                        logger.warning(
