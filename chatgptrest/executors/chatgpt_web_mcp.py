...
def _thought_guard_short_answer_missing_trace_details(
    *,
    ...
    if isinstance(obs, dict):
        thought_for_present = bool(obs.get("thought_for_present"))
        if not thought_for_present:
            thought_seconds = obs.get("thought_seconds")
            thought_for_present = isinstance(thought_seconds, (int, float)) and int(thought_seconds) > 0
    if thought_for_present:
        return None
