...
_CHATGPT_WAIT_TRANSIENT_ERROR_RE = re.compile(
    r"("
...
_CHATGPT_TRANSIENT_DRIVER_RESULT_RE = re.compile(
    r"("
    r"targetclosederror|"
    r"browser has been closed|"
    r"browser not launched|"
...
_CHATGPT_SELF_CHECK_MANUAL_REPAIR_REASONS = frozenset({
...
_CHATGPT_PREFLIGHT_BLOCKED_REASONS = frozenset({
    "frontend_rate_limit",
    "manual_pro_session",
    "auth",
    "cloudflare",
...
def _looks_like_chatgpt_wait_transient_error(exc: BaseException | str) -> bool:
    error_type = ""
    error_text = ""
    ...
    if error_type_l in {
        ...
        return True
    return bool(_CHATGPT_WAIT_TRANSIENT_ERROR_RE.search(error_text))


def _looks_like_chatgpt_transient_driver_result(result: dict[str, Any]) -> bool:
    status = str(result.get("status") or "").strip().lower()
    if status not in {"error", "blocked", "cooldown"}:
        return False
    error_text = f"{str(result.get('error_type') or '')} {str(result.get('error') or '')}".strip()
    return bool(_CHATGPT_TRANSIENT_DRIVER_RESULT_RE.search(error_text))


def _chatgpt_wait_transient_failure_limit() -> int:
    return max(1, min(_cfg.wait_transient_failure_limit, 8))


def _chatgpt_send_transient_retry_limit() -> int:
    return max(0, min(_cfg.send_transient_retry_limit, 4))


def _chatgpt_wait_transient_retry_after_seconds() -> int:
    return max(
        30,
        min(
            int(human_visible_web_retry_after_seconds(_cfg.wait_transient_retry_after_seconds, max_seconds=300.0)),
            300,
        ),
    )


def _chatgpt_self_check_timeout_seconds() -> int:
    return max(10, min(_cfg.self_check_timeout_seconds, 180))


def _chatgpt_self_check_manual_repair_reason(payload: dict[str, Any]) -> str | None:
    ...
    if isinstance(blocked_state, dict):
        ...
        if reason in _CHATGPT_SELF_CHECK_MANUAL_REPAIR_REASONS:
            return reason
    combined = f"{str(payload.get('error_type') or '')} {str(payload.get('error') or '')}".strip().lower()
    for marker in _CHATGPT_SELF_CHECK_MANUAL_REPAIR_REASONS:
...
def _chatgpt_self_check_error_summary(payload: dict[str, Any]) -> str:
    error_type = str(payload.get("error_type") or "ChatGPTSelfCheckFailed").strip() or "ChatGPTSelfCheckFailed"
    error = str(payload.get("error") or "").strip() or "<empty error>"
    blocked_state = payload.get("blocked_state")
    blocked_reason = ""
    if isinstance(blocked_state, dict):
        blocked_reason = str(blocked_state.get("reason") or "").strip().lower()
    if blocked_reason:
        return f"{error_type}: {error} (blocked_reason={blocked_reason})"
...
def _db_chatgpt_web_hold() -> dict[str, Any] | None:
    raw = str(os.environ.get("CHATGPTREST_DB_PATH") or "").strip()
    path = Path(raw).expanduser() if raw else Path(__file__).resolve().parents[2] / "state" / "jobdb.sqlite3"
    ...
    try:
        ...
        return None
    return hold.to_detail() if hold is not None else None
...
def _live_missing_input_files(file_paths: list[str] | None) -> list[str]:
...
def _thought_guard_require_trace_for_research() -> bool:
    return _cfg.thought_guard_require_trace_for_research
...
_THOUGHT_GUARD_PRESETS = frozenset({"pro_extended", "thinking_extended", "thinking_heavy"})
...
def _thought_guard_has_thought_trace(obs: dict[str, Any] | None) -> bool:
    if not isinstance(obs, dict):
        return False
    if bool(obs.get("thought_for_present")):
        return True
    thought_seconds = obs.get("thought_seconds")
    return isinstance(thought_seconds, (int, float)) and int(thought_seconds) > 0


def _thought_guard_short_answer_missing_trace_details(
    *,
    preset: str,
    status: str,
    ...
    if str(preset or "").strip().lower() not in _THOUGHT_GUARD_PRESETS:
        return None
    if str(status or "").strip().lower() != "completed":
        return None
    if not str(conversation_url or "").strip():
        return None
    min_chars_required = int(max(0, int(min_chars_required or 0)))
    if min_chars_required <= 0:
        return None

    answer_chars = len(str(answer_text or "").strip())
    if answer_chars <= 0 or answer_chars >= min_chars_required:
        return None

    if _thought_guard_has_thought_trace(obs):
        return None

    return {
        "reason": "missing_thought_for_short_answer",
        "missing_observation": not bool(obs),
        "thought_for_present": False,
...
def _thought_guard_research_answer_missing_trace_details(
    *,
    preset: str,
    status: str,
    conversation_url: str | None,
    ...
    if preset_l not in _THOUGHT_GUARD_PRESETS:
        return None
    if str(status or "").strip().lower() != "completed":
        return None
    if not str(conversation_url or "").strip():
        return None
    if not is_research_contract_params(params):
        return None
    answer_chars = len(str(answer_text or "").strip())
    if answer_chars <= 0:
        return None
    if _thought_guard_has_thought_trace(obs):
        return None
    return {
        "reason": "missing_thought_for_research_answer",
        "missing_observation": not bool(obs),
        "thought_for_present": False,
        "answer_chars": int(answer_chars),
...
def _thought_guard_completed_fail_closed_reason(
    *,
    ...
    if str(status or "").strip().lower() != "completed":
        return None
    reason = str((guard_details or {}).get("reason") or "").strip().lower()
    if not reason:
        return None
    if reason in {"missing_thought_for_short_answer", "missing_thought_for_research_answer"}:
        return reason
    if bool((guard_details or {}).get("require_thought_for")) and reason == "missing_thought_for":
        return reason
    try:
        min_chars = max(0, int(min_chars_required or 0))
    except Exception:
        min_chars = 0
    answer_chars = len(str(answer_text or "").strip())
    if min_chars > 0 and 0 < answer_chars < min_chars and reason in {"thought_too_short", "skipping", "answer_now"}:
        return reason
    return None
...
class ChatGPTWebMcpExecutor(BaseExecutor):
    ...
    async def run(self, *, job_id: str, kind: str, input: dict[str, Any], params: dict[str, Any]) -> ExecutorResult:  # noqa: A002
        ...
        hold = _db_chatgpt_web_hold()
        if hold is not None:
            reason = str(hold.get("reason") or "chatgpt_web_hold").strip()
            retry_after = hold.get("retry_after_seconds")
            return ExecutorResult(
                status="blocked",
                answer=f"ChatGPT Web automation hold active: {reason}",
                meta={
                    "error_type": "Blocked",
                    "error": f"ChatGPT Web automation hold active: {reason}",
                    "retry_after_seconds": retry_after,
                    "not_before": hold.get("blocked_until"),
                    "chatgpt_web_hold": hold,
                    "trace_id": _trace_id,
                },
            )
        ...
        try:
            ...
            if isinstance(preflight, dict) and bool(preflight.get("blocked")):
                ...
                status = (
                    "blocked"
                    if reason.strip().lower() in _CHATGPT_PREFLIGHT_BLOCKED_REASONS
                    else ("cooldown" if wait_seconds > 0 else "blocked")
                )
                return ExecutorResult(
                    status=status,
        ...
        if phase != "wait" and deep_research and send_timeout_raw is None:
            deep_research_send_floor = max(0, int(_cfg.deep_research_send_timeout_seconds or 0))
            if deep_research_send_floor > 0:
                send_timeout_seconds = max(send_timeout_seconds, deep_research_send_floor)
        ...
        thought_guard_require_trace_for_research = _thought_guard_require_trace_for_research()
        ...
        if phase != "wait":
            missing_input_files = _live_missing_input_files(file_paths if isinstance(file_paths, list) else None)
            if missing_input_files:
                preview = ", ".join(missing_input_files[:3])
                if len(missing_input_files) > 3:
                    preview += f" (+{len(missing_input_files) - 3} more)"
                message = (
                    "One or more declared input.file_paths no longer exist on disk. "
                    "Resend with valid file_paths or recreate the missing files before retrying. "
                    f"Missing: {preview}"
                )
                return ExecutorResult(
                    status="error",
                    ...
                    meta={
                        ...
                        "error": message,
                        "missing_file_paths": list(missing_input_files),
                    },
                )
        ...
        async def _chatgpt_self_check_payload(*, conversation_url: str | None) -> dict[str, Any]:
            tool_args: dict[str, Any] = {"timeout_seconds": _chatgpt_self_check_timeout_seconds()}
            if conversation_url:
                tool_args["conversation_url"] = str(conversation_url).strip()
            try:
                result = await asyncio.to_thread(
                    self._client.call_tool,
                    tool_name="chatgpt_web_self_check",
                    tool_args=tool_args,
                    timeout_sec=float(_chatgpt_self_check_timeout_seconds()) + 15.0,
                )
            except Exception as exc:
                return {
                    "ok": False,
                    "status": "error",
                    "error_type": "ChatGPTSelfCheckError",
            ...
            if not isinstance(result, dict):
                return {
                    ...
                    "error": "chatgpt_web_self_check returned non-dict result",
                }
            return dict(result)
        ...
        async def _wait_loop(*, conversation_url: str | None, idempotency_key: str | None) -> tuple[dict[str, Any], str]:
            ...
            transient_failure_limit = _chatgpt_wait_transient_failure_limit()
            deadline = _now() + float(max_wait_seconds)
            transient_failures = 0

            while _now() < deadline:
                remaining_wait_seconds = max(30, min(wait_timeout_seconds, int(deadline - _now())))
                try:
                    return await self._wait_loop_core(
                        ...
                        max_wait_seconds=remaining_wait_seconds,
                        min_chars=min_chars,
                        on_wait_result=_on_chatgpt_wait_result,
                        now_fn=_now,
                    )
                except Exception as exc:
                    if not _looks_like_chatgpt_wait_transient_error(exc):
                        raise
                    transient_failures += 1
                    self_check = await _chatgpt_self_check_payload(conversation_url=url or None)
                    manual_reason = _chatgpt_self_check_manual_repair_reason(self_check)
                    if bool(self_check.get("ok")) and transient_failures <= transient_failure_limit:
                        await asyncio.sleep(min(1.0 + float(transient_failures), 3.0))
                        continue

                    retry_after = _chatgpt_wait_transient_retry_after_seconds()
                    recovery = {
                        "action": ("manual_followup" if manual_reason else "requeue_wait"),
                        "failure_count": int(transient_failures),
                        ...
                        return (
                            {
                                ...
                                "error_type": str(self_check.get("error_type") or "VerificationRequired"),
                                "error": f"{error_text}; self_check={_chatgpt_self_check_error_summary(self_check)}",
                                "retry_after_seconds": int(retry_after),
                                "not_before": _now() + float(retry_after),
                                "wait_transient_failures": int(transient_failures),
                                "transient_runtime_recovery": recovery,
                            },
                            url,
                        )
                    return (
        ...
        if phase == "wait":
            ...
            send_transient_retries = 0
            send_transient_limit = _chatgpt_send_transient_retry_limit()
            while True:
                try:
                    result = await asyncio.to_thread(
                        self._client.call_tool,
                    ...
                    if send_transient_retries < send_transient_limit and _looks_like_chatgpt_wait_transient_error(exc):
                        self_check = await _chatgpt_self_check_payload(conversation_url=conversation_url)
                        recovered_url = ""
                        idem_sent = None
                        try:
                        ...
                        if not conversation_url:
                            try:
                                recovered_url = await _resolve_conversation_url_from_idempotency(primary_key)
                            except Exception:
                                recovered_url = ""
                        recovered_thread_url = _normalize_chatgpt_conversation_url(recovered_url) or ""
                        if bool(self_check.get("ok")):
                            send_transient_retries += 1
                            await asyncio.sleep(min(1.0 + float(send_transient_retries), 3.0))
                            continue
                        wait_seconds = _chatgpt_wait_transient_retry_after_seconds()
                        return ExecutorResult(
                            status="cooldown",
                            ...
                            meta={
                                ...
                                "error": (
                                    f"{exc}; self_check={_chatgpt_self_check_error_summary(self_check)}"
                                ),
                                "retry_after_seconds": wait_seconds,
                                "not_before": _now() + wait_seconds,
                                "transient_runtime_recovery": {
                                    "phase": "send",
                                    "failure_count": int(send_transient_retries + 1),
                                    "self_check": self_check,
                                },
                                "send_phase_evidence": {
                                    "idempotency_sent": idem_sent,
                                    "recovered_conversation_url": str(recovered_thread_url or "").strip() or None,
                                    "requested_conversation_url": str(conversation_url or "").strip() or None,
                                    "safe_to_retry_send": bool(idem_sent is False and not str(recovered_thread_url or "").strip()),
                                },
                            },
                        )
                    wait_seconds = 60.0
                ...
                if (
                    ...
                    and send_transient_retries < send_transient_limit
                    and _looks_like_chatgpt_transient_driver_result(result)
                ):
                    self_check = await _chatgpt_self_check_payload(conversation_url=conversation_url)
                    if bool(self_check.get("ok")):
                        send_transient_retries += 1
                        await asyncio.sleep(min(1.0 + float(send_transient_retries), 3.0))
                        continue
                    result.setdefault("transient_runtime_recovery", {})
                    try:
                        result["transient_runtime_recovery"] = dict(result.get("transient_runtime_recovery") or {})
                    except Exception:
                        result["transient_runtime_recovery"] = {}
                    result["transient_runtime_recovery"].update(
                        {
                            "phase": "send",
                            "failure_count": int(send_transient_retries + 1),
                            "self_check": self_check,
                    ...
                    if not str(result.get("error") or "").strip():
                        result["error"] = _chatgpt_self_check_error_summary(self_check)
                break
            ...
            if status == "in_progress" and not conversation_url:
                ...
                if recovered:
                    ...
                    wait_seconds = human_visible_web_retry_after_seconds(90.0, max_seconds=1800.0)
            ...
            # For thinking/pro presets, a missing/too-short "Thought for" trace or a visible
            # Skipping/Answer-now marker indicates degraded generation. Optionally trigger a
            # single UI Regenerate attempt on the same conversation without re-sending a prompt.
            should_thought_guard = bool(preset in _THOUGHT_GUARD_PRESETS and (not deep_research or thought_guard_require_thought_for))
            if should_thought_guard:
                ...
                if not abnormal:
                    short_without_trace = _thought_guard_short_answer_missing_trace_details(
                        preset=preset,
                        status=status,
                        conversation_url=conversation_url,
                        answer_text=str(result.get("answer") or ""),
                        min_chars_required=int(min_chars),
                        obs=obs,
                    )
                    if short_without_trace:
                        abnormal = True
                        guard_details = {**guard_details, **short_without_trace}
                if not abnormal and thought_guard_require_trace_for_research:
                    research_without_trace = _thought_guard_research_answer_missing_trace_details(
                        preset=preset,
                        status=status,
                        conversation_url=conversation_url,
                        params=params,
                    ...
                    if research_without_trace:
                        abnormal = True
                        guard_details = {**guard_details, **research_without_trace}
                if abnormal:
                    ...
                    thought_guard_fail_closed_reason = _thought_guard_completed_fail_closed_reason(
                    ...
                    if thought_guard_auto_regen and conversation_url:
                        try:
                            ...
                            if isinstance(regen, dict) and bool(regen.get("ok")) and str(regen.get("status") or "").strip().lower() == "completed":
                                ...
                                result = regen
                                status = "completed"
                                thought_guard_fail_closed_reason = None
                            else:
                                regen_status = (
                                    str((regen or {}).get("status") or "").strip().lower()
                                    if isinstance(regen, dict)
                                    else ""
                                )
                                if isinstance(regen, dict) and bool(regen.get("ok")) and regen_status in {
                                    "in_progress",
                                    "queued",
                                    "cooldown",
                                    "blocked",
                                    ...
                                    regen["_thought_guard"].update(
                                        {
                                            "enabled": True,
                                            "action": "regenerate_pending",
                                            "regenerate_status": regen_status,
                                            "prev": guard_details,
                                        }
                                    )
                                    result = regen
                                    ...
                                    try:
                                        ...
                                        result["_thought_guard"]["regenerate_status"] = (
                                            regen_status if isinstance(regen, dict) else type(regen).__name__
                                        )
                                    except Exception:
                                        result["_thought_guard"]["action"] = "regenerate_skipped"
                    if thought_guard_fail_closed_reason and status == "completed":
                        try:
                            if not str(result["_thought_guard"].get("action") or "").strip():
                                result["_thought_guard"]["action"] = "fail_closed"
                            result["_thought_guard"]["fail_closed"] = True
                            result["_thought_guard"]["fail_closed_reason"] = thought_guard_fail_closed_reason
                        except Exception:
                            pass
                        result["status"] = "needs_followup"
                        result.setdefault("error_type", "ThoughtGuardMissingTrace")
                        result.setdefault(
                            "error",
                            (
                                "ChatGPT Pro/thinking answer completed without a usable thinking trace "
                                "for the requested quality contract; require same-session retry "
                                "or explicit follow-up before treating it as external review evidence."
                            ),
                        )
                        status = "needs_followup"
