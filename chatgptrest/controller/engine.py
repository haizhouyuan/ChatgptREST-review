...
class ControllerEngine:
    ...
    def _emit_runtime_event(
        ...
        *,
        ...
        event_bus = self._state.get("event_bus")
    ...
    def ask(
        ...
        *,
        ...
        if kb_direct_completion_allowed(route_plan["graph_state"]) and route_plan["kb_chunks"]:
            ...
            self._emit_runtime_event(
                ...
                data={
                    "run_id": run_id,
                    "route": route,
                    "artifact_ids": list(evidence_refs),
                    "artifact_id": str(evidence_refs[0]) if evidence_refs else "",
                    "kb_hit_count": int(route_plan["kb_hit_count"] or 0),
                },
            )
        ...
        if execution_kind == "coding_agent":
            step_result, dispatch_request = self._dispatch_route_to_coding_agent(
                run_id=run_id,
                question=question,
                route_plan=route_plan,
                stable_context=stable_context,
            ...
            with connect(self._cfg.db_path) as conn:
                ...
                self._persist_step_result(
                    ...
                    run_id=run_id,
                    work_id=_RUN_STEP_EXECUTE,
                    title="Dispatch route into the coding-agent execution lane",
                    kind="execution",
                    owner="controller",
                    lane=(
                        f"coding_agent:{str((dispatch_request or {}).get('resolved_executor', {}).get('executor_id') or '')}"
                        if dispatch_request is not None
                        else "coding_agent"
                    ),
                    step_result=step_result,
                    depends_on=[_RUN_STEP_PLAN],
                    route=route,
                    provider=str((dispatch_request or {}).get("resolved_executor", {}).get("executor_id") or "coding_agent"),
                    preset="coding_agent",
                )
                conn.commit()
            if dispatch_request is not None:
                self._start_coding_agent_dispatch_worker(**dispatch_request)
            snapshot = self.get_run_snapshot(run_id=run_id)
            selected_executor = str((dispatch_request or {}).get("resolved_executor", {}).get("executor_id") or "coding_agent")
            return {
                "ok": True,
                "trace_id": trace_id,
                "run_id": run_id,
        ...
        params_obj = {
            ...
            "min_chars": self._default_job_min_chars(route=route, stable_context=stable_context),
    ...
    def _requested_execution_lane(self, *, stable_context: dict[str, Any], route_plan: dict[str, Any]) -> str:
        lane_policy = stable_context.get("lane_policy")
        if isinstance(lane_policy, dict):
            lane = str(lane_policy.get("execution_lane") or "").strip().lower()
            if lane:
                return lane
        execution_request = stable_context.get("execution_request")
        if isinstance(execution_request, dict):
            lane = str(execution_request.get("requested_execution_lane") or "").strip().lower()
            if lane:
                return lane
        return str(route_plan.get("executor_lane") or "").strip().lower()

    def _default_job_min_chars(self, *, route: str, stable_context: dict[str, Any]) -> int:
        if route not in {"quick_ask", "kb_answer", "clarify", "hybrid"}:
            return 800
        scenario_pack = stable_context.get("scenario_pack")
        if isinstance(scenario_pack, dict):
            provider_hints = scenario_pack.get("provider_hints")
            planning_mode = ""
            if isinstance(provider_hints, dict):
                planning_mode = str(provider_hints.get("planning_mode") or "").strip().lower()
            scenario = str(scenario_pack.get("scenario") or "").strip().lower()
            output_shape = str(scenario_pack.get("output_shape") or "").strip().lower()
            if (
                planning_mode == "compact_next_steps"
                and scenario == "planning"
                and output_shape == "planning_memo"
            ):
                return 60
        return 200
    ...
    def _build_objective_plan(
        ...
        *,
        ...
        requested_execution_lane = self._requested_execution_lane(stable_context=stable_context, route_plan=route_plan)
        ...
        elif requested_execution_lane == "coding_agent":
            objective_kind = "coding_agent_delivery"
        ...
        elif objective_kind == "coding_agent_delivery":
            success_criteria.append({"type": "coding_agent_execution_tracked", "value": True})
        else:
    ...
    def _resolve_execution_kind(self, *, route_plan: dict[str, Any], stable_context: dict[str, Any]) -> str:
        ...
        if route == "action":
            return "effect"
        requested_execution_lane = self._requested_execution_lane(stable_context=stable_context, route_plan=route_plan)
        if requested_execution_lane == "coding_agent":
            return "coding_agent"
        execution_preference = self._scenario_execution_preference(stable_context=stable_context)
    ...
    def _dispatch_route_to_coding_agent(
        self,
        *,
        run_id: str,
        question: str,
        ...
        execution_request = dict(stable_context.get("execution_request") or {})
        resolved = resolve_coding_agent_executor(
            requested_executor=execution_request.get("requested_executor"),
            requested_family=execution_request.get("requested_executor_family"),
            requested_model=execution_request.get("requested_model"),
            requested_effort=execution_request.get("requested_effort"),
            request_source=str(execution_request.get("request_source") or "task_intake.context"),
        )
        if resolved is None:
            return (
                StepResult(
                    work_status="WAITING_HUMAN",
                    controller_status="WAITING_HUMAN",
                    summary="No coding-agent executor could be resolved for this planning request.",
                    next_action={"type": "select_executor", "status": "blocking"},
                    output={"error": "coding_agent_executor_missing"},
                    delivery={
                        "status": "waiting_human",
                        "summary": "A coding-agent lane was requested, but no executor could be resolved.",
                        "blockers": ["Missing coding-agent executor"],
                    },
                ),
                None,
            )
        if not resolved.ready:
            return (
                StepResult(
                    work_status="WAITING_HUMAN",
                    controller_status="WAITING_HUMAN",
                    summary=f"Requested coding-agent executor `{resolved.executor_id}` is not ready on this host.",
                    next_action={
                        "type": "select_executor",
                        "status": "blocking",
                        "executor_id": resolved.executor_id,
                    },
                    output={"error": "coding_agent_executor_not_ready", "executor_id": resolved.executor_id},
                    delivery={
                        "status": "waiting_human",
                        "summary": f"Coding-agent executor `{resolved.executor_id}` is not ready on this host.",
                        "blockers": [f"executor_not_ready:{resolved.executor_id}"],
                    },
                ),
                None,
            )
        output = {
            ...
            "executor_id": resolved.executor_id,
            "executor_family": resolved.family,
            "status": "submitted",
        }
        return (
            StepResult(
                work_status="RUNNING",
                controller_status="WAITING_EXTERNAL",
                summary=f"Coding-agent executor `{resolved.executor_id}` submitted and tracked by the controller.",
                next_action={
                    "type": "await_coding_agent_completion",
                    "status": "pending",
                    "executor_id": resolved.executor_id,
                    "executor_family": resolved.family,
                },
                ...
                delivery={
                    "status": "submitted",
                    "summary": f"Submitted to coding-agent executor `{resolved.executor_id}`.",
                    "execution_lane": "coding_agent",
                    "executor_id": resolved.executor_id,
                },
            ),
            {
                ...
                "route": str(route_plan.get("route") or ""),
                "question": question,
                "stable_context": dict(stable_context or {}),
                "resolved_executor": resolved.to_public_dict(),
                "timeout_seconds": int(timeout_seconds),
            },
        )

    def _start_coding_agent_dispatch_worker(
        self,
        *,
        run_id: str,
        route: str,
        question: str,
        stable_context: dict[str, Any],
        resolved_executor: dict[str, Any],
        timeout_seconds: int,
    ) -> None:
        worker = threading.Thread(
            target=self._run_coding_agent_dispatch,
            kwargs={
                "run_id": run_id,
                "route": route,
                "question": question,
            ...
            daemon=True,
            name=f"controller-coding-agent-{run_id[:8]}",
        )
        worker.start()

    def _run_coding_agent_dispatch(
        self,
        *,
        run_id: str,
        route: str,
        ...
        resolved = resolve_coding_agent_executor(
            requested_executor=resolved_executor.get("executor_id"),
            requested_family=resolved_executor.get("family"),
            requested_model=resolved_executor.get("requested_model"),
            requested_effort=resolved_executor.get("requested_effort"),
            request_source=str(resolved_executor.get("request_source") or "task_intake.context"),
        )
        if resolved is None:
            return
        try:
            result = run_coding_agent_executor(
                resolved=resolved,
                question=question,
                stable_context=stable_context,
                trace_id=str(stable_context.get("trace_id") or run_id),
                timeout_seconds=timeout_seconds,
            )
            if result.ok:
                summary = result.summary or result.answer[:240] or f"Coding-agent executor `{resolved.executor_id}` completed."
                step_result = StepResult(
                    ...
                    delivery={
                        ...
                        "blockers": [],
                        "decisions": [
                            {"type": "route", "value": route, "reason": "coding_agent_completed"},
                            {"type": "execution_lane", "value": "coding_agent", "reason": result.executor_id},
                        ],
                        "artifacts": [f"coding_agent:{run_id}"],
                    },
                    artifacts=[
                        ControllerArtifact(
                            artifact_id=f"coding_agent:{run_id}",
                            work_id=_RUN_STEP_EXECUTE,
                            kind="coding_agent_result",
                            title=f"Coding-agent result: {result.executor_id}",
                ...
                summary = result.error or f"Coding-agent executor `{resolved.executor_id}` failed."
                step_result = StepResult(
                    work_status="FAILED",
                    controller_status="FAILED",
                    summary=summary,
                    next_action={
                        "type": "investigate_or_retry",
                        "status": "blocking",
                        "executor_id": resolved.executor_id,
                    },
                    output={
                    ...
                    artifacts=[
                        ControllerArtifact(
                            ...
                            work_id=_RUN_STEP_EXECUTE,
                            kind="coding_agent_result",
                            title=f"Coding-agent result: {resolved.executor_id}",
                            path=result.artifact_path,
                            metadata={
                                "execution_lane": "coding_agent",
                                "executor_id": resolved.executor_id,
            ...
            step_result = StepResult(
                work_status="FAILED",
                controller_status="FAILED",
                summary=f"Coding-agent executor failed: {exc}",
                next_action={"type": "investigate_or_retry", "status": "blocking", "executor_id": resolved.executor_id},
                output={"error": str(exc), "executor_id": resolved.executor_id, "execution_lane": "coding_agent"},
                delivery={"status": "failed", "summary": f"Coding-agent executor failed: {exc}"},
            )
        try:
            with connect(self._cfg.db_path) as conn:
                conn.execute("BEGIN IMMEDIATE")
                self._persist_step_result(
                    ...
                    run_id=run_id,
                    work_id=_RUN_STEP_EXECUTE,
                    title="Dispatch the selected route to a coding-agent execution lane",
                    kind="execution",
                    owner="controller",
                    lane=f"coding_agent:{resolved.executor_id}",
                    step_result=step_result,
                    depends_on=[_RUN_STEP_PLAN],
                    provider=resolved.executor_id,
                    preset="coding_agent",
                    route=route,
                )
                conn.commit()
    ...
    def _job_completion_view(self, *, job: Any) -> dict[str, Any]:
        job_id = str(getattr(job, "job_id", "") or "").strip()
        payload: dict[str, Any] = {}
        if job_id:
            try:
                result_path = artifacts.job_dir(self._cfg.artifacts_dir, job_id) / "result.json"
        ...
        source = {
            ...
            "answer_path": str(getattr(job, "answer_path", "") or ""),
            "answer_chars": getattr(job, "answer_chars", None),
            "answer_format": str(getattr(job, "answer_format", "") or ""),
            "conversation_export_path": str(getattr(job, "conversation_export_path", "") or ""),
            **payload,
        }
        completion_contract = completion_contract_from_job_like(source)
        canonical_answer = canonical_answer_from_job_like(source)
        return {
            "completion_contract": dict(completion_contract),
            "canonical_answer": dict(canonical_answer),
            "answer_state": get_completion_answer_state(source),
            "canonical_ready": bool(canonical_answer.get("ready")),
            "authoritative_job_id": get_authoritative_job_id(source),
            "authoritative_answer_path": get_authoritative_answer_path(source),
            "answer_provenance": dict(get_answer_provenance(source)),
        }
    ...
    def _reconcile_job_work_item(self, conn: Any, *, run: dict[str, Any], work_item: dict[str, Any]) -> bool:
        ...
        if status_value == "completed":
            completion_view = self._job_completion_view(job=job)
            authoritative_job_id = str(completion_view.get("authoritative_job_id") or "").strip() or None
            authoritative_answer_path = str(completion_view.get("authoritative_answer_path") or "").strip() or None
            answer_state = str(completion_view.get("answer_state") or "").strip().lower() or "partial"
            canonical_ready = bool(completion_view.get("canonical_ready"))
            contract_answer_chars = dict(completion_view.get("completion_contract") or {}).get("answer_chars")
        else:
            completion_view = {}
            authoritative_job_id = None
            authoritative_answer_path = None
            answer_state = "partial"
            canonical_ready = False
            contract_answer_chars = None
        if status_value == "completed" and canonical_ready and authoritative_answer_path:
            ...
            step_result = StepResult(
                ...
                output={
                    **dict(work_item.get("output") or {}),
                    ...
                    "answer_path": authoritative_answer_path,
                    "answer_state": answer_state,
                    "authoritative_job_id": authoritative_job_id,
                    "authoritative_answer_path": authoritative_answer_path,
                    "answer_provenance": dict(completion_view.get("answer_provenance") or {}),
                    "completion_contract": dict(completion_view.get("completion_contract") or {}),
                    "canonical_answer": dict(completion_view.get("canonical_answer") or {}),
                ...
                artifacts=[
                    ControllerArtifact(
                        ...
                        metadata={
                            ...
                            "authoritative_job_id": authoritative_job_id,
                            "answer_chars": (
                                int(contract_answer_chars)
                                if isinstance(contract_answer_chars, int)
                                else job.answer_chars
                            ),
                        },
                    )
                ],
            ...
            return True
        if status_value == "completed":
            summary = "External job completed with provisional answer; awaiting authoritative completion."
            step_result = StepResult(
                work_status="RUNNING",
                controller_status="WAITING_EXTERNAL",
                summary=summary,
                next_action={
                    ...
                    "job_id": job_id,
                    "provider": str(run.get("provider") or ""),
                    "objective_step_id": str(work_item.get("work_id") or _RUN_STEP_EXECUTE),
                    "answer_state": answer_state,
                    "authoritative_job_id": authoritative_job_id,
                    "authoritative_answer_path": authoritative_answer_path,
                },
