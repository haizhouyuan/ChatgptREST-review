...
def _build_prompt(*, question: str, stable_context: Mapping[str, Any], resolved: CodingAgentResolvedExecutor) -> str:
    ...
    available_inputs = task_intake.get("available_inputs")
    task_context = dict(task_intake.get("context") or {})
    lines = [
    ...
    if isinstance(available_inputs, Mapping):
        local_material_summary = str(available_inputs.get("local_material_preflight_summary") or "").strip()
        if not local_material_summary:
            local_material_summary = str(task_context.get("local_material_preflight_summary") or "").strip()
        if local_material_summary:
            lines.append("Host local material preflight:")
            lines.append(local_material_summary)
