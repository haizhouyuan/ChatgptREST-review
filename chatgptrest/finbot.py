...
from chatgptrest.core.path_resolver import resolve_finagent_root
...
from chatgptrest.finbot_modules.evidence_binding import (  # noqa: F401
    build_claim_evidence_bindings as _build_claim_evidence_bindings,
)
from chatgptrest.finbot_modules.negative_evidence import (  # noqa: F401
    build_counterevidence_packets as _build_counterevidence_packets,
)
from chatgptrest.finbot_modules.posture_guard import (  # noqa: F401
    evaluate_posture_guard as _evaluate_posture_guard,
)
from chatgptrest.finbot_modules.promotion_packet import (  # noqa: F401
    build_promotion_packet as _build_promotion_packet,
)
from chatgptrest.finbot_modules.peer_snapshot import (  # noqa: F401
    build_peer_snapshot as _build_peer_snapshot,
)
from chatgptrest.finbot_modules.transcript_provider import (  # noqa: F401
    build_transcript_packet as _build_transcript_packet,
)
from chatgptrest.finbot_modules.primary_data_enrich import (  # noqa: F401
    build_primary_data_packet as _build_primary_data_packet,
)
...
DEFAULT_FINAGENT_ROOT = resolve_finagent_root(start=__file__)
...
def _write_latest_and_history_payload(*, latest_path: Path, history_dir: Path, payload_name: str, payload: dict[str, Any]) -> dict[str, str]:
    history_dir.mkdir(parents=True, exist_ok=True)
    stamped = history_dir / f"{int(time.time())}_{payload_name}"
    _write_json(stamped, payload)
    _write_json(latest_path, payload)
    return {"latest_path": str(latest_path), "history_path": str(stamped)}
                            ...
                            supporting_sources.append(
                                {
                                    ...
                                    "excerpt": _text_value(source.get("excerpt") or source.get("exact_quote") or source.get("evidence_excerpt")),
                                }
                            )
                    else:
                        name = _text_value(source)
                        if name:
                            supporting_sources.append({"source_id": "", "name": name, "detail_href": "", "contribution_role": "", "excerpt": ""})
            rows.append(
                {
                    "claim": claim,
                    ...
                    "exact_quote": _text_value(item.get("exact_quote") or item.get("exact_excerpt") or item.get("excerpt")),
                }
            )
        else:
...
def _build_claim_citation_edges(
    *,
    ...
    citation_by_source_id = {
        _text_value(row.get("source_id")): row
        for row in citation_objects
        if _text_value(row.get("source_id"))
    }
    for binding in (evidence_bindings or {}).get("bindings") or []:
        claim_id = _text_value(binding.get("claim_id"))
        citation = citation_by_source_id.get(_text_value(binding.get("source_id")) or "")
        if not claim_id or citation is None:
            continue
        support_type = _text_value(binding.get("stance") or "support")
        ...
        edges.append(
            {
                ...
                "confidence": "high" if bool(binding.get("is_exact_excerpt")) else "medium",
                "note": _text_value(binding.get("excerpt")),
                "is_load_bearing": bool((claim_lookup.get(claim_id) or {}).get("is_load_bearing")),
                "missing_primary_evidence": bool(binding.get("missing_primary_evidence")),
            }
        )
...
def _build_theme_run_item(theme: dict[str, Any], run_result: dict[str, Any]) -> InboxItem | None:
    ...
    if run_result.get("promotion_packet"):
        payload["promotion_packet"] = dict(run_result.get("promotion_packet") or {})
...
def _build_claim_lane_prompt(
    *,
    bundle: dict[str, Any],
    kol_summary: dict[str, Any] | None,
    transcript_packet: dict[str, Any] | None = None,
) -> tuple[str, str]:
    opportunity = dict(bundle.get("opportunity") or {})
    theme_blocks = [_render_theme_context(item) for item in bundle.get("related_themes") or []]
    source_blocks = [_render_source_context(item) for item in bundle.get("related_sources") or []]
    ...
    if transcript_packet and bool(transcript_packet.get("available")):
        context_payload["transcript_packet"] = transcript_packet
    system_prompt = (
    ...
    prompt = (
        ...
        "claim,evidence_grade,importance,why_it_matters,next_check,falsification_condition,support_note,exact_quote,supporting_sources。"
        "`supporting_sources` 必须是数组，每个元素字段为：name,detail_href,contribution_role,excerpt。"
        "如果没有 exact quote，就把 `exact_quote` 留空，不要伪造原文。"
        "如果上下文里有 transcript excerpt，优先把 transcript 里的原文当作 first-hand source 使用。"
        "\n然后输出中文 Markdown，按小节：\n"
        "## Core claims\n## Claim ledger\n## Supporting evidence\n## Critical unknowns\n## Source priorities\n"
        f"\n上下文：\n```json\n{_json_dumps(context_payload)}\n```"
    )
...
def _build_skeptic_lane_prompt(
    *,
    bundle: dict[str, Any],
    claim_lane: dict[str, Any],
    kol_summary: dict[str, Any] | None = None,
    claim_evidence_bindings: dict[str, Any] | None = None,
    counterevidence_packets: dict[str, Any] | None = None,
    transcript_packet: dict[str, Any] | None = None,
) -> tuple[str, str]:
    opportunity = dict(bundle.get("opportunity") or {})
    context_payload = {
        ...
        "kol_suite": kol_summary or {},
        "claim_evidence_bindings": claim_evidence_bindings or {},
        "counterevidence_packets": counterevidence_packets or {},
    }
    if transcript_packet and bool(transcript_packet.get("available")):
        context_payload["transcript_packet"] = transcript_packet
    system_prompt = (
        ...
        "不要重复多头论点。优先对 load-bearing claims 逐条找反证。"
    )
    prompt = (
        "请针对这个机会给出最强的反方结构。\n\n"
        "先输出 ```json```，字段固定为："
        ...
        "如果上下文里已有 counterevidence packet，请优先围绕这些 packet 组织反方，而不是复述 claim lane。"
        "如果 transcript 已提供管理层原话或 Q&A，请优先从 transcript 里提取反证和 timing risk。"
        "\n然后输出中文 Markdown，按小节：\n"
        "## Bear case\n## Risk register\n## Thesis breakers\n## Timing risks\n## Competing paths\n## Disconfirming signals\n"
        f"\n上下文：\n```json\n{_json_dumps(context_payload)}\n```"
    )
...
def _build_expression_lane_prompt(
    *,
    ...
    if peer_snapshot:
        context_payload["peer_snapshot"] = peer_snapshot
    system_prompt = (
...
def _compose_research_package_markdown(*, markdown: str, lane_summaries: dict[str, Any]) -> str:
    ...
    policy = dict(lane_summaries.get("policy_result") or {})
    promotion_packet = dict(lane_summaries.get("promotion_packet") or {})
    claim_evidence_bindings = list((lane_summaries.get("claim_evidence_bindings") or {}).get("bindings") or [])
    counterevidence_packets = list((lane_summaries.get("counterevidence_packets") or {}).get("packets") or [])
    ...
    if claim_evidence_bindings:
        lines.append("")
        lines.append("### Claim evidence bindings")
        for row in claim_evidence_bindings:
            claim_id = _text_value(row.get("claim_id"))
            source_name = _text_value(row.get("source_name")) or _text_value(row.get("source_id")) or "unbound"
            if bool(row.get("missing_primary_evidence")):
                lines.append(f"- {claim_id}: missing primary evidence via {source_name}")
                continue
            excerpt = _text_value(row.get("excerpt"))
            lines.append(f"- {claim_id} ← {source_name}: {excerpt or 'excerpt unavailable'}")
    if skeptic:
        lines.append("")
    ...
    if counterevidence_packets:
        lines.append("")
        lines.append("### Counterevidence packets")
        for row in counterevidence_packets:
            lines.append(
                "- "
                f"{_text_value(row.get('claim_id'))} | stance={_text_value(row.get('stance')) or 'n/a'} | "
                f"{_text_value(row.get('excerpt')) or 'no packet excerpt'}"
            )
    if expression:
        lines.append("")
    ...
    if policy:
        lines.append("")
        lines.append("### Policy guard")
        lines.append(f"- raw decision: {_text_value(policy.get('raw_current_decision')) or 'n/a'}")
        lines.append(f"- max allowed posture: {_text_value(policy.get('max_allowed_posture')) or 'n/a'}")
        lines.append(f"- promote_to_paid: {bool(policy.get('promote_to_paid'))}")
        for row in policy.get("blocked_reasons") or []:
            lines.append(f"- blocked: {row}")
    if promotion_packet:
        lines.append("")
        lines.append("### Promotion packet")
        lines.append(f"- why_now: {_text_value(promotion_packet.get('why_now')) or 'n/a'}")
        lines.append(f"- promote_bool: {bool(promotion_packet.get('promote_bool'))}")
        for row in promotion_packet.get("first_hand_source_to_check") or []:
            lines.append(f"- first_hand_source_to_check: {_text_value(row.get('name')) or _text_value(row.get('source_id'))}")
    return "\n".join(line for line in lines if line is not None).strip() + "\n"
...
def _should_promote_research_package(package_payload: dict[str, Any]) -> bool:
    ...
    policy_result = dict(package_payload.get("policy_result") or {})
    if "promote_to_paid" in policy_result:
        return bool(policy_result.get("promote_to_paid"))
...
def _build_research_package_item(package_payload: dict[str, Any]) -> InboxItem:
    ...
    policy_result = dict(package_payload.get("policy_result") or {})
    blocked_reasons = list(policy_result.get("blocked_reasons") or [])
    if should_promote:
        action_hint = "Read the dossier. This high-priority opportunity qualifies for promotion to paid tier."
        if blocked_reasons:
            action_hint += " Review blocked reasons before promotion."
    else:
        ...
        if blocked_reasons:
            action_hint += f" Blocked by: {blocked_reasons[0]}"
    return InboxItem(
        ...
        severity="warning" if should_promote or _text_value(package_payload.get("current_decision")) in {"deepen_now", "prepare_candidate"} else "accent",
...
def opportunity_deepen(
    *,
    ...
    transcript_packet = _build_transcript_packet(
        candidate_id=candidate_id,
        symbol=ticker,
    )
    previous_peer_snapshot = dict((previous_payload or {}).get("peer_snapshot") or {})
    ...
    with ThreadPoolExecutor(max_workers=2, thread_name_prefix="finbot-lane") as executor:
        ...
        def _run_claim() -> dict[str, Any]:
            cs, cp = _build_claim_lane_prompt(
                bundle=bundle,
                kol_summary=None,
    ...
    key_sources = _coerce_string_list(claim_structured.get("key_sources") or bundle.get("opportunity", {}).get("suggested_sources") or [])
    related_sources = list(bundle.get("related_sources") or [])
    provisional_source_scorecard = _build_source_scorecard(
        related_sources=related_sources,
        key_sources=key_sources,
    )
    provisional_claim_ledger = _coerce_claim_ledger(claim_structured.get("claim_ledger") or []) or _default_claim_ledger(
    ...
    provisional_claim_ledger = _enrich_claim_ledger_with_sources(
        claim_ledger=provisional_claim_ledger,
        source_scorecard=provisional_source_scorecard,
    )
    provisional_claim_ledger = _annotate_claim_rows(candidate_id=candidate_id, claim_ledger=provisional_claim_ledger)
    provisional_claim_objects = _build_claim_objects(
        candidate_id=candidate_id,
        claim_ledger=provisional_claim_ledger,
        previous_payload=previous_payload,
        disconfirming_signals=[],
    )
    provisional_claim_bindings = _build_claim_evidence_bindings(
        candidate_id=candidate_id,
        claim_objects=provisional_claim_objects,
        claim_ledger=provisional_claim_ledger,
        source_scorecard=provisional_source_scorecard,
    )

    # Stage 2: Skeptic lane (depends on claim output)
    skeptic_system, skeptic_prompt = _build_skeptic_lane_prompt(
        bundle=bundle,
        claim_lane=claim_structured,
        kol_summary=kol_summary,
        claim_evidence_bindings=provisional_claim_bindings,
    ...
    with ThreadPoolExecutor(max_workers=2, thread_name_prefix="finbot-stage3") as executor:
        def _run_expression() -> dict[str, Any]:
            es, ep = _build_expression_lane_prompt(
                ...
                peer_snapshot=previous_peer_snapshot,
            )
    ...
    # WP5: Build peer snapshot after expression lane
    peer_snapshot = _build_peer_snapshot(
        candidate_id=candidate_id,
        leader_expression=expression_leader,
        valuation_frame=expression_structured.get("valuation_frame"),
        symbol=ticker,
    )
    ...
    key_sources = _coerce_string_list(structured.get("key_sources") or key_sources or bundle.get("opportunity", {}).get("suggested_sources") or [])
    ...
    claim_evidence_bindings = _build_claim_evidence_bindings(
        candidate_id=candidate_id,
    ...
    claim_citation_edges = _build_claim_citation_edges(
        ...
        evidence_bindings=claim_evidence_bindings,
    )
    counterevidence_packets = _build_counterevidence_packets(
        candidate_id=candidate_id,
        claim_objects=claim_objects,
    ...
    provisional_payload = {
        ...
        "counterevidence_packets": counterevidence_packets,
    }
    ...
    # WP7: Build primary data enrichment packet
    primary_data_packet = _build_primary_data_packet(
        candidate_id=candidate_id,
        symbol=ticker,
        claim_evidence_bindings=claim_evidence_bindings.get("bindings") if claim_evidence_bindings else None,
        counterevidence_packets=counterevidence_packets.get("packets") if counterevidence_packets else None,
    )
    ...
    package_payload = {
        ...
        # WP5-7: New enrichment artifacts
        "peer_snapshot": peer_snapshot,
        "transcript_packet": transcript_packet,
        "primary_data_packet": primary_data_packet,
    }
    ...
    promotion_packet = _build_promotion_packet(
        logical_key=candidate_id,
        candidate_id=candidate_id,
        thesis_name=_text_value(package_payload.get("thesis_name")),
        current_decision=_text_value(package_payload.get("current_decision")),
        why_now=_text_value(package_payload.get("headline") or package_payload.get("why_not_investable_yet") or bundle["opportunity"].get("note")),
        source_scorecard=source_scorecard,
        novelty_hint=[*skeptic_disconfirming, *claim_core_claims[:2]],
        blocked_by=list(package_payload.get("blocking_facts") or []),
        tier=tier_tag,
    )
    promotion_enrichment = dict(primary_data_packet.get("promotion_enrichment") or {})
    if not bool(promotion_enrichment.get("has_official_disclosure")):
        promotion_packet.setdefault("blocked_by", []).append("No official disclosure attached to promotion packet.")
        promotion_packet["promote_bool"] = False
    if not bool(promotion_enrichment.get("first_hand_anchor_found")):
        promotion_packet.setdefault("blocked_by", []).append("Primary data seam did not find a first-hand anchor yet.")
        promotion_packet["promote_bool"] = False
    first_hand_sources = list(promotion_packet.get("first_hand_source_to_check") or [])
    for row in list(primary_data_packet.get("filing_evidence") or []):
        source_id = _text_value(row.get("source_id"))
        if source_id and source_id not in first_hand_sources:
            first_hand_sources.append(source_id)
    if first_hand_sources:
        promotion_packet["first_hand_source_to_check"] = first_hand_sources
    package_payload["promotion_packet"] = promotion_packet
    policy_result = _evaluate_posture_guard(
        package_payload=package_payload,
        claim_evidence_bindings=claim_evidence_bindings,
        counterevidence_packets=counterevidence_packets,
    )
    package_payload["policy_result"] = policy_result
    blocked_reasons = [row for row in policy_result.get("blocked_reasons") or [] if _text_value(row)]
    if blocked_reasons:
        existing_blockers = list(package_payload.get("blocking_facts") or [])
        for row in blocked_reasons:
            if row not in existing_blockers:
                existing_blockers.append(row)
        package_payload["blocking_facts"] = existing_blockers
    synced_blockers: list[str] = []
    for row in list(package_payload.get("blocking_facts") or []):
        text = _text_value(row)
        if text and text not in synced_blockers:
            synced_blockers.append(text)
    promotion_packet["blocked_by"] = synced_blockers
    package_payload["promotion_packet"] = promotion_packet
    max_allowed_posture = _text_value(policy_result.get("max_allowed_posture"))
    if max_allowed_posture:
        package_payload["current_decision"] = max_allowed_posture
    package_payload["missing_primary_evidence"] = list(policy_result.get("missing_evidence") or [])
    package_payload["missing_counterevidence"] = list(policy_result.get("missing_counterevidence") or [])
    package_payload["promote_to_paid"] = bool(policy_result.get("promote_to_paid"))
    ...
    lane_summaries["policy_result"] = policy_result
    lane_summaries["promotion_packet"] = promotion_packet
    context_payload = {
        "candidate_id": candidate_id,
    ...
    evidence_binding_path = history_dir / "claim_evidence_bindings.json"
    counterevidence_path = history_dir / "counterevidence_packets.json"
    policy_path = history_dir / "policy_result.json"
    promotion_packet_path = history_dir / "promotion_packet.json"
    peer_snapshot_path = history_dir / "peer_snapshot.json"
    transcript_packet_path = history_dir / "transcript_packet.json"
    primary_data_packet_path = history_dir / "primary_data_packet.json"
    ...
    _write_json(evidence_binding_path, claim_evidence_bindings)
    _write_json(counterevidence_path, counterevidence_packets)
    _write_json(policy_path, policy_result)
    _write_json(promotion_packet_path, promotion_packet)
    _write_json(peer_snapshot_path, peer_snapshot)
    _write_json(transcript_packet_path, transcript_packet)
    _write_json(primary_data_packet_path, primary_data_packet)
    package_payload["guardrail_artifacts"] = {
        "claim_evidence_bindings": str(evidence_binding_path),
        "counterevidence_packets": str(counterevidence_path),
        "policy_result": str(policy_path),
        "promotion_packet": str(promotion_packet_path),
        "peer_snapshot": str(peer_snapshot_path),
        "transcript_packet": str(transcript_packet_path),
        "primary_data_packet": str(primary_data_packet_path),
    }
...
_DISCOVERY_STRATEGY_ROTATION = ("value", "quality", "growth", "momentum")
...
def _pick_discovery_strategy(*, now_ts: float | None = None) -> str:
    ts = now_ts if now_ts is not None else time.time()
    day_bucket = int(ts // 86400)
    return _DISCOVERY_STRATEGY_ROTATION[day_bucket % len(_DISCOVERY_STRATEGY_ROTATION)]


def _is_any_market_open(*, now_struct: time.struct_time | None = None) -> bool:
    local = now_struct or time.localtime()
    if local.tm_wday >= 5:
        return False
    minutes = local.tm_hour * 60 + local.tm_min
    asia_open = 9 * 60 <= minutes < 16 * 60
    us_open = minutes >= (21 * 60 + 30) or minutes < (4 * 60)
    return asia_open or us_open
...
def theme_radar_scout(
    *,
    ...
    if isinstance(package_result, dict):
        result["promotion_packet"] = dict((package_result.get("package") or {}).get("promotion_packet") or {})
    return result
...
def theme_batch_run(
    *,
    ...
    all_themes = list(catalog.get("themes") or [])
    selected_theme_slug = _text_value(theme_slug)
    if selected_theme_slug:
        themes = [
            theme
            for theme in all_themes
            if _text_value(theme.get("theme_slug")) == selected_theme_slug
        ]
        if not themes:
            return {
                "ok": False,
                "error": "theme_not_found",
                "requested_theme_slug": selected_theme_slug,
                "catalog_path": str(catalog_path),
            }
    else:
        themes = all_themes[: max(1, int(limit))]
    ...
    for theme in themes:
        ...
        promotion_packet = _build_promotion_packet(
            logical_key=theme_slug,
            candidate_id="",
            thesis_name=str(theme.get("title") or theme_slug),
            current_decision=str(result.get("recommended_posture") or ""),
            why_now=str(spec_context.get("why_now") or spec_context.get("thesis_statement") or theme.get("title") or ""),
            source_scorecard=[],
            novelty_hint=[
                str((result.get("best_expression") or {}).get("entity") or ""),
                str(spec_context.get("why_mispriced") or ""),
            ],
            blocked_by=["Theme batch output still needs candidate-level first-hand source before promotion."],
            tier=tier_tag,
        )
        promotion_path = run_root / "promotion_packet.json"
        _write_json(promotion_path, promotion_packet)
        result = {
            **result,
            "promotion_packet": promotion_packet,
            "promotion_packet_path": str(promotion_path),
        }
        promotion_packets.append(
            {
                "theme_slug": theme_slug,
                "promotion_packet_path": str(promotion_path),
                "promote_bool": bool(promotion_packet.get("promote_bool")),
            }
        )
        ...
        "runs": [
            {
                ...
                "promotion_packet_path": result.get("promotion_packet_path"),
            }
...
def daily_work(
    *,
    ...
    if radar.get("promotion_packet"):
        payload["promotion_packets"] = [dict(radar.get("promotion_packet") or {})]
    ...
    if include_theme_batch:
        ...
        payload["promotion_packets"] = [
            *(payload.get("promotion_packets") or []),
            *list(theme_batch.get("promotion_packets") or []),
        ]
    return payload
