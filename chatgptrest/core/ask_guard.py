...
class AskClientProfile:
    ...
    max_in_flight_by_kind: dict[str, int] | None = None
...
def _file_content_fingerprint_token(path_value: Any) -> dict[str, Any]:
    ...
    try:
        stat = path.stat()
        if not path.is_file():
            return {"path": path.as_posix(), "kind": "non_file"}
        max_bytes = max(0, int(os.environ.get("CHATGPTREST_ASK_GUARD_FILE_FINGERPRINT_MAX_BYTES", str(64 * 1024 * 1024)) or 0))
        base: dict[str, Any] = {
            "name": path.name,
            "size": int(stat.st_size),
        }
        if max_bytes <= 0 or int(stat.st_size) > max_bytes:
        ...
        return {"path": raw, "kind": "unreadable"}
...
def _profile_from_payload(raw: dict[str, Any]) -> AskClientProfile:
    ...
    raw_max_by_kind = raw.get("max_in_flight_by_kind")
    max_by_kind: dict[str, int] = {}
    if isinstance(raw_max_by_kind, dict):
        for raw_kind, raw_limit in raw_max_by_kind.items():
            kind = _normalize_token(raw_kind, max_chars=100)
            if not kind:
                continue
            try:
    ...
    return AskClientProfile(
        ...
        max_in_flight_by_kind=max_by_kind,
    )
...
def _safe_non_pro_preset(kind: str) -> str | None:
    return safe_non_pro_preset_for_kind(_normalize_token(kind))
    ...
    if normalized_kind == "chatgpt_web.ask":
        return HTTPException(
            ...
            detail={
                ...
                "detail": "interactive coding clients must use the public MCP automation-kernel instead of direct /v1/jobs chatgpt_web.ask",
                "x_client_name": requested_name or profile.client_id,
                "client_id": profile.client_id,
                "trust_class": profile.trust_class,
                "hint": "Use automation_ask / automation_result / automation_job_* on http://127.0.0.1:18712/mcp. Maintenance exceptions must use a maintenance-registered client instead of params.allow_direct_live_chatgpt_ask.",
            },
        )
    if normalized_kind in {"gemini_web.ask", "qwen_web.ask"}:
        return HTTPException(
            ...
            detail={
                ...
                "reason": "interactive_client_must_use_public_agent",
                "detail": "interactive coding clients must use the public MCP automation-kernel instead of low-level /v1/jobs ask calls",
                "kind": normalized_kind,
                "x_client_name": requested_name or profile.client_id,
                "client_id": profile.client_id,
                "trust_class": profile.trust_class,
                "hint": "Use the public MCP automation-kernel (automation_ask / automation_result / automation_job_*) instead of low-level /v1/jobs ask calls.",
            },
        )
    return _authorization_violation(
        error="low_level_ask_use_public_agent",
        reason="interactive_client_must_use_public_agent",
        detail="interactive coding clients must use the public MCP automation-kernel instead of low-level ask",
        profile=profile,
        audit=audit,
        hint="Use automation_ask / automation_result / automation_job_* on http://127.0.0.1:18712/mcp.",
    )


def _runtime_prompt_fingerprint(*, kind: str, input_obj: dict[str, Any], params_obj: dict[str, Any]) -> str:
    file_refs = [
        _file_content_fingerprint_token(item)
        for item in list(input_obj.get("file_paths") or [])
        if str(item).strip()
    ]
    file_refs = sorted((item for item in file_refs if item), key=_stable_json)
    payload = {
...
def prepare_low_level_ask_runtime_controls(
    *,
    kind: str,
    ...
    runtime_control = dict(guard_payload.get("runtime_control") or {})
    if not runtime_control.get("prompt_fingerprint"):
        runtime_control["prompt_fingerprint"] = _runtime_prompt_fingerprint(kind=kind, input_obj=input_obj, params_obj=params_obj)
...
def enforce_low_level_ask_runtime_controls(
    *,
    ...
    prompt_fingerprint = str(runtime_control.get("prompt_fingerprint") or "").strip()
    ...
    if max_in_flight_jobs > 0:
        rows = conn.execute(
            """
            SELECT job_id, status, phase, not_before, updated_at
              FROM jobs
        ...
        if len(rows) >= max_in_flight_jobs:
            ...
            now = time.time()
            not_before_values: list[float] = []
            active_jobs: list[dict[str, Any]] = []
            for row in list(rows)[:5]:
                job_id = str(row[0] or "").strip()
                if not job_id:
                    continue
                phase = str(row[2] or "").strip() if len(row) > 2 else ""
                raw_not_before = row[3] if len(row) > 3 else None
                try:
                    not_before = float(raw_not_before) if raw_not_before is not None else None
                except Exception:
                    not_before = None
                if not_before is not None and not_before > now:
                    not_before_values.append(not_before)
                active_jobs.append(
                    {
                        "job_id": job_id,
            ...
            if not_before_values:
                retry_after_seconds = max(1, int(min(not_before_values) - now))
            raise HTTPException(
                status_code=429,
                detail={
                    ...
                    "safe_next_action": "wait_for_active_job_or_cancel_confirmed_stale_job_with_reason",
                },
            )

    normalized_kind = _normalize_token(kind, max_chars=100)
    max_by_kind_raw = runtime_control.get("max_in_flight_by_kind")
    max_by_kind = dict(max_by_kind_raw) if isinstance(max_by_kind_raw, dict) else {}
    max_in_flight_for_kind = max(0, int(max_by_kind.get(normalized_kind) or 0))
    if max_in_flight_for_kind > 0:
        rows = conn.execute(
            """
        ...
        if len(rows) >= max_in_flight_for_kind:
            active_job_ids = [str(row[0]) for row in list(rows)[:5] if str(row[0] or "").strip()]
            raise HTTPException(
                status_code=429,
                detail={
                    "error": "low_level_ask_client_kind_concurrency_exceeded",
                    "error_type": "LowLevelAskClientKindConcurrencyExceeded",
                    "reason": "registered_client_kind_max_in_flight_exceeded",
                    "detail": "registered low-level ask client has reached its in-flight limit for this provider kind",
                    "client_id": client_id,
                    "kind": str(kind),
                    "max_in_flight_for_kind": max_in_flight_for_kind,
                    "active_job_ids": active_job_ids,
                    "safe_next_action": "wait_for_active_job_or_use_non_chatgpt_web_provider",
                },
            )
    ...
    *,
    ...
    guard_payload = {
        ...
        "runtime_control": {
            ...
            "max_in_flight_by_kind": dict(profile.max_in_flight_by_kind or {}),
        },
    }
    ...
    *,
    ...
    if not bool(decision.get("allow_pro", True)) and _is_pro_like_request(kind=normalized_kind, preset=preset, params_obj=params):
        ...
        if not fallback_preset:
            capability_profile = provider_capability_profile_for_kind(normalized_kind)
            raise HTTPException(
                status_code=403,
                detail={
