...
def resolve_evomap_vector_db_path(raw: str = "") -> str:
    candidate = (
        raw.strip()
        or os.environ.get("EVOMAP_VECTOR_DB", "").strip()
        or os.environ.get("OPENMIND_EVOMAP_VECTOR_DB", "").strip()
        or str(REPO_ROOT / "data" / "evomap_vectors.db")
    )
    return _expand(candidate)
