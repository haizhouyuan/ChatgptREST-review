...
REGISTRY: tuple[EnvVar, ...] = (
    ...
    EnvVar("CHATGPTREST_REQUIRE_EXPLICIT_MCP_CANCEL_REASON", EnvType.BOOL, False,
           "Require explicit reason argument for MCP job cancel tools", "core"),
    ...
    EnvVar("CHATGPTREST_GEMINI_SEND_RETRY_DELAY", EnvType.FLOAT, 90.0,
           "Delay before retrying a transient Gemini send within the same executor pass", "gemini",
           min_value=30.0, max_value=1800.0),
    EnvVar("CHATGPTREST_GEMINI_WAIT_TRANSIENT_RETRY_AFTER_SECONDS", EnvType.INT, 90,
    ...
    EnvVar("CHATGPTREST_WEB_HUMAN_RETRY_FLOOR_SECONDS", EnvType.INT, 90,
           "Global minimum retry interval for browser-visible web retries", "core", min_value=30, max_value=3600),
    EnvVar("CHATGPTREST_WEB_HUMAN_RETRY_JITTER_MAX_SECONDS", EnvType.INT, 15,
           "Max jitter added to browser-visible web retries", "core", min_value=0, max_value=300),
    EnvVar("CHATGPTREST_WEB_RETRY_BUDGET_WINDOW_SECONDS", EnvType.INT, 900,
           "Rolling window for browser-visible retry budget accounting", "core", min_value=60, max_value=86_400),
    EnvVar("CHATGPTREST_WEB_RETRY_BUDGET_MAX_PER_JOB", EnvType.INT, 4,
           "Max browser-visible retry schedules per job within the budget window (0 = disabled)", "core", min_value=0, max_value=1000),
    EnvVar("CHATGPTREST_WEB_RETRY_BUDGET_MAX_PER_PROVIDER", EnvType.INT, 12,
           "Max browser-visible retry schedules per provider within the budget window (0 = disabled)", "core", min_value=0, max_value=10_000),
    EnvVar("CHATGPTREST_WEB_RETRY_BUDGET_WARN_RATIO", EnvType.FLOAT, 0.8,
           "Warn threshold ratio for browser-visible retry budget hot state", "core", min_value=0.1, max_value=1.0),
    EnvVar("CHATGPTREST_INFRA_RETRY_AFTER_SECONDS", EnvType.INT, 120,
           "Retry delay for infrastructure errors", "core", min_value=5),
    EnvVar("CHATGPTREST_UI_RETRY_AFTER_SECONDS", EnvType.INT, 90,
           "Retry delay for UI errors", "core", min_value=5),
    EnvVar("CHATGPTREST_WAIT_INFRA_RETRY_AFTER_SECONDS", EnvType.INT, 90,
           "Retry delay for wait-phase infra errors", "core", min_value=5),
    EnvVar("CHATGPTREST_WAIT_UI_RETRY_AFTER_SECONDS", EnvType.INT, 90,
           "Retry delay for wait-phase UI errors", "core", min_value=5),
    EnvVar("CHATGPT_UNSENT_TRANSIENT_RETRY_AFTER_SECONDS", EnvType.INT, 90,
           "Retry delay when a prompt was not sent because of a transient ChatGPT UI/driver error", "core",
    ...
    EnvVar("CHATGPTREST_CODEX_SRE_MODEL_DEFAULT", EnvType.STRING, "gpt-5.3-codex-spark",
           "Default model for sre.fix_request diagnosis when caller does not specify one", "repair"),
    EnvVar("CHATGPTREST_CODEX_SRE_REASONING_EFFORT", EnvType.STRING, "low",
           "Reasoning effort for sre.fix_request resume/default diagnosis", "repair"),
    EnvVar("CHATGPTREST_CODEX_SRE_FRESH_REASONING_EFFORT", EnvType.STRING, "medium",
           "Reasoning effort for fresh sre.fix_request diagnosis", "repair"),
    EnvVar("CHATGPTREST_CODEX_SRE_FAIL_OPEN_TO_MANUAL", EnvType.BOOL, True,
           "Return a structured manual route when Codex SRE diagnosis fails", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_MODEL_P0", EnvType.STRING, "gpt-5.3-codex-spark",
           "Default model for repair.open_pr mode p0", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_MODEL_P1", EnvType.STRING, "gpt-5.3-codex",
           "Default model for repair.open_pr mode p1", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_MODEL_P2", EnvType.STRING, "gpt-5.3-codex",
           "Default model for repair.open_pr mode p2", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_REASONING_EFFORT_P0", EnvType.STRING, "low",
           "Default reasoning effort for repair.open_pr mode p0", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_REASONING_EFFORT_P1", EnvType.STRING, "medium",
           "Default reasoning effort for repair.open_pr mode p1", "repair"),
    EnvVar("CHATGPTREST_CODEX_OPEN_PR_REASONING_EFFORT_P2", EnvType.STRING, "high",
           "Default reasoning effort for repair.open_pr mode p2", "repair"),
    EnvVar("CHATGPTREST_CODEX_RUNNER_PRESERVE_OPENAI_ENV", EnvType.BOOL, False,
           "Preserve ambient OPENAI_* env vars for Codex CLI subprocesses", "repair"),
