...
_COMPACT_PLANNING_OUTPUT_RE = re.compile(r"(?:output requirements|输出要求)\s*planning_memo", re.IGNORECASE)
_COMPACT_PLANNING_THREE_BULLETS_RE = re.compile(r"(?:三条(?:无序)?列表|three (?:unordered )?bullet)", re.IGNORECASE)
_GENERIC_COMPACT_OUTPUT_RE = re.compile(
    r"("
    r"\d+\s*条(?:以内|之内)?|"
    r"\d+\s*点(?:以内|之内)?|"
    r"控制在.{0,12}(条|点|要点|列表|bullet)|"
    r"不超过.{0,12}(条|点|要点|列表|bullet)|"
    r"(简要|精简|精炼).{0,12}(回答|回复|总结|结论)|"
    r"(要点列表|要点回答|bullet list)"
    r")",
    re.I,
)
_CONTINUATION_NUDGE_RE = re.compile(
    r"(?:"
    r"^(?:please\s+)?(?:continue|proceed|go on|keep going|resume)(?:\s+(?:the|this|your)?\s*(?:answer|analysis|review|work|task|documents?))?[.!。！\s]*$|"
    r"^(?:you (?:seem|look) (?:stuck|blocked)|(?:it|this) (?:seems|looks) stuck|stuck|no progress).{0,100}(?:continue|proceed|keep going|resume)?[.!。！\s]*$|"
    r"^(?:我看你)?(?:卡住|没进度|没有进度|好久没进度|一直是.{0,40})(?:了)?[，,。；;：:\\s]*(?:请|麻烦)?(?:继续|接着|往下|继续完成|继续审查|继续分析)[。.!！\s]*$|"
    r"^(?:请|麻烦)?(?:继续|接着|往下|继续完成|继续审查|继续分析)[。.!！\s]*$"
    r")",
    re.IGNORECASE,
)
    ...
    *,
    ...
    if isinstance(messages, list) and messages:
        ...
        for item in messages:
            ...
            out_item: dict[str, Any] = {"role": role, "text": text}
            if "status" in item:
                out_item["status"] = item.get("status")
            if isinstance(item.get("metadata"), dict):
                out_item["metadata"] = item.get("metadata")
            out.append(out_item)
...
def _looks_like_continuation_nudge(text: str) -> bool:
    normalized = normalize_text(text)
    if not normalized or len(normalized) > 240:
        return False
    lowered = normalized.lower()
    if (
        re.search(r"(?:继续|接着|往下|continue|proceed|keep going|resume)", lowered, re.IGNORECASE)
        and re.search(r"(?:卡住|没进度|没有进度|好久|一直是|reading documents|stuck|no progress)", lowered, re.IGNORECASE)
    ):
        return True
    return bool(_CONTINUATION_NUDGE_RE.search(normalized))
...
def extract_answer_from_conversation_export_obj(
    *,
    ...
    if best_idx is not None:
        ...
        candidate_is_complete: list[Any] = []
        candidate_finish_types: list[Any] = []
        next_role_after_match: str | None = None
        subsequent_user_turn_count = 0
        continuation_nudge_count = 0
        contaminated_after_match = False
        for idx in range(int(best_idx) + 1, len(messages)):
            message = messages[idx]
            if not isinstance(message, dict):
                continue
            role = str(message.get("role") or "").strip().lower()
            if next_role_after_match is None and role:
                next_role_after_match = role
            if role == "user":
                subsequent_user_turn_count += 1
                if _looks_like_continuation_nudge(str(message.get("text") or "")):
                    continuation_nudge_count += 1
                    continue
                contaminated_after_match = True
                break
            if role != "assistant":
                continue
            ...
            candidate_is_complete.append(metadata.get("is_complete"))
            candidate_finish_types.append(finish_details.get("type"))
        ...
        info["next_role_after_match"] = next_role_after_match
        info["subsequent_user_turn_count"] = int(subsequent_user_turn_count)
        info["continuation_nudge_count"] = int(continuation_nudge_count)
        info["thread_contaminated_after_match"] = bool(
            next_role_after_match == "user"
            or contaminated_after_match
            or subsequent_user_turn_count > continuation_nudge_count
        )
        if not candidates:
        ...
        chosen_idx = int(best[0])
        chosen_quality = candidate_qualities[chosen_idx]
        info["answer_quality"] = chosen_quality
        if chosen_idx < len(candidate_statuses):
            info["chosen_assistant_status"] = candidate_statuses[chosen_idx]
        if chosen_idx < len(candidate_is_complete):
            info["chosen_assistant_is_complete"] = candidate_is_complete[chosen_idx]
        if chosen_idx < len(candidate_finish_types):
            info["chosen_assistant_finish_type"] = candidate_finish_types[chosen_idx]
        if str(info.get("chosen_assistant_status") or "").strip().lower() == "in_progress":
...
def classify_answer_quality(
    ...
    *,
    ...
    bullet_line_count = len(re.findall(r"^[\s]*[-*•]\s", text, re.MULTILINE))
    numbered_line_count = len(re.findall(r"^[\s]*\d+[.)]\s", text, re.MULTILINE))

    question = str(question_text or "")
    compact_planning_contract = bool(
        question
        and _COMPACT_PLANNING_OUTPUT_RE.search(question)
        and _COMPACT_PLANNING_THREE_BULLETS_RE.search(question)
    )
    generic_compact_contract = bool(question and _GENERIC_COMPACT_OUTPUT_RE.search(question))
    ...
    # Compact planning lanes intentionally produce short three-bullet answers.
    # Do not treat these as suspect when the prompt explicitly asks for a
    # `planning_memo` in a three-bullet shape.
    if chars < 100:
        if compact_planning_contract and (bullet_line_count + numbered_line_count) >= 3:
            return "final"
        if generic_compact_contract and (
            (bullet_line_count + numbered_line_count) >= 2 or sentence_like_count >= 2
        ):
            return "final"
    ...
    if chars < 400 and structural_markers == 0 and sentence_like_count < 3:
        if generic_compact_contract and sentence_like_count >= 2:
            return "final"
        return "suspect_short_answer"
