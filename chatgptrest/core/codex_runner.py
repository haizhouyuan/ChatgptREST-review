...
def _codex_subprocess_env(extra_env: dict[str, str] | None = None) -> dict[str, str]:
    env = dict(os.environ)
    preserve_openai_env = _env_truthy("CHATGPTREST_CODEX_RUNNER_PRESERVE_OPENAI_ENV", False)
    if not preserve_openai_env:
        for key in list(env):
            if key.startswith("OPENAI_") or key.startswith("openai_"):
                env.pop(key, None)
    if extra_env:
...
def codex_exec_with_schema(
    *,
    ...
    try:
        ...
        run_kwargs["env"] = _codex_subprocess_env(env)
