...
def _looks_like_unsent_web_send_transport_failure(error_type: str | None, error: str | None) -> bool:
    haystack = f"{error_type or ''}\n{error or ''}".lower()
    if "sse stream timeout" in haystack or "deadline exceeded" in haystack:
        return True
    if "target crashed" in haystack or "target closed" in haystack or "page crashed" in haystack:
        return True
    return False
...
def _attachment_stats_for_recovery_event(conn: sqlite3.Connection, job: "JobRecord") -> dict[str, Any]:
    row = conn.execute("SELECT input_json, client_json FROM jobs WHERE job_id = ?", (job.job_id,)).fetchone()
    input_json = str(row["input_json"] or "") if row is not None else ""
    client_json = str((row["client_json"] if row is not None else None) or job.client_json or "")
    input_obj = _json_obj(input_json)
    client_obj = _json_obj(client_json)
    file_paths = input_obj.get("file_paths")
    staging = client_obj.get("mcp_attachment_staging")
        ...
        for item in files:
            if isinstance(item, dict):
                try:
                    ...
                    pass
    return {
        "file_count": len(file_paths) if isinstance(file_paths, list) else 0,
        "staged_file_count": len(files) if isinstance(files, list) else 0,
        "staged_total_bytes": int(total_bytes),
        "mcp_staged": bool(isinstance(files, list) and files),
    }


def _record_web_send_retry_exhausted_event(
    conn: sqlite3.Connection,
    *,
    artifacts_dir: Any,
    job: "JobRecord",
    ...
    if phase != "send" or not is_web_ask_kind(job.kind):
        return
    if not _looks_like_unsent_web_send_transport_failure(error_type, error):
        return
    attachment_stats = _attachment_stats_for_recovery_event(conn, job)
    payload = {
        "kind": job.kind,
        "status": status.value,
        "phase": phase,
        ...
        "error_preview": str(error or "").strip()[:500],
        "guard_reason": guard_reason,
        "safe_next_action": (
            "retry_same_request_with_larger_send_timeout_or_slimmer_attachments_"
            "before_opening_more_parallel_chatgpt_web_jobs"
        ),
        "recommended_params": {
            "send_timeout_seconds": 240,
            "max_wait_seconds": 7200,
        },
        "attachment_stats": attachment_stats,
    }
    insert_event(conn, job_id=job.job_id, type="web_send_retry_exhausted", payload=payload)
    artifacts.append_event(artifacts_dir, job.job_id, type="web_send_retry_exhausted", payload=payload)
...
def _validate_conversation_url_for_kind(*, kind: str, conversation_url: str | None) -> None:
    ...
    if platform == "qwen" and provider in {"chatgpt", "gemini"}:
        raise ValueError(
            ...
            "Qwen threads are retired; use a ChatGPT or Gemini conversation instead."
        )
    if platform in {"chatgpt", "gemini"} and provider == "qwen":
        raise ValueError(
            f"conversation_url host is not Qwen ({url}); cannot use kind={k}. "
            "qwen_web.ask has been retired and is no longer available."
        )
...
def create_job(
    ...
    *,
    ...
    if str(kind or "").strip().lower().startswith("qwen_web."):
        raise ValueError("qwen_web.ask has been retired and is no longer available")
...
def request_cancel(
    ...
    *,
    ...
    def _try_cancel_nonrunning_intermediate_immediately(current_job: JobRecord) -> JobRecord | None:
        if current_job.status not in {JobStatus.NEEDS_FOLLOWUP, JobStatus.COOLDOWN, JobStatus.BLOCKED}:
            return None
        reason_message = (f"cancel requested: {reason_text}" if reason_text else "cancel requested")
        changed = conn.execute(
            """
        ...
        if not changed:
            raise RuntimeError("failed to cancel intermediate job (concurrent update?)")
        insert_event(
            conn,
            job_id=job_id,
            type="status_changed",
            payload={"from": current_job.status.value, "to": JobStatus.CANCELED.value},
        )
        ...
        canceled_job = get_job(conn, job_id=job_id)
        if canceled_job is None:
            raise RuntimeError("failed to reload canceled job")
        _write_canceled_result_payload(canceled_job)
        return canceled_job
    ...
    immediate_intermediate_cancel = _try_cancel_nonrunning_intermediate_immediately(job)
    if immediate_intermediate_cancel is not None:
        insert_event(conn, job_id=job_id, type="cancel_requested", payload=payload)
        artifacts.append_event(artifacts_dir, job_id, type="cancel_requested", payload=payload)
        return immediate_intermediate_cancel
    conn.execute(
...
def _build_claim_where(
    *,
    ...
    pause_active: int,
    pause_all: int,
    auto_blocked_pause: int,
    chatgpt_web_hold_active: int,
) -> tuple[str, list[Any]]:
...
def _build_claim_order_by(*, phase_filter: str | None) -> str:
    ...
    only wait workers and send workers get explicit priority ordering.

    Send workers should keep user-facing asks ahead of repair.* jobs. Repair
    jobs are still important, but they must not starve the canonical ask path
    after a transient failure.
    """

    if phase_filter == "send":
        return """
            CASE
              WHEN kind LIKE 'repair.%'
                THEN 1
              ELSE 0
            END ASC,
            not_before ASC,
...
def claim_next_job(
    ...
    *,
    ...
    auto_blocked_pause = int(
        pause_active and str(pause.reason or "").strip().lower().startswith("auto_blocked:")
    )
    chatgpt_web_hold_active = int(get_chatgpt_web_hold(conn, now=now) is not None)
    ...
    where_sql, where_params = _build_claim_where(
        ...
        pause_all=pause_all,
        auto_blocked_pause=auto_blocked_pause,
        chatgpt_web_hold_active=chatgpt_web_hold_active,
    )
...
def store_answer_result(
    ...
    *,
    ...
    params_obj = parse_job_params_json(row["params_json"] if row is not None else None)
    ...
    last_event_type = str(latest_event["type"] or "").strip() if latest_event is not None else ""
    widget_export_available = False
    if job.conversation_export_path:
        try:
            widget_export_available = widget_export_available_from_path(
                artifacts.resolve_artifact_path(artifacts_dir, str(job.conversation_export_path)),
            )
        except Exception:
            widget_export_available = False
    ...
    completion_contract = build_completion_contract(
        ...
        authoritative_answer_path=meta.answer_path,
        min_chars_required=min_chars_required,
        last_event_type=last_event_type or None,
        reason_type=None,
        completion_quality=("completed_under_min_chars" if last_event_type == "completion_guard_completed_under_min_chars" else "final"),
        conversation_export_path=job.conversation_export_path,
        widget_export_available=widget_export_available,
        research_contract=is_research_contract_params(params_obj),
    )
    canonical_answer = build_canonical_answer_record(
        status=JobStatus.COMPLETED.value,
    ...
    try:
        ...
        insert_event(
            conn,
            job_id=job_id,
            type="completion_contract_recorded",
            payload=completion_contract,
        )
        insert_event(
            conn,
            job_id=job_id,
            type="canonical_answer_recorded",
    ...
    *,
    ...
    if effective_phase != "wait" and job.max_attempts > 0 and job.attempts >= job.max_attempts:
        ...
        if extended_max_attempts is None:
            ...
            _record_web_send_retry_exhausted_event(
                conn,
                artifacts_dir=artifacts_dir,
                job=job,
