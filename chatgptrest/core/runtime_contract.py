...
def public_agent_cancel_allowlist_state() -> dict[str, Any]:
    allowlist = parse_client_name_allowlist(os.environ.get("CHATGPTREST_ENFORCE_CANCEL_CLIENT_NAME_ALLOWLIST", ""))
    client_name = public_agent_client_name().strip().lower()
    if not allowlist:
        return {
            "enforced": False,
...
def public_agent_mcp_runtime_contract_state() -> dict[str, Any]:
    ...
    cancel_allowlist_state = public_agent_cancel_allowlist_state()
    host, port = public_agent_mcp_host_port()
    allowlisted = bool(allowlist_state.get("allowlisted"))
    cancel_allowlisted = bool(cancel_allowlist_state.get("allowlisted"))
    runtime_ok = bool(token) and allowlisted
    cancel_runtime_ok = runtime_ok and cancel_allowlisted
    return {
        "ok": runtime_ok,
        "source": source,
        ...
        "cancel_allowlist_enforced": bool(cancel_allowlist_state.get("enforced")),
        "cancel_allowlist": list(cancel_allowlist_state.get("allowlist") or []),
        "cancel_allowlisted": cancel_allowlisted,
        "cancel_runtime_contract_ok": cancel_runtime_ok,
