...
from chatgptrest.api.schemas import (
    ...
    PauseSetRequest,
    PauseView,
    RetryBudgetProviderView,
    RetryBudgetStatusView,
...
from chatgptrest.core.web_retry_budget import summarize_recent_web_retry_budget
...
def make_ops_router(cfg: AppConfig) -> APIRouter:
    ...
    def ops_status() -> OpsStatusView:
        ...
        retry_budget_summary: dict[str, Any] = {}
        with connect(cfg.db_path) as conn:
            pause = get_pause_state(conn)
            ...
            retry_budget_summary = summarize_recent_web_retry_budget(conn, now=now)
        ...
        if retry_budget_summary.get("hot_providers"):
            attention_reasons.append("retry_budget_hot")
        if int(retry_budget_summary.get("recent_budget_exceeded_events") or 0) > 0:
            attention_reasons.append("retry_budget_exceeded")
        return OpsStatusView(
            ...
            retry_budget_window_seconds=int(retry_budget_summary.get("window_seconds") or 0),
            retry_budget_recent_by_provider=dict(retry_budget_summary.get("recent_by_provider") or {}),
            retry_budget_hot_providers=list(retry_budget_summary.get("hot_providers") or []),
            retry_budget_exceeded_events=int(retry_budget_summary.get("recent_budget_exceeded_events") or 0),
            attention_reasons=attention_reasons,
            last_job_event_id=last_event_id,
            build=build,
        )

    @router.get("/v1/ops/retry-budget", response_model=RetryBudgetStatusView)
    def ops_retry_budget() -> RetryBudgetStatusView:
        now = time.time()
        with connect(cfg.db_path) as conn:
            summary = summarize_recent_web_retry_budget(conn, now=now)
        return RetryBudgetStatusView(
            now=float(summary.get("now") or now),
            window_seconds=int(summary.get("window_seconds") or 0),
            max_per_job=int(summary.get("max_per_job") or 0),
            max_per_provider=int(summary.get("max_per_provider") or 0),
            warn_ratio=float(summary.get("warn_ratio") or 0.0),
            recent_budget_exceeded_events=int(summary.get("recent_budget_exceeded_events") or 0),
            hot_providers=list(summary.get("hot_providers") or []),
            providers=[RetryBudgetProviderView(**item) for item in list(summary.get("providers") or [])],
        )
