...
ALL_MODELS = ["chatgpt_pro", "gemini_deepthink", "chatgpt_dr", "gemini_dr"]
...
def _hit_plane(source: str) -> str:
    if source == "kb":
        return "kb_working_set"
    if source in {"evomap", "evomap_staged_fallback"}:
        return "canonical_knowledge"
    if source == PLANNING_REVIEW_SOURCE:
...
def _hit_explainability(hit: dict[str, Any]) -> dict[str, Any]:
    source = str(hit.get("source") or "").strip()
    meta_key = ""
    selection_reason = "Retrieved from advisor recall."
    if source == "kb":
        selection_reason = "Matched KB evidence through FTS recall."
    elif source in {"evomap", "evomap_staged_fallback"}:
        meta_key = "evomap_meta"
        selection_reason = "Matched EvoMap graph knowledge through runtime retrieval."
    elif source == PLANNING_REVIEW_SOURCE:
        meta_key = "planning_pack_meta"
        selection_reason = "Matched reviewed planning runtime-pack evidence."
    meta = hit.get(meta_key) if meta_key else {}
    meta = meta if isinstance(meta, dict) else {}
    return {
        "source": source,
        "plane": _hit_plane(source),
        "selection_reason": selection_reason,
        "score": float(hit.get("score") or 0.0),
        "promotion_status": str(meta.get("promotion_status") or ""),
        "review_domain": str(meta.get("review_domain") or ""),
        "pack_version": str(meta.get("pack_version") or ""),
        "groundedness": meta.get("groundedness"),
    }


def _annotate_hits_with_explainability(hits: list[dict[str, Any]]) -> list[dict[str, Any]]:
    annotated: list[dict[str, Any]] = []
    for hit in hits:
        entry = dict(hit)
        entry["explainability"] = _hit_explainability(entry)
...
def _build_promotion_audit(
    *,
    ...
    return {
        ...
        "evomap": _promotion_status_counts(evomap_hits, meta_key="evomap_meta"),
        "planning_review_pack": _promotion_status_counts(planning_hits, meta_key="planning_pack_meta"),
        "explainability": "Promotion audit is populated from EvoMap and planning reviewed hits when those sources are present.",
    }


def _build_consult_explainability(
    *,
    auto_context: bool,
    kb_scope_enabled: bool,
    planning_scope_enabled: bool,
    kb_hits: list[dict[str, Any]],
    planning_hits: list[dict[str, Any]],
    ...
    return {
        ...
        "context_injection": [
            {
                "source": "kb",
                "enabled": bool(auto_context and kb_scope_enabled),
                "injected": bool(kb_hits),
                "hit_count": len(kb_hits),
                "plane": _hit_plane("kb"),
                "reason": (
                    "Auto context injected KB snippets into the fan-out prompt."
                    if kb_hits
                    else "KB auto context was enabled but no qualifying KB snippets were found."
                ),
            },
            {
                "source": PLANNING_REVIEW_SOURCE,
                "enabled": bool(planning_scope_enabled),
                "injected": bool(planning_hits),
                "hit_count": len(planning_hits),
                "plane": _hit_plane(PLANNING_REVIEW_SOURCE),
                "reason": (
                    "Explicit planning_review scope injected reviewed planning runtime-pack snippets."
                    if planning_hits
                    else "planning_review scope was enabled but no reviewed planning snippets matched."
                ),
            },
        ],
        "fanout_policy": "Injected context blocks are prepended to the user question before multi-model fan-out.",
        "context_block_count": len(context_blocks),
    }


def _build_recall_explainability(
    *,
    kb_scope_enabled: bool,
    evomap_scope_enabled: bool,
    planning_scope_enabled: bool,
    kb_hits: list[dict[str, Any]],
    evomap_hits: list[dict[str, Any]],
    planning_hits: list[dict[str, Any]],
    ...
    return {
        "source_scope": list(source_scope),
        "selection_policy": f"Merged all enabled source hits by descending score and kept the top {top_k}.",
        "sources": [
            {
                "source": "kb",
                "enabled": bool(kb_scope_enabled),
                "retrieved": len(kb_hits),
                "plane": _hit_plane("kb"),
                "reason": (
                    "KB FTS recall was enabled for advisor recall."
                    if kb_scope_enabled
                    else "KB recall was explicitly excluded by source_scope."
                ),
            },
            {
                "source": "evomap",
                "enabled": bool(evomap_scope_enabled),
                "retrieved": len(evomap_hits),
                "plane": _hit_plane("evomap"),
                "reason": (
                    "EvoMap graph recall was enabled for advisor recall."
                    if evomap_scope_enabled
                    else "EvoMap recall was explicitly excluded by source_scope."
                ),
            },
            {
                "source": PLANNING_REVIEW_SOURCE,
                "enabled": bool(planning_scope_enabled),
                "retrieved": len(planning_hits),
                "plane": _hit_plane(PLANNING_REVIEW_SOURCE),
                "reason": (
                    "planning_review scope enabled reviewed planning runtime-pack recall."
                    if planning_scope_enabled
                    else "planning runtime-pack recall was not opted in."
                ),
            },
        ],
    }
...
def make_consult_router(cfg: AppConfig) -> APIRouter:
    ...
    async def advisor_consult(request: Request, body: dict[str, Any] = Body(...)) -> dict[str, Any]:
        ...
        explainability = _build_consult_explainability(
            auto_context=auto_context,
            kb_scope_enabled=kb_scope_enabled,
            planning_scope_enabled=planning_scope_enabled,
            kb_hits=kb_hits,
            planning_hits=planning_hits,
            context_blocks=context_blocks,
            source_scope=source_scope,
        )
        promotion_audit = _build_promotion_audit(
            kb_hits=kb_hits,
            evomap_hits=[],
            planning_hits=planning_hits,
        )
        ...
        consultation = {
            ...
            "question": question,
            "enriched_question": enriched_question if context_blocks else None,
            "models": models,
    ...
    async def advisor_consult_result(consultation_id: str) -> dict[str, Any]:
        ...
        out: dict[str, Any] = {
            ...
            "explainability": consultation.get("explainability"),
            "promotion_audit": consultation.get("promotion_audit"),
    ...
    async def advisor_recall(body: dict[str, Any] = Body(...)) -> dict[str, Any]:
        ...
        hits = _annotate_hits_with_explainability(all_hits[:top_k])
        ...
        explainability = _build_recall_explainability(
            kb_scope_enabled=kb_scope_enabled,
            evomap_scope_enabled=evomap_scope_enabled,
            planning_scope_enabled=planning_scope_enabled,
            kb_hits=kb_hits,
            evomap_hits=evomap_hits,
            planning_hits=planning_hits,
            source_scope=source_scope,
            top_k=top_k,
        )
