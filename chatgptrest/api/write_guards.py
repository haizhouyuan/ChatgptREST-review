...
from chatgptrest.core.client_request_auth import resolve_registered_client_auth_profile
from chatgptrest.core.env import truthy_env as _truthy_env

_CODING_AGENT_LOW_LEVEL_ASK_BLOCKED_CLIENTS = {
    "antigravity",
    "chatgptrest_ask",
...
_CODING_AGENT_LOW_LEVEL_ASK_ALLOWED_CLIENTS = {
...
def _registered_low_level_ask_client(request: Request) -> str:
    header_client_id = (request.headers.get("x-client-id") or "").strip().lower()
    header_client_name = (request.headers.get("x-client-name") or "").strip().lower()
    for lookup_key in (header_client_id, header_client_name):
        if not lookup_key:
            continue
        profile = resolve_registered_client_auth_profile(lookup_key)
        if profile is not None:
            return profile.client_id
    return ""


def enforce_client_name_allowlist(request: Request, *, allow_registered_low_level_ask: bool = False) -> None:
    ...
    if client_name and client_name in allowlist:
        return
    if allow_registered_low_level_ask and _registered_low_level_ask_client(request):
        return
    ...
    *,
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "allowed_client_names": sorted(allowlist),
            "hint": "Use the public MCP automation-kernel surface (automation_ask / automation_result / automation_job_*) for live asks. For tightly controlled exceptions, allowlist the client or set params.allow_direct_live_chatgpt_ask=true.",
        },
    )


def enforce_coding_agent_low_level_ask_submission(
    request: Request,
    *,
    kind: str,
    client_obj: dict[str, Any] | None = None,
    ...
    if normalized_kind not in {"gemini_web.ask", "qwen_web.ask"}:
        return
    ...
    body_client_name = str(client_obj.get("name") or "").strip().lower()
    effective_client_name = header_client_name or body_client_name
    if not effective_client_name or effective_client_name in _CODING_AGENT_LOW_LEVEL_ASK_ALLOWED_CLIENTS:
        return
    if effective_client_name not in _CODING_AGENT_LOW_LEVEL_ASK_BLOCKED_CLIENTS:
        return
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "reason": "public_agent_is_required_for_coding_agents",
            "detail": "coding-agent low-level /v1/jobs ask submission is blocked for this provider",
            "kind": normalized_kind,
            "x_client_name": effective_client_name,
            "allowed_client_names": sorted(_CODING_AGENT_LOW_LEVEL_ASK_ALLOWED_CLIENTS),
            "hint": "Use the public MCP automation-kernel surface (automation_ask / automation_result / automation_job_*) instead of low-level /v1/jobs ask calls.",
        },
    )
