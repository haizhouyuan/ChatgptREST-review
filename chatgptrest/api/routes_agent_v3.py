...
_TERMINAL_AGENT_STATUSES = {"completed", "failed", "cancelled", "needs_followup", "needs_input"}
...
def _build_agent_response(
    *,
    ...
    if job_id:
        payload["provenance"]["job_id"] = job_id
    if consultation_id:
        payload["provenance"]["consultation_id"] = consultation_id
    if artifacts_list:
...
def _job_snapshot(*, cfg, job_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "agent_status": _agent_status_from_job_status(job_status),
        "answer": answer,
...
def _controller_snapshot(controller: ControllerEngine, *, run_id: str) -> dict[str, Any]:
    ...
    job_id = str(run.get("final_job_id") or delivery.get("job_id") or "")
    controller_status = str(run.get("controller_status") or "")
    attachment_confirmation = None
    if controller_status == "FAILED" and job_id:
        try:
            attachment_confirmation = _attachment_confirmation_for_job(cfg=load_config(), job_id=job_id)
        except Exception:
            attachment_confirmation = None
    if attachment_confirmation:
        return {
...
def _scenario_pack_payload(pack: Any) -> dict[str, Any]:
    return dict(summarize_scenario_pack(pack) or {})
...
def make_v3_agent_router() -> APIRouter:
    ...
    def _refresh_session_state(session: dict[str, Any]) -> dict[str, Any]:
        ...
        if kind == "job" and session.get("job_id"):
            ...
            snapshot = _controller_snapshot(controller, run_id=str(session["run_id"]))
            session["status"] = snapshot["agent_status"]
            session["last_answer"] = snapshot.get("answer", "")
    ...
    async def agent_turn(request: Request, body: dict = Body(...)):
        ...
        compiled_prompt = build_prompt_from_strategy(
            ...
            contract=ask_contract,
            model_provider="gemini" if goal_hint == "image" else "chatgpt",
            custom_context=context,
        )
            ...
            def _run_turn() -> dict[str, Any]:
                ...
                if goal_hint == "image":
                    ...
                    _upsert_session(
                        ...
                        last_message=message,
                        last_answer="",
                        provenance={"route": "image", "provider_path": ["gemini_web"], "final_provider": "gemini_web", "job_id": job_id},
                    )
                ...
                if goal_hint in {"gemini_research", "gemini_deep_research"}:
                    deep_research = goal_hint == "gemini_deep_research"
                    preset = "pro" if deep_research else "deep_think"
                    run_id = f"run_{uuid.uuid4().hex[:12]}"
                    job_id = _submit_direct_job(
                        cfg=cfg,
                        kind="gemini_web.ask",
                        ...
                        client_obj={"name": "agent_v3", "goal_hint": goal_hint, "session_id": session_id},
                        idempotency_key=f"agent-gemini:{session_id}:{goal_hint}:{int(time.time())}",
                    )
                    route_name = "deep_research" if deep_research else "research"
                    _upsert_session(
                        session_id,
                        kind="job",
                        run_id=run_id,
                        ...
                        last_message=message,
                        last_answer="",
                        provenance={"route": route_name, "provider_path": ["gemini_web"], "final_provider": "gemini_web", "job_id": job_id},
                    )
                    snapshot = _wait_for_job_completion(cfg=cfg, job_id=job_id, timeout_seconds=timeout_seconds)
                    artifacts_list = []
                    if snapshot.get("conversation_url"):
                ...
                _upsert_session(
                    ...
                    provenance={
                        ...
                        **({"job_id": str(result.get("job_id"))} if result.get("job_id") else {}),
                    },
                )
