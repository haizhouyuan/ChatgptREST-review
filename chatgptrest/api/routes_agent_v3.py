...
def _opencli_artifact_rows(paths: list[str] | None) -> list[dict[str, Any]]:
    kind_map = {
        "request.json": "opencli_request",
        "stdout.txt": "opencli_stdout",
        "stderr.txt": "opencli_stderr",
        "diagnostics.json": "opencli_diagnostics",
        "result.json": "opencli_result",
        "answer.md": "opencli_answer",
        "doctor.txt": "opencli_doctor",
    }
    ...
    for raw_path in list(paths or []):
        ...
        rows.append(
            {
                "kind": kind_map.get(name, "opencli_artifact"),
                "path": path_text,
                "title": name,
            }
        )
    ...
    async def agent_turn(request: Request, body: dict = Body(...)):
        ...
        try:
            def _run_turn() -> dict[str, Any]:
                ...
                # Phase 2: Explicit opencli narrow lane
                # Check for explicit opencli execution request
                execution_request_raw = (task_intake.context or {}).get("execution_request")
                if execution_request_raw and isinstance(execution_request_raw, dict):
                    executor_kind = str(execution_request_raw.get("executor_kind", "")).strip()
                    if executor_kind == "opencli":
                        ...
                        try:
                            opencli_request = OpenCLIExecutionRequest.from_dict(execution_request_raw)
                        except Exception as exc:
                            logger.error("Failed to parse opencli execution request: %s", exc)
                            response = _build_agent_response(
                                session_id=session_id,
                                run_id=f"run_{uuid.uuid4().hex[:12]}",
                                status="failed",
                                answer=f"Invalid opencli execution request: {exc}",
                                route="opencli",
                        ...
                        # Execute via OpenCLIExecutor
                        policy = OpenCLIExecutionPolicy()
                        executor = OpenCLIExecutor(policy=policy)
                        opencli_result = executor.execute(opencli_request)
                        opencli_artifacts = _opencli_artifact_rows(opencli_result.artifacts)

                        # Build answer from opencli result
                        if opencli_result.ok:
                            answer_artifact = [item for item in opencli_artifacts if item.get("kind") == "opencli_answer"]
                            if answer_artifact:
                                try:
                                    answer = Path(str(answer_artifact[0].get("path") or "")).read_text(encoding="utf-8")
                                except Exception:
                                    answer = f"OpenCLI execution succeeded. Structured result: {json.dumps(opencli_result.structured_result, indent=2)}"
                            else:
                                answer = f"OpenCLI execution succeeded. Structured result: {json.dumps(opencli_result.structured_result, indent=2)}"
                            status = "completed"
                        else:
                            ...
                            if answer_artifact:
                                try:
                                    ...
                                    answer = f"OpenCLI execution failed: {opencli_result.error_message}"
                            else:
                                answer = f"OpenCLI execution failed: {opencli_result.error_message}"
                            status = "failed"
                        ...
                        response = _build_agent_response(
                            ...
                            final_provider="opencli",
                            job_id="",
                            artifacts_list=opencli_artifacts,
                            contract=response_contract,
                            trace_id=trace_id,
                            provider_request=None,
                        )
