...
class CompletionContractView(BaseModel):
    kind: Optional[str] = None
    answer_state: Optional[str] = None
    finality_reason: Optional[str] = None
    answer_chars: Optional[int] = None
    min_chars_required: Optional[int] = None
    authoritative_job_id: Optional[str] = None
    authoritative_answer_path: Optional[str] = None
    answer_provenance: Dict[str, Any] = Field(default_factory=dict)
    export_available: bool = False
    widget_export_available: bool = False


class CanonicalAnswerView(BaseModel):
...
class JobView(BaseModel):
    ...
    requested_execution_lane: Optional[str] = None
    effective_execution_lane: Optional[str] = None
    requested_provider: Optional[str] = None
    effective_provider: Optional[str] = None
    requested_preset: Optional[str] = None
    effective_preset: Optional[str] = None
    ...
    provider_capability_profile: Dict[str, Any] = Field(default_factory=dict)
    premium_allowed: Optional[bool] = None
    premium_justified: Optional[bool] = None
    task_object_contract: Dict[str, Any] = Field(default_factory=dict)
    completion_contract: Optional[CompletionContractView] = None
...
class RetryBudgetProviderView(BaseModel):
    provider_id: str
    recent_retry_count: int = 0
    recent_budget_exceeded_count: int = 0
...
class RetryBudgetStatusView(BaseModel):
    ok: bool = True
    now: float
    window_seconds: int
    max_per_job: int
    ...
    warn_ratio: float
    recent_budget_exceeded_events: int = 0
    hot_providers: list[str] = Field(default_factory=list)
    providers: list[RetryBudgetProviderView] = Field(default_factory=list)
...
class OpsStatusView(BaseModel):
    ...
    retry_budget_window_seconds: int = 0
    retry_budget_recent_by_provider: Dict[str, int] = Field(default_factory=dict)
    retry_budget_hot_providers: list[str] = Field(default_factory=list)
    retry_budget_exceeded_events: int = 0
