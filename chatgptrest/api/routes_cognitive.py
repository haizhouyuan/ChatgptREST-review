...
class GraphQueryRequest(BaseModel):
    query: str = Field(min_length=1)
    scopes: list[str] = Field(default_factory=lambda: ["business"])
    repo: str = ""
    project_id: str = ""
        ...
        result = service.capture(
            [
                MemoryCaptureItem(
                    ...
                    project_id=item.project_id.strip(),
                    object_payload=dict(item.object_payload or {}),
                    require_complete_identity=(
                        body.require_complete_identity
                        if item.require_complete_identity is None
                        else bool(item.require_complete_identity)
                    ),
                )
                for item in body.items
            ]
