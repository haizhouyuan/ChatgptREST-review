...
def _find_answer_path(job_dir: Path, result_obj: dict[str, Any]) -> Path | None:
    authoritative = resolve_authoritative_answer_artifact(result_obj, artifacts_dir=job_dir.parent.parent)
...
def _format_md(report: dict[str, Any]) -> str:
    ...
    if answer_path:
        ...
        lines.append(f"- answer_state: `{answer.get('answer_state')}` final_ready={answer.get('final_ready')}")
        authoritative_answer_path = answer.get("authoritative_answer_path")
        if authoritative_answer_path:
            lines.append(f"- authoritative_answer_path: `{authoritative_answer_path}`")
...
def verify_job(*, artifacts_dir: Path, job_id: str, min_similarity: float) -> dict[str, Any]:
    ...
    completion_contract = completion_contract_from_job_like(result)
    answer_state = get_completion_answer_state(result)
    authoritative_answer_path = get_authoritative_answer_path(result)
    answer_provenance = get_answer_provenance(result)
    final_answer_ready = is_authoritative_answer_ready(result)
    ...
    answer_path = _find_answer_path(job_dir, result)
    answer_text = ""
    answer_sha256 = ""
    answer_chars = 0
    fence_balanced = True
    ...
    if status == "completed" and answer_state != "final":
        warnings.append("completed_not_final")
    if authoritative_answer_path and answer_path is not None and str(answer_path).endswith(authoritative_answer_path):
        notes.append("authoritative_answer_selected")
    ...
    report: dict[str, Any] = {
        ...
        "answer": {
            ...
            "final_ready": final_answer_ready,
