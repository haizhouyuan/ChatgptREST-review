...
def _policy_error_for_request(*, provider: str, preset: str, allow_live_chatgpt_smoke: bool) -> str | None:
    ...
    if not bool(allow_live_chatgpt_smoke):
        return "cold client smoke against live chatgpt_web.ask is blocked by default; use gemini/qwen or pass --allow-live-chatgpt-smoke for a controlled exception"
    if preset_n != "auto":
...
def _build_system_observed_payload(
    *,
    ...
    payload: dict[str, Any] = {
        "ok": str(job_info.get("status") or "").lower() == "completed",
        "provider": provider,
        "preset": preset,
        "question": question,
        "docs_read": docs_read,
        "commands": commands,
        "job_succeeded": str(job_info.get("status") or "").lower() == "completed",
        "job_id": str(job_info.get("job_id") or "").strip(),
        "final_status": str(job_info.get("status") or "").strip(),
...
def _salvage_client_summary(
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "job_succeeded": str(job_obj.get("final_status") or "").strip().lower() == "completed",
        "job_id": str(job_obj.get("job_id") or "").strip(),
        "final_status": str(job_obj.get("final_status") or "").strip(),
        "answer_path": str(answer_path),
        "conversation_path": str(conversation_path),
        "answer_preview": _read_answer_preview(answer_path),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--provider", choices=["chatgpt", "gemini", "qwen"], default="gemini")
