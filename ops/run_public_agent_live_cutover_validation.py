...
from chatgptrest.eval.public_agent_live_cutover_validation import (
    run_public_agent_live_cutover_validation,
    write_public_agent_live_cutover_report,
)


OUT_DIR = Path("docs/dev_log/artifacts/public_agent_live_cutover_validation_20260323")
...
def main() -> int:
    report = run_public_agent_live_cutover_validation()
    basename = _next_report_basename(OUT_DIR)
    json_path, md_path = write_public_agent_live_cutover_report(report, out_dir=OUT_DIR, basename=basename)
    print(
        json.dumps(
            {
