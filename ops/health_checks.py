...
# HTTP status codes that confirm service liveness (non-error responses)
_LIVENESS_HTTP_CODES = {401, 403, 404, 405}
...
def check_http(label: str, url: str, *, timeout: int = 5) -> dict[str, Any]:
    """Check HTTP endpoint liveness.

    Only specific 4xx codes confirm liveness (auth required, wrong path, etc.).
    5xx means the service is listening but unhealthy — NOT counted as ok.
    ...
    try:
        ...
        if exc.code in _LIVENESS_HTTP_CODES:
...
def summarize_runtime_quick() -> dict[str, Any]:
    ...
    for svc in registry.get("services", []):
        if "liveness_url" in svc:
            result = check_http(svc["name"], svc["liveness_url"])
