...
def _classify_outcome(
    *,
    ...
    if isinstance(answer_obj, dict):
        ...
        answer_chunk = answer
    if status == "completed" and answer_chunk:
    ...
    if status == "in_progress" and (export_path or has_export_event):
...
def _has_export_signal(get_obj: dict[str, Any] | None, events_obj: dict[str, Any] | None) -> bool:
    ...
    return bool(export_path or has_export_event)
    ...
    *,
    ...
    for spec in _selected_live_specs(live_env):
        ...
        if job_id:
            ...
            if str((get_obj or {}).get("status") or "").strip() == "completed":
