...
def main() -> int:
    ...
    if isinstance(result.structured_result, dict):
        result_shape = f"dict keys={list(result.structured_result.keys())}"
    elif isinstance(result.structured_result, list):
        result_shape = f"list len={len(result.structured_result)}"
    else:
        result_shape = type(result.structured_result).__name__
    print(f"  Structured result shape: {result_shape}")
