...
CHROME_NO_SANDBOX="${CHROME_NO_SANDBOX:-auto}"
...
CHROME_DISABLE_EXTENSIONS="${CHROME_DISABLE_EXTENSIONS:-0}"
...
CHROME_PROFILE_STATE_HELPER="${ROOT_DIR}/ops/chrome_profile_state.py"
...
chrome_sandbox_available() {
  local sandbox_bin="${CHROME_SANDBOX_BIN:-/opt/google/chrome/chrome-sandbox}"
  [[ "$(id -u)" -ne 0 ]] || return 1
  [[ -x "${sandbox_bin}" ]] || return 1
  [[ -u "${sandbox_bin}" ]] || return 1
  return 0
}
...
resolve_no_sandbox() {
  case "${CHROME_NO_SANDBOX}" in
    ...
    0|false|no)
      ...
      ;;
    auto|"")
      if chrome_sandbox_available; then
        echo "0"
      else
        echo "1"
      fi
      ;;
    *)
      echo "Unsupported CHROME_NO_SANDBOX value: ${CHROME_NO_SANDBOX}" >&2
      return 1
      ;;
  esac
}
...
normalize_profile_state() {
  [[ -f "${CHROME_PROFILE_STATE_HELPER}" ]] || return 0
  python3 "${CHROME_PROFILE_STATE_HELPER}" mark-clean \
    --user-data-dir "${CHROME_USER_DATA_DIR}" \
    --profile-dir "${CHROME_PROFILE_DIR}" \
    >/dev/null 2>&1 || true
}
...
CHROME_USE_NO_SANDBOX="$(resolve_no_sandbox)"
normalize_profile_state
...
if [[ "${CHROME_USE_NO_SANDBOX}" == "1" ]]; then
  args+=(--no-sandbox)
fi
...
if [[ "${CHROME_DISABLE_EXTENSIONS}" == "1" ]]; then
...
echo "  no_sandbox=${CHROME_USE_NO_SANDBOX} (requested=${CHROME_NO_SANDBOX})"
...
echo "  disable_extensions=${CHROME_DISABLE_EXTENSIONS}"
echo "  log=${LOG_FILE}"
