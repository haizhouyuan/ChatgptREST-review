...
_EARLYOOM_BLOCK_RE = re.compile(r"^==== (?P<ts>\S+) pid=(?P<pid>\d+) .*? name=(?P<name>.+?) ====$", re.MULTILINE)
...
def _is_readable_db_path(path: Path) -> bool:
    try:
...
def _resolve_openmind_db_path(env_names: tuple[str, ...], filename: str) -> str:
    """Resolve OpenMind DBs across normal and isolated Codex HOME values.

    Explicit env wins, even if it is wrong, so real misconfiguration remains
    visible. Without env, prefer the current HOME path but fall back to the
    passwd home; Codex sessions often run with an isolated HOME while the live
    OpenMind DBs stay under the login user's real home.
    """
    ...
    candidates: list[Path] = [Path(os.path.expanduser(f"~/.openmind/{filename}"))]
    real_home = _passwd_home().strip()
    if real_home:
        candidates.append(Path(real_home) / ".openmind" / filename)
    ...
    for candidate in deduped:
        if _is_readable_db_path(candidate):
...
def _earlyoom_log_path() -> str:
    raw = os.environ.get("CHATGPTREST_EARLYOOM_LOG_PATH", "").strip()
    if raw:
        return os.path.expanduser(raw)
    stamp = datetime.now().strftime("%Y-%m-%d")
    return f"/var/log/earlyoom/earlyoom_kills_{stamp}.log"
...
def _read_tail(path: Path, *, max_bytes: int = 262_144) -> str:
    try:
        ...
        sudo_text = _read_tail_via_sudo(path, max_bytes=max_bytes)
        if sudo_text is not None:
            return sudo_text
        raise exc


def _read_tail_via_sudo(path: Path, *, max_bytes: int) -> str | None:
    try:
        result = subprocess.run(
            ["sudo", "-n", "tail", "-c", str(int(max_bytes)), str(path)],
            check=False,
            text=True,
    ...
    if "No such file" in stderr or "cannot open" in stderr and "No such file" in stderr:
        return ""
    return None


def _check_earlyoom_recent_kills(
    log_path: str | None = None,
    *,
    now: float | None = None,
    window_seconds: int = 900,
) -> dict[str, Any]:
    """Surface recent earlyoom kills as first-class health attention."""
    path = Path(log_path or _earlyoom_log_path())
    try:
        text = _read_tail(path)
    except PermissionError as exc:
        return {
            "check": "earlyoom_recent_kills",
            "ok": False,
            "recent_count": None,
            "log_path": str(path),
    ...
    matches = list(_EARLYOOM_BLOCK_RE.finditer(text))
    ...
    for index, match in enumerate(matches):
        ...
        try:
            ...
            continue
        if event_ts < now_ts - float(window_seconds):
            continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    kb_path = _resolve_openmind_db_path(("OPENMIND_KB_SEARCH_DB", "OPENMIND_KB_PATH"), "kb_search.db")
    mem_path = _resolve_openmind_db_path(("OPENMIND_MEMORY_DB",), "memory.db")
    ...
    results.append(_check_earlyoom_recent_kills())
