...
for dropin_dir in "${SRC_DIR}"/chatgptrest-*.service.d; do
  [[ -d "${dropin_dir}" ]] || continue
  base="$(basename -- "${dropin_dir}")"
  mkdir -p "${UNIT_DST}/${base}"
  for conf in "${dropin_dir}"/*.conf; do
    [[ -f "${conf}" ]] || continue
    conf_base="$(basename -- "${conf}")"
    install_unit "${conf}" "${UNIT_DST}/${base}/${conf_base}"
    echo "Installed ${UNIT_DST}/${base}/${conf_base}"
  done
done
...
echo ""
echo "Optional (read-only monitor projection refresh):"
echo "  systemctl --user enable --now chatgptrest-monitor-projection-refresh.timer"
echo ""
