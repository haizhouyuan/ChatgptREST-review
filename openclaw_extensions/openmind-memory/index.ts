...
const PROJECT_ID_ALIASES: Record<string, string> = {
  "两轮车车身业务": "shortmobility",
  shortmobility: "shortmobility",
  "行星滚柱丝杠": "prs",
  prs: "prs",
};

const PROJECT_INFERENCE_PATTERNS: Array<{ projectId: string; patterns: RegExp[] }> = [
  {
    projectId: "shortmobility",
    patterns: [/两轮车车身业务/i, /shortmobility/i, /0497/i, /金彭/i, /绿源/i],
  },
  {
    projectId: "prs",
    patterns: [/行星滚柱丝杠/i, /\bprs\b/i, /太普/i],
  },
];

const NON_PROJECT_PROJECT_INFERENCE_SUPPRESS_PATTERNS = [
  /绩效/i,
  /考核/i,
  /考核表/i,
  /个人绩效/i,
...
function normalizeProjectId(value: unknown): string {
  ...
  return PROJECT_ID_ALIASES[text] ?? PROJECT_ID_ALIASES[text.toLowerCase()] ?? text;
}

function inferProjectIdFromText(text: string): string {
  const normalized = String(text ?? "").trim();
  if (!normalized) {
    return "";
  }
  for (const entry of PROJECT_INFERENCE_PATTERNS) {
...
function shouldSuppressProjectInference(text: string): boolean {
  const normalized = String(text ?? "").trim();
  if (!normalized) {
    return false;
  }
  return NON_PROJECT_PROJECT_INFERENCE_SUPPRESS_PATTERNS.some((pattern) => pattern.test(normalized));
}
...
function buildResolveContextRequest(
  ...
  const threadId =
    params.threadId != null && params.threadId !== "" ? String(params.threadId) : params.sessionId ?? "";
  return {
    query: params.query,
...
const memoryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    function scopeKey(args: {
      ...
      return String(args.sessionKey ?? args.sessionId ?? args.agentId ?? "").trim();
    }
    ...
    function resolveProjectId(args: {
      ...
      const explicit = normalizeProjectId(args.projectId);
      const suppressInference = shouldSuppressProjectInference(String(args.text ?? ""));
      if (explicit) {
        const key = scopeKey(args);
        if (key) {
          sessionProjects.set(key, explicit);
        }
        return explicit;
      }
      ...
      const inferred = inferProjectIdFromText(String(args.text ?? ""));
      const key = scopeKey(args);
      const cached = normalizeProjectId(key ? sessionProjects.get(key) : "");
      const fallback = normalizeProjectId(cfg.projectId);
      const resolved = inferred || cached || fallback;
      if (resolved && key) {
        sessionProjects.set(key, resolved);
      }
      return resolved;
    }
    ...
    if (cfg.autoRecall) {
      api.on("before_agent_start", async (event, ctx) => {
        ...
        const cacheKey = hashKey([ctx.sessionKey ?? "", prompt, projectId, cfg.repo, cfg.graphScopes.join(",")]);
