...
const PROJECT_INFERENCE_PATTERNS: Array<{ projectId: string; patterns: RegExp[] }> = [
  {
    ...
    patterns: [/两轮车车身业务/i, /shortmobility/i, /0497/i, /金彭/i, /绿源/i, /袁海州/i, /孙群慧/i],
  },
  {
    projectId: "prs",
    patterns: [/行星滚柱丝杠/i, /\bprs\b/i, /太普/i, /蔡总/i],
  },
];
...
const telemetryPlugin = {
  ...
  register(api: OpenClawPluginApi) {
    ...
    function resolveProjectId(args: {
      ...
      const sessionKey = transportSessionKey(args);
      const cached = normalizeProjectId(sessionProjects.get(sessionKey) ?? "");
      const resolved = inferred || cached;
      if (resolved) {
        sessionProjects.set(sessionKey, resolved);
      }
      return resolved;
    }

    function ensureRunContext(
      ctx: { sessionKey?: string; sessionId?: string; agentId?: string } | undefined,
      hintText = "",
    ): RunContext {
      const sessionKey = transportSessionKey(ctx);
      const existing = runs.get(sessionKey);
      if (existing) {
        const resolvedProjectId = resolveProjectId({ ...ctx, text: hintText });
        if (resolvedProjectId && existing.projectId !== resolvedProjectId) {
          existing.projectId = resolvedProjectId;
        }
        return existing;
      }
      const agentId = String(ctx?.agentId ?? "unknown").trim() || "unknown";
      const projectId = resolveProjectId({ ...ctx, text: hintText });
    ...
    api.on("before_agent_start", async (event, ctx) => {
      const run = ensureRunContext(ctx, String(event.prompt ?? ""));
      const sessionKey = transportSessionKey(ctx);
      queue.enqueue({
    ...
    api.on("agent_end", async (event, ctx) => {
      ...
      sessionProjects.delete(sessionKey);
    });
