    ...
    with env_ctx:
        async with _page_slot(kind="gemini", ctx=ctx), async_playwright() as p:
            ...
            try:
                ...
                async def _collect_visible_tools() -> list[dict[str, Any]]:
                ...
                try:
                    ...
                    tools_btn["visible"] = True
                    tools = await _collect_visible_tools()
                    if not any(_GEMINI_DEEP_THINK_TOOL_RE.search(str(t.get("text") or "")) for t in tools):
                        expanded_more = False
                        try:
                            ...
                            expanded_more = False
                        if expanded_more:
                            extra = await _collect_visible_tools()
                            seen: set[tuple[str, Any]] = set()
                            merged: list[dict[str, Any]] = []
                            for item in [*tools, *extra]:
                                key = (str(item.get("text") or ""), item.get("checked"))
                                if key in seen:
                                    continue
                                seen.add(key)
                                merged.append(item)
