...
_GEMINI_TOOLS_MORE_RE = re.compile(
    r"^(更多(?:工具|选项|選項)?|More(?:\s+(?:tools|options))?)$",
    re.I,
)
...
async def _gemini_expand_more_tools_submenu(page) -> bool:
    """
    Gemini can fold some tools (for example Deep Think) under a second-level
    "More/更多" item inside the prompt-area tools menu.

    Return True when a visible "More" entry was clicked; False otherwise.
    """

    candidates = [
        page.locator("div.cdk-overlay-pane:visible [role='menuitem']").filter(has_text=_GEMINI_TOOLS_MORE_RE),
        page.locator("div.cdk-overlay-container:visible [role='menuitem']").filter(has_text=_GEMINI_TOOLS_MORE_RE),
        page.locator("div.cdk-overlay-pane:visible button").filter(has_text=_GEMINI_TOOLS_MORE_RE),
        page.locator("div.cdk-overlay-container:visible button").filter(has_text=_GEMINI_TOOLS_MORE_RE),
        page.locator("button.toolbox-drawer-item-list-button").filter(has_text=_GEMINI_TOOLS_MORE_RE),
    ]
        ...
        for locator in fuzzy_candidates:
            ...
            for i in range(min(int(count), 16)):
                ...
                if not _gemini_tool_label_matches(label_re=label_re, text=text):
                    continue
                return item
        if not expanded_more:
            try:
                expanded_more = await _gemini_expand_more_tools_submenu(page)
            except Exception:
                expanded_more = False
            if expanded_more:
                continue
