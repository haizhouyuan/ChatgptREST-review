...
async def _open_gemini_page(p, cfg: ChatGPTWebConfig, *, conversation_url: str | None, ctx: Context | None):
    ...
    if use_cdp:
        ...
        async def _open_over_cdp() -> tuple[Any, Any, Any, bool]:
            ...
            page = None
            if _gemini_reuse_existing_cdp_page():
...
async def _gemini_click_upload_menu_item(page, *, label_re: re.Pattern[str]) -> None:
    # The menu is rendered as a floating popover. Prefer visible items.
    ...
    if not await item.count():
        item = page.locator("button, [role='menuitem']").filter(has_text=label_re).first
    if not await item.count():
        raise RuntimeError(f"Gemini upload menu item not found: {label_re.pattern}")
    await item.click()
    await _human_pause(page)
...
async def _gemini_attach_drive_file(page, *, query: str, ctx: Context | None) -> None:
    ...
    await _gemini_open_upload_menu(page, ctx=ctx)
    await _gemini_click_upload_menu_item(
        page,
        label_re=re.compile(r"(云端硬盘|雲端硬碟|Google\\s*Drive|From\\s*Drive|Add\\s*from\\s*Drive)", re.I),
    )
    await _gemini_wait_for_drive_picker_modal_open(page, timeout_ms=20_000)
...
def _gemini_clean_response_text(text: str) -> str:
    ...
    while lines and re.match(r"^(显示思路|Show (thoughts?|reasoning))\b", lines[0].strip(), re.I):
        lines.pop(0)
    ...
    # Some Gemini UI variants include wrapper labels in the extracted innerText, e.g.:
    #   分析
    #   Gemini said
    # Strip these if they appear as leading scaffolding (otherwise we may return a non-answer).
    while lines and re.fullmatch(r"(分析|Analysis)\s*", lines[0].strip(), re.I):
        lines.pop(0)
        while lines and not lines[0].strip():
            lines.pop(0)
    while lines and re.fullmatch(r"Gemini\s*said\s*", lines[0].strip(), re.I):
        lines.pop(0)
        while lines and not lines[0].strip():
            lines.pop(0)
    return "\n".join(lines)
