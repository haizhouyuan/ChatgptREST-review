...
async def _gemini_enter_image_prompt(page, prompt_box, prompt: str) -> None:
    try:
...
async def _gemini_capture_image_bytes_from_page(page, *, src: str) -> tuple[bytes, str]:
    imgs = page.locator("main img")
    count = await imgs.count()
    for idx in range(min(count, 20)):
        img = imgs.nth(idx)
        try:
            current_src = str((await img.get_attribute("src")) or "").strip()
        except Exception:
            continue
        if current_src != str(src or "").strip():
            continue
        raw = await img.screenshot(type="png")
        return raw, "image/png"
    raise RuntimeError(f"Rendered Gemini image element not found for src: {src}")
            ...
            async with _page_slot(kind="gemini", ctx=ctx), async_playwright() as p:
                ...
                try:
                    ...
                    if not resume_only:
                        ...
                        prompt_box = await _gemini_focus_prompt_box(page, prompt_box)
                        await _gemini_enter_image_prompt(page, prompt_box, prompt)
                        await _human_pause(page)
                    ...
                    for img in images:
                        ...
                        try:
                            ...
                            raw, mime_type = await _gemini_capture_image_bytes_from_page(page, src=src)
                        ext = {
                            "image/png": "png",
                            "image/jpeg": "jpg",
                            "image/jpg": "jpg",
