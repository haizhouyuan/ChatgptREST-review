...
from chatgptrest.core.web_retry_policy import human_visible_web_retry_after_int
...
def _gemini_infra_retry_after_seconds() -> int:
    ...
    try:
        ...
        val = 120
    return human_visible_web_retry_after_int(val, max_seconds=3600, jitter=False)
...
def _gemini_classify_error_type(*, error_text: str, fallback: str) -> str:
    ...
    if "gemini is not available in this region" in hay or "not available in this region" in hay:
        return "GeminiUnsupportedRegion"
    if (
        "redirected to google sign-in" in hay
        or "google sign-in" in hay
        or "google sign in" in hay
        or "log in to google" in hay
        or "sign in to continue" in hay
    ):
    ...
    if "gemini import code unavailable" in hay:
        return "GeminiImportCodeUnavailable"
    if (
        "gemini upload menu button not found" in hay
        or "gemini upload menu item not found" in hay
        or "timed out waiting for visible google drive picker iframe" in hay
        or "timed out waiting for drive picker search input" in hay
        or "timed out waiting for drive picker insert button" in hay
        or "timed out waiting for google drive picker to close" in hay
    ):
        return "GeminiDriveAttachUnavailable"
