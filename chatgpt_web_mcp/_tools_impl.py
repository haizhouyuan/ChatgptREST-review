...
async def _chatgpt_ensure_fresh_chat_context(
    page,
    *,
    landing_url: str,
    ctx: Context | None,
    ...
    """
    Ensure a top-level ask is not sent into an unrelated existing ChatGPT thread.

    Strategy:
    - If the current page is not a thread URL, we're already safe.
    - Otherwise try the UI "New chat" affordance first.
    - If that does not move us off the thread, hard-navigate to the landing URL.
    - If we are still on a thread afterwards, fail closed.
    """
    before_url = str(getattr(page, "url", "") or "").strip()
    before_thread_id = _chatgpt_conversation_id_from_url(before_url)
    result: dict[str, Any] = {
        "ok": True,
        "action": "already_non_thread",
    ...
    try:
        ...
        clicked = False
        result["new_chat_error"] = f"{type(exc).__name__}: {exc}"
    result["clicked_new_chat"] = bool(clicked)
    try:
        await _human_pause(page)
    except Exception:
        pass

    after_new_chat_url = str(getattr(page, "url", "") or "").strip()
    result["after_new_chat_url"] = after_new_chat_url
    if not _chatgpt_conversation_id_from_url(after_new_chat_url):
        result["after_url"] = after_new_chat_url
        return result

    result["action"] = "landing_url"
    try:
        await _goto_with_retry(page, str(landing_url or "").strip(), ctx=ctx)
    except Exception as exc:
        result["landing_navigation_error"] = f"{type(exc).__name__}: {exc}"
    after_landing_url = str(getattr(page, "url", "") or "").strip()
    result["after_url"] = after_landing_url
    result["after_thread_id"] = _chatgpt_conversation_id_from_url(after_landing_url)
    if not result["after_thread_id"]:
        return result
...
def _chatgpt_auto_verification_pending_success_wait_seconds() -> float:
    raw = (os.environ.get("CHATGPT_AUTO_VERIFICATION_PENDING_SUCCESS_WAIT_SECONDS") or "").strip()
    if not raw:
        return 90.0
    try:
        return max(1.0, float(raw))
...
def _chatgpt_frontend_rate_limit_cooldown_seconds() -> int:
    raw = (os.environ.get("CHATGPT_FRONTEND_RATE_LIMIT_COOLDOWN_SECONDS") or "").strip()
    if not raw:
        return max(60 * 60, _chatgpt_blocked_cooldown_seconds())
    try:
...
def _chatgpt_action_allowed_during_blocked(*, action: str, reason: str) -> bool:
    ...
    if normalized_reason == "frontend_rate_limit":
        return False
...
_CHATGPT_FRONTEND_RATE_LIMIT_MODAL_RE = _compile_env_regex(
    "CHATGPT_FRONTEND_RATE_LIMIT_MODAL_REGEX",
    r"("
    r"modal-conversation-history-rate-limit|"
    r"conversation[-\s]+history[-\s]+rate[-\s]+limit"
    r")",
    re.I,
)
...
def _chatgpt_frontend_rate_limit_error_marker(text: str | None) -> bool:
    return bool(_CHATGPT_FRONTEND_RATE_LIMIT_MODAL_RE.search(str(text or "")))


async def _chatgpt_frontend_rate_limit_observation(
    page,
    *,
    ...
    if _chatgpt_frontend_rate_limit_error_marker(error_text):
        signals.append("error_text:conversation_history_rate_limit_modal")

    selector = "#modal-conversation-history-rate-limit, [data-testid='modal-conversation-history-rate-limit']"
    try:
        modal = page.locator(selector).first
        if await modal.count():
            try:
                visible = bool(await modal.is_visible())
            except Exception:
                visible = True
            if visible:
                signals.append("dom:conversation_history_rate_limit_modal")
                try:
                    modal_text = _compact_ws(await modal.inner_text())
                except Exception:
                    modal_text = ""
    except Exception:
        pass
    ...
    if not signals:
        try:
            found = await page.evaluate(
                """() => {
  const el = document.querySelector('#modal-conversation-history-rate-limit,[data-testid="modal-conversation-history-rate-limit"]');
  if (!el) return null;
  const rect = el.getBoundingClientRect ? el.getBoundingClientRect() : null;
  const visible = !rect || (rect.width > 0 && rect.height > 0);
  return { visible, text: (el.innerText || el.textContent || '').trim() };
}"""
            )
            if isinstance(found, dict) and bool(found.get("visible")):
                signals.append("dom_eval:conversation_history_rate_limit_modal")
                modal_text = _compact_ws(str(found.get("text") or ""))
        except Exception:
            pass
    ...
    if modal_text:
        observation["modal_text"] = modal_text[:500]
    if error_text and _chatgpt_frontend_rate_limit_error_marker(error_text):
        observation["trigger_error"] = str(error_text)[:500]
    return observation


async def _chatgpt_set_frontend_rate_limit_blocked(
    page,
    *,
    ctx: Context | None,
    phase: str,
    ...
    url = ""
    try:
        title, url, _body = await _chatgpt_page_snapshot(page)
    except Exception:
        try:
    ...
    if artifacts is None:
        artifacts = await _capture_debug_artifacts(page, label=f"chatgpt_{phase}_frontend_rate_limit")

    state = await _chatgpt_set_blocked(
        reason="frontend_rate_limit",
        cooldown_seconds=_chatgpt_frontend_rate_limit_cooldown_seconds(),
        artifacts=artifacts,
        extra={
            "phase": phase,
            "title": title,
            ...
            "signals": list(observation.get("signals") or []),
            "modal_text": str(observation.get("modal_text") or "")[:500],
            "trigger_error_type": trigger_error_type,
            "trigger_error": str(trigger_error or "")[:500],
        },
    )
    when = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(float(state.get("blocked_until") or 0.0)))
    await _ctx_info(ctx, f"ChatGPT frontend rate-limit modal detected; blocking automation until {when}.")
    return state


async def _chatgpt_frontend_rate_limit_result_from_exception(
    page,
    *,
    ctx: Context | None,
    phase: str,
    ...
    observation = await _chatgpt_frontend_rate_limit_observation(page, error_text=error_text)
    if not observation:
        return None

    debug_artifacts = dict(artifacts or {})
    if artifacts is None and page is not None:
        try:
            debug_artifacts = await _capture_debug_artifacts(page, label=f"{phase}_frontend_rate_limit")
        except Exception:
            debug_artifacts = {}

    effective_url = str(conversation_url or "").strip()
    if not effective_url and page is not None:
        try:
            effective_url = str(getattr(page, "url", "") or "").strip()
        except Exception:
            effective_url = ""

    blocked_state = await _chatgpt_set_frontend_rate_limit_blocked(
        page,
        ctx=ctx,
        phase=phase,
    ...
    result.update(
        {
            ...
            "error": "ChatGPT frontend rate-limit modal is visible; automation is blocked to avoid escalating 429s.",
    ...
    *,
    ...
    while True:
        try:
            ...
            if pending2:
                result["pending"] = True
                if "html:verification_success_waiting" in pending2:
                    pending_success_deadline = max(
                        pending_success_deadline,
                        time.time() + _chatgpt_auto_verification_pending_success_wait_seconds(),
                    )
            effective_deadline = max(observe_deadline, pending_success_deadline)
            if (not result["attempted"]) or time.time() >= effective_deadline or not pending2:
                break
...
async def _raise_if_chatgpt_blocked(
    ...
    *,
    ...
    frontend_rate_limit = await _chatgpt_frontend_rate_limit_observation(page)
    if frontend_rate_limit:
        artifacts = await _capture_debug_artifacts(page, label=f"chatgpt_{phase}_frontend_rate_limit")
        state = await _chatgpt_set_frontend_rate_limit_blocked(
            page,
            ctx=ctx,
            phase=phase,
            observation=frontend_rate_limit,
            artifacts=artifacts,
            connection=connection,
        )
        when = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(float(state.get("blocked_until") or 0.0)))
        msg = (
            "ChatGPT frontend rate-limit modal is visible "
            "(modal-conversation-history-rate-limit). "
            "Stop automated ChatGPT Web actions and wait for the cooldown before retrying."
        )
        if artifacts:
            msg += f" Debug artifacts: {artifacts}"
...
def _chatgpt_should_skip_file_input(*, input_id: str | None, accept: str | None, is_image: bool) -> bool:
    normalized_id = str(input_id or "").strip().lower()
    normalized_accept = str(accept or "").strip().lower()
    if normalized_id in {"upload-photos", "upload-camera"}:
        return True
    if (not is_image) and normalized_accept.startswith("image/"):
        return True
    return False
...
async def _upload_file_via_menu(page, *, file_path: Path, ctx: Context | None) -> bool:
    ...
    try:
        for idx in range(min(6, await inputs.count())):
            ...
            if _chatgpt_should_skip_file_input(input_id=cand_id, accept=accept, is_image=is_image):
                continue
            chosen = cand
            break
    except Exception:
    ...
    for existing in reversed(pages):
        ...
        if target_conv_id:
            if current_conv_id != target_conv_id:
                continue
        elif current_conv_id:
            # For top-level asks without an explicit conversation_url, never reuse an
            # existing thread tab. Doing so can silently send unrelated user prompts
            # into an old conversation when the UI "New chat" affordance drifts.
            continue
        try:
...
async def _open_chatgpt_page(p, cfg: ChatGPTWebConfig, *, conversation_url: str | None, ctx: Context | None):
    ...
    if use_cdp:
        ...
        async def _open_over_cdp() -> tuple[Any, Any, Any, bool]:
            ...
            try:
                setattr(page, "_chatgptrest_reused_existing_cdp_page", bool(reused_existing))
            except Exception:
                pass
            try:
                ...
                pass
            bring_existing_to_front = _truthy_env("CHATGPTREST_CDP_BRING_EXISTING_PAGE_TO_FRONT", False)
            if (not reused_existing) or bring_existing_to_front:
                try:
...
def _chatgpt_should_preserve_cdp_page(
    *,
    ...
    conversation_url: str | None,
    page_url: str | None,
    reused_existing_cdp_page: bool = False,
) -> bool:
    if bool(close_context):
        return False
    normalized_tool = str(tool or "").strip().lower()
    if normalized_tool == "chatgpt_web_conversation_export":
        return bool(reused_existing_cdp_page and _chatgpt_conversation_id_from_url(str(page_url or "").strip()))
...
async def _chatgpt_maybe_close_page(
    ...
    *,
    ...
    preserve = _chatgpt_should_preserve_cdp_page(
        ...
        reused_existing_cdp_page=bool(getattr(page, "_chatgptrest_reused_existing_cdp_page", False)),
    )
    if preserve:
        return False
    try:
...
async def chatgpt_web_conversation_export(
    ...
    cfg = _load_config()
    backend_mode_norm = str(backend_mode or "auto").strip().lower()
    if backend_mode_norm not in {"auto", "dom_only"}:
        backend_mode_norm = "auto"
    try:
    ...
    with env_ctx:
        async with _page_slot(kind="chatgpt", ctx=ctx), async_playwright() as p:
            ...
            try:
                ...
                async def _build_dom_export(*, backend_status: int, backend_error_text: str | None) -> str:
                    messages = None
                    ...
                    if not isinstance(messages, list) or not messages:
                        raise RuntimeError(
                            f"backend-api conversation fetch failed (HTTP {backend_status}): "
                            f"{backend_error_text or 'no body'}"
                        )
                    ...
                    export_obj["export_kind"] = "dom_messages"
                    export_obj["conversation_url"] = effective_url
                    export_obj["backend_status"] = backend_status
                    export_obj["backend_error"] = backend_error_text
                    export_obj["backend_mode"] = backend_mode_norm
                    export_obj["messages"] = messages
                    return json.dumps(export_obj, ensure_ascii=False, indent=2)

                if backend_mode_norm == "dom_only":
                    export_kind = "dom_messages"
                    backend_error = "backend_fetch_skipped"
                    status_code = 0
                    conversation_json = await _build_dom_export(
                        backend_status=status_code,
                        backend_error_text=backend_error,
                    )
                else:
                ...
                result = {
                    ...
                    **saved,
                    ...
                    "backend_mode": backend_mode_norm,
                    "preview": preview,
                    "elapsed_seconds": round(time.time() - started_at, 3),
                    "run_id": run_id,
                    "blocked_state": await _chatgpt_read_blocked_state(),
                ...
                frontend_result = await _chatgpt_frontend_rate_limit_result_from_exception(
                    page,
                    ctx=ctx,
                    phase="conversation_export_error",
                    exc=exc,
                    started_at=started_at,
                    run_id=run_id,
                    conversation_url=url,
                    ...
                    _maybe_append_call_log(
                        {
                            ...
                            "elapsed_seconds": frontend_result.get("elapsed_seconds"),
                            "run_id": run_id,
                            "params": {
                                "timeout_seconds": timeout_seconds,
                                "conversation_url": conversation_url,
                            ...
                            "debug_artifacts": frontend_result.get("debug_artifacts"),
                        }
                    )
                    return frontend_result
                blocked_state = await _chatgpt_read_blocked_state()
    ...
    with env_ctx:
        async with _page_slot(kind="chatgpt", ctx=ctx), async_playwright() as p:
            ...
            try:
                ...
                if not conversation_url:
                    fresh_chat = await _chatgpt_ensure_fresh_chat_context(
                        page,
                        landing_url=cfg.url,
                        ctx=ctx,
                    )
                    if not bool(fresh_chat.get("ok")):
                        msg = (
                            "Unable to guarantee a fresh ChatGPT thread for a top-level ask; "
                            "refusing to send into an existing conversation."
                        )
                        result = {
                            "ok": False,
                            "status": "error",
                        ...
                        if idempotency is not None:
                            try:
                                await _idempotency_update(
                                    ...
                                    conversation_url="",
                                    result=result,
                                    error=f"ConversationFreshThreadUnavailable: {msg}",
                                )
                            except Exception:
                                pass
                        return result
                    if str(fresh_chat.get("action") or "").strip().lower() != "already_non_thread":
                        await _find_prompt_box(page, timeout_ms=30_000)
                        await _wait_for_message_list_to_settle(page)
                        conversation_url_effective = (page.url or "").strip()
                        _mark("fresh_chat_context")
                ...
                if page is not None:
                    ...
                    if ctx and artifacts:
                        await _ctx_info(ctx, f"Saved debug artifacts: {artifacts}")
                    frontend_rate_limit = await _chatgpt_frontend_rate_limit_observation(
                        page,
                        error_text=_coerce_error_text(exc),
                    )
                    if frontend_rate_limit:
                        blocked_state = await _chatgpt_set_frontend_rate_limit_blocked(
                            ...
                            artifacts=artifacts,
                            trigger_error_type=type(exc).__name__,
                            trigger_error=_coerce_error_text(exc),
                        )
                        result = {
                            "ok": False,
                            "status": _blocked_status_from_state(blocked_state),
                        ...
                        if idempotency is not None:
                            try:
                                await _idempotency_update(
                                    ...
                                    conversation_url=str(result.get("conversation_url") or ""),
                                    result=result,
                                    error="ChatGPTFrontendRateLimit",
                                )
                            except Exception:
                                pass
                        return result
                ...
                if not retry_after and _looks_like_transient_playwright_error(exc):
                    base_retry_after = float(_env_int("CHATGPT_UNSENT_TRANSIENT_RETRY_AFTER_SECONDS", 90))
                    retry_after = human_visible_web_retry_after_seconds(base_retry_after, max_seconds=1800.0)
                    status = "cooldown"
                result = {
                    "ok": False,
                    "status": status,
                ...
                try:
                    ...
                    if frontend_result is not None:
                        event: dict[str, Any] = {
                            "tool": "chatgpt_web_wait",
                            "status": frontend_result.get("status"),
                            "conversation_url": frontend_result.get("conversation_url"),
                            "elapsed_seconds": frontend_result.get("elapsed_seconds"),
                            "run_id": run_id,
                            "params": {
                                "timeout_seconds": timeout_seconds,
                        ...
                        if frontend_result.get("debug_artifacts"):
                            event["debug_artifacts"] = frontend_result.get("debug_artifacts")
                        _maybe_append_call_log(event)
                        return frontend_result
