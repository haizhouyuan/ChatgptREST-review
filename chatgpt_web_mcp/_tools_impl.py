...
def _blocked_status_from_state(state: dict[str, Any] | None) -> str:
    ...
    if reason in {"network", "unusual_activity"}:
        return "cooldown"
    return "blocked"
...
async def _chatgpt_enforce_not_blocked(*, ctx: Context | None, action: str) -> None:
    ...
    if reason in {"network", "unusual_activity"} and action in {"wait", "conversation_export"}:
        return
...
async def _chatgpt_try_auto_verification_click(
    ...
    *,
    ...
    try:
        ...
        result["resolved"] = bool((not signals2) and (not verify2))
    except Exception as exc:
    ...
    *,
    ...
    if cloudflare_signals:
        ...
        state = await _chatgpt_set_blocked(
            reason="cloudflare",
            cooldown_seconds=_chatgpt_verification_cooldown_seconds(),
            artifacts=artifacts,
            extra={
                "phase": phase,
                "title": title,
...
async def _chatgpt_refresh_page(page, *, ctx: Context | None, reason: str, phase: str) -> None:
    ...
    try:
        ...
        try:
            await _goto_with_retry(page, page.url or _load_config().url, ctx=ctx)
        except Exception:
            pass
    await _human_pause(page)
        ...
        async with _page_slot(kind="chatgpt", ctx=ctx), async_playwright() as p:
            ...
            try:
                ...
                async def _recover_upload_surface(*, reason: str, phase: str) -> bool:
                    ...
                    try:
                        await _chatgpt_refresh_page(page, ctx=ctx, reason=reason, phase=phase)
                    ...
                    if not refreshed_once:
                        ...
                        try:
                            ...
                            if bool(refresh_guard.get("allowed")):
                                await _chatgpt_refresh_page(page, ctx=ctx, reason="wait timeout", phase="wait_timeout")
    ...
    with env_ctx:
        async with _page_slot(kind="chatgpt", ctx=ctx), async_playwright() as p:
            ...
            try:
                ...
                await _find_prompt_box(page, timeout_ms=max(5_000, int(timeout_seconds * 1000)))
                await _chatgpt_refresh_page(page, ctx=ctx, reason="explicit refresh", phase="refresh")
                await _wait_for_message_list_to_settle(page)
                result: dict[str, Any] = {
                    "ok": True,
                    "status": "completed",
