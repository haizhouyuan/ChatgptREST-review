...
DEFAULT_AGENT_SAME_SESSION_REPAIR_MAX_ATTEMPTS = 3
...
def _load_driver_blocked_state(chatgptrest_root: Path) -> dict[str, Any]:
    state_path = (chatgptrest_root / "state" / "driver" / "chatgpt_blocked_state.json").resolve()
...
def _should_auto_same_session_repair(
    *,
    ...
    if str(next_action.get("type") or "").strip().lower() != "same_session_repair":
        return False
    if str(next_action.get("error_type") or "").strip().lower() != "blocked":
        return False
...
def _run_agent_wait_with_recovery(
    *,
    ...
    raise RuntimeError("advisor_agent_wait transport recovery exhausted", detail) from last_exc
...
def _run_agent_turn(python_bin: Path, args: argparse.Namespace) -> int:
    ...
    try:
        ...
        lock_enabled = _chatgpt_review_lane_enabled(args, legacy_preset=legacy_preset)
        ...
        with _provider_lane_lock(
            ...
            if lane_lock:
                runtime["provider_lane_lock"] = dict(lane_lock)
            if out_summary:
            ...
            while True:
                ...
                result = _run_mcp_tool(
                    ...
                    arguments=tool_args,
                    timeout_seconds=request_timeout_seconds,
                    request_id=3 + (repair_attempts * 10),
                    session=mcp_session,
                )
                runtime = _agent_runtime_payload(
                    session_id=session_id,
                ...
                if should_wait:
                    ...
                    wait_result = _run_agent_wait_with_recovery(
                        mcp_url=str(args.public_mcp_url),
                        session_id=str(result.get("session_id") or session_id),
                        agent_timeout_seconds=agent_timeout_seconds,
                        request_timeout_seconds=request_timeout_seconds,
                        request_id=4 + (repair_attempts * 10),
                        session=mcp_session,
                    )
                ...
                blocked_state = _load_driver_blocked_state(root)
                if result and _should_auto_same_session_repair(
                    args=args,
                    result=result,
                    ...
                    repair_attempts += 1
                    wait_seconds = _repair_next_delay_seconds(result=result, blocked_state=blocked_state, cap_seconds=300)
                    runtime = _agent_runtime_payload(
                        session_id=str(result.get("session_id") or session_id),
                        agent_timeout_seconds=agent_timeout_seconds,
                        request_timeout_seconds=request_timeout_seconds,
                        delivery_mode=delivery_mode,
                        submission_stage="same_session_repair_wait",
                        submission_started=True,
                        mcp_session=mcp_session,
                    )
                    runtime["same_session_repair"] = {
                        "attempt": repair_attempts,
                        "wait_seconds": int(wait_seconds),
                        "blocked_state_reason": str(blocked_state.get("reason") or ""),
                    }
                    if lane_lock:
                        runtime["provider_lane_lock"] = dict(lane_lock)
                    if out_summary:
        ...
        if result is None:
            raise RuntimeError("advisor-agent wrapper produced no result")
        if repair_attempts > 0:
            result.setdefault("auto_resumed", True)
            result.setdefault("same_session_repair_attempts", repair_attempts)
        if out_summary:
            runtime = _agent_runtime_payload(
                session_id=str(result.get("session_id") or session_id),
                agent_timeout_seconds=agent_timeout_seconds,
