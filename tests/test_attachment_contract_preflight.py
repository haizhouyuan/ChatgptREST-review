...
def test_detect_missing_attachment_contract_ignores_slash_delimited_concepts() -> None:
    signal = detect_missing_attachment_contract(
        kind="chatgpt_web.ask",
        input_obj={"question": "请对 episodic/semantic/procedural memory 做结构化比较。"},
        params_obj={"purpose": "research"},
    )
    assert signal is None


def test_detect_missing_attachment_contract_ignores_importer_review_concepts() -> None:
    signal = detect_missing_attachment_contract(
        kind="chatgpt_web.ask",
        input_obj={"question": "请先说明 /importer/review 的分工，再给我 importer/review execute/dry-run 的区别。"},
        params_obj={"purpose": "research"},
    )
    assert signal is None


def test_detect_missing_attachment_contract_ignores_uri_like_tokens() -> None:
    signal = detect_missing_attachment_contract(
        kind="chatgpt_web.ask",
        input_obj={"question": "请阅读 s://github.com/langchain-ai/langgraph 并总结。"},
        params_obj={"purpose": "research"},
    )
    assert signal is None
...
def test_create_job_rejects_high_risk_missing_attachment_contract(env: dict[str, Path]) -> None:
    app = create_app()
    ...
    res = client.post(
        ...
        json={
            "kind": "chatgpt_web.ask",
            "input": {"question": "Read the local review bundle at /vol1/work/review_bundle_v1.md and review it."},
            "params": {"preset": "auto", "purpose": "review"},
        },
        headers={"Idempotency-Key": "attachment-contract-create-fail"},
    )

    assert res.status_code == 400
    detail = res.json()["detail"]
    assert detail["error"] == "attachment_contract_missing"
    assert detail["error_type"] == "AttachmentContractMissing"
    assert detail["attachment_contract"]["high_risk"] is True


def test_chatgpt_executor_fails_closed_when_declared_file_path_is_missing(tmp_path: Path) -> None:
    ex = ChatGPTWebMcpExecutor(mcp_url="http://127.0.0.1:0/mcp")
    ex._client = _NeverCalledToolCaller()  # type: ignore[assignment]
    missing = tmp_path / "missing.md"
    res = asyncio.run(
        ex.run(
            job_id="job-chatgpt-missing-file",
            kind="chatgpt_web.ask",
            input={"question": "Read the attachment.", "file_paths": [missing.as_posix()]},
            params={"preset": "auto", "phase": "send", "max_wait_seconds": 60},
        )
    )
    assert res.status == "error"
    meta = res.meta or {}
    assert meta.get("error_type") == "AttachmentFileNotFound"
    assert meta.get("missing_file_paths") == [missing.as_posix()]
...
def test_worker_records_attachment_contract_event_and_issue_family(
    ...
    monkeypatch.setenv("CHATGPTREST_FAIL_CLOSED_HIGH_RISK_ATTACHMENT_CONTRACT_ON_CREATE", "0")
    app = create_app()
    client = TestClient(app)
