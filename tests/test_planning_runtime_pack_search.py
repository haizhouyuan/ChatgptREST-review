...
def test_search_planning_runtime_pack_returns_only_runtime_visible_atoms(tmp_path: Path, monkeypatch) -> None:
    ...
    assert hits[0]["planning_pack_meta"]["ready_for_explicit_consumption"] is True
    assert hits[0]["planning_pack_meta"]["bundle_generated_at"] == "2026-04-01T00:00:00+00:00"
    assert hits[0]["planning_pack_meta"]["bundle_freshness"] in {"fresh", "stale", "unknown"}


def test_planning_runtime_pack_bundle_status_reports_readiness_and_freshness(tmp_path: Path, monkeypatch) -> None:
    bundle, _ = _write_pack_bundle(tmp_path)
    monkeypatch.setenv("CHATGPTREST_PLANNING_RUNTIME_PACK_BUNDLE_DIR", str(bundle))
    ...
    assert status["ready_for_explicit_consumption"] is True
    assert status["pack_version"] == "pack"
    assert status["bundle_generated_at"] == "2026-04-01T00:00:00+00:00"
    assert status["bundle_freshness"] in {"fresh", "stale"}
    assert status["scope"] == {"opt_in_only": True, "default_runtime_cutover": False}
