...
def test_finbot_theme_radar_scout_writes_pending_inbox_item(tmp_path: Path, monkeypatch) -> None:
            ...
            "structured": (
                {
                    ...
                    "claim_ledger": [
                        {
                            "claim": "TSMC CPO 配套条件正在形成",
                            "evidence_grade": "high",
                            "importance": "high",
                            "why_it_matters": "决定 CPO 何时从主题信号升级为可验证产线故事。",
                            "next_check": "TSMC 下一次法说会明确提到 CPO / CoWoS 资源分配。",
    ...
    assert saved_package["claim_evidence_bindings"]["bindings"][0]["is_exact_excerpt"] is True
    assert saved_package["claim_evidence_bindings"]["bindings"][0]["missing_primary_evidence"] is False
    assert saved_package["counterevidence_packets"]["packets"][0]["stance"] == "weaken"
    assert saved_package["policy_result"]["missing_evidence"] == []
    assert saved_package["policy_result"]["missing_counterevidence"] == []
    assert Path(saved_package["guardrail_artifacts"]["policy_result"]).exists()
...
def test_finbot_theme_batch_run_writes_pending_items(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["promotion_packets"][0]["theme_slug"] == "transformer"
    assert saved["payload"]["promotion_packet"]["tier"] == "paid-tier"


def test_finbot_theme_batch_run_filters_single_theme_slug(tmp_path: Path, monkeypatch) -> None:
    catalog_path = tmp_path / "catalog.json"
    catalog_path.write_text(
        json.dumps(
            {
                ...
                "themes": [
                    {
                        "theme_slug": "transformer",
                        "title": "变压器超级周期",
                        "spec_path": "transformer.yaml",
                        "events_path": "transformer.json",
                        "as_of": "2026-03-15",
                    },
                    {
                        "theme_slug": "commercial_space",
                        "title": "商业航天",
                        "spec_path": "commercial_space.yaml",
                        "events_path": "commercial_space.json",
                        "as_of": "2026-03-15",
                    },
                ],
            }
    ...
    def _fake_run(*, args: list[str], **_: object) -> dict[str, object]:
        theme_slug = args[args.index("--theme-slug") + 1]
        return {
            "theme_slug": theme_slug,
            "recommended_posture": "watch_only",
            "best_expression": {"entity": "Rocket Lab"},
            "run_root": f"/tmp/{theme_slug}",
        }

    monkeypatch.setattr(finbot, "_run_finagent_script_json", _fake_run)

    payload = finbot.theme_batch_run(
        root=tmp_path,
    ...
    assert payload["ok"] is True
    assert payload["theme_count"] == 1
    assert payload["requested_theme_slug"] == "commercial_space"
    assert payload["runs"][0]["theme_slug"] == "commercial_space"
    assert len(payload["created_items"]) == 1
    theme_state = json.loads((tmp_path / "themes" / "commercial-space" / "latest.json").read_text(encoding="utf-8"))
    assert theme_state["theme_slug"] == "commercial_space"
    assert theme_state["best_expression"] == "Rocket Lab"


def test_finbot_theme_batch_run_returns_theme_not_found(tmp_path: Path) -> None:
    catalog_path = tmp_path / "catalog.json"
    catalog_path.write_text(
        json.dumps(
            {
    ...
    assert payload["error"] == "theme_not_found"
    assert payload["requested_theme_slug"] == "commercial_space"
...
def test_finbot_daily_work_composes_refresh_watchlist_and_radar(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **_: {
            "ok": True,
            "created": False,
            "promotion_packet": {"candidate_id": "candidate-1", "promote_bool": True},
        },
    )
    monkeypatch.setattr(
        finbot,
        ...
        lambda **_: {
            "ok": True,
            "created_items": [{"item_id": "theme-1"}],
            "promotion_packets": [{"theme_slug": "theme-a", "promote_bool": False}],
        },
    )
    ...
    assert payload["promotion_packets"][0]["candidate_id"] == "candidate-1"
    assert payload["promotion_packets"][1]["theme_slug"] == "theme-a"
...
def test_finbot_opportunity_deepen_writes_dossier_and_inbox(tmp_path: Path, monkeypatch) -> None:
    ...
    assert payload["claim_evidence_bindings"]["bindings"][0]["source_name"] == "TSMC IR"
    assert payload["claim_evidence_bindings"]["bindings"][0]["is_exact_excerpt"] is True
    assert payload["counterevidence_packets"]["packets"][0]["stance"] == "weaken"
    assert payload["policy_result"]["missing_evidence"] == []
    assert payload["policy_result"]["missing_counterevidence"] == []
    assert payload["promotion_packet"]["blocked_by"] == []
    assert payload["peer_snapshot"]["candidate_id"] == "candidate_tsmc_cpo"
    assert payload["peer_snapshot"]["valuation_driver"]
    assert payload["transcript_packet"]["candidate_id"] == "candidate_tsmc_cpo"
    assert payload["transcript_packet"]["available"] is False
    assert payload["primary_data_packet"]["candidate_id"] == "candidate_tsmc_cpo"
    assert payload["primary_data_packet"]["promotion_enrichment"]["promotion_recommendation"] == "no_primary_data"
    assert payload["history"]["status"] == "new"
    assert "action_distance_after" in payload["history"]
    assert Path(payload["guardrail_artifacts"]["peer_snapshot"]).exists()
    assert Path(payload["guardrail_artifacts"]["transcript_packet"]).exists()
    assert Path(payload["guardrail_artifacts"]["primary_data_packet"]).exists()
...
def test_finbot_claim_evidence_bindings_flag_missing_primary_excerpt() -> None:
    bindings = finbot._build_claim_evidence_bindings(
        candidate_id="candidate_demo",
        claim_objects=[{"claim_id": "clm_demo", "is_load_bearing": True, "importance": "high"}],
        claim_ledger=[
            {
                "claim_id": "clm_demo",
                "claim": "Demo claim",
    ...
    assert bindings["bindings"][0]["claim_id"] == "clm_demo"
    assert bindings["bindings"][0]["missing_primary_evidence"] is True
    assert bindings["bindings"][0]["is_exact_excerpt"] is False


def test_finbot_daily_work_skips_market_discovery_when_closed(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(finbot, "refresh_dashboard_projection", lambda: {"ok": True})
    monkeypatch.setattr(finbot, "watchlist_scout", lambda **_: {"ok": True, "created": False})
    monkeypatch.setattr(finbot, "theme_radar_scout", lambda **_: {"ok": True, "created": False})
    monkeypatch.setattr(finbot, "_is_any_market_open", lambda: False)

    payload = finbot.daily_work(
        root=tmp_path,
        include_source_refresh=False,
        include_theme_batch=False,
        include_market_discovery=True,
    )

    assert payload["ok"] is True
    assert payload["market_discovery"]["reason"] == "market_closed"
    assert payload["market_discovery"]["market_open"] is False


def test_finbot_posture_guard_treats_no_refute_packet_as_missing_counterevidence() -> None:
    result = finbot._evaluate_posture_guard(
        package_payload={
            "candidate_id": "candidate_demo",
            "tier": "free-tier",
            "current_decision": "deepen_now",
            ...
            "promotion_packet": {
                "promote_bool": True,
                "first_hand_source_to_check": [{"source_id": "src_demo", "name": "Demo Source"}],
            },
        },
        claim_evidence_bindings={
            "bindings": [{"claim_id": "clm_demo", "missing_primary_evidence": False, "is_exact_excerpt": True}]
        },
        counterevidence_packets={
            "packets": [{"claim_id": "clm_demo", "stance": "no_refute_found", "excerpt": ""}]
        },
    )

    assert result["missing_evidence"] == []
    assert result["missing_counterevidence"] == ["clm_demo"]
    assert result["max_allowed_posture"] == "prepare_candidate"
    assert result["promote_to_paid"] is True
