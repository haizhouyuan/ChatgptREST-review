...
from chatgptrest.governance.authority_anchor import AuthorityAnchor, AuthorityDocumentStatus
from chatgptrest.kernel.context_assembler import ContextAssembler
...
def _synthetic_authority_anchor(
    *,
    project_id: str = "prj-alpha",
    ...
    return AuthorityAnchor(
        ...
        anchor_path=f"/tmp/{project_id}/_project_context.md",
        planning_base="/tmp",
        frozen_facts=["Alpha launch is not confirmed and remains blocked."],
        style_rules=["去掉不是而是结构，直接陈述判断。"],
        pinned_authority_inputs=["/tmp/authority-alpha.md"],
        current_phase_framing="- Keep the Alpha launch blocked until audit clears.",
        schema_gaps=list(schema_gaps or []),
        missing_docs=list(missing_docs or []),
        authority_docs=[
            AuthorityDocumentStatus(
                path="/tmp/authority-alpha.md",
                exists=not missing_docs,
                non_empty=not missing_docs,
                size_bytes=128 if not missing_docs else 0,
                modified_at="2026-04-08T00:00:00+00:00",
                age_days=0.0,
                stale_status="fresh",
            )
        ],
        project_context="Alpha launch blocked until audit clears.\nDo not use 不是…而是… phrasing.",
    )
...
def test_context_resolver_prioritizes_authority_and_surfaces_conflicts(monkeypatch) -> None:
    _seed_runtime()
    runtime = get_advisor_runtime()
    runtime.memory.stage_and_promote(
        MemoryRecord(
            category="conversation",
            key="working:sess-1:alpha-conflict",
            value={"role": "assistant", "message": "Alpha launch is confirmed and ready for shipment."},
            confidence=0.9,
            source=MemorySource(
                type=SourceType.USER_INPUT.value,
                agent="advisor",
        ...
        MemoryTier.WORKING,
        reason="seed conflicting working-memory guidance",
    )
    monkeypatch.setattr(
        "chatgptrest.cognitive.context_service.load_authority_anchor",
        lambda project_id: _synthetic_authority_anchor(project_id=project_id),
    )
    resolver = ContextResolver(runtime)

    result = resolver.resolve(
        ContextResolveOptions(
            query="What is the current Alpha launch posture?",
            session_id="sess-1",
            account_id="acct-1",
            agent_id="advisor",
            role_id="planning",
    ...
    assert result.ok is True
    assert result.prompt_prefix.startswith("## Authority Anchor")
    assert result.metadata["authority_governance"]["present"] is True
    assert result.metadata["authority_governance"]["conflict_count"] >= 1
    assert any(block["kind"] == "authority" for block in result.metadata["explainability"]["context_blocks"])


def test_context_resolver_marks_stale_and_missing_authority_as_degraded(monkeypatch) -> None:
    _seed_runtime()
    runtime = get_advisor_runtime()
    monkeypatch.setattr(
        "chatgptrest.cognitive.context_service.load_authority_anchor",
        lambda project_id: _synthetic_authority_anchor(
            project_id=project_id,
            stale_status="stale",
            missing_docs=["/tmp/missing-alpha.md"],
            schema_gaps=["missing_owner"],
        ),
    )
    resolver = ContextResolver(runtime)
    ...
    assert "authority_anchor_stale" in result.degraded_sources
    assert "authority_docs_missing" in result.degraded_sources
    assert "authority_schema_partial" in result.degraded_sources
    assert result.metadata["authority_governance"]["missing_docs"] == ["/tmp/missing-alpha.md"]


def test_context_resolver_treats_personal_graph_gap_as_optional_for_project_scoped_packets(monkeypatch) -> None:
    _seed_runtime()
    runtime = get_advisor_runtime()
    monkeypatch.setattr(
        "chatgptrest.cognitive.context_service.load_authority_anchor",
    ...
    assert result.ok is True
    assert "personal_graph_empty" not in result.degraded_sources
    assert "personal_graph_empty" in result.metadata["optional_gap_sources"]


def test_context_assembler_protects_authority_under_token_pressure(monkeypatch) -> None:
    class _StubMemory:
        def get_working_context(self, **kwargs):  # noqa: ANN003
            return []

        def get_episodic(self, **kwargs):  # noqa: ANN003
            return []

        def get_semantic(self, **kwargs):  # noqa: ANN003
            return [
                MemoryRecord(
                    category="user_profile",
                    key="semantic-guidance",
                    value={"guidance": " ".join(["generic"] * 400)},
                    confidence=0.9,
                    source=MemorySource(
                        type=SourceType.USER_INPUT.value,
                        agent="advisor",
    ...
    monkeypatch.setattr(
        "chatgptrest.kernel.context_assembler.load_authority_anchor",
        lambda project_id: _synthetic_authority_anchor(project_id=project_id),
    )
    assembler = ContextAssembler(memory_manager=_StubMemory(), max_tokens=3600)

    pack = assembler.build(
        query="Summarize Alpha launch posture.",
        session_id="sess-1",
        project_id="prj-alpha",
        semantic_limit=1,
    )
    ...
    assert pack.sources
    assert pack.sources[0].source_type == "authority"
    assert any(source.source_type == "authority" for source in pack.sources)
...
def test_context_resolver_project_scope_filters_active_context() -> None:
    _seed_runtime()
    runtime = get_advisor_runtime()
    runtime.memory.stage_and_promote(
        MemoryRecord(
    ...
    assert result.ok is True
    assert "Beta Project" in result.prompt_prefix
    assert "Alpha Project" not in result.prompt_prefix
    assert result.metadata["project_id"] == "prj-002"
...
def test_context_resolver_recalls_imported_work_memory_and_exposes_import_hits(tmp_path) -> None:  # noqa: ANN001
    ...
    assert result.metadata["work_memory_scope_hits"]["active_project"] == "account_role_project"
