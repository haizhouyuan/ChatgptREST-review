...
def test_context_resolve_includes_authority_governance_metadata(monkeypatch) -> None:  # noqa: ANN001
    client = _make_router_client()
    _seed_runtime_state()

    monkeypatch.setattr(
        "chatgptrest.cognitive.context_service.load_authority_anchor",
        lambda project_id: AuthorityAnchor(
            ...
            stale_status="fresh",
            anchor_path="/tmp/alpha/_project_context.md",
            planning_base="/tmp",
            frozen_facts=["Alpha launch is not confirmed."],
            style_rules=["去掉不是而是结构。"],
            pinned_authority_inputs=["/tmp/alpha-authority.md"],
            current_phase_framing="- Keep Alpha blocked until audit clears.",
            schema_gaps=[],
            missing_docs=[],
            authority_docs=[
                AuthorityDocumentStatus(
                    path="/tmp/alpha-authority.md",
                    exists=True,
                    non_empty=True,
                    size_bytes=128,
            ...
            project_context="Alpha stays blocked until audit clears.",
        ),
    )
    ...
    response = client.post(
        "/v2/context/resolve",
        json={
            "query": "What is the Alpha launch posture?",
            "session_key": "sess-1",
            "account_id": "acct-1",
            "agent_id": "advisor",
            "role_id": "planning",
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["prompt_prefix"].startswith("## Authority Anchor")
    assert body["metadata"]["authority_governance"]["present"] is True
    retrieval_plan = {item["kind"]: item for item in body["metadata"]["retrieval_plan"]}
    assert retrieval_plan["authority"]["resolved"] is True
    assert retrieval_plan["authority"]["plane"] == "authority_anchor"
...
def test_context_resolve_project_scope_filters_active_context() -> None:
    client = _make_router_client()
    runtime = get_advisor_runtime()

    for project_id, name in (("prj-alpha", "Alpha Project"), ("prj-beta", "Beta Project")):
        runtime.memory.stage_and_promote(
            MemoryRecord(
    ...
    body = response.json()
    assert body["ok"] is True
    assert "Beta Project" in body["prompt_prefix"]
    assert "Alpha Project" not in body["prompt_prefix"]
    assert body["metadata"]["project_id"] == "prj-beta"
...
def test_knowledge_ingest_writes_kb_and_graph(tmp_path, monkeypatch) -> None:
    ...
    assert graph_atom.scope_project == "ChatgptREST"
...
def test_memory_capture_and_context_resolve_honor_project_scope() -> None:
    client = _make_router_client()
    _seed_runtime_state()
    ...
    capture = client.post(
        ...
        json={
            "items": [
                {
                    "title": "PRS preference",
                    "content": "Treat the PRS execution card as the active operating baseline.",
                    "trace_id": "tr-memory-capture-prs",
                    "session_key": "sess-prs-origin",
                    "account_id": "acct-prs",
                    "source_ref": "openclaw://session/sess-prs-origin/agent_end",
                    "project_id": "prs",
                }
            ]
        },
    ...
    good = client.post(
        "/v2/context/resolve",
        json={
            "query": "What is the active baseline for this project?",
            "session_key": "sess-prs-ask",
            "account_id": "acct-prs",
            "project_id": "prs",
            "sources": ["memory"],
        },
    ...
    assert "execution card" in captured_block["text"]

    miss = client.post(
        "/v2/context/resolve",
        json={
            "query": "What is the active baseline for this project?",
            "session_key": "sess-other-project",
            "account_id": "acct-prs",
    ...
    miss_body = miss.json()
    assert all(block["source_type"] != "captured" for block in miss_body["context_blocks"])
    assert miss_body["metadata"]["project_id"] == "shortmobility"
...
def test_telemetry_ingest_preserves_identity_contract_fields() -> None:
    ...
    feedback = runtime.memory.get_episodic(category="execution_feedback", project_id="prs", limit=10)
    assert len(feedback) == 1
    assert feedback[0].source["project_id"] == "prs"
