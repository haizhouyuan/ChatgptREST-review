...
def test_gemini_single_small_text_attachment_is_inlined(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    def _fake_upload(*, job_id: str, file_paths: list[str]) -> list[dict[str, Any]]:  # noqa: ARG001
        ...
        upload_called = True
        raise AssertionError("drive upload should not run for inline single-text lane")
    ...
    res = asyncio.run(
        executor.run(
            job_id="job",
            kind="gemini_web.ask",
            input={"question": "read", "file_paths": [str(sample)]},
            params={"preset": "pro", "timeout_seconds": 120, "max_wait_seconds": 120, "min_chars": 0},
        )
    )
    assert res.status == "completed"
    ...
    assert calls
    assert calls[0]["tool_name"] == "gemini_web_ask_pro"
    assert "drive_files" not in calls[0]["tool_args"]
    assert "Inlined Attachment Context" in calls[0]["tool_args"]["question"]
    assert "Line one." in calls[0]["tool_args"]["question"]
    assert isinstance(res.meta.get("attachment_preprocess"), dict)
    assert res.meta["attachment_preprocess"].get("single_text_inlined") is True
    assert res.meta["attachment_preprocess"].get("uploaded_count") == 0


def test_gemini_large_single_text_attachment_falls_back_to_drive(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    sample = tmp_path / "large_note.md"
    sample.write_text("A" * 20_000, encoding="utf-8")

    upload_calls: list[list[str]] = []
    calls: list[dict[str, Any]] = []

    def _fake_upload(*, job_id: str, file_paths: list[str]) -> list[dict[str, Any]]:  # noqa: ARG001
        upload_calls.append(list(file_paths))
        return [
            {
                "src_path": str(sample),
                "drive_name": sample.name,
                "drive_id": "large-id",
                "drive_url": "https://drive.google.com/open?id=large-id",
                "upload_completed": True,
            }
        ]
    ...
    assert res.status == "completed"
    assert upload_calls == [[str(sample)]]
    assert calls
    assert calls[0]["tool_name"] == "gemini_web_ask_pro"
    assert calls[0]["tool_args"]["drive_files"] == ["https://drive.google.com/open?id=large-id"]
    assert "Inlined Attachment Context" not in calls[0]["tool_args"]["question"]
    assert isinstance(res.meta.get("attachment_preprocess"), dict)
    assert res.meta["attachment_preprocess"].get("single_text_inline_skipped") is True
    assert res.meta["attachment_preprocess"].get("inline_skip_reason") == "file_too_large"
    assert res.meta["attachment_preprocess"].get("uploaded_count") == 1
