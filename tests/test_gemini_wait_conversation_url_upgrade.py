...
class _MismatchedThreadClient:
    ...
    def call_tool(self, *, tool_name: str, tool_args: dict, timeout_sec: float) -> dict:  # noqa: ARG002
        ...
        return {
            ...
            "observed_conversation_url": "https://gemini.google.com/app/zzzz9999",
...
def test_gemini_wait_loop_keeps_requested_thread_on_thread_mismatch() -> None:
    ex = GeminiWebMcpExecutor(mcp_url="http://127.0.0.1:0/mcp")
    client = _MismatchedThreadClient()
    ex._client = client  # type: ignore[assignment]

    res = asyncio.run(
        ex.run(
            job_id="job-gemini-wait-thread-mismatch",
            kind="gemini_web.ask",
            input={
                "question": "hello",
                "conversation_url": "https://gemini.google.com/app/abc123xyz",
    ...
    assert res.meta and res.meta.get("conversation_url") == "https://gemini.google.com/app/abc123xyz"
    assert res.meta.get("error_type") == "GeminiConversationThreadMismatch"
    assert res.meta.get("observed_conversation_url") == "https://gemini.google.com/app/zzzz9999"
    assert res.meta.get("retry_after_seconds") == 120
