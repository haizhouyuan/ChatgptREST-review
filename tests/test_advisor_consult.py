...
def test_consult_returns_explainability_and_persists_promotion_audit(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    body = response.json()
    assert body["planning_context_injected"] is True
    assert body["promotion_audit"]["planning_review_pack"] == {"active": 1}
    planning_entry = next(
        item for item in body["explainability"]["context_injection"]
        if item["source"] == "planning_review_pack"
    )
    assert planning_entry["injected"] is True
    assert planning_entry["hit_count"] == 1

    followup = client.get(f"/v1/advisor/consult/{body['consultation_id']}")
    assert followup.status_code == 200
    followup_body = followup.json()
    assert followup_body["promotion_audit"]["planning_review_pack"] == {"active": 1}
    planning_entry = next(
        item for item in followup_body["explainability"]["context_injection"]
        if item["source"] == "planning_review_pack"
    )
    assert planning_entry["injected"] is True


def test_recall_returns_hit_explainability_and_promotion_audit_for_planning_review(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    assert body["promotion_audit"]["planning_review_pack"] == {"active": 1}
    assert body["hits"][0]["source"] == "planning_review_pack"
    assert body["hits"][0]["explainability"]["plane"] == "planning_review_pack"
    planning_source = next(
        item for item in body["explainability"]["sources"]
        if item["source"] == "planning_review_pack"
    )
    assert planning_source["enabled"] is True
    assert planning_source["retrieved"] == 1


def test_recall_surfaces_evomap_promotion_audit_and_hit_explainability(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    db.put_document(
        Document(
            ...
            source="evomap",
            project="launch",
            raw_ref="/tmp/doc_evomap.md",
            title="Launch playbook",
        )
    )
    db.put_episode(Episode(episode_id="ep_evomap", doc_id="doc_evomap", episode_type="note", title="Launch"))
    db.put_atom(
        Atom(
            atom_id="at_evomap",
            episode_id="ep_evomap",
            atom_type="procedure",
            question="How to validate launch readiness?",
            answer="Run the validation suite and inspect the launch gate summary.",
            canonical_question="How to validate launch readiness?",
    ...
    app = create_app()
    client = TestClient(app)
    response = client.post("/v1/advisor/recall", json={"query": "validate launch readiness"})
    assert response.status_code == 200
    body = response.json()
    assert body["promotion_audit"]["evomap"] == {"active": 1}
    evomap_hit = next(hit for hit in body["hits"] if hit["artifact_id"] == "at_evomap")
    assert evomap_hit["explainability"]["source"] == "evomap"
    assert evomap_hit["explainability"]["promotion_status"] == "active"
