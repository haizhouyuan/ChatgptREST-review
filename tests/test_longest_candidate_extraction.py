...
def test_final_looking_in_progress_assistant_is_not_returned() -> None:
    """A final-looking text node is still not final while its assistant message is in_progress."""
    export = _make_mapping_export([
        ("user", "Please produce the final operating plan.", "finished_successfully", "text"),
        (
            "assistant",
            (
                ...
                "3. Promote only after evidence review.\n"
            ),
            "in_progress",
            "text",
        ),
    ...
    answer, info = extract_answer_from_conversation_export_obj(
        obj=export,
        question="Please produce the final operating plan.",
        deep_research=False,
        allow_fallback_last_assistant=False,
    )
    ...
    assert answer is None
    assert info.get("answer_source") == "matched_in_progress_partial"
    assert info.get("chosen_assistant_status") == "in_progress"
...
def test_missing_reply_marks_thread_contamination_when_next_turn_is_user() -> None:
    export = _make_export([
        {"role": "user", "text": "Research the 3D printing AI workflows."},
        {"role": "user", "text": "Unrelated Q1 planning follow-up."},
        {"role": "assistant", "text": "This answer belongs to the unrelated follow-up."},
    ])
    answer, info = extract_answer_from_conversation_export_obj(
        obj=export,
        question="Research the 3D printing AI workflows.",
        deep_research=True,
        allow_fallback_last_assistant=False,
    )
    assert answer is None
    assert info.get("answer_source") == "matched_but_missing_assistant"
    assert info.get("thread_contaminated_after_match") is True
    assert info.get("next_role_after_match") == "user"
    assert info.get("subsequent_user_turn_count") == 1
