...
def test_run_direct_provider_execution_gate_passes_with_monkeypatched_live_calls(monkeypatch) -> None:
    ...
    def fake_post_job(**kwargs):
        if kwargs["kind"] == "chatgpt_web.ask":
            assert kwargs["client_name"] == mod.DEFAULT_BLOCKED_CLIENT_NAME
            return {"status_code": 403, "body": {"detail": {"error": "direct_live_chatgpt_ask_blocked", "x_client_name": mod.DEFAULT_BLOCKED_CLIENT_NAME}}}
        assert kwargs["client_name"] == mod.DEFAULT_MAINT_CLIENT_NAME
