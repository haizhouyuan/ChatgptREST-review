...
def test_quality_guard_allows_compact_planning_three_bullet_answer_under_100_chars() -> None:
    question = (
        "## Output Requirements\n"
        "planning_memo\n\n"
        "要求：1）直接输出三条无序列表；2）每条一句。\n"
    )
    answer = (
        "- 确认供应链恢复窗口，并确保各方协同推进。\n"
        "- 与供应商、销售和交付团队进行对接，确保本周进展。\n"
        "- 在恢复窗口确认前，避免承诺具体的导入日期。\n"
    )
    assert len(answer.strip()) < 100
    quality = classify_answer_quality(answer, question_text=question)
    assert quality == "final", f"Expected final for compact planning answer, got {quality}"


def test_quality_guard_keeps_sub_100_bullets_suspect_without_compact_planning_contract() -> None:
    answer = (
        "- 先同步。\n"
        "- 再确认。\n"
        "- 最后推进。\n"
    ...
    quality = classify_answer_quality(answer)
    assert quality == "suspect_short_answer", (
        f"Expected suspect_short_answer without compact planning contract, got {quality}"
    )


def test_quality_guard_allows_generic_compact_multisentence_answer() -> None:
    question = "只基于附件，用中文用 4 条以内要点回答：当前最明确的结论是什么？下一步最该补的验证是什么？"
    answer = (
        "当前最明确的结论是 Graphiti 仍是当前已评估方案中的领先路线。"
        "下一步最该补的是 clean-room 完整重跑与 provenance audit，这两项是现阶段最关键的验证。"
    )
    quality = classify_answer_quality(answer, question_text=question)
    assert quality == "final", f"Expected final for generic compact answer, got {quality}"
...
def test_quality_guard_blocks_review_shallow_verdict_without_required_path_citations() -> None:
    question = (
    ...
    quality = classify_answer_quality(answer, question_text=question)
    assert quality == "suspect_review_shallow_verdict", (
        f"Expected suspect_review_shallow_verdict, got {quality}"
    )


def test_quality_guard_allows_review_with_concrete_file_paths_when_prompt_requires_them() -> None:
    question = (
        "Please formally review this patch.\n"
        "- Findings first.\n"
        "- For each finding, cite the problematic path.\n"
        "- Required reading includes worker.py and conversation_exports.py.\n"
    )
    answer = (
        "## Findings\n\n"
        "### High\n"
        "**Path**: /vol1/1000/projects/ChatgptREST/chatgptrest/worker/worker.py\n"
        "- `_run_once()` still only regenerates on short/meta/context-failure cases and misses long shallow review verdicts.\n"
        "\n"
        "### Medium\n"
        "**Path**: /vol1/1000/projects/ChatgptREST/chatgptrest/core/conversation_exports.py\n"
        "- `classify_answer_quality()` lacks a review-specific branch when the prompt explicitly requires path citations.\n"
    )
    quality = classify_answer_quality(answer, question_text=question)
    assert quality == "final", f"Expected final, got {quality}"
