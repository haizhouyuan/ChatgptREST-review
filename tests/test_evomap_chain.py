...
from chatgptrest.evomap.knowledge.chain_builder import (
    BackfillStats,
    CanonicalQuestionBackfillStats,
    ChainStats,
    backfill_canonical_question,
    build_chains,
    backfill_valid_from,
    derive_canonical_question,
    normalize_question,
    run_p1_migration,
)
...
class TestCanonicalQuestionBackfill:

    def test_derive_canonical_question_from_specific_question(self):
        derived, source = derive_canonical_question("How should we prepare for the visit?")
        assert derived == "How should we prepare for the visit?"
        assert source == "question"

    def test_derive_canonical_question_skips_generic_heading(self):
        derived, source = derive_canonical_question("结论")
        assert derived == ""
        assert source == "generic_heading"

    def test_derive_canonical_question_normalizes_activity_event_question(self):
        derived, source = derive_canonical_question("What workflow.completed event occurred?")
        assert derived == "activity: workflow.completed"
        assert source == "activity_pattern"

    def test_backfill_canonical_question_updates_missing_rows_only(self, db):
        _make_atom(db, "at_plain", canonical_question="", valid_from=1000.0)
        _make_atom(db, "at_event", canonical_question="", valid_from=1000.0)
        db.put_atom(
            Atom(
                atom_id="at_generic",
                episode_id="ep_1",
        ...
        db.put_atom(
            Atom(
                ...
                question="ignored",
                answer="summary",
                canonical_question="Already set",
                valid_from=1000.0,
                status="scored",
            )
        )
        conn = db.connect()
        conn.execute("UPDATE atoms SET question = ? WHERE atom_id = ?", ("What workflow.completed event occurred?", "at_event"))
        db.commit()

        stats = backfill_canonical_question(db)

        assert stats.from_question == 0
        assert stats.from_activity_pattern == 1
        assert stats.skipped_generic == 1
        assert stats.already_set == 1
        assert stats.skipped_blank == 1
        assert db.get_atom("at_plain").canonical_question == ""
        assert db.get_atom("at_event").canonical_question == "activity: workflow.completed"
        assert db.get_atom("at_generic").canonical_question == ""
        assert db.get_atom("at_existing").canonical_question == "Already set"
...
class TestChainConstruction:
    ...
    def test_metadata_only_chain_build_preserves_existing_promotion_status(self, db):
        _make_atom(db, "at_old", canonical_question="How to deploy?", valid_from=1000.0)
        _make_atom(db, "at_new", canonical_question="How to deploy?", valid_from=2000.0)
        conn = db.connect()
        conn.execute(
            "UPDATE atoms SET promotion_status='active', promotion_reason='manual_active' WHERE atom_id='at_new'"
        )
        conn.execute(
            "UPDATE atoms SET promotion_status='candidate', promotion_reason='manual_candidate' WHERE atom_id='at_old'"
        )
        db.commit()

        stats = build_chains(db, apply_promotion_semantics=False)

        old = db.get_atom("at_old")
        new = db.get_atom("at_new")
        assert old.chain_id == new.chain_id
        assert old.chain_rank == 1
        assert new.chain_rank == 2
        assert new.is_chain_head == 1
        assert old.superseded_by == "at_new"
        assert old.promotion_status == "candidate"
        assert new.promotion_status == "active"
        assert old.promotion_reason == "manual_candidate"
        assert new.promotion_reason == "manual_active"
        assert stats.multi_atom_chains == 1
...
class TestEmptyCanonical:
    ...
    def test_metadata_only_chain_build_does_not_rewrite_blank_canonical_atoms(self, db):
        _make_atom(db, "at_no_cq", canonical_question="", valid_from=1000.0)
        conn = db.connect()
        conn.execute(
            "UPDATE atoms SET promotion_status='active', promotion_reason='keep_me' WHERE atom_id='at_no_cq'"
        )
        db.commit()

        build_chains(db, apply_promotion_semantics=False)
        atom = db.get_atom("at_no_cq")
        assert atom.promotion_status == "active"
        assert atom.promotion_reason == "keep_me"
        assert atom.chain_id == ""
