...
def test_execution_result_serialization_accepts_list_payload():
    """Structured result may be a JSON array, not only a dict."""
    result = OpenCLIExecutionResult(
        ok=True,
        executor_kind="opencli",
        command_id="test.cmd",
        ...
        error_type="",
        error_message="",
        structured_result=[{"title": "item"}],
        artifacts=["/tmp/test.json"],
    )
    data = result.to_dict()
    assert isinstance(data["structured_result"], list)
    assert data["structured_result"][0]["title"] == "item"

    restored = OpenCLIExecutionResult.from_dict(data)
    assert isinstance(restored.structured_result, list)
    assert restored.structured_result[0]["title"] == "item"
