...
def test_compact_repair_evidence_for_prompt_truncates_large_sections() -> None:
    compact = repair_mod._compact_repair_evidence_for_prompt(  # noqa: SLF001
        {
            "events_tail": "e" * 50_000,
            "target_job": {"last_error": "boom " * 1_000},
            "nested": {"payload": "x" * 5_000},
        },
        string_limit=120,
        events_tail_limit=240,
    )
    assert isinstance(compact, dict)
    assert len(str(compact["events_tail"])) < 400
    assert len(str(compact["target_job"]["last_error"])) < 400
    assert len(str(compact["nested"]["payload"])) < 300


def test_build_codex_autofix_prompt_caps_large_evidence_payload() -> None:
    prompt = repair_mod._build_codex_autofix_prompt(  # noqa: SLF001
        agents_md="",
        playbook_md="playbook policy",
        repo_memory_text="",
        bootstrap_memory_text="",
        evidence=repair_mod._compact_repair_evidence_for_prompt(  # noqa: SLF001
            {"events_tail": "z" * 100_000, "blob": "q" * 100_000},
            string_limit=1_000,
            events_tail_limit=2_000,
        ),
    )
    assert "=== AGENTS.md (truncated) ===" not in prompt
