...
def test_gemini_expand_more_tools_submenu_supports_more_variants() -> None:
    src = inspect.getsource(server._gemini_expand_more_tools_submenu)
    assert "div.cdk-overlay-pane:visible [role='menuitem']" in src
    assert "div.cdk-overlay-container:visible [role='menuitem']" in src
    assert "button.toolbox-drawer-item-list-button" in src
