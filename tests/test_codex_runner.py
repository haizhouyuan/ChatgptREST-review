...
def test_codex_exec_with_schema_builds_cmd_and_parses_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(cmd, *, input, text, check, capture_output, timeout, env):  # noqa: ANN001,A002,ARG001
...
def test_codex_resume_last_message_json_builds_cmd_and_parses_output(
    ...
    def fake_run(cmd, *, input, text, check, capture_output, timeout, cwd, env):  # noqa: ANN001,A002,ARG001
        captured["cmd"] = list(cmd)
        captured["cwd"] = cwd
...
def test_codex_exec_with_schema_scrubs_openai_env_by_default(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    schema_path = tmp_path / "schema.json"
    schema_path.write_text("{}", encoding="utf-8")
    out_json = tmp_path / "out.json"
    captured: dict[str, object] = {}
    ...
    monkeypatch.delenv("CHATGPTREST_CODEX_RUNNER_PRESERVE_OPENAI_ENV", raising=False)
...
def test_codex_exec_with_schema_can_preserve_openai_env_when_enabled(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    schema_path = tmp_path / "schema.json"
    schema_path.write_text("{}", encoding="utf-8")
    out_json = tmp_path / "out.json"
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(subprocess, "run", fake_run)
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setenv("CHATGPTREST_CODEX_RUNNER_PRESERVE_OPENAI_ENV", "1")
    ...
    assert env.get("CHATGPTREST_FOO") == "bar"
