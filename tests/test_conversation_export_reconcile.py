...
from chatgptrest.core.conversation_exports import render_conversation_export_markdown
from chatgptrest.core.conversation_exports import extract_answer_from_conversation_export_obj
...
def test_extract_answer_allows_benign_continuation_nudge_after_match(tmp_path: Path) -> None:
    q = "Please execute the attached Stage 2.5 gate Pro review request."
    ...
    _write_export(
        ...
        [
            {"role": "user", "text": q},
            {"role": "assistant", "text": "我会先检查附件。"},
            {"role": "user", "text": "我看你卡住了，好久没进度，一直是reading documents，请继续"},
            {"role": "assistant", "text": final, "status": "finished_successfully"},
        ],
    )
    payload = json.loads(export_path.read_text(encoding="utf-8"))

    ans, info = extract_answer_from_conversation_export_obj(
        obj=payload,
        question=q,
        allow_fallback_last_assistant=False,
    )

    assert ans == final.strip()
    assert info["subsequent_user_turn_count"] == 1
    assert info["continuation_nudge_count"] == 1
    assert info["thread_contaminated_after_match"] is False
    assert info["selection_strategy"] == "longest_final_quality"


def test_extract_answer_keeps_substantive_second_user_turn_contaminated(tmp_path: Path) -> None:
    q = "Please execute the attached Stage 2.5 gate Pro review request."
    final = "## verdict\n\nPASS\n\n" + ("具体发现。\n" * 120)
    export_path = tmp_path / "conversation.json"
    _write_export(
        ...
        [
            {"role": "user", "text": q},
            {"role": "assistant", "text": "我会先检查附件。"},
            {"role": "user", "text": "请改成 Deep Research，并顺便评估另一个项目。"},
            {"role": "assistant", "text": final, "status": "finished_successfully"},
        ],
    )
    payload = json.loads(export_path.read_text(encoding="utf-8"))
    ...
    assert ans == "我会先检查附件。"
    assert info["subsequent_user_turn_count"] == 1
    assert info["continuation_nudge_count"] == 0
    assert info["thread_contaminated_after_match"] is True
...
def test_extract_answer_uses_deep_research_widget_export_when_mapping_only_has_stub(tmp_path: Path) -> None:
    q = "请调研长期工作记忆架构。"
    report = "## Long-Term Memory Architecture\n\n" + ("内容。" * 1200)
    export_path = tmp_path / "conversation.json"
    payload = {
        ...
        "current_node": "a1",
        "deep_research_widget_export": {"markdown": report, "meta": {"source_urls_count": 12}},
    }
...
def test_render_markdown_includes_deep_research_widget_export(tmp_path: Path) -> None:
    q = "请调研长期工作记忆架构。"
    report = "## 推荐架构\n\n- 事件层\n- 状态层\n"
    export_path = tmp_path / "conversation.json"
    payload = {
        ...
        "current_node": "u1",
        "deep_research_widget_export": {"markdown": report},
    }
    export_path.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    ans = _extract_answer_from_conversation_export(export_path=export_path, question=q, deep_research=True)
    assert ans == report.strip()
    markdown = render_conversation_export_markdown(export_obj=payload, conversation_id="conv-dr-2")
    assert "## assistant" in markdown
    assert "## 推荐架构" in markdown
