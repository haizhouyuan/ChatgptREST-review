...
def test_runtime_wires_activity_ingest_for_live_telemetry(monkeypatch, tmp_path) -> None:
    ...
    try:
        ...
        episodic = runtime.memory.get_episodic(category="execution_feedback", project_id="prs", limit=10)
        assert len(episodic) == 1
        assert episodic[0].source["project_id"] == "prs"
...
def test_runtime_logs_missing_feishu_client_module_as_info(caplog, monkeypatch) -> None:
    monkeypatch.delenv("QWEN_API_KEY", raising=False)
    ...
    assert runtime.feishu is not None
    assert any("FeishuApiClient module unavailable" in record.message for record in caplog.records)
    assert not any(record.levelname == "WARNING" and "FeishuApiClient unavailable" in record.message for record in caplog.records)
