...
def env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("CHATGPTREST_EXTRA_ALLOWED_FILE_ROOTS", str(tmp_path))
...
def _complete_web_ask_with_answer(
    *,
    client: TestClient,
    env: dict[str, Path],
    idempotency_key: str,
    ...
    created = client.post(
        "/v1/jobs",
        json={"kind": "chatgpt_web.ask", "input": {"question": "research please"}, "params": {"preset": "auto"}},
...
def test_store_answer_result_writes_completion_contract_and_event(
    env: dict[str, Path],
) -> None:
    app = create_app()
    client = TestClient(app)
    r = client.post(
        "/v1/jobs",
        json={"kind": "chatgpt_web.ask", "input": {"question": "hello"}, "params": {"preset": "auto", "min_chars": 800}},
        headers={"Idempotency-Key": "completion-contract-result"},
    )
    assert r.status_code == 200
    job_id = r.json()["job_id"]
    ...
    assert contract["authoritative_answer_path"] == payload["path"]
    assert contract["min_chars_required"] == 800
    assert canonical["ready"] is True
    assert canonical["answer_state"] == "final"
    assert canonical["authoritative_answer_path"] == payload["path"]

    with connect(env["db_path"]) as conn:
        row = conn.execute(
            "SELECT payload_json FROM job_events WHERE job_id = ? AND type = ? ORDER BY id DESC LIMIT 1",
            (job_id, "completion_contract_recorded"),
        ).fetchone()
        canonical_row = conn.execute(
            "SELECT payload_json FROM job_events WHERE job_id = ? AND type = ? ORDER BY id DESC LIMIT 1",
            (job_id, "canonical_answer_recorded"),
        ).fetchone()
    assert row is not None
    event_payload = json.loads(str(row["payload_json"] or "{}"))
    assert event_payload["answer_state"] == "final"
    assert event_payload["authoritative_answer_path"] == payload["path"]
    assert canonical_row is not None
    canonical_event = json.loads(str(canonical_row["payload_json"] or "{}"))
    assert canonical_event["ready"] is True
    assert canonical_event["authoritative_answer_path"] == payload["path"]


def test_completed_suspect_short_answer_stays_provisional(env: dict[str, Path]) -> None:
    app = create_app()
    client = TestClient(app)
    job_id = _complete_web_ask_with_answer(
        client=client,
        env=env,
        idempotency_key="completion-quality-short-provisional",
        answer="ok",
    )
    ...
    payload = result.json()
    assert payload["completion_quality"] == "suspect_short_answer"
    assert payload["completion_contract"]["answer_state"] == "provisional"
    assert payload["completion_contract"]["finality_reason"] == "suspect_short_answer"
    assert payload["canonical_answer"]["ready"] is False
    assert payload["canonical_answer"]["answer_state"] == "provisional"


def test_completed_suspect_short_answer_answer_route_returns_409(env: dict[str, Path]) -> None:
    app = create_app()
    client = TestClient(app)
    job_id = _complete_web_ask_with_answer(
        client=client,
        env=env,
        idempotency_key="completion-quality-short-answer-gate",
        answer="ok",
    )

    ans = client.get(f"/v1/jobs/{job_id}/answer?offset=0&max_chars=200")
    assert ans.status_code == 409
    detail = ans.json()["detail"]
    assert detail["status"] == "completed"
    assert detail["answer_state"] == "provisional"
    assert detail["canonical_ready"] is False
    assert detail["action_hint"] == "review_completed_answer"


def test_legacy_completion_contract_marks_nonfinal_completion_quality_provisional() -> None:
    contract = completion_contract_from_job_like(
        {
            "job_id": "legacy-short-1",
            "status": "completed",
            "completion_quality": "suspect_short_answer",
            "answer_path": "jobs/legacy-short-1/answer.md",
            "answer_chars": 2,
        }
    )
    assert contract["answer_state"] == "provisional"
    assert contract["finality_reason"] == "suspect_short_answer"
