...
def test_mcp_job_wait_background_runner_delivers_push_hook(monkeypatch: pytest.MonkeyPatch):
    mod = _load_mcp_server_module()
    deliveries: list[dict[str, object]] = []

    async def fake_wait(**_kwargs):  # noqa: ANN003
        return {"ok": True, "job_id": "jpush1", "status": "completed", "path": "/tmp/answer.md"}

    async def fake_push(*, state, job=None):  # noqa: ANN001
        deliveries.append({"state": dict(state), "job": dict(job or {})})
        return {"ok": True, "delivered": True, "channel": "hook"}

    monkeypatch.setattr(mod, "_chatgptrest_job_wait_impl", fake_wait)
    monkeypatch.setattr(mod, "_maybe_deliver_background_wait_push", fake_push)
    monkeypatch.setattr(mod, "_tmux_notify", lambda _msg: None)

    async def scenario() -> None:
        started = await mod._background_wait_start(  # noqa: SLF001
            job_id="jpush1",
            cfg=BackgroundWaitConfig(
                ...
                notify_controller=False,
                notify_done=False,
                push_context={"origin": "hermes-workbench", "task_id": "tsk-push-001"},
            ),
        )
        assert started["ok"] is True
        ...
        assert final["watch_status"] == "completed"
        assert final["push_delivery"]["ok"] is True

    asyncio.run(scenario())
    assert deliveries
    assert deliveries[0]["state"]["push_context"]["task_id"] == "tsk-push-001"
    assert deliveries[0]["job"]["status"] == "completed"
