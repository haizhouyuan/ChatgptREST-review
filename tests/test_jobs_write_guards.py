...
def test_registered_generate_image_client_bypasses_global_allowlist(
    env: dict[str, Path],  # noqa: ARG001
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("CHATGPTREST_ENFORCE_CLIENT_NAME_ALLOWLIST", "chatgptrest-mcp")
    monkeypatch.setenv("CHATGPTREST_REQUIRE_TRACE_HEADERS_FOR_WRITE", "1")
    monkeypatch.setenv("CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT", "internal-secret")
    app = create_app()
    client = TestClient(app)

    body = {
        "kind": "gemini_web.generate_image",
        "input": {"prompt": "生成一张极简蓝白技术海报"},
    ...
    allowed = client.post(
        ...
        headers={
            **_signed_headers(
                client_lookup="chatgptrest_gemini_generate_image_submit",
                client_instance="gemini-generate-1",
                body_payload=body,
                environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-secret"},
            ),
            "Idempotency-Key": "wg-gemini-generate-1",
        },
    )
