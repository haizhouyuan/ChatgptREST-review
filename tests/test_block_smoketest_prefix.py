...
def test_allows_direct_live_chatgpt_ask_for_allowlisted_client(env: dict[str, Path]):
    app = create_app()
    client = TestClient(app)
    r = client.post(
        "/v1/jobs",
        ...
        headers={
            "Idempotency-Key": "direct-live-chatgpt-allowed",
            "X-Client-Name": "chatgptrest-admin-mcp",
        },
    )
    assert r.status_code == 200


def test_allows_direct_live_chatgpt_ask_when_explicitly_overridden(env: dict[str, Path]):
    app = create_app()
    client = TestClient(app)
    r = client.post(
        "/v1/jobs",
