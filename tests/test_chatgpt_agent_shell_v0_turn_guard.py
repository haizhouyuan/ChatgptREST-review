...
def test_needs_followup_then_completed_consumes_next_turn_and_succeeds(
    ...
    wait_sequence = [
        ...
        {
            "status": "completed",
            "conversation_url": "https://chatgpt.com/c/followup-ok",
            "completion_contract": {
                "answer_state": "final",
                "authoritative_answer_path": "jobs/job-2/answer.md",
            },
        },
    ]
...
def test_wait_job_requires_research_finality_before_returning_completed(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    mod = _load_module()
    responses = [
        (200, {"job_id": "job-1", "status": "completed", "completion_contract": {"answer_state": "provisional"}}),
        (
            200,
            {
                "job_id": "job-1",
    ...
    def fake_http_request(self, **kwargs):  # noqa: ANN001
    ...
    agent = mod.ChatGPTAgentV0(
        ...
        api_token="",
        state_root=tmp_path,
        session_id="test-v0-finality-wait",
        dry_run=False,
        auto_rollback=False,
    )
    agent.state.max_wait_seconds = 5.0

    waited = agent._wait_job("job-1")
    assert waited["status"] == "completed"
    assert waited["completion_contract"]["answer_state"] == "final"
    assert responses == []
