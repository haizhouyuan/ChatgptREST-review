...
def test_auto_blocked_pause_still_defers_chatgpt_but_not_gemini(env: dict[str, Path]):
    app = create_app()
    client = TestClient(app)

    paused = client.post(
        "/v1/ops/pause",
        json={"mode": "send", "duration_seconds": 60, "reason": "auto_blocked:cloudflare"},
    )
    assert paused.status_code == 200
    assert paused.json()["mode"] == "send"
    assert paused.json()["active"] is True
    ...
    chatgpt = client.post(
        ...
        headers={"Idempotency-Key": "ops-auto-blocked-chatgpt"},
    )
    assert chatgpt.status_code == 200
    chatgpt_job = chatgpt.json()
    assert chatgpt_job["retry_after_seconds"] is not None
    assert chatgpt_job["retry_after_seconds"] > 0

    chatgpt_events = client.get(f"/v1/jobs/{chatgpt_job['job_id']}/events?after_id=0&limit=50")
    assert chatgpt_events.status_code == 200
    assert "job_deferred_by_pause" in [e["type"] for e in chatgpt_events.json()["events"]]
    ...
    gemini = client.post(
        ...
        headers={"Idempotency-Key": "ops-auto-blocked-gemini"},
    )
    assert gemini.status_code == 200
    gemini_job = gemini.json()
    assert gemini_job["retry_after_seconds"] is None

    gemini_events = client.get(f"/v1/jobs/{gemini_job['job_id']}/events?after_id=0&limit=50")
    assert gemini_events.status_code == 200
    assert "job_deferred_by_pause" not in [e["type"] for e in gemini_events.json()["events"]]
...
def test_ops_retry_budget_surfaces_hot_provider_and_recent_counts(env: dict[str, Path], monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("CHATGPTREST_WEB_RETRY_BUDGET_MAX_PER_PROVIDER", "5")
    monkeypatch.setenv("CHATGPTREST_WEB_RETRY_BUDGET_WARN_RATIO", "0.8")
    with connect(env["db_path"]) as conn:
        conn.execute("BEGIN IMMEDIATE")
        for idx in range(4):
            conn.execute(
    ...
    try:
        ...
        assert status.status_code == 200
        body = status.json()
        assert body["retry_budget_window_seconds"] == 900
        assert body["retry_budget_recent_by_provider"]["chatgpt"] == 4
        assert body["retry_budget_hot_providers"] == ["chatgpt"]
        assert body["retry_budget_exceeded_events"] == 1
        assert "retry_budget_hot" in body["attention_reasons"]
        assert "retry_budget_exceeded" in body["attention_reasons"]

        retry_budget = client.get("/v1/ops/retry-budget")
        assert retry_budget.status_code == 200
        rb = retry_budget.json()
        assert rb["window_seconds"] == 900
        assert rb["recent_budget_exceeded_events"] == 1
        assert rb["hot_providers"] == ["chatgpt"]
        provider = next(item for item in rb["providers"] if item["provider_id"] == "chatgpt")
        assert provider["recent_retry_count"] == 4
        assert provider["recent_budget_exceeded_count"] == 1
        assert provider["hot"] is True
        assert provider["exceeded"] is False
    finally:
        monkeypatch.setattr(time, "time", real_time_time)
