...
class TestDispatch:
    ...
    def test_dispatch_skill_gap_records_capability_gap(self, tmp_path, monkeypatch):
        ...
        ctx = ContextPackage(
            ...
            project_card={
                "title": "调研中国两轮电动车市场品牌",
                "description": "调研中国两轮电动车市场品牌",
            },
        )
        result = dispatcher.dispatch(ctx, target_agent="main")

        assert result["status"] == "skill_gap"
        assert result["capability_gap_ids"]
        gaps = market_gate.get_capability_gap_recorder().fetch_gaps(status="open")
        assert gaps[0].capability_id == "market_research"

    def test_dispatch_skill_gap_surfaces_market_candidates(self, tmp_path, monkeypatch):
        monkeypatch.setenv("OPENMIND_SKILL_PLATFORM_DB", str(tmp_path / "skill_platform.db"))
        ...
        recorder.register_market_candidate(
            ...
            summary="candidate for dispatch fallback",
        )
        dispatcher = AgentDispatcher()
        ctx = ContextPackage(
            trace_id="tr_gap_market",
            project_card={
                "title": "调研中国两轮电动车市场品牌",
                "description": "调研中国两轮电动车市场品牌",
            },
