...
def test_work_memory_manager_retrieves_cross_session_active_context_by_account_role() -> None:
    ...
    assert metadata["scope_hits"]["active_project"] == "account_role_project"
    assert metadata["scope_hits"]["handoff"] == "account_project"
...
def test_work_memory_manager_missing_thread_degrades_but_keeps_account_scope() -> None:
    ...
    assert metadata["scope_hits"]["decision_ledger"] == "account_role_project"
...
def test_work_memory_manager_project_scope_filters_active_context() -> None:
    memory = MemoryManager(":memory:")
    manager = WorkMemoryManager(memory)
    ...
    for project_id, name in (("prj-alpha", "Alpha Project"), ("prj-beta", "Beta Project")):
        manager.write_from_capture(
            category="active_project",
            title=name,
            content=f"{name} remains active.",
            summary=f"{name} remains active.",
            payload={
