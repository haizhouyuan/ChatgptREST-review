...
class TestCooldownState:
    ...
    def test_send_phase_blocked_cooldown_projects_to_same_session_repair(self, monkeypatch):
        """验证 send-phase verification cooldown 会投影成 needs_followup."""
        monkeypatch.setenv("OPENMIND_API_KEY", "test-key")
        monkeypatch.setenv("OPENMIND_AUTH_MODE", "strict")

        projected = _project_job_snapshot_for_public_agent(
            {
                "job_id": "job-cooldown-1",
                "job_status": JobStatus.COOLDOWN.value,
                "phase": "send",
                "conversation_url": "",
        ...
        assert projected["agent_status"] == "needs_followup"
        assert projected["next_action"]["type"] == "same_session_repair"
        assert projected["next_action"]["retry_after_seconds"] == 120
        assert projected["next_action"]["error_type"] == "Blocked"
