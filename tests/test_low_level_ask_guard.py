...
def test_apply_low_level_ask_guard_limits_fails_for_gemini_when_non_pro_fallback_unavailable() -> None:
    with pytest.raises(HTTPException) as exc_info:
        ask_guard.apply_low_level_ask_guard_limits(
            kind="gemini_web.ask",
            params_obj={"preset": "pro"},
            guard_payload={
                "resolved_client_id": "limits-bot",
                "decision": {
                    "decision": "allow_with_limits",
                    "allow_pro": False,
                },
    ...
    detail = exc.detail
    assert detail["error"] == "low_level_ask_limit_unenforceable"
    assert detail["reason"] == "allow_with_limits_requires_unavailable_non_pro_preset"
    assert detail["provider_capability_profile"]["provider_id"] == "gemini"
    assert detail["provider_capability_profile"]["safe_non_pro_fallback"] is None


def test_apply_low_level_ask_guard_limits_leaves_non_pro_chatgpt_preset_unchanged() -> None:
    params, applied = ask_guard.apply_low_level_ask_guard_limits(
        kind="chatgpt_web.ask",
        params_obj={"preset": "auto"},
        guard_payload={
            "decision": {
                "decision": "allow_with_limits",
                "allow_pro": False,
...
def test_internal_submit_wrappers_runtime_dedupe_blocks_recent_duplicate(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT", "internal-submit-secret")
    upload_root = Path("/tmp/chatgptrest_uploads/test_internal_submit_dedupe")
    upload_root.mkdir(parents=True, exist_ok=True)
    packet_path = upload_root / f"{tmp_path.name}_example_packet.zip"
    packet_path.write_bytes(b"placeholder packet")
    app = create_app()
    client = TestClient(app)
    body = {
        "kind": "chatgpt_web.ask",
        "input": {
            "question": "Please review this attached design packet and list the highest priority issues.",
            "file_paths": [str(packet_path)],
        },
        "params": {"preset": "pro_extended"},
    }
    headers1 = _signed_headers(
        client_lookup="chatgptrest_chatgpt_ask_submit",
        client_instance="internal-submit-dedupe-1",
        method="POST",
        path="/v1/jobs",
        body_payload=body,
        environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-submit-secret"},
        nonce="internal-submit-dedupe-nonce-1",
    )
    headers2 = _signed_headers(
        client_lookup="chatgptrest_chatgpt_ask_submit",
        client_instance="internal-submit-dedupe-1",
        ...
        body_payload=body,
        environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-submit-secret"},
        nonce="internal-submit-dedupe-nonce-2",
    )

    first = client.post("/v1/jobs", json=body, headers={**headers1, "Idempotency-Key": "internal-submit-dedupe-1"})
    assert first.status_code == 200

    second = client.post("/v1/jobs", json=body, headers={**headers2, "Idempotency-Key": "internal-submit-dedupe-2"})
    assert second.status_code == 409
    detail = second.json()["detail"]
    assert detail["error"] == "low_level_ask_duplicate_recently_submitted"
    assert detail["reason"] == "duplicate_recent_low_level_ask"
    assert detail["client_id"] == "internal-submit-wrappers"


def test_internal_submit_wrappers_runtime_dedupe_uses_attachment_content(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT", "internal-submit-secret")
    upload_root = Path("/tmp/chatgptrest_uploads/test_internal_submit_content_dedupe")
    first_dir = upload_root / f"{tmp_path.name}_a"
    second_dir = upload_root / f"{tmp_path.name}_b"
    first_dir.mkdir(parents=True, exist_ok=True)
    second_dir.mkdir(parents=True, exist_ok=True)
    first_packet = first_dir / "review_packet.zip"
    second_packet = second_dir / "review_packet.zip"
    first_packet.write_bytes(b"same packet bytes")
    second_packet.write_bytes(b"same packet bytes")
    app = create_app()
    client = TestClient(app)
    ...
    body1 = _body(first_packet)
    body2 = _body(second_packet)
    headers1 = _signed_headers(
        client_lookup="chatgptrest_chatgpt_ask_submit",
        client_instance="internal-submit-content-dedupe-1",
        method="POST",
        path="/v1/jobs",
        body_payload=body1,
        environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-submit-secret"},
        nonce="internal-submit-content-dedupe-nonce-1",
    )
    headers2 = _signed_headers(
        client_lookup="chatgptrest_chatgpt_ask_submit",
        client_instance="internal-submit-content-dedupe-1",
        ...
        body_payload=body2,
        environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-submit-secret"},
        nonce="internal-submit-content-dedupe-nonce-2",
    )

    first = client.post("/v1/jobs", json=body1, headers={**headers1, "Idempotency-Key": "internal-content-dedupe-1"})
    assert first.status_code == 200

    second = client.post("/v1/jobs", json=body2, headers={**headers2, "Idempotency-Key": "internal-content-dedupe-2"})
    assert second.status_code == 409
    detail = second.json()["detail"]
    assert detail["error"] == "low_level_ask_duplicate_recently_submitted"
...
def test_internal_submit_wrappers_chatgpt_kind_single_flight_blocks_second_job(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT", "internal-submit-secret")
    ...
    def _post(idx: int, question: str) -> Any:
        ...
        headers = _signed_headers(
            client_lookup="chatgptrest_chatgpt_ask_submit",
            client_instance="internal-submit-kind-single-flight-1",
            method="POST",
            path="/v1/jobs",
            body_payload=body,
            environ={"CHATGPTREST_ASK_HMAC_SECRET_INTERNAL_SUBMIT": "internal-submit-secret"},
            nonce=f"internal-submit-kind-single-flight-nonce-{idx}",
        )
        return client.post("/v1/jobs", json=body, headers={**headers, "Idempotency-Key": f"internal-kind-single-flight-{idx}"})

    first = _post(1, "Please review proposal A and list the highest priority risks.")
    assert first.status_code == 200

    second = _post(2, "Please review proposal B and list the highest priority risks.")
    assert second.status_code == 429
    detail = second.json()["detail"]
    assert detail["error"] == "low_level_ask_client_kind_concurrency_exceeded"
    assert detail["kind"] == "chatgpt_web.ask"
    assert detail["active_job_ids"] == [first.json()["job_id"]]
...
def test_runtime_max_in_flight_jobs_blocks_third_request(
    ...
    assert detail["safe_next_action"] == "wait_for_active_job_or_cancel_confirmed_stale_job_with_reason"
    assert detail["retry_after_seconds"] >= 1
    assert detail["active_jobs"][0]["job_id"] == first.json()["job_id"]
    assert detail["active_jobs"][1]["job_id"] == second.json()["job_id"]
