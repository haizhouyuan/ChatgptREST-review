...
class TestAtomGroundedness:
    ...
    def test_planning_profile_downweights_code_symbol_when_path_is_grounded(self):
        answer = (
            "Use /vol1/1000/projects/ChatgptREST/docs/runbook.md as the runtime anchor "
            "and compare FooBarBazPlanningSymbol before the visit."
        )
        planning = check_atom_groundedness(
            "at_plan",
            answer,
            32503680000.0,
            weight_profile="planning",
        )
        standard = check_atom_groundedness(
            "at_code",
            answer,
            32503680000.0,
            weight_profile="code_procedure",
        )
        assert planning.weight_profile == "planning"
        assert standard.weight_profile == "code_procedure"
        assert planning.overall > standard.overall
        assert planning.overall >= 0.7
        assert standard.overall < 0.7

    def test_enforce_promotion_gate_infers_planning_profile_from_document_source(self, db):
        db.put_document(
            Document(
                doc_id="doc_plan",
                source="planning",
                project="planning",
                raw_ref="/tmp/doc_plan.md",
                title="Plan",
            )
        )
        db.put_episode(Episode(episode_id="ep_plan", doc_id="doc_plan"))
        db.put_atom(
            Atom(
                atom_id="at_plan",
                episode_id="ep_plan",
                ...
                valid_from=32503680000.0,
            )
        )
        db.commit()

        passed, record = enforce_promotion_gate(db, "at_plan", threshold=0.7)
