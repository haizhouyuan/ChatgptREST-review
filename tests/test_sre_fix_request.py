...
def test_sre_fix_request_creates_open_pr_followup(env: dict[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert report["codex"]["model"] == "gpt-5.3-codex-spark"
    assert report["codex"]["fresh_reasoning_effort"] == "medium"
    ...
    with connect(env["db_path"]) as conn:
        pr_row = conn.execute(
            "SELECT kind, status, params_json FROM jobs WHERE kind = 'repair.open_pr' ORDER BY created_at DESC LIMIT 1"
        ).fetchone()
        assert pr_row is not None
        assert str(pr_row["status"]) == "queued"
        params_obj = json.loads(str(pr_row["params_json"] or "{}"))
        assert "model" not in params_obj
...
def test_sre_fix_request_writes_controller_taskpack_projection(
    ...
    monkeypatch.setattr(
        sre_exec,
        "codex_resume_last_message_json",
        lambda **kwargs: fake_codex_exec_with_schema(**kwargs),  # noqa: ARG005
    )
...
def test_sre_fix_request_records_prompt_stats_and_compacts_prompt_context(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    app = create_app()
    ...
    def fake_codex_exec_with_schema(**kwargs):  # noqa: ANN003
        ...
        return CodexExecResult(
            ...
            output={
                "summary": "Need operator review.",
                "root_cause": "Use the compact prompt context.",
                "route": "manual",
                "confidence": "medium",
                "rationale": "The prompt now carries only compact context.",
            },
        )
    ...
    context_pack = {
        "global_memory_md": "/tmp/global-memory.md",
        "global_memory_excerpt": "G" * 9_000,
        "recent_failures": [{"job_id": f"job-{idx}", "error": "E" * 2_000} for idx in range(6)],
        "system_state": {"driver": "degraded", "chrome": "alive"},
    }
    created = client.post(
        "/v1/jobs",
        ...
        headers=_auth_headers("sre-prompt-stats"),
    )
    assert created.status_code == 200
    fix_job_id = created.json()["job_id"]

    ran = asyncio.run(_run_once(cfg=load_config(), worker_id="sre-prompt-stats", lease_ttl_seconds=60, kind_prefix="sre."))
    assert ran is True

    assert "global_memory_md" not in captured["prompt"]
    assert "<truncated" in captured["prompt"]

    report_path = env["artifacts_dir"] / "jobs" / fix_job_id / "sre_fix_report.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["prompt_stats"]["prompt_chars"] == len(captured["prompt"])
    assert report["prompt_stats"]["estimated_tokens"] > 0
    assert any(section["title"] == "Provided context pack" for section in report["prompt_stats"]["sections"])


def test_sre_fix_request_codex_failure_falls_back_to_manual(
    env: dict[str, Path],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    app = create_app()
    client = TestClient(app)
    target_job_id = _create_completed_dummy_job(client)
    issue_id = _report_issue(env["db_path"], job_id=target_job_id, symptom="runtime signature unclear")
    ...
    def fake_codex_exec_with_schema(**kwargs):  # noqa: ANN003
        return CodexExecResult(
            ...
            error="responses websocket unavailable",
            stderr="responses websocket unavailable",
        )
    ...
    created = client.post(
        ...
        json={
            "kind": "sre.fix_request",
            "input": {"issue_id": issue_id, "job_id": target_job_id, "symptom": "runtime signature unclear"},
            "params": {"resume_lane": False},
        },
        headers=_auth_headers("sre-codex-error-manual"),
    )
    assert created.status_code == 200
    fix_job_id = created.json()["job_id"]

    ran = asyncio.run(_run_once(cfg=load_config(), worker_id="sre-manual-1", lease_ttl_seconds=60, kind_prefix="sre."))
    assert ran is True
    ...
    report_path = env["artifacts_dir"] / "jobs" / fix_job_id / "sre_fix_report.json"
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report["runner_mode"] == "codex_error_manual_fallback"
    assert report["decision"]["route"] == "manual"
    assert report["codex"]["fresh_result"]["ok"] is False
    assert "responses websocket unavailable" in str(report["codex"]["fresh_result"]["error"])
