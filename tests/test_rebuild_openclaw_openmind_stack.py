...
def test_build_config_rewrites_bindings_and_plugins(monkeypatch, tmp_path: Path) -> None:
    ...
    assert cfg["bindings"] == [
        ...
        {"type": "route", "agentId": "feishu-intake", "match": {"channel": "feishu", "accountId": "default"}},
    ]
    ...
    assert telemetry_cfg["defaultModel"] == "MiniMax-M2.7-highspeed"
    ...
    assert set(by_id) == {"main", "feishu-intake", "maintagent", "finbot"}
    ...
    assert by_id["feishu-intake"]["model"] == rebuild.DEFAULT_MINIMAX_MODEL_REF
    assert by_id["feishu-intake"]["tools"]["profile"] == "full"
    assert by_id["feishu-intake"]["tools"]["allow"] == list(rebuild.FEISHU_INTAKE_TOOL_ALLOW)
    assert by_id["feishu-intake"]["tools"]["sandbox"]["tools"]["allow"] == list(rebuild.FEISHU_INTAKE_TOOL_ALLOW)
    assert "alsoAllow" not in by_id["feishu-intake"]["tools"]
    assert "sandbox" not in by_id["feishu-intake"]
    assert "skills" not in by_id["feishu-intake"]
    ...
    assert by_id["maintagent"]["skills"] == ["chatgptrest-call"]
    ...
    assert by_id["finbot"]["skills"] == ["chatgptrest-call"]
    ...
    assert cfg["models"]["providers"]["minimax"]["models"][0]["id"] == "MiniMax-M2.7-highspeed"
    ...
    assert cfg["tools"]["agentToAgent"]["allow"] == ["main", "feishu-intake", "maintagent", "finbot"]
    ...
    assert cfg["session"]["dmScope"] == "per-account-channel-peer"
...
def test_build_config_extends_skill_dirs() -> None:
    ...
    assert cfg["skills"]["allowBundled"] == [
        "general_core",
        "maint_core",
        "market_scan_quarantine",
        "research_core",
    ]


def test_build_config_omits_extra_dirs_when_no_runtime_local_bundle_materialization(monkeypatch) -> None:
    class _Bundle:
    ...
    class _Registry:
        bundles = {
            "general_core": _Bundle("general_core"),
            "maint_core": _Bundle("maint_core"),
            "research_core": _Bundle("research_core"),
            "market_scan_quarantine": _Bundle("market_scan_quarantine"),
        }

        def available_skill_ids_for_bundles(self, bundle_ids, *, platform: str = "", runtime_local_only: bool = False):
            return []

    monkeypatch.setattr(rebuild, "get_canonical_registry", lambda: _Registry())

    cfg = rebuild.build_config(_sample_config())

    assert cfg["skills"]["load"]["extraDirs"] == []
...
def test_build_config_defaults_to_lean_topology() -> None:
    ...
    assert set(by_id) == {"main", "feishu-intake"}
    assert by_id["main"]["tools"]["profile"] == "coding"
...
def test_build_config_injects_openmind_api_key_from_env_file(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "openmind-advisor" not in cfg["plugins"]["entries"]
...
def test_active_agent_specs_switches_main_and_optional_maintagent() -> None:
    ...
    assert [spec.agent_id for spec in lean_specs] == ["main", "feishu-intake"]
    assert lean_specs[0].tool_also_allow == rebuild.MAIN_LEAN_TOOL_ADDITIONS
    assert lean_specs[0].allow_agents == ()
    assert lean_specs[1].tool_profile == "full"
    assert lean_specs[1].tool_allow == rebuild.FEISHU_INTAKE_TOOL_ALLOW
    assert lean_specs[1].tool_also_allow == ()
    assert [spec.agent_id for spec in ops_specs] == ["main", "feishu-intake", "maintagent", "finbot"]
...
def test_install_openmind_plugins_skips_matching_linked_installs(tmp_path: Path, monkeypatch) -> None:
    ...
    _seed_plugin_snapshot(plugin_dir, "openmind-memory")
        ...
        json.dumps(
            {
                "plugins": {
                    "installs": {
                        "openmind-memory": {
                            "source": "path",
                            "sourcePath": str(rebuild.REPO_ROOT / "openclaw_extensions" / "openmind-memory"),
...
def test_install_openmind_plugins_adopts_existing_extension_without_reinstall(tmp_path: Path, monkeypatch) -> None:
    ...
    installs = json.loads((tmp_path / "openclaw.json").read_text(encoding="utf-8"))["plugins"]["installs"]
    record = installs["openmind-memory"]
    assert record["source"] == "path"
    assert record["sourcePath"] == str((rebuild.REPO_ROOT / "openclaw_extensions" / "openmind-memory").resolve())
...
def test_install_openmind_plugins_removes_untracked_stale_extension_before_install(tmp_path: Path, monkeypatch) -> None:
    ...
    plugin_dir.mkdir(parents=True)
    (plugin_dir / "openclaw.plugin.json").write_text(json.dumps({"id": "openmind-memory"}), encoding="utf-8")
    (plugin_dir / "index.ts").write_text("// stale memory plugin\n", encoding="utf-8")
    (tmp_path / "openclaw.json").write_text(json.dumps({"plugins": {"installs": {}}}), encoding="utf-8")
...
def test_ensure_workspaces_writes_managed_role_files(tmp_path: Path, monkeypatch) -> None:
    patched_agents = (
        ...
        rebuild.AgentSpec(
            "feishu-intake",
            str(tmp_path / "feishu-intake"),
            "minimax/MiniMax-M2.7-highspeed",
            tool_profile="minimal",
            tool_also_allow=("openmind_work_material_ops",),
        ),
    ...
    feishu_intake_agents = (tmp_path / "feishu-intake" / "AGENTS.md").read_text(encoding="utf-8")
    feishu_intake_tools = (tmp_path / "feishu-intake" / "TOOLS.md").read_text(encoding="utf-8")
    feishu_intake_role_packs = (tmp_path / "feishu-intake" / "ROLE_PACKS.md").read_text(encoding="utf-8")
    ...
    assert "dedicated work-only intake lane for Feishu" in feishu_intake_agents
    assert "legacy `openmind_work_ingress_turn` ingress policy tool is retired" in feishu_intake_agents
    assert "Use `openmind_work_material_ops` only for exact local material inspection" in feishu_intake_agents
    assert "must not move or rename anything before the work frame and archive structure are confirmed" in feishu_intake_agents
    assert "Never tell the user to use `exec`" in feishu_intake_agents
    assert "/vol1/1000/projects/planning" in feishu_intake_agents
    assert "This workspace is work-only." in feishu_intake_tools
    assert "legacy work-ingress/advisor control plane is retired" in feishu_intake_tools
    assert "openmind_work_material_ops" in feishu_intake_tools
    assert "work-ingress/advisor control plane is retired" in feishu_intake_role_packs
    assert "openmind_work_material_ops" in feishu_intake_role_packs
    assert "do not move or rename files on the first turn of a mixed ask" in feishu_intake_role_packs
    assert "`exec` is intentionally unavailable here" in feishu_intake_role_packs
