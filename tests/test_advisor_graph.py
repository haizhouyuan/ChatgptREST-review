...
def test_execute_quick_ask_uses_context_resolver_with_identity(monkeypatch):
    from chatgptrest.advisor import simple_routes
    import chatgptrest.cognitive.context_service as context_service_mod
    ...
    def _fake_quick_ask(message, trace_id="", kb_search_fn=None):  # noqa: ANN001
        return SimpleNamespace(
            answer="done",
            status="ok",
            to_dict=lambda: {"answer": "done", "status": "ok"},
        )

    class _FakeResolver:
        def __init__(self, runtime):  # noqa: ANN001
            seen["runtime"] = runtime

        def resolve(self, options):  # noqa: ANN001
            seen["session_id"] = options.session_id
            seen["account_id"] = options.account_id
            seen["thread_id"] = options.thread_id
            seen["role_id"] = options.role_id
            return SimpleNamespace(prompt_prefix="## Active Context\nseeded")

    monkeypatch.setattr(simple_routes, "quick_ask", _fake_quick_ask)
    monkeypatch.setattr(context_service_mod, "ContextResolver", _FakeResolver)
    runtime = SimpleNamespace(memory=object(), kb_hub=None, evomap_knowledge_db=None)
    result = execute_quick_ask(
        {
            "normalized_message": "What is the current project state?",
            "kb_has_answer": False,
            "kb_top_chunks": [],
            "trace_id": "trace-1",
            "session_id": "sess-1",
