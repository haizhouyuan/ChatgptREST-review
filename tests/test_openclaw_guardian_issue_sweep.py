...
def test_projection_only_mode_disables_mutation_and_orch_report(guardian) -> None:
    parser = guardian.build_parser()
    args = parser.parse_args(["--projection-only", "--include-orch-report"])
    args.no_autofix = False
    args.no_notify = False
    args.include_orch_report = True

    normalized = guardian._apply_projection_only_mode(args)

    assert normalized.projection_only is True
    assert normalized.no_autofix is True
    assert normalized.no_notify is True
    assert normalized.include_orch_report is False
