...
def test_min_chars_guard_completes_trivial_prompt_short_answer(tmp_path) -> None:
    db_path = tmp_path / "jobdb.sqlite3"
    with connect(db_path) as conn:
        should_complete, details = _min_chars_guard_should_complete_under_min_chars(
            conn=conn,
    ...
    assert should_complete is True
    assert details.get("decision_reason") == "trivial_prompt_short_answer"
...
def test_min_chars_guard_research_contract_blocks_stalled_completion(tmp_path) -> None:
    db_path = tmp_path / "jobdb.sqlite3"
    with connect(db_path) as conn:
        _insert_min_chars_downgrade(conn=conn, job_id="j7", ts=10.0, answer_chars=1000)
        _insert_min_chars_downgrade(conn=conn, job_id="j7", ts=20.0, answer_chars=1000)
        conn.commit()
        should_complete, details = _min_chars_guard_should_complete_under_min_chars(
            conn=conn,
            job_id="j7",
    ...
    assert should_complete is False
    assert details.get("decision_reason") == "stalled"
    assert details.get("research_contract_blocked") is True


def test_min_chars_guard_research_contract_complete_final_requests_followup(tmp_path) -> None:
    db_path = tmp_path / "jobdb.sqlite3"
    with connect(db_path) as conn:
        should_complete, details = _min_chars_guard_should_complete_under_min_chars(
            conn=conn,
            job_id="j-complete-final",
            answer_chars=3776,
    ...
    assert should_complete is False
    assert details.get("decision_reason") == "semantic_final_under_min_chars"
    assert details.get("research_contract_blocked") is True
    assert details.get("terminal_action") == "needs_followup"


def test_min_chars_guard_research_contract_cap_exceeded_requests_followup(tmp_path) -> None:
    db_path = tmp_path / "jobdb.sqlite3"
    with connect(db_path) as conn:
        for i in range(10):
            _insert_min_chars_downgrade(conn=conn, job_id="j8", ts=float(i), answer_chars=1000)
        conn.commit()
        should_complete, details = _min_chars_guard_should_complete_under_min_chars(
            conn=conn,
            job_id="j8",
