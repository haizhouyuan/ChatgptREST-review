...
class _SendTimeoutExceptionRecoveringMcpClient:
    def __init__(self) -> None:
        self.calls: list[tuple[str, dict]] = []
    ...
    def call_tool(self, *, tool_name: str, tool_args: dict, timeout_sec: float) -> dict:  # noqa: ARG002
        ...
        if tool_name == "gemini_web_ask_pro":
            raise RuntimeError(
                "ToolCallError: mcp_http tool gemini_web_ask_pro failed on attempt 1/15: "
                "McpHttpError: SSE stream timeout (deadline exceeded)."
            )
        if tool_name == "gemini_web_idempotency_get":
            assert tool_args.get("tool_name") == "gemini_web_ask_pro"
            assert tool_args.get("include_result") is True
            return {
                "ok": True,
                "status": "completed",
                "record": {
        ...
        if tool_name == "gemini_web_wait":
            ...
            return {
                "ok": True,
                "status": "completed",
                "answer": "timeout recovered",
                "conversation_url": "https://gemini.google.com/app/recovered-after-timeout",
            }
        raise AssertionError(f"unexpected tool: {tool_name}")


class _DeepThinkPendingMcpClient:
    def __init__(self) -> None:
        self.calls: list[tuple[str, dict]] = []
    ...
    def call_tool(self, *, tool_name: str, tool_args: dict, timeout_sec: float) -> dict:  # noqa: ARG002
        ...
        if tool_name == "gemini_web_wait":
            ...
            return {
                "ok": True,
                "status": "completed",
                "answer": "deep think recovered",
                "conversation_url": "https://gemini.google.com/app/recovered-thread-9999",
            }
        raise AssertionError(f"unexpected tool: {tool_name}")


class _PendingReplayMcpClient:
    def __init__(self) -> None:
        self.calls: list[tuple[str, dict]] = []
    ...
    def call_tool(self, *, tool_name: str, tool_args: dict, timeout_sec: float) -> dict:  # noqa: ARG002
        ...
        if tool_name == "gemini_web_wait":
            ...
            return {
                "ok": True,
                "status": "completed",
                "answer": "pending replay recovered",
                "conversation_url": "https://gemini.google.com/app/recovered-thread-pending",
            }
        raise AssertionError(f"unexpected tool: {tool_name}")
...
def test_gemini_send_timeout_exception_recovers_blank_in_progress_and_enters_wait() -> None:
    ex = GeminiWebMcpExecutor(mcp_url="http://127.0.0.1:0/mcp")
    client = _SendTimeoutExceptionRecoveringMcpClient()
    ex._client = client  # type: ignore[assignment]
    res = asyncio.run(
        ex.run(
            job_id="job-gemini-send-exception-idempotency-recover",
            kind="gemini_web.ask",
            input={"question": "请继续分析这个主题。"},
            params={"preset": "pro", "timeout_seconds": 60, "max_wait_seconds": 60},
        )
    ...
    assert isinstance(res.meta, dict)
    assert res.meta.get("conversation_url") == "https://gemini.google.com/app/recovered-after-timeout"


def test_gemini_full_send_with_deep_think_pending_enters_wait() -> None:
    ex = GeminiWebMcpExecutor(mcp_url="http://127.0.0.1:0/mcp")
    client = _DeepThinkPendingMcpClient()
    ex._client = client  # type: ignore[assignment]
    res = asyncio.run(
        ex.run(
            job_id="job-gemini-deep-think-pending",
            kind="gemini_web.ask",
            input={"question": "请继续分析这个主题。"},
            params={"preset": "deep_think", "timeout_seconds": 60, "max_wait_seconds": 60},
        )
    )

    assert res.status == "completed"
    assert [name for name, _ in client.calls] == ["gemini_web_ask_pro_deep_think", "gemini_web_wait"]
    assert isinstance(res.meta, dict)
    assert res.meta.get("conversation_url") == "https://gemini.google.com/app/recovered-thread-9999"


def test_gemini_full_send_with_replayed_pending_recovery_enters_wait() -> None:
    ex = GeminiWebMcpExecutor(mcp_url="http://127.0.0.1:0/mcp")
    client = _PendingReplayMcpClient()
    ex._client = client  # type: ignore[assignment]
    res = asyncio.run(
        ex.run(
            job_id="job-gemini-deep-think-replayed-pending",
            kind="gemini_web.ask",
            input={"question": "请继续分析这个主题。"},
            params={"preset": "deep_think", "timeout_seconds": 60, "max_wait_seconds": 60},
        )
    ...
    assert [name for name, _ in client.calls] == ["gemini_web_ask_pro_deep_think", "gemini_web_wait"]
    assert isinstance(res.meta, dict)
    assert res.meta.get("conversation_url") == "https://gemini.google.com/app/recovered-thread-pending"
