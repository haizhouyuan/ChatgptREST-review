...
def test_memory_capture_service_persists_project_scope_for_captured_memory() -> None:
    runtime = _runtime()
    service = MemoryCaptureService(runtime)
    ...
    result = service.capture(
        [
            MemoryCaptureItem(
                ...
                content="Use the PRS execution card as the current source of truth.",
                summary="PRS execution card is current source of truth.",
                trace_id="trace-captured-project",
                session_id="sess-1",
                account_id="acct-1",
                agent_id="advisor",
                role_id="planning",
    ...
    item = result.results[0]
    assert item.ok is True
    records = runtime.memory.get_episodic(category="captured_memory", project_id="prs", limit=10)
    assert len(records) == 1
    assert records[0].source["project_id"] == "prs"
    assert records[0].value["project_id"] == "prs"
