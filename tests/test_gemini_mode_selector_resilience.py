...
def test_gemini_drive_attach_timeout_with_hidden_accounts_iframe_keeps_drive_classification() -> None:
    assert (
        _gemini_classify_error_type(
            error_text=(
                "Timed out waiting for visible Google Drive picker iframe. "
                "picker_debug={\"dom_iframes\":[{\"src\":\"https://accounts.google.com/RotateCookiesPage\",\"visible\":false}]}"
            ),
            fallback="RuntimeError",
        )
        == "GeminiDriveAttachUnavailable"
...
def test_gemini_clean_response_text_strips_anchor_only_transcript_labels() -> None:
    assert gemini_core._gemini_clean_response_text("显示思路\nGemini 说\n") == ""
    assert gemini_core._gemini_clean_response_text("你说\n显示思路\nGemini 說\n") == ""


def test_gemini_clean_response_text_preserves_real_body_after_transcript_labels() -> None:
    cleaned = gemini_core._gemini_clean_response_text(
        "你说\n显示思路\nGemini 说\n\n- 第一条\n- 第二条\n- 第三条"
    )
    assert cleaned == "- 第一条\n- 第二条\n- 第三条"
...
def test_gemini_should_import_code_requires_explicit_flag() -> None:
    assert not gemini_ask._gemini_should_import_code(
        github_repo="https://github.com/example/repo",
        enable_import_code=False,
    )
    assert gemini_ask._gemini_should_import_code(
        github_repo="https://github.com/example/repo",
        enable_import_code=True,
    )


def test_gemini_resolve_repo_context_prefers_explicit_hint() -> None:
    github_repo, repo_hint, effective = gemini_ask._gemini_resolve_repo_context(
        github_repo="https://github.com/example/repo",
        repo_context_hint="https://github.com/example/review-repo",
    )
    assert github_repo == "https://github.com/example/repo"
    assert repo_hint == "https://github.com/example/review-repo"
    assert effective == "https://github.com/example/review-repo"
...
def test_gemini_tools_more_regex_matches_folded_menu_variants() -> None:
    assert gemini_core._GEMINI_TOOLS_MORE_RE.search("更多")
    assert gemini_core._GEMINI_TOOLS_MORE_RE.search("更多工具")
    assert gemini_core._GEMINI_TOOLS_MORE_RE.search("More")
    assert gemini_core._GEMINI_TOOLS_MORE_RE.search("More tools")


def test_gemini_find_tool_item_expands_more_submenu_once(monkeypatch) -> None:
    class _FakePage:
    ...
    async def _fake_first_visible(locator, *, max_scan: int = 10):
        tag = str(locator)
        if "tool-match" in tag and state["expanded"]:
            return object()
        return None

    async def _fake_expand(_page) -> bool:
        if state["expanded"]:
            return False
        state["expanded"] = True
        return True
    ...
    class _FakeLocator:
        ...
        async def count(self) -> int:
            return 1 if "tool-match" in self.tag and state["expanded"] else 0

        async def is_visible(self) -> bool:
            return "tool-match" in self.tag and state["expanded"]

        async def inner_text(self) -> str:
            return "Deep Think" if "tool-match" in self.tag else ""

    def _fake_locator(selector: str):
        tag = "tool-match" if "menuitemcheckbox" in selector else selector
        return _FakeLocator(tag)

    page = _FakePage()
    page.locator = _fake_locator  # type: ignore[attr-defined]

    monkeypatch.setattr(gemini_core, "_gemini_open_tools_drawer", _noop)
    monkeypatch.setattr(gemini_core, "_gemini_first_visible", _fake_first_visible)
    monkeypatch.setattr(gemini_core, "_gemini_expand_more_tools_submenu", _fake_expand)

    got = asyncio.run(
        gemini_core._gemini_find_tool_item(
            page,
            label_re=re.compile(r"(Deep\s*Think|深度思考|深入思考)", re.I),
            timeout_seconds=0.6,
        )
    )
