...
def test_closeout_forwards_after_checker_passes(monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(mod.subprocess, "run", fake_run)
    monkeypatch.setenv("OPENMIND_API_KEY", "mirror-key-from-env")
    ...
    assert seen_env["CHATGPTREST_TELEMETRY_MIRROR_API_KEY"] == "mirror-key-from-env"


def test_closeout_loads_mirror_key_from_config_file(monkeypatch, tmp_path) -> None:
    mod = _load_script()
    cfg = tmp_path / "chatgptrest.env"
    cfg.write_text("OPENMIND_API_KEY=file-openmind-key\n", encoding="utf-8")
    monkeypatch.delenv("OPENMIND_API_KEY", raising=False)
    monkeypatch.delenv("CHATGPTREST_TELEMETRY_MIRROR_API_KEY", raising=False)
    monkeypatch.setenv("CHATGPTREST_CLOSEOUT_ENV_FILES", cfg.as_posix())

    env = mod._build_closeout_env()

    assert env["OPENMIND_API_KEY"] == "file-openmind-key"
    assert env["CHATGPTREST_TELEMETRY_MIRROR_API_KEY"] == "file-openmind-key"
