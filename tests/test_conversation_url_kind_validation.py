...
def test_rejects_qwen_kind_with_chatgpt_conversation_url(env: dict[str, Path]):
    ...
    assert "provider_removed" in r.text
...
def test_rejects_cross_provider_parent_job_id_followup(env: dict[str, Path]):
    ...
    parent = {
        "kind": "gemini_web.ask",
        "input": {
            "question": "Review the attached Gemini thread and extract three implementation risks with mitigations.",
    ...
    child = {
        "kind": "chatgpt_web.ask",
        "input": {"question": "Continue this review and extract the top risks in JSON.", "parent_job_id": parent_id},
        "params": {"preset": "pro_extended"},
    }
