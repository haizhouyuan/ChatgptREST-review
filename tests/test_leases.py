...
def test_retryable_ask_extension_limit_counts_across_error_types(env: dict[str, Path], monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("CHATGPTREST_RETRYABLE_SEND_ATTEMPTS_CAP", "6")
    monkeypatch.setenv("CHATGPTREST_RETRYABLE_SEND_MAX_EXTENSIONS", "1")
    app = create_app()
    client = TestClient(app)
    payload = {
        "kind": "gemini_web.ask",
        "input": {"question": "请给出当前 W1 live 阻塞的最小解释。"},
        "params": {"preset": "pro"},
    }
    r = client.post("/v1/jobs", json=payload, headers={"Idempotency-Key": "lease-ask-extend-cross-error-1"})
    assert r.status_code == 200
    job_id = r.json()["job_id"]
    ...
    for _ in range(max_attempts - 1):
        ...
        with connect(env["db_path"]) as conn:
            ...
            stored = store_retryable_result(
                ...
                error="SSE stream timeout (deadline exceeded)",
            )
            conn.commit()
        assert stored.status == JobStatus.COOLDOWN
    ...
    with connect(env["db_path"]) as conn:
        conn.execute("BEGIN IMMEDIATE")
        first_extension = store_retryable_result(
            conn,
            artifacts_dir=env["artifacts_dir"],
            job_id=job_id,
            worker_id="w1",
        ...
        row = conn.execute("SELECT status, attempts, max_attempts FROM jobs WHERE job_id = ?", (job_id,)).fetchone()
        conn.commit()
    assert first_extension.status == JobStatus.COOLDOWN
    assert row is not None
    assert int(row["max_attempts"]) == max_attempts + 1

    with connect(env["db_path"]) as conn:
        conn.execute("BEGIN IMMEDIATE")
        extended_job = claim_next_job(conn, artifacts_dir=env["artifacts_dir"], worker_id="w1", lease_ttl_seconds=60)
        conn.commit()
    assert extended_job is not None
    assert extended_job.attempts == max_attempts + 1

    with connect(env["db_path"]) as conn:
        conn.execute("BEGIN IMMEDIATE")
        second_result = store_retryable_result(
            conn,
            artifacts_dir=env["artifacts_dir"],
            job_id=job_id,
            worker_id="w1",
            lease_token=str(extended_job.lease_token or ""),
            status=JobStatus.COOLDOWN,
            not_before=time.time() - 1,
            error_type="InfraError",
            error="Locator.count: Target crashed",
        )
        row = conn.execute("SELECT status, attempts, max_attempts, last_error_type FROM jobs WHERE job_id = ?", (job_id,)).fetchone()
        ...
        conn.commit()
    assert second_result.status == JobStatus.ERROR
    assert row is not None
    assert str(row["status"]) == JobStatus.ERROR.value
    assert int(row["max_attempts"]) == max_attempts + 1
    assert str(row["last_error_type"]) == "MaxAttemptsExceeded"
    assert skipped is not None
    payload_out = json.loads(str(skipped["payload_json"] or "{}"))
    assert payload_out.get("guard_reason") == "retryable_extension_limit_reached:1/1"
    with connect(env["db_path"]) as conn:
        recovery = conn.execute(
            "SELECT payload_json FROM job_events WHERE job_id = ? AND type = ? ORDER BY id DESC LIMIT 1",
            (job_id, "web_send_retry_exhausted"),
        ).fetchone()
    assert recovery is not None
    recovery_payload = json.loads(str(recovery["payload_json"] or "{}"))
    assert recovery_payload.get("terminal_error_type") == "MaxAttemptsExceeded"
    assert recovery_payload.get("recommended_params", {}).get("send_timeout_seconds") == 240
...
def test_worker_claim_retries_transient_database_locked(env: dict[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    cfg = load_config()
    real_connect = worker_mod.connect
    attempts = 0
    sleeps: list[float] = []
    ...
    class _LockedConnection:
        ...
        def commit(self) -> None:
            raise AssertionError("commit should not run on locked connection")
    ...
    def fake_connect(path: Path):  # noqa: ANN202
        nonlocal attempts
        attempts += 1
        if attempts <= 2:
            return _ConnectionContext(_LockedConnection())
        return real_connect(path)

    async def fake_sleep(delay: float) -> None:
        sleeps.append(delay)

    monkeypatch.setenv("CHATGPTREST_WORKER_CLAIM_DB_LOCK_RETRY_ATTEMPTS", "3")
    monkeypatch.setenv("CHATGPTREST_WORKER_CLAIM_DB_LOCK_RETRY_BASE_MS", "200")
    monkeypatch.setattr(worker_mod, "connect", fake_connect)
    monkeypatch.setattr(worker_mod.asyncio, "sleep", fake_sleep)

    ran = asyncio.run(worker_mod._run_once(cfg=cfg, worker_id="w-lock-retry", lease_ttl_seconds=60))
