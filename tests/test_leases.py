...
def test_retryable_qwen_cdp_limit_becomes_needs_followup(env: dict[str, Path], monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    qwen_cdp_error = (
        "Qwen CDP connect failed (RuntimeError: CDP connect failed without a specific error). "
    ...
    with connect(env["db_path"]) as conn:
        ...
        row = conn.execute("SELECT status, last_error_type, last_error FROM jobs WHERE job_id = ?", (job_id,)).fetchone()
        ev_skip = conn.execute(
            "SELECT payload_json FROM job_events WHERE job_id = ? AND type = ? ORDER BY id DESC LIMIT 1",
            (job_id, "max_attempts_extension_skipped"),
        ).fetchone()
    ...
    assert stored.status == JobStatus.NEEDS_FOLLOWUP
    assert row is not None
    assert str(row["status"]) == JobStatus.NEEDS_FOLLOWUP.value
    assert str(row["last_error_type"] or "") == "QwenCdpUnavailable"
    assert "Qwen CDP connect failed" in str(row["last_error"] or "")
    assert ev_skip is not None
    payload_obj = json.loads(str(ev_skip["payload_json"] or "{}"))
    assert payload_obj.get("guard_reason") == "retryable_extension_limit_reached:0/0"
