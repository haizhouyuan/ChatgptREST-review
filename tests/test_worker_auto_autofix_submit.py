...
def test_worker_auto_autofix_submits_for_web_retry_budget_needs_followup(env: dict[str, Path]):
    app = create_app()
    client = TestClient(app)
    payload = {
        "kind": "chatgpt_web.ask",
        "input": {"question": "Continue the deep research and finish the report."},
        "params": {"preset": "deep_research"},
    }
    r = client.post("/v1/jobs", json=payload, headers={"Idempotency-Key": "autofix-web-retry-budget"})
    assert r.status_code == 200
    job_id = r.json()["job_id"]

    cfg = load_config()
    conversation_url = "https://chatgpt.com/c/00000000-0000-0000-0000-000000000003"
    asyncio.run(
        _maybe_submit_worker_autofix(
            cfg=cfg,
            job_id=job_id,
            kind="chatgpt_web.ask",
            status="needs_followup",
            error_type="WebRetryBudgetExceeded",
            error="chatgpt browser-visible retry budget exceeded within 900s",
    ...
    with connect(env["db_path"]) as conn:
        ...
        assert "clear_blocked" in allow_actions
        assert "refresh" in allow_actions
        assert "restart_driver" in allow_actions
        events = conn.execute(
            "SELECT type FROM job_events WHERE job_id = ? AND type = ?",
            (job_id, "auto_autofix_submitted"),
        ).fetchall()
...
def test_worker_auto_autofix_skips_blank_gemini_timeout_without_thread(env: dict[str, Path]):
    app = create_app()
    client = TestClient(app)
    payload = {
        "kind": "gemini_web.ask",
        "input": {"question": "请整理供应链恢复窗口的下一步安排。"},
        "params": {"preset": "pro"},
    }
    r = client.post("/v1/jobs", json=payload, headers={"Idempotency-Key": "autofix-gemini-blank-timeout"})
    assert r.status_code == 200
    job_id = r.json()["job_id"]
    ...
    with connect(env["db_path"]) as conn:
        ...
        events = conn.execute(
            "SELECT type FROM job_events WHERE job_id = ? AND type = ?",
            (job_id, "auto_autofix_skipped_blank_gemini_cooldown"),
        ).fetchall()
        assert events


def test_worker_auto_autofix_skips_same_session_repair_gate(env: dict[str, Path]) -> None:
    app = create_app()
    client = TestClient(app)
    payload = {
        "kind": "gemini_web.ask",
        "input": {"question": "继续整理来访准备。"},
        "params": {"preset": "pro"},
    }
    r = client.post("/v1/jobs", json=payload, headers={"Idempotency-Key": "autofix-gemini-thread-mismatch"})
    assert r.status_code == 200
    job_id = r.json()["job_id"]
    ...
    with connect(env["db_path"]) as conn:
        ...
        row = conn.execute(
            ...
            (job_id, "auto_autofix_skipped_by_gate"),
        ).fetchone()
        assert row is not None
        payload_obj = json.loads(str(row["payload_json"] or "{}"))
        gate = payload_obj.get("gate") or {}
        assert gate.get("terminal_action") == "same_session_repair"
        assert gate.get("allow_codex_autofix") is False
