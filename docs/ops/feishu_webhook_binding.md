...
curl -s -X POST http://127.0.0.1:18711/v2/advisor/webhook \
...
| 本地测试（集成态默认） | `http://127.0.0.1:18711/v2/advisor/webhook` |

说明：
集成态 ChatgptREST 默认把 `/v2/advisor/*` 挂在统一 FastAPI host `18711`。只有显式单独运行 dedicated advisor 服务时，才应把本地 webhook 指向其它端口。
