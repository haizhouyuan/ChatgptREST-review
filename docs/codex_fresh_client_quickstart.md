...
- `127.0.0.1:18711` 是 ChatgptREST REST API base，只给 `/v1/*`、`/v2/*`、`/v3/*`。
- `127.0.0.1:18712/mcp` 才是 public advisor-agent MCP；不要把 `18712` 当成 REST API base。
- Claude Code / Antigravity 的 JSON MCP 配置必须写成 `"type": "http"` + `"url": "http://127.0.0.1:18712/mcp"`；不要用 legacy `serverURL`。
- 如果这台机器上的客户端配置可能漂了，先运行 `python3 ops/check_public_mcp_client_configs.py --fix`，再重启对应客户端会话。
