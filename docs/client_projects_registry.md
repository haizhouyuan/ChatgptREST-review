...
- 客户端文档同步状态（2026-03-25）：`planning/codexread/storyplay/homeagent/research/openclaw` 的当前入口文档已同步到 “public advisor-agent MCP first”；repo-local skill `planning/skills-src/chatgptrest-call` 与 `ppt-banana-review-loop` 已同步边界。`finchat` 与 `maint(sim-dev)` 也已更新工作树文件，但两仓当前尚无 `HEAD`，需在各自仓初始化/提交时正式固化。
...
- 凡是还要调用低层 `/v1/jobs` web ask 的自动化客户端，必须先登记到 `ops/policies/ask_client_registry.json`，明确：
  - `client_id`
  - `source_type`
  - `trust_class`
  - `allowed_surfaces`
...
- automation wrapper 若仍保留 `low_level_jobs`，现在必须是 `auth_mode=hmac`；registry-name-only 低层 ask 会被服务端认定为 misconfigured 并 fail-closed。
- 未登记或来源不明的 low-level ask 调用视为不合规，默认 fail-closed；交互式 coding client 仍然必须走 public advisor-agent MCP，不得回退到 `/v1/jobs`。
...
3) 按登记表逐个同步客户端项目文档（至少更新两处）：
   ...
   - 如项目内存在 ChatgptREST 调用 skill / wrapper 说明（例如 repo-local `SKILL.md`），也必须同步“public advisor-agent MCP first，low-level ask maintenance-only”
...
- 2026-02-24: 写路径追踪门禁 + 取消归因硬化（可选强制）：
  ...
  - 新增 public MCP 可选门禁：`CHATGPTREST_REQUIRE_EXPLICIT_MCP_CANCEL_REASON=1` 时，`automation_job_cancel` / `chatgptrest_job_cancel` 必须显式传 `reason`，否则 MCP 工具在发出 HTTP `/cancel` 前 fail-closed。
...
- 2026-03-19: direct live ChatGPT ask 旁路封堵（默认开启）：
  ...
  - 客户端建议：对 coding agent，真实 live ask 一律走 public MCP `automation_ask`（`http://127.0.0.1:18712/mcp`）；不要再用 `curl/chatgptrestctl` 直接创建 `chatgpt_web.ask`，也不要把 `/v3/agent/*` 当成 coding agent 的默认接入面。
...
- 2026-03-23: coding-agent northbound 文档与接入面对齐：
  - Codex / Claude Code / Antigravity 的默认入口统一为 public MCP automation-kernel-v1：`http://127.0.0.1:18712/mcp`。
...
- 2026-03-25: coding-agent 低层 ask 入口进一步收口：
  - 新增阻断：coding-agent client identity 直接调用 `POST /v1/jobs kind=gemini_web.ask|qwen_web.ask` 默认返回 `coding_agent_low_level_ask_blocked`（HTTP 403）。
  - wrapper `skills-src/chatgptrest-call/scripts/chatgptrest_call.py` 的 legacy jobs fallback 改为 maintenance-only，必须显式带 `--maintenance-legacy-jobs`，并使用保留 client name `chatgptrestctl-maint`；如果它真的触发低层 web ask，还必须配置对应 HMAC secret env。
  - 客户端建议：所有 agent 文档、skill、runbook、示例命令都不要再把 `--no-agent`、`/v1/jobs` 或 `/v3/agent/turn` 作为默认入口；统一改写为 public advisor-agent MCP first。
- 2026-03-25: low-level ask 身份注册与授权门禁：
  - 新增强制要求：低层 `chatgpt_web.ask|gemini_web.ask|qwen_web.ask` 调用必须携带已登记的来源身份（`X-Client-Id` 或 `X-Client-Name`）。
  - maintenance/internal 例外（例如 `chatgptrest-admin-mcp`、`chatgptrestctl-maint`、`internal-submit-wrappers`、`finagent-event-extractor`）现在不再是“名字登记即放行”，而是 HMAC-scoped identity。
  - 新增错误语义：`low_level_ask_client_identity_required`、`low_level_ask_client_not_registered`、`low_level_ask_client_auth_failed`、`low_level_ask_intent_blocked`。
  - 维护例外不再依赖 `params.allow_direct_live_chatgpt_ask=true`；真正的例外只保留给 registry 里的 maintenance/internal profile。
  - 客户端建议：如果是自动化 bot/pipeline/timer，需要先补 client registry、provenance headers、以及 HMAC secret（若 profile 要求），再决定是否真的需要低层 ask；不要把 extractor/sufficiency gate/JSON-only microtask 塞进 live web ask。
- 2026-03-27: wrapper low-level ask 治理收口：
  - `planning-wrapper` 是唯一仍保留低层 web ask 的自动化 wrapper 身份；该 lane 现已升级为 HMAC-scoped，并增加运行时并发上限与近期重复提交去重。
  - `openclaw-wrapper` 收口为 `public_agent_mcp` only；外部 low-level `/v1/jobs` 不再允许。
  - `advisor-automation` 收口为 internal-runtime identity；外部 low-level `/v1/jobs` 不再允许复用 `advisor_*` aliases。
  - `finbot-wrapper` 继续保持 public-agent-only，不开放 low-level ask。
- 2026-04-26: ChatGPT Web 共享 Pro 通道 admission 收口：
  - `internal-submit-wrappers` 和 `planning-wrapper` 的低层 `chatgpt_web.ask` 现在按 kind 单飞；同一注册客户端已有 `queued/in_progress` ChatGPT Web ask 时，新请求返回 `low_level_ask_client_kind_concurrency_exceeded`（HTTP 429），而不是继续进入浏览器队列。
  - 近期重复提交去重从路径级升级为附件内容级：同名同大小同 SHA-256 的 packet 即使换了 staging 目录，也会命中 `low_level_ask_duplicate_recently_submitted`（HTTP 409）。
  - 客户端建议：遇到 409 复用 `existing_job_id`；遇到 429 等 active job 完成或改用非 ChatGPT Web provider，不要通过换 idempotency key / 换临时目录重试。
