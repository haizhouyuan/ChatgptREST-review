...
- `opencli` is **not** a public low-level `kind=*` job contract in v1.
- The current integration is intentionally narrow:
  - execution only through policy-gated server-side lanes,
  - operator diagnostics via `chatgptrestctl opencli doctor|policy|smoke`,
  - allowlisted commands only.
- Current policy boundary:
  - no arbitrary CLI passthrough,
  - no auto-install as part of the ChatgptREST execution contract,
  - no implicit projection of opencli execution as generic provider-selection success.
- If public-agent responses include opencli execution provenance, that provenance is executor-specific metadata, not a provider-family contract change.
