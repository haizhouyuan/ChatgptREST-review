...
- Pro safety guardrails (default on):
  ...
  - Direct low-level `POST /v1/jobs kind=chatgpt_web.ask` is blocked by default with HTTP 403
    `detail.error="direct_live_chatgpt_ask_blocked"` unless one of the following is true:
    - `params.allow_direct_live_chatgpt_ask=true`
    - caller `X-Client-Name` is allowlisted by `CHATGPTREST_DIRECT_LIVE_CHATGPT_CLIENT_ALLOWLIST`
    - request is an in-process FastAPI `TestClient` harness
    Preferred path for real live asks is `/v3/agent/turn` or `advisor_agent_turn` via the public MCP surface.
