...
Mode:
- Prefer Claude Code Agent Teams.
- Optimize for quality, coverage, evidence, and actionable outcomes first.
- Do not optimize for low token usage.
...
Task identity lock:
- This is NOT the older "public advisor-agent facade implementation" task.
- This is NOT a feature-building task unless business validation uncovers repo-local defects that must be fixed.
- This IS the OpenClaw / OpenMind unified agent business integration validation task.
- Your mission is to execute BI-01 through BI-14 from the test plan, collect evidence, fix repo-local issues when discovered, rerun affected tests/business cases, and produce an acceptance result.

Execution behavior requirements:
- Do not ask the user whether to continue.
- Do not stop after reading docs and proposing todos.
- Start executing.
- If Agent Teams is available, use it.
- If Agent Teams is unavailable or partially unavailable, automatically degrade to a single-session execution model and continue the same mission.
- Only return `blocked` if you hit a real external blocker after attempting the automated gate and relevant business cases.

Required startup self-check:
- Before doing any substantial work, confirm to yourself that the loaded task is correct by extracting from the test plan:
  - the automated gate suites
  - BI-01 through BI-14 scope
  - the fact that OpenClaw plugin convergence to `/v3/agent/turn` is part of the acceptance criteria
- If your internal plan/todos drift to building `/v3/agent/*`, public MCP, CLI convergence, or other previously completed facade implementation items as the primary task, treat that as context drift and correct yourself immediately.
...
Preferred Agent Teams topology:
...
Single-session fallback rules:
- If Agent Teams cannot be used, keep the same lane structure logically but execute sequentially in one session.
- Preserve the same deliverable quality and evidence standard.
- Do not report `blocked` merely because Agent Teams is unavailable.
...
- one walkthrough doc with `_v1.md` or higher version suffix
