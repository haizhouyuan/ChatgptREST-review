...
1) `docs/contract_v1.md`：对外 REST v1 契约（/v1/jobs、/wait、/answer、/conversation…）。
2) `docs/runbook.md`：启动/重启/排障清单（端口、env、脚本、常见故障，含 `Incident-Class Runbook` 与 Codex 唤醒执行合同）。
3) `docs/repair_agent_playbook.md`：Repair Agent 权限/步骤/规则（自动修复执行基线）。
4) `docs/handoff_chatgptrest_history.md`：历史问题与修复记录（按问题→代码路径→提交/测试）。
5) `docs/ops_rollout_plan_p0p1p2_safe_enable_20251228.md`：哪些能力“已合入但默认不启用”，以及启用策略。
6) `docs/chatgpt_web_ui_reference.md`：ChatGPT Web UI 快照参考（本地自动生成，默认不入 git；指向 `artifacts/ui_snapshots/`）。
7) `skills-src/chatgptrest-call/SKILL.md`：给 Codex/agent 的统一调用入口（`chatgptrestctl` wrapper）。
8) `artifacts/monitor/open_issue_list/latest.md`：当前 open / in_progress issue 视图（自动导出）。
9) `artifacts/monitor/open_issue_list/history_tail.md`：最近 issue 生命周期演进快照（自动导出）。
