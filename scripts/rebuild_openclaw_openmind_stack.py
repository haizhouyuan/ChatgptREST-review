...
DEFAULT_MINIMAX_MODEL_REF = "minimax/MiniMax-M2.7-highspeed"
...
TOPOLOGY_AGENT_IDS = {
    "lean": ("main", "feishu-intake"),
    "ops": ("main", "feishu-intake", "maintagent", "finbot"),
}
...
AGENTS: tuple[AgentSpec, ...] = (
    ...
    AgentSpec(
        agent_id="feishu-intake",
        workspace="/vol1/1000/openclaw-workspaces/feishu-intake",
        model=DEFAULT_MINIMAX_MODEL_REF,
        name="Feishu Intake",
        # Use an explicit allowlist instead of minimal+alsoAllow. OpenClaw's live
        # plugin-tool bootstrap only preloads plugin tools from explicit allowlists,
        # so work-intake plugin tools would otherwise disappear at runtime.
        tool_profile="full",
        tool_allow=FEISHU_INTAKE_TOOL_ALLOW,
        sandbox=AGENT_SANDBOX_OVERRIDES["feishu-intake"],
    ),
    AgentSpec(
        agent_id="maintagent",
...
def _workspace_identity(spec: AgentSpec) -> str:
    labels = {
        ...
        "feishu-intake": ("Feishu Intake", "work intake router", "focused / fail-closed"),
...
def _workspace_tools(spec: AgentSpec) -> str:
    ...
    role_specific = {
        ...
        "feishu-intake": [
            "- This workspace is work-only. Treat Feishu messages, screenshots, and files as work materials by default.",
            "- The legacy work-ingress/advisor control plane is retired; do not call `openmind_work_ingress_turn` or `openmind_advisor_ask`.",
            "- Use `openmind_work_material_ops` only for exact-path local material inspection or controlled moves inside allowlisted roots.",
            "- Current allowlisted material roots are `/vol1/1000/projects/planning` (read-write) and `/vol1/1000/projects/ChatgptREST` (read-only). Fail closed instead of improvising outside them.",
            "- Higher-order planning/research routing is no longer provided by this ChatgptREST stack.",
            "- For mixed asks, the first turn may inspect exact files, but must not move or rename anything before the work frame and archive structure are confirmed.",
            "- `exec` is not part of this lane. Never tell the user to use or wait for `exec`.",
            "- Do not ask the user for screenshots or manual file summaries when the file path is already directly accessible from the allowlisted roots.",
            "- If the message is clearly non-work, fail closed with a short redirect instead of improvising.",
        ],
        "maintagent": [
...
- The legacy `openmind_advisor_ask` control-plane path is retired; do not rely on it for project routing.
- Determine project association with rules first, then cache, and fail closed into a short clarification when project signals conflict.
- Treat task mode as explicit runtime state: choose among `continue`, `branch`, `status`, `new`, or `clarify`.
- If you cannot confidently tell whether the user means continue vs branch vs status, ask a short clarification instead of guessing.
...
""",
        "feishu-intake": """# AGENTS.md

## Role

- You are the dedicated work-only intake lane for Feishu.
- Your primary job is to route work asks into the correct execution lane.
- Treat screenshots, files, and quoted messages as structured work materials, not as invitations for free-form image commentary.

## Operating rules

- The legacy `openmind_work_ingress_turn` ingress policy tool is retired.
- Treat `/vol1/1000/projects/planning` as the primary read-write work-material root and `/vol1/1000/projects/ChatgptREST` as a read-only reference root.
- Use `openmind_work_material_ops` only for exact local material inspection or controlled moves inside allowlisted roots.
- Do not expect ChatgptREST to provide higher-order advisor/planning routing from this lane anymore.
- For mixed asks, the first turn may inspect exact files, but must not move or rename anything before the work frame and archive structure are confirmed.
- `exec` is not available in this lane. Never tell the user to use `exec`.
- If the ask is clearly non-work, fail closed with a short redirect instead of improvising.
...
def _workspace_role_packs(spec: AgentSpec) -> str:
    if spec.agent_id != "main":
        if spec.agent_id == "feishu-intake":
            return (
                "# ROLE_PACKS.md\n\n"
                "- This workspace does not host independent role packs.\n"
                "- The legacy work-ingress/advisor control plane is retired in this stack.\n"
                "- This lane may only use `openmind_work_material_ops` for exact-path local material inspection or controlled moves.\n"
                "- Do not attempt higher-order planning/research routing through ChatgptREST from this lane.\n"
                "- Even when explicit local paths are present, do not move or rename files on the first turn of a mixed ask; wait until the work frame and archive destination are explicit.\n"
                "- `exec` is intentionally unavailable here.\n"
            )
...
- OpenMind advisor:
  - retired in this stack; do not call `openmind_advisor_ask`
...
def build_agents_section(current_cfg: dict[str, Any], *, topology: str) -> dict[str, Any]:
    ...
    for spec in active_specs:
        sandbox_config = dict(spec.sandbox) if spec.sandbox else None
        sandbox_tool_policy = None
        if sandbox_config and isinstance(sandbox_config.get("tools"), dict):
            sandbox_tool_policy = dict(sandbox_config.pop("tools"))
        entry: dict[str, Any] = {
            "id": spec.agent_id,
            ...
            "tools": {
                ...
                **({"sandbox": {"tools": sandbox_tool_policy}} if sandbox_tool_policy else {}),
            },
        }
        if spec.name:
...
def build_bindings(current_cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if primary_account:
        bindings.append({"type": "route", "agentId": "feishu-intake", "match": {"channel": "feishu", "accountId": primary_account}})
    return bindings
