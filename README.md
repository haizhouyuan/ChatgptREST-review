...
- The repo wrapper `skills-src/chatgptrest-call/scripts/chatgptrest_call.py` now auto-loads missing maintenance env from `~/.config/chatgptrest/chatgptrest.env` and `/vol1/maint/MAIN/secrets/credentials.env`, so `chatgptrestctl-maint` HMAC/API tokens do not need an extra manual export step.
...
- `ChatGPT Pro` and `Deep Research` are separate modes. `preset=pro_extended` is the Pro long-answer path; add `deep_research=true` only when the user explicitly asked for Deep Research.
- For internal technical judgment / architecture-review asks on `ChatGPT Pro`, prefer attaching a local context memo (`--file-path /tmp/local_context.md`) and explicitly instructing `不要使用网页搜索，不要切到 deep research，只基于附件和常识判断`. Bare Pro prompts on repo/runtime questions can otherwise burn the browser retry budget on visible search/tool steps without ever surfacing the final answer text.
